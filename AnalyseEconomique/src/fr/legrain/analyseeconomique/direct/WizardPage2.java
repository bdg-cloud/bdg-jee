...
import fr.legrain.analyseeconomique.actions.CompositePage2;
...
public class WizardPage2 extends WizardPage implements ILgrWizardPage {
	...
	public void setVisible(boolean visible) {
		if(((WizardAnalyseEconomiqueDirect)getWizard()).getModel().getCheminFichierCompta()!=null) {
			((CompositePage2)control).getTxtFichierCompta()
			.setText(((WizardAnalyseEconomiqueDirect)getWizard()).getModel().getCheminFichierCompta());
		}
		if(!((WizardAnalyseEconomiqueDirect)getWizard()).getModel().getNouveauDocument()) {
	...
	public void finishPage() {
		if (isPageComplete()) {
			saveToModel();
			if(((WizardAnalyseEconomiqueDirect)getWizard()).getModel().getNouveauDocument()) {
				((WizardAnalyseEconomiqueDirect)getWizard()).getModel().copieFichierCompta(ConstLiasse.C_FICHIER_COMPTA_LOCAL);
				((WizardAnalyseEconomiqueDirect)getWizard()).getModel().lectureInfosCompta(ConstLiasse.C_FICHIER_COMPTA_LOCAL);
				((WizardAnalyseEconomiqueDirect)getWizard()).getModel().sauveInfosComptaXML(ConstLiasse.C_FICHIER_XML_COMPTA_INITIAL);
				
//				((WizardAnalyseEconomique)getWizard()).repartitionAvecProgression();
				((WizardAnalyseEconomiqueDirect)getWizard()).getModel().sauveRepartXML(ConstLiasse.C_FICHIER_XML_REPART_INITIAL);
...
//			/////////////////////////////////////////////
	...
	public void saveToModel() {
		((WizardAnalyseEconomiqueDirect)getWizard()).getModel().setCheminFichierCompta(((CompositePage2)control).getTxtFichierCompta().getText());
	}
