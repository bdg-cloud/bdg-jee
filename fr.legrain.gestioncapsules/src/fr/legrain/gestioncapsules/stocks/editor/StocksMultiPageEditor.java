...
import fr.legrain.gestioncapsules.stocks.controllers.PaCalculReportController;
...
//public class MultiPageEditor extends SharedHeaderFormEditor {
	...
	public static final String ID_EDITOR = "fr.legrain.gestioncapsules.stocks.editor.StocksMultiPageEditor"; //$NON-NLS-1$
	...
	private EditorEtatStocksPeriode editorEtatStocksPeriode = null;
	private EditorEtatStocksReport editorEtatStocksReport= null;
	
	String labelPageStocks = "Stock CRD";
	String labelPageReportStocks = "Calcul des reports de stocks CRD";
	String labelPageEtatStocks = "Etat des stocks CRD";
	String labelPageEtatStocksPeriode = "Etat des mouvements de CRD";
	String labelPageEtatStocksReport = "Etat des reports de stocks CRD";
	/**
...
//	}
	...
	void createPageQueEditeur() {
		try {
			...
			String iconPageEtatStocksPeriode = "/icons/printer.png";
			...
			editorEtatStocksPeriode = new EditorEtatStocksPeriode(this,em);
			...
			listePageEditeur.add(editorEtatStocksPeriode);
			
			int index = addPage(editorStocks, getEditorInput());
			setPageText(index, labelPageStocks);
			setPageImage(index, Activator.getImageDescriptor(iconPageStocks).createImage());
			
			index = addPage(editorCalculReport, getEditorInput());
			setPageText(index, labelPageReportStocks);
			setPageImage(index, Activator.getImageDescriptor(iconPageReportStocks).createImage());
			
			index = addPage(editorEtatStocksReport, getEditorInput());
			setPageText(index, labelPageEtatStocksReport);
			setPageImage(index, Activator.getImageDescriptor(iconPageEtatStocksReport).createImage());
			
			index = addPage(editorEtatStocks, getEditorInput());
			setPageText(index, labelPageEtatStocks);
			setPageImage(index, Activator.getImageDescriptor(iconPageEtatStocks).createImage());
			
			index = addPage(editorEtatStocksPeriode, getEditorInput());
			setPageText(index, labelPageEtatStocksPeriode);
			setPageImage(index, Activator.getImageDescriptor(iconPageEtatStocksPeriode).createImage());
			...
			mapEditorController.put(editorEtatStocksPeriode, editorEtatStocksPeriode.getController());
			...
			editorEtatStocksPeriode.getController().addChangementDePageListener(this);
...
//			editorCalculReport.getController().addDeclencheCommandeControllerListener(editorStocks.getController());
			...
			paramAfficheEtatStocks.setTitreFormulaire(labelPageEtatStocksPeriode);
			((PaEtatStocksPeriodeController)editorEtatStocksPeriode.getController()).configPanel(paramAfficheEtatStocks);
