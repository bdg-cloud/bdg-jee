...
public class PaEtatStocksController extends JPABaseControllerSWTStandard implements
RetourEcranListener {
	
	static Logger logger = Logger.getLogger(PaEtatStocksController.class.getName());		
...
//		new LgrDozerMapper<TaReportStock,SWTStocks>();
	...
	public PaEtatStocksController(PaEtatStocks vue) {
		this(vue,null);
	}

	public PaEtatStocksController(PaEtatStocks vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
...
//	}
	...
	public PaEtatStocksController getThis(){
		return this;
	}
...
//		criteres.setExclusionQteSsUnite(vue.getCbExclusion().getSelection());
		...
		if(vue.getCbEditSynthese().getSelection()){
			List<TaEtatStockCapsules> listeEtatStockFinal = etatStocks.calculEtatStocks(criteres,new TaReportStockCapsulesDAO(getEm()).recupDerniereDateReportStock(),true,true);
		...
		if(vue.getCbEditMouvDate().getSelection()){
			List<TaEtatStockCapsules> listeEtatStockFinal = etatStocks.calculEtatStocksMouvements(criteres,new TaReportStockCapsulesDAO(getEm()).recupDerniereDateReportStock(),true);
			ConstEdition constEdition = new ConstEdition();
			Impression impression = new Impression(listeEtatStockFinal,constEdition);
...
//		if(aideDisponible()){
			try {
				...
				switch ((PaEtatStocksController.this.dao.getModeObjet().getMode())) {
				case C_MO_CONSULTATION:					
				case C_MO_EDITION:
				case C_MO_INSERTION:
...
//						paramAfficheAideRecherche.setChampsIdentifiant(Const.C_ID_ARTICLE);
						...
						paramAfficheAideRecherche.setDebutRecherche(vue.getTfCODE_ARTICLE().getText());
						paramAfficheAideRecherche.setControllerAppelant(PaEtatStocksController.this);
...
//					}					
				...
				if (paramAfficheAideRecherche.getJPQLQuery() != null) {
					...
					paAideController.addRetourEcranListener(PaEtatStocksController.this);
