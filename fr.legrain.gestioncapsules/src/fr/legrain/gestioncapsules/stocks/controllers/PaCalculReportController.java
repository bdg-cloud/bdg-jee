...
import fr.legrain.gestioncapsules.dao.TaReportStockCapsules;
...
RetourEcranListener {
	...
	private TaReportStockCapsulesDAO dao = null;//new TaReportStockDAO();
	...
	public PaCalculReportController(PaCalculReportSWT vue,EntityManager em) {
		...
		dao = new TaReportStockCapsulesDAO(getEm());
		try {
			setVue(vue);
...
//			end
			...
			for (TaEtatStockCapsules taEtatStock : listeReports) {
				...
				TaTitreTransport taTitreTransport = taTitreTransportDAO.findByCode(taEtatStock.getCodeTitreTransport());
