...
import fr.legrain.licencebdg.pluginLicenceBDG;
 ...
 * </ul>
 */
public class licenceBDGMultiPageEditor extends AbstractLgrMultiPageEditorSupport implements 
ILgrRetourEditeur, IChangementDePageListener,IChangementMasterEntityListener , IChangementDeSelectionListener,
IAnnuleToutListener{

	
	public static final String ID_EDITOR = "fr.legrain.licenceBdg.multi"; //$NON-NLS-1$
	static Logger logger = Logger.getLogger(licenceBDGMultiPageEditor.class.getName());
	...
	protected EditorLicenceBDG editorLicence = null;

	/**
	 * Creates a multi-page editor example.
	 */
	public licenceBDGMultiPageEditor() {
		super();
	...
	protected void createPageQueEditeur() {
		try {
			editorLicence = new EditorLicenceBDG(this,em);
			//Initialisation des pages/editeurs composant le multipage editor
			String labelPage1 = editorLicence.getTitle();
			...
			listePageEditeur.add(editorLicence);
			
			
			int index = addPage(editorLicence, getEditorInput());
			setPageText(index, labelPage1);
			setPageImage(index, pluginLicenceBDG.getImageDescriptor(iconPage1).createImage());
			//liaison entre la selection du controller principal et le multipage editor
			editorLicence.getController().addChangementDeSelectionListener(this);
			
			mapEditorController.put(editorLicence, editorLicence.getController());
			
			editorLicence.getController().addChangementDePageListener(this);

			editorLicence.getController().addAnnuleToutListener(this);

			editeurCourant = editorLicence;
			ParamAfficheLicenceBDG paramAfficheRelanceFacture = new ParamAfficheLicenceBDG();
			editorLicence.getController().configPanel(paramAfficheRelanceFacture);
