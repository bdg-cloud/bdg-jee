...
import fr.legrain.gestCom.Module_Tiers.ModelBdg;
...
import fr.legrain.licencebdg.divers.MessagesEcran;
import fr.legrain.licencebdg.divers.ParamAfficheLicenceBDG;
...
public class SWTPaBDGController extends JPABaseControllerSWTStandard 
implements RetourEcranListener, ISelectionListener{

	static Logger logger = Logger.getLogger(SWTPaBDGController.class.getName());	

	private PaBDGSWT vue = null;
	private TaLicenceBdgDAO dao = null;
	private String idSupportAbon = null;

	private Object ecranAppelant = null;
	private SWTSupportAbonLogiciel swtBdg;
	private SWTSupportAbonLogiciel swtOld;
	private Realm realm;
	private DataBindingContext dbc;

	private Class classModel = SWTSupportAbonLogiciel.class;
	private ModelBdg modelBdg = new ModelBdg();	
	...
	private LgrDozerMapper<SWTSupportAbonLogiciel,TaLicenceBdg> mapperUIToModel  = new LgrDozerMapper<SWTSupportAbonLogiciel,TaLicenceBdg>();
	private LgrDozerMapper<TaLicenceBdg,SWTSupportAbonLogiciel> mapperModelToUI  = new LgrDozerMapper<TaLicenceBdg,SWTSupportAbonLogiciel>();
	private TaLicenceBdg taLicence = null;

//	private Impression impression = new Impression();

	public SWTPaBDGController(PaBDGSWT vue) {
		this(vue,null);
	}

	public SWTPaBDGController(PaBDGSWT vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		dao = new TaLicenceBdgDAO(getEm());
...
//			initSashFormWeight();
	...
	public void bind(PaBDGSWT paEpiceaSWT){
		bind(paEpiceaSWT,null);
	}
	
	public void bindID(PaBDGSWT paEpiceaSWT, int idSupportAbon ){
		SWTSupportAbonLogiciel swtEpiceaTmp = new SWTSupportAbonLogiciel();
		modelBdg.mapping(dao.findById(idSupportAbon), swtEpiceaTmp);
		bind(paEpiceaSWT, swtEpiceaTmp);
	}
	
	public void bindCode(PaBDGSWT paEpiceaSWT, String code ){
		SWTSupportAbonLogiciel swtEpiceaTmp = new SWTSupportAbonLogiciel();
		modelBdg.mapping(dao.findByCode(code), swtEpiceaTmp);
		bind(paEpiceaSWT, swtEpiceaTmp);
	}
	
	public void bind(PaBDGSWT paEpiceaSWT, SWTSupportAbonLogiciel selection ){
		try {
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());
				dbc = new DataBindingContext(realm);
				if(dao.dataSetEnModif()){
					selection.setCodeSupportAbon(swtBdg.getCodeSupportAbon());
					swtBdg = selection;
					modelBdg.mapping(selection,taLicence);
				}else{
				if(selection!=null && selection.getIdSupportAbon()!=null) {
					taLicence =  dao.findById(selection.getIdSupportAbon());
					
					/*selection =*/ modelBdg.mapping(taLicence, selection);
					swtBdg = selection;
				} else {
					if(swtBdg==null) {
						swtBdg = new SWTSupportAbonLogiciel();
					}
					selection = swtBdg;
				}
				}
	...
	private void changementDeSelection() {		
		if(swtBdg!=null) {
			if(swtBdg.getIdSupportAbon()!=null) {
				taLicence = dao.findById(swtBdg.getIdSupportAbon());
			}
			//null a tester ailleurs, car peut etre null en cas d'insertion
			fireChangementDeSelection(new ChangementDeSelectionEvent(SWTPaBDGController.this));
		}
		initEtatComposant();
		initAbonnement();
		vue.getTfDateDebAbon().setEnabled(false);
		vue.getTfDateFinAbon().setEnabled(false);
		if(swtBdg.getEntreprise()!=null)vue.getTfEntreprise().setText(swtBdg.getEntreprise());
		if(swtBdg.getCodeEntite()!=null)vue.getTfCodeEntite().setText(swtBdg.getCodeEntite());
		if(swtBdg.getNomTiers()!=null)vue.getTfNomTiers().setText(swtBdg.getNomTiers());
		if(swtBdg.getPrenomTiers()!=null)vue.getTfPrenomTiers().setText(swtBdg.getPrenomTiers());
	}
	...
	public ResultAffiche configPanel(ParamAffiche param){
		...
		if (param!=null){
			...
			if(param.getTitreFormulaire()!=null){
				...
				vue.getLaTitreFormulaire().setText(ParamAfficheLicenceBDG.C_TITRE_FORMULAIRE);
			}

			if(param.getSousTitre()!=null){
				vue.getLaTitreFenetre().setText(param.getSousTitre());
			} else {
				vue.getLaTitreFenetre().setText(ParamAfficheLicenceBDG.C_SOUS_TITRE);
			}
			...
			if(modelBdg!=null && modelBdg.getListeEntity()!=null)
				modelBdg.getListeEntity().clear();
			if(modelBdg!=null && modelBdg.getListeObjet()!=null)
			modelBdg.getListeObjet().clear();
			...
			if(param.getSelected()!=null 
				...
				if(param.getCodeDocument()!=null) {
					SWTSupportAbonLogiciel licence = modelBdg.recherche(Const.C_CODE_SUPPORT_ABON, param.getCodeDocument());
					if(licence!=null) {
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfCodeSupport(), new InfosVerifSaisie(new TaLicenceBdg(),Const.C_CODE_SUPPORT_ABON,null));
		mapInfosVerifSaisie.put(vue.getTfCommentaire(), new InfosVerifSaisie(new TaLicenceBdg(),Const.C_COMMENTAIRE,null));
		mapInfosVerifSaisie.put(vue.getTfCommercial(), new InfosVerifSaisie(new TaTiers(),Const.C_CODE_TIERS,null));
		mapInfosVerifSaisie.put(vue.getTfDateAcquisition(), new InfosVerifSaisie(new TaLicenceBdg(),Const.C_DATE_ACQUISITION,null));
...
//		mapInfosVerifSaisie.put(vue.getTfDateFinAbon(), new InfosVerifSaisie(new TaHebergement(),Const.C_DATE_FIN_ABON,null));
	...
	protected void initActions() {		
		...
		vue.getBtnAbonnement().addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				if(taLicence!=null && taLicence.getTaAbonnements().size()>0)actAfficheAbonnement();
	...
	public SWTPaBDGController getThis(){
		return this;
	}
	...
	public boolean onClose() throws ExceptLgr {
		...
		if(retour) {
			if (ecranAppelant instanceof SWTPaAideControllerSWT) {
				setListeEntity(getModelBdg().remplirListe(getEm()));
	...
	protected void actInserer() throws Exception{
		try {
			if(dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION)==0) {
				VerrouInterface.setVerrouille(true);
				setSwtOldLicence();
				taLicence = new TaLicenceBdg();//ne pas déplacer pour que les autres écrans récupère le bon masterEntity
				...
				swtBdg.setCodeSupportAbon(dao.genereCodeJPA());
				validateUIField(Const.C_CODE_SUPPORT_ABON,swtBdg.getCodeSupportAbon());//permet de verrouiller le code genere
				modelBdg.mapping(swtBdg, taLicence);
		...
		try {
			...
			if(isUtilise())MessageDialog.openInformation(vue.getShell(), MessagesEcran
				...
				if (MessageDialog.openConfirm(vue.getShell(), MessagesEcran
					...
					TaLicenceBdg u = dao.findById(swtBdg.getIdSupportAbon());
					dao.supprimer(u);
					dao.commit(transaction);
					if(containsEntity(u)) modelBdg.getListeEntity().remove(u);
					taLicence=null;
	...
	protected boolean aideDisponible() {
		...
		switch ((SWTPaBDGController.this.dao.getModeObjet().getMode())) {
		case C_MO_CONSULTATION:
		case C_MO_EDITION:
		case C_MO_INSERTION:										
			...
			try{
				...
				case C_MO_INSERTION:										
					...
					if(getFocusCourantSWT().equals(vue.getTfGroupe())){
						...
						paramAfficheAideRecherche.setControllerAppelant(SWTPaBDGController.this);
	...
	public IStatus validateUI() throws Exception {
		if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
			...
			mapperUIToModel.map((SWTSupportAbonLogiciel) selectedEpicea.getValue(),taLicence);
		}
		return null;
	}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		...
		try {
			...
			if(nomChamp.equals(Const.C_COMMERCIAL)) {
				...
				TaLicenceBdg u = new TaLicenceBdg();
				...
				if(swtBdg!=null && swtBdg.getIdSupportAbon()!=null) {
					u.setIdSupportAbon(swtBdg.getIdSupportAbon());
				}
...
//						mapperUIToModel.map(swtBdg,taLicence);
					...
					if(!enregistreToutEnCours) {
						...
						swtBdg.setIdSupportAbon(taLicence.getIdSupportAbon());
...
//						!dao.autoriseModification(taLicence);	
		return (swtBdg!=null &&
				swtBdg.getIdTiers()!=null && !dao.recordModifiable(dao.getNomTable(),swtBdg.getIdTiers()))
				|| !dao.autoriseModification(taLicence);
	}
...
//		vue.getTfCodeArticle().setEditable(false);
	...
	public SWTSupportAbonLogiciel getSwtOld() {
		return swtOld;
	}
	...
	public void setSwtOldLicence() {
		if(swtBdg!=null) {
			this.swtOld=SWTSupportAbonLogiciel.copy(swtBdg);
...
//			}
	...
	//		}
	//	}
	public boolean containsEntity(TaLicenceBdg entite){
		if(modelBdg.getListeEntity()!=null){
			for (Object e : modelBdg.getListeEntity()) {
				if(((TaLicenceBdg)e).getIdSupportAbon()==
	...
	public boolean setSwtOldEpiceaRefresh() {
		try {	
			if (swtBdg!=null){
				TaLicenceBdg taOld =dao.findById(taLicence.getIdSupportAbon());
	...
	public void setVue(PaBDGSWT vue) {
		super.setVue(vue);
		this.vue = vue;
	}
	...
	public void initAbonnement(){
		...
		if(swtBdg.getDateDebAbon()==null || swtBdg.getDateFinAbon()==null ){
		...
		}else if(swtBdg.getDateFinAbon().before(taInfoEntreprise.getDatedebInfoEntreprise())||
				swtBdg.getDateDebAbon().after(taInfoEntreprise.getDatefinInfoEntreprise())){
			vue.getLaHorsAbon().setVisible(true);
		}
	...
	protected void actRefresh() throws Exception {
		...
		} else if(swtBdg!=null && swtBdg.getIdSupportAbon()!=null) {
			idActuel = swtBdg.getIdSupportAbon();
		}
		if (taLicence!=null && swtBdg!=null) {
			modelBdg.mapping(taLicence, swtBdg);
		}
			initAbonnement();
	}finally{
		vue.setCursor(Display.getCurrent().getSystemCursor(SWT.CURSOR_ARROW));
	...
	protected void actRefreshCourant() throws Exception {
		...
		int rang = modelBdg.getListeObjet().indexOf(selectedEpicea.getValue());
		mapperModelToUI.map(taLicence, swtOld);
		if (rang>0) modelBdg.getListeObjet().set(rang, swtOld);
		rang =((WritableList)tableViewer.getInput()).indexOf(swtOld);
		if (rang>0){
			((WritableList)tableViewer.getInput()).set(rang, swtOld);
		}else{
		writableList =new WritableList(realm, modelBdg.getListeObjet(),
				classModel);
		tableViewer.setInput(writableList);
		}
		if(idActuel!=0) {
			tableViewer.setSelection(new StructuredSelection(modelBdg.recherche(Const.C_ID_SUPPORT_ABON
					, idActuel)),true);
		}else
	...
	public TaLicenceBdgDAO getDao() {
		return dao;
	}



	public SWTSupportAbonLogiciel getSwtBdg() {
		return swtBdg;
	}



	public TaLicenceBdg getTaLicence() {
		return taLicence;
	}
	...
	public void setDao(TaLicenceBdgDAO dao) {
		this.dao = dao;
	};
...
//		modelBdg.setJPQLQuery(null);
//		modelBdg.setListeEntity(null);
//		try {
//			actRefresh();
//		} catch (Exception e1) {
//			logger.error("", e1);
...
//	}
	...
	public void actCreerAbonnement(TaLicenceBdg taLicence, final Boolean fermer ){
		try{
			final ParamAffiche param = new ParamAfficheAbonnement();
			...
			if(taLicence.getTaAbonnements()==null || taLicence.getTaAbonnements().size()==0){
				...
				if(taRTSupport!=null){
					...
					if(taRTSupport.getTaTAbonnement()!=null){
						...
						swtAbonnement.setDateDebut(taLicence.getDateAcquisition());	
						if(LibDate.PremierJourMois(taLicence.getDateAcquisition()).compareTo((taLicence.getDateAcquisition()))!=0){
