 ...
 * </ul>
 */
public class ListeAbonnementMultiPageEditor extends fr.legrain.licence.editors.AbstractLgrMultiPageEditorSupport implements 
ILgrRetourEditeur, IChangementDePageListener,IChangementMasterEntityListener , IChangementDeSelectionListener,
IAnnuleToutListener{

	
	public static final String ID_EDITOR = "fr.legrain.listeAbonnement.multi"; //$NON-NLS-1$
	static Logger logger = Logger.getLogger(ListeAbonnementMultiPageEditor.class.getName());
	...
	protected EditorListeAbonnement editorAbonnement = null;

	/**
	 * Creates a multi-page editor example.
	 */
	public ListeAbonnementMultiPageEditor() {
		super();
	...
	protected void createPageQueEditeur() {
		try {
			editorAbonnement = new EditorListeAbonnement(this,em);
			//Initialisation des pages/editeurs composant le multipage editor
			String labelPage1 = editorAbonnement.getTitle();
			...
			listePageEditeur.add(editorAbonnement);
			
			
			int index = addPage(editorAbonnement, getEditorInput());
			setPageText(index, labelPage1);
			setPageImage(index, pluginAbonnement.getImageDescriptor(iconPage1).createImage());
			//liaison entre la selection du controller principal et le multipage editor
			editorAbonnement.getController().addChangementDeSelectionListener(this);
			
			mapEditorController.put(editorAbonnement, editorAbonnement.getController());
			
			editorAbonnement.getController().addChangementDePageListener(this);

			editorAbonnement.getController().addAnnuleToutListener(this);

			editeurCourant = editorAbonnement;
			ParamAfficheAbonnement paramAfficheRelanceFacture = new ParamAfficheAbonnement();
			editorAbonnement.getController().configPanel(paramAfficheRelanceFacture);
...
//		masterEntity = ((SWTPaReglementMultipleController)editorLicence.getController()).getTaDocument();
//		((SWTPaReglementMultipleController)editorLicence.getController()).
...
//		masterEntity = ((SWTPaReglementMultipleController)editorLicence.getController()).getTaDocument();
//		((SWTPaReglementMultipleController)editorLicence.getController()).getPaReglementController().setMasterEntity(masterEntity);
//		((SWTPaReglementMultipleController)editorLicence.getController()).getAffectationReglementController().setMasterEntity(masterEntity);
////		if(masterEntity!=null && masterEntity.getCodeDocument()!=null) {
////			setPartName(masterEntity.getCodeDocument());
////		} else {
//			setPartName("Règlement");
...
//				SWTAbonnement support = new SWTAbonnement();
...
//				support.setDateDebut(documentSelection.getDateDocument());
//				support.setDateFin(documentSelection.getDateDocument());
//
//
