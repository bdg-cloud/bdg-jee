...
import fr.legrain.abonnement.editors.FormEditorFinalisationAbonnement;
...
//						new FormEditorInputAbonnement(), FormEditorFinalisationAbonnement.ID
//					);
...
//	}
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			IEditorPart editor = LgrFormEditor.verifEditeurOuvert(FormEditorFinalisationAbonnement.ID);
			if(editor==null){
				IEditorPart e = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().openEditor(
				new FormEditorInputAbonnement(), FormEditorFinalisationAbonnement.ID
			);
