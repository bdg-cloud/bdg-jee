...
import fr.legrain.abonnement.ecrans.PaCompositeSectionParam;
...
//	public static final String etatEnCoursCode = "ENCOURS";
	...
	private PaFormPageAvis vue = null;
	...
	private BigDecimal montantDepart=BigDecimal.ZERO;
	private boolean nonTransforme=true;
...
//	protected LgrModifyListener2 lgrModifyListener2 = new LgrModifyListener2();
	...
	public ParamControllerMini(FormPageControllerPrincipal masterContoller, PaFormPageAvis vue, EntityManager em) {
		this.vue = vue;
	...
	public void actCreer(IStructuredSelection selection,final ParamAffiche param ){
		try{
			masterController.getDocEcheanceController().actEnregistrer();
		}catch (Exception e) {
			logger.error("",e);
		}
	}
	public ParamControllerMini getThis() {
		return this;
	}
	/* Permet le rafraîchissement des differents composites quand on clique sur le bouton */
	private Action refreshAction = new Action("Rechercher",pluginAbonnement.getImageDescriptor(PaCompositeSectionParam.iconPath)) { 
		@Override 
		public void run() { 
			datedeb = LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb());
			...
			codeEtat = vue.getCompositeSectionParam().getCbEtat().getText();
			codeTiers = vue.getCompositeSectionParam().getTfCodeTiers().getText();
			nonTransforme = vue.getCompositeSectionParam().getBtnNonTransforme().getSelection();
			montantDepart = LibConversion.stringToBigDecimal(vue.getCompositeSectionParam().getTfMontantDepart().getText());
...
//						String contributorName = confElements[jj].getContributor().getName();
	...
	protected void initActions() {	
		if(!toolBarInitialise) {
			...
			vue.getCompositeSectionParam().getBtnRefesh().setImage(pluginAbonnement.getImageDescriptor(PaCompositeSectionParam.iconRefreshPath).createImage());
			...
			vue.getCompositeSectionResultats().getBtnCreer().setImage(pluginAbonnement.getImageDescriptor(PaCompositeSectionParam.iconRefreshPath).createImage());
			vue.getCompositeSectionResultats().getBtnCreer().addSelectionListener(new SelectionListener() {
				...
				public void widgetSelected(SelectionEvent e) {
					IStructuredSelection selection =((IStructuredSelection)masterController.getDocEcheanceController().
							getTableViewer().getSelection());
					IHMEnteteAvisEcheance documentSelection= (IHMEnteteAvisEcheance)selection.getFirstElement();
...
//					}				
			...
			vue.getCompositeSectionParam().getBtnAideTiers().setImage(pluginAbonnement.getImageDescriptor(PaCompositeSectionParam.iconFindPath).createImage());
...
//						&&listeGestionnaireEditorSupportAbon.containsKey(taTAbonnement.getCodeTAbonnement())
			...
			vue.getCompositeSectionParam().getBtnNonTransforme().setSelection(true);
			vue.getCompositeSectionParam().getTfMontantDepart().setText("0.1");
...
//			refreshAction.run();
		...
		//}
			...
			codeEtat = vue.getCompositeSectionParam().getCbEtat().getText();
			codeTiers = vue.getCompositeSectionParam().getTfCodeTiers().getText();
			vue.getCompositeSectionParam().getBtnNonTransforme().setEnabled(suppressionUniquement);
			vue.getCompositeSectionParam().getTfMontantDepart().setEnabled(suppressionUniquement);
...
//					ctrlUnChampsSWT(getFocusAvantAideSWT());
	...
	public BigDecimal getMontantDepart() {
		return montantDepart;
	}

	public boolean isNonTransforme() {
		return nonTransforme;
	}
