...
public class SWTPaAbonnementController extends JPABaseControllerSWTStandard 
implements RetourEcranListener, ISelectionListener{

	static Logger logger = Logger.getLogger(SWTPaAbonnementController.class.getName());	

	private PaAbonnementSWT vue = null;
...
//	private Impression impression = new Impression();
	...
	public SWTPaAbonnementController(PaAbonnementSWT vue) {
		this(vue,null);
	}

	public SWTPaAbonnementController(PaAbonnementSWT vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
...
//		daoDocumentTiers = new TaDocumentTiersDAO(getEm());
	...
	public void bind(PaAbonnementSWT paEpiceaSWT){
		bind(paEpiceaSWT,null);
	}
	
	public void bindID(PaAbonnementSWT paEpiceaSWT, int idAbonnement ){
	...
	public void bindCode(PaAbonnementSWT paEpiceaSWT, String code ){
		SWTAbonnement swtEpiceaTmp = new SWTAbonnement();
		modelAbonnement.mapping(dao.findByCode(code), swtEpiceaTmp);
		bind(paEpiceaSWT, swtEpiceaTmp);
	}
	
	public void bind(PaAbonnementSWT paEpiceaSWT, SWTAbonnement selection ){
		try {
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());
				dbc = new DataBindingContext(realm);
				if(dao.dataSetEnModif()){
					swtAbonnement = selection;
//					selection.setCodeSupportAbon(swtAbonnement.getCodeSupportAbon());
//					selection.setIdSupportAbon(swtAbonnement.getIdSupportAbon());
//					selection.setCodeTiers(swtAbonnement.getCodeTiers());
//					selection.setCodeTAbonnement(swtAbonnement.getCodeTAbonnement());
//					selection.setCodeArticle(swtAbonnement.getCodeArticle());
//					selection.setIdTiers(swtAbonnement.getIdTiers());
//					selection.setIdTAbonnement(swtAbonnement.getIdTAbonnement());
//					selection.setIdArticle(swtAbonnement.getIdArticle());					
//					selection.setDateDebut(swtAbonnement.getDateDebut());
//					selection.setDateFin(swtAbonnement.getDateFin());					
					modelAbonnement.mapping(selection,taAbonnement);
				}else{
				if(selection!=null && selection.getIdAbonnement()!=0) {
					taAbonnement =  dao.findById(selection.getIdAbonnement());
					
					/*selection =*/ modelAbonnement.mapping(taAbonnement, selection);
					swtAbonnement = selection;
				} else {
					if(swtAbonnement==null) {
						swtAbonnement = new SWTAbonnement();
					}
					selection = swtAbonnement;
				}
				}

				if(taAbonnement!=null && taAbonnement.getTaTiers()!=null)
					vue.getLaCodeTiers().setText("Code tiers : "+taAbonnement.getTaTiers().getCodeTiers());
	...
	private void changementDeSelection() {		
		if(swtAbonnement!=null) {
			...
			fireChangementDeSelection(new ChangementDeSelectionEvent(SWTPaAbonnementController.this));
		}
		initEtatComposant();
		vue.getLaHorsAbon().setVisible(false);
		if(swtAbonnement.getDateDebut()==null || swtAbonnement.getDateFin()==null){
...
//		mapComposantChamps.put(vue.getTfCodeAbonnement(),Const.C_CODE_ABONNEMENT);
	...
	public SWTPaAbonnementController getThis(){
		return this;
	}
...
//				validateUIField(Const.C_CODE_ABONNEMENT,swtAbonnement.getCodeAbonnement());//permet de verrouiller le code genere
		...
		try {
			...
			if(isUtilise())MessageDialog.openInformation(vue.getShell(), MessagesEcran
				...
				if (MessageDialog.openConfirm(vue.getShell(), MessagesEcran
					...
					TaAbonnement u = dao.findById(swtAbonnement.getIdAbonnement());
					dao.supprimer(u);
	...
	protected boolean aideDisponible() {
		...
		switch ((SWTPaAbonnementController.this.dao.getModeObjet().getMode())) {
		case C_MO_CONSULTATION:
		case C_MO_EDITION:
		case C_MO_INSERTION:										
		...
		try {
			IStatus s = null;
			 ...
			 if(nomChamp.equals(Const.C_CODE_ARTICLE)) {
				...
				if(swtAbonnement!=null ) {
					u.setIdAbonnement(swtAbonnement.getIdAbonnement());
				}else
					if (swtAbonnement!=null){
						swtAbonnement.setIdAbonnement(0);
					}
...
//						mapperUIToModel.map(swtBdg,taLicence);
					...
					if(!enregistreToutEnCours) {
						...
						swtAbonnement.setIdAbonnement(taAbonnement.getIdAbonnement());
...
//						!dao.autoriseModification(taLicence);	
		return (swtAbonnement!=null &&
				swtAbonnement.getIdTiers()!=null && !dao.recordModifiable(dao.getNomTable(),swtAbonnement.getIdTiers()))
	...
	public void setSwtOldLicence() {
		if(swtAbonnement!=null) {
			this.swtOld=SWTAbonnement.copy(swtAbonnement);
...
//			}
	...
	public void setVue(PaAbonnementSWT vue) {
		super.setVue(vue);
		this.vue = vue;
	}
...
//		mapComposantDecoratedField.put(vue.getTfCodeAbonnement(),vue.getFieldCodeAbonnement());
	...
	protected void actRefresh() throws Exception {
		...
		} else if(swtAbonnement!=null ) {
			idActuel = swtAbonnement.getIdAbonnement();
		}
		...
		if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION) == 0) {
			...
			if (taAbonnement!=null && swtAbonnement!=null) {
				mapperModelToUI.map(taAbonnement, swtAbonnement);
			}
		}
