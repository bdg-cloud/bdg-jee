...
public class ModelAbonnement /*extends AbstractConcurrentModel*/ implements IModelGeneral, ILgrListModel<TaAbonnement>  {
	static Logger logger = Logger.getLogger(ModelAbonnement.class.getName());
	LinkedList<SWTAbonnement> listeObjet = new LinkedList<SWTAbonnement>();
	private Collection<TaAbonnement> listeEntity = null;
	private String JPQLQuery = null;
	private Class<SWTAbonnement> typeObjet;
	private LgrDozerMapper<TaAbonnement,SWTAbonnement> mapperModelToUI = null;
	private IlgrMapper<SWTAbonnement, TaAbonnement> lgrMapper = null;
//	private AbstractLgrDAO<TaAbonnement> dao;
	...
	public class MyContentProvider implements IStructuredContentProvider  {
		...
		public WritableList elements = new WritableList(listeObjet, SWTAbonnement.class);
		...
		public void updateElement(int index) {
			...
			try {
//				if(dao==null) 
//					dao = new TaAbonnementDAO();
//				if(daoAbonnement==null)
//					daoAbonnement= new TaAbonnementDAO();
//				
//				SWTAbonnement t = remplirListeLazy(dao.getEntityManager(),index);
//				if(t==null) {
//					t = new SWTAbonnement();
...
//				viewer.replace(t, index);
	...
	public SWTAbonnement mapping(TaAbonnement abonnement, SWTAbonnement swtAbonnement) {
		if(daoAbonnement==null)
			daoAbonnement= new TaAbonnementDAO();
		if(swtAbonnement==null)
			swtAbonnement = new SWTAbonnement();
		if(abonnement==null)abonnement=new TaAbonnement();

		swtAbonnement.setIdAbonnement(abonnement.getIdAbonnement());
		if(abonnement.getTaArticle()!=null){
			swtAbonnement.setCodeArticle(abonnement.getTaArticle().getCodeArticle());
			swtAbonnement.setIdArticle(abonnement.getTaArticle().getIdArticle());
		}
		
		if(abonnement.getTaTAbonnement()!=null){
			swtAbonnement.setCodeTAbonnement(abonnement.getTaTAbonnement().getCodeTAbonnement());
			swtAbonnement.setIdTAbonnement(abonnement.getTaTAbonnement().getIdTAbonnement());
		}

		if(abonnement.getTaSupportAbon()!=null){
			swtAbonnement.setCodeSupportAbon(abonnement.getTaSupportAbon().getCodeSupportAbon());
			swtAbonnement.setIdSupportAbon(abonnement.getTaSupportAbon().getIdSupportAbon());
		}

		if(abonnement.getTaTiers()!=null){
			swtAbonnement.setIdTiers(abonnement.getTaTiers().getIdTiers());
			swtAbonnement.setCodeTiers(abonnement.getTaTiers().getCodeTiers());
		}
		swtAbonnement.setDateDebut(abonnement.getDateDebut());
		swtAbonnement.setDateFin(abonnement.getDateFin());
		swtAbonnement.setCommentaire(abonnement.getCommentaire());

		return swtAbonnement;
	}
	
	public TaAbonnement mapping( SWTAbonnement swtAbonnement,TaAbonnement abonnement) {
		if(abonnement==null)
			abonnement = new TaAbonnement();
		TaTiersDAO daoTiers=new TaTiersDAO();
		TaTiers tiers=null;
		TaArticleDAO daoArticle = new TaArticleDAO();
		TaArticle article =null;
		TaSupportAbonDAO daoSupportAbon = new TaSupportAbonDAO();
		TaSupportAbon supportAbon =null;
		TaTAbonnementDAO daoTAbonnement = new TaTAbonnementDAO();
		TaTAbonnement TAbonnement =null;

		try {
			if(swtAbonnement.getCodeTiers()!=null)
				tiers=daoTiers.findByCode(swtAbonnement.getCodeTiers());
		} catch (Exception e) {}
		try {
			if(swtAbonnement.getCodeArticle()!=null)
				article=daoArticle.findByCode(swtAbonnement.getCodeArticle());
		} catch (Exception e) {}
		try {
			if(swtAbonnement.getCodeSupportAbon()!=null)
				supportAbon=daoSupportAbon.findByCode(swtAbonnement.getCodeSupportAbon());
		} catch (Exception e) {}
		try {
			if(swtAbonnement.getCodeTAbonnement()!=null)
				TAbonnement=daoTAbonnement.findByCode(swtAbonnement.getCodeTAbonnement());
		} catch (Exception e) {}		

		abonnement.setDateDebut(swtAbonnement.getDateDebut());
		abonnement.setDateFin(swtAbonnement.getDateFin());
		abonnement.setIdAbonnement(swtAbonnement.getIdAbonnement());
		abonnement.setTaArticle(article);
		abonnement.setTaTiers(tiers);
		abonnement.setTaTAbonnement(TAbonnement);
		abonnement.setTaSupportAbon(supportAbon);
		abonnement.setCommentaire(swtAbonnement.getCommentaire());
		return abonnement;
	}
	
	public void addToModel(final TableViewer viewer, TaAbonnement licence) {
		SWTAbonnement swtEpicea = mapping(licence,null);
	...
	public void refreshModel(final TableViewer viewer, TaAbonnement licence) {
		SWTAbonnement swtEpicea = recherche(Const.C_ID_SUPPORT_ABON, licence.getIdAbonnement());
	...
	public void removeFromModel(final TableViewer viewer, TaAbonnement licence) {
		SWTAbonnement swtEpicea = recherche(Const.C_ID_SUPPORT_ABON, licence.getIdAbonnement());
...
//		Thread t = new Thread() {
//			public void run() {
//				TaAbonnementDAO dao =new TaAbonnementDAO();
//				Collection<TaAbonnement> l = null;
...
//				}
//				if(l != null) {
//					final Collection<TaAbonnement> lfinal = l;
...
//					
//					
//					for(TaAbonnement ti : l){
//						final TaAbonnement licence = ti;
...
//		t.start();
	...
	public SWTAbonnement remplirListeLazy(EntityManager em, int id) {
//		System.err.println("remplirListeLazy " + id);
//		TaAbonnementDAO dao =new TaAbonnementDAO(em);
		SWTAbonnement swtEpicea = null;
//		Date dateDeb = new Date();
//		Collection<TaAbonnement> l = new ArrayList<TaAbonnement>();
//		if(listeEntity==null) {
//			listeEntity = new LinkedList<TaAbonnement>();
//		}
//		TaAbonnement t = dao.findById(id);
//		if(t!=null) {
//			listeEntity.add(t);
//
//			TaAbonnement tiers = t;
//			swtEpicea = new SWTAbonnement();
...
//						if(licence.getAccepte()!=null)swtEpicea.setAccepte(licence.getAccepte());else swtEpicea.setAccepte(false);
	...
	public LinkedList<SWTAbonnement> remplirListe(EntityManager em) {
//		TaAbonnementDAO dao =new TaAbonnementDAO(em);
//		SWTAbonnement swtEpicea = null;
//		Date dateDeb = new Date();
//		Collection<TaAbonnement> l = null;
...
//		logger.info("temp remplirListe "+new Date(dateFin.getTime()-dateDeb.getTime()));
	...
	public SWTAbonnement recherche(String propertyName, Object value, boolean startWith) {
	...
	public SWTAbonnement recherche(String propertyName, Object value) {
		return recherche(propertyName,value,false);
	}
	
	public LinkedList<SWTAbonnement> getListeObjet() {
		return listeObjet;
	}

	public Collection<TaAbonnement> getListeEntity() {
		return listeEntity;
	}

	public void setListeEntity(Collection<TaAbonnement> listeEntity) {
		this.listeEntity = listeEntity;
	}

	public void setListeObjet(LinkedList<SWTAbonnement> listeObjet) {
	...
	public void customMapper(CustomFieldMapperIFLGR custom){
		//passage EJB
//		if(mapperModelToUI==null)mapperModelToUI=new LgrDozerMapper<TaAbonnement, SWTAbonnement>();
//		mapperModelToUI.getMapper().setCustomFieldMapper(custom);
	}

	public LgrDozerMapper<TaAbonnement, SWTAbonnement> getMapperModelToUI() {
		return mapperModelToUI;
	}

	public void setMapperModelToUI(LgrDozerMapper<TaAbonnement, SWTAbonnement> mapperModelToUI) {
