...
import fr.legrain.lgrBackup.dao.TaLgrBackup;
import fr.legrain.lgrBackup.dao.TaLgrBackupDAO;
...
public class ModelLgrBackup /*extends AbstractConcurrentModel*/ implements IModelGeneral, ILgrListModel<TaLgrBackup>  {
	static Logger logger = Logger.getLogger(ModelLgrBackup.class.getName());
	LinkedList<SWTSupportAbonLogiciel> listeObjet = new LinkedList<SWTSupportAbonLogiciel>();
	private Collection<TaLgrBackup> listeEntity = null;
	private String JPQLQuery = null;
	private Class<SWTSupportAbonLogiciel> typeObjet;
	private LgrDozerMapper<TaLgrBackup,SWTSupportAbonLogiciel> mapperModelToUI = null;
	private IlgrMapper<SWTSupportAbonLogiciel, TaLgrBackup> lgrMapper = null;
//	private AbstractLgrDAO<TaLgrBackup> dao;
	private TaLgrBackupDAO dao;
	...
	public class MyContentProvider implements IStructuredContentProvider  {
		...
		public void updateElement(int index) {
			...
			try {
				if(dao==null) 
					dao = new TaLgrBackupDAO();
				if(daoAbonnement==null)
					daoAbonnement= new TaAbonnementDAO();
				
				SWTSupportAbonLogiciel t = remplirListeLazy(entityManager,index);
				if(t==null) {
	...
	public SWTSupportAbonLogiciel mapping(TaLgrBackup licence, SWTSupportAbonLogiciel swtSupport) {
		if(daoAbonnement==null)
			daoAbonnement= new TaAbonnementDAO();
		if(swtSupport==null)
			swtSupport = new SWTSupportAbonLogiciel();
		if(licence==null)licence=new TaLgrBackup();
	...
	public TaLgrBackup mapping( SWTSupportAbonLogiciel swtEpicea,TaLgrBackup licence) {
		if(licence==null)
			licence = new TaLgrBackup();
	...
	public void addToModel(final TableViewer viewer, TaLgrBackup licence) {
	...
	public void refreshModel(final TableViewer viewer, TaLgrBackup licence) {
	...
	public void removeFromModel(final TableViewer viewer, TaLgrBackup licence) {
		SWTSupportAbonLogiciel swtEpicea = recherche(Const.C_ID_SUPPORT_ABON, licence.getIdSupportAbon());
	...
	public void initCPThread(final TableViewer viewer, final ViewPart v,final ProgressBar bar, 
		...
		Thread t = new Thread() {
			public void run() {
				TaLgrBackupDAO dao =new TaLgrBackupDAO();
				Collection<TaLgrBackup> l = null;
...
//					}
				...
				if(l != null) {
					final Collection<TaLgrBackup> lfinal = l;
					...
					for(TaLgrBackup ti : l){
						final TaLgrBackup licence = ti;
	...
	public SWTSupportAbonLogiciel remplirListeLazy(EntityManager em, int id) {
		...
		Collection<TaLgrBackup> l = new ArrayList<TaLgrBackup>();
		if(listeEntity==null) {
			listeEntity = new LinkedList<TaLgrBackup>();
		}
		TaLgrBackup t = dao.findById(id);
		if(t!=null) {
			listeEntity.add(t);
...
//				if(licence.getAccepte()!=null)swtEpicea.setAccepte(licence.getAccepte());else swtEpicea.setAccepte(false);
	...
	public Collection<TaLgrBackup> getListeEntity() {
		return listeEntity;
	}

	public void setListeEntity(Collection<TaLgrBackup> listeEntity) {
	...
	public void customMapper(CustomFieldMapperIFLGR custom){
		//passage EJB
//		if(mapperModelToUI==null)mapperModelToUI=new LgrDozerMapper<TaLgrBackup, SWTSupportAbonLogiciel>();
//		mapperModelToUI.getMapper().setCustomFieldMapper(custom);
	}

	public LgrDozerMapper<TaLgrBackup, SWTSupportAbonLogiciel> getMapperModelToUI() {
		return mapperModelToUI;
	}

	public void setMapperModelToUI(LgrDozerMapper<TaLgrBackup, SWTSupportAbonLogiciel> mapperModelToUI) {
