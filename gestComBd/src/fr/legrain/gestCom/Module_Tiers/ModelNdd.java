...
import fr.legrain.supportNDD.dao.TaNDD;
import fr.legrain.supportNDD.dao.TaNDDDAO;
import fr.legrain.tiers.dao.TaFamilleTiers;
import fr.legrain.tiers.dao.TaFamilleTiersDAO;
import fr.legrain.tiers.dao.TaTiers;
import fr.legrain.tiers.dao.TaTiersDAO;

public class ModelNdd /*extends AbstractConcurrentModel*/ implements IModelGeneral, ILgrListModel<TaNDD>  {
	static Logger logger = Logger.getLogger(ModelNdd.class.getName());
	LinkedList<SWTSupportAbonLogiciel> listeObjet = new LinkedList<SWTSupportAbonLogiciel>();
	private Collection<TaNDD> listeEntity = null;
	private String JPQLQuery = null;
	private Class<SWTSupportAbonLogiciel> typeObjet;
	private LgrDozerMapper<TaNDD,SWTSupportAbonLogiciel> mapperModelToUI = null;
	private IlgrMapper<SWTSupportAbonLogiciel, TaNDD> lgrMapper = null;
//	private AbstractLgrDAO<TaNDD> dao;
...
//				viewer.replace(t, index);
	...
	public SWTSupportAbonLogiciel mapping(TaNDD licence, SWTSupportAbonLogiciel swtSupport) {
		if(daoAbonnement==null)
			daoAbonnement= new TaAbonnementDAO();
		if(swtSupport==null)
			swtSupport = new SWTSupportAbonLogiciel();
		if(licence==null)licence=new TaNDD();
	...
	public TaNDD mapping( SWTSupportAbonLogiciel swtEpicea,TaNDD licence) {
		if(licence==null)
			licence = new TaNDD();
	...
	public void addToModel(final TableViewer viewer, TaNDD licence) {
	...
	public void refreshModel(final TableViewer viewer, TaNDD licence) {
	...
	public void removeFromModel(final TableViewer viewer, TaNDD licence) {
		SWTSupportAbonLogiciel swtEpicea = recherche(Const.C_ID_SUPPORT_ABON, licence.getIdSupportAbon());
	...
	public void initCPThread(final TableViewer viewer, final ViewPart v,final ProgressBar bar, 
		...
		Thread t = new Thread() {
			public void run() {
				TaNDDDAO dao =new TaNDDDAO();
				Collection<TaNDD> l = null;
...
//					}
				...
				if(l != null) {
					final Collection<TaNDD> lfinal = l;
	...
	public SWTSupportAbonLogiciel remplirListeLazy(EntityManager em, int id) {
		...
		Collection<TaNDD> l = new ArrayList<TaNDD>();
		if(listeEntity==null) {
			listeEntity = new LinkedList<TaNDD>();
		}
		TaNDD t = dao.findById(id);
		if(t!=null) {
			listeEntity.add(t);
...
//				if(licence.getAccepte()!=null)swtEpicea.setAccepte(licence.getAccepte());else swtEpicea.setAccepte(false);
	...
	public Collection<TaNDD> getListeEntity() {
		return listeEntity;
	}

	public void setListeEntity(Collection<TaNDD> listeEntity) {
	...
	public void customMapper(CustomFieldMapperIFLGR custom){
		//passage EJB
//		if(mapperModelToUI==null)mapperModelToUI=new LgrDozerMapper<TaNDD, SWTSupportAbonLogiciel>();
//		mapperModelToUI.getMapper().setCustomFieldMapper(custom);
	}

	public LgrDozerMapper<TaNDD, SWTSupportAbonLogiciel> getMapperModelToUI() {
		return mapperModelToUI;
	}

	public void setMapperModelToUI(LgrDozerMapper<TaNDD, SWTSupportAbonLogiciel> mapperModelToUI) {
