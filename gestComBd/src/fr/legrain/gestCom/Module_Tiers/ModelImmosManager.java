...
import fr.legrain.supportImmosManager.dao.TaImmosManager;
import fr.legrain.supportImmosManager.dao.TaImmosManagerDAO;
import fr.legrain.tiers.dao.TaFamilleTiers;
import fr.legrain.tiers.dao.TaFamilleTiersDAO;
import fr.legrain.tiers.dao.TaTiers;
import fr.legrain.tiers.dao.TaTiersDAO;

public class ModelImmosManager /*extends AbstractConcurrentModel*/ implements IModelGeneral, ILgrListModel<TaImmosManager>  {
	static Logger logger = Logger.getLogger(ModelImmosManager.class.getName());
	LinkedList<SWTSupportAbonLogiciel> listeObjet = new LinkedList<SWTSupportAbonLogiciel>();
	private Collection<TaImmosManager> listeEntity = null;
	private String JPQLQuery = null;
	private Class<SWTSupportAbonLogiciel> typeObjet;
	private LgrDozerMapper<TaImmosManager,SWTSupportAbonLogiciel> mapperModelToUI = null;
	private IlgrMapper<SWTSupportAbonLogiciel, TaImmosManager> lgrMapper = null;
//	private AbstractLgrDAO<TaImmosManager> dao;
...
//				viewer.replace(t, index);
	...
	public SWTSupportAbonLogiciel mapping(TaImmosManager licence, SWTSupportAbonLogiciel swtSupport) {
		if(daoAbonnement==null)
			daoAbonnement= new TaAbonnementDAO();
		if(swtSupport==null)
			swtSupport = new SWTSupportAbonLogiciel();
		if(licence==null)licence=new TaImmosManager();
	...
	public TaImmosManager mapping( SWTSupportAbonLogiciel swtEpicea,TaImmosManager licence) {
		if(licence==null)
			licence = new TaImmosManager();
	...
	public void addToModel(final TableViewer viewer, TaImmosManager licence) {
	...
	public void refreshModel(final TableViewer viewer, TaImmosManager licence) {
	...
	public void removeFromModel(final TableViewer viewer, TaImmosManager licence) {
		SWTSupportAbonLogiciel swtEpicea = recherche(Const.C_ID_SUPPORT_ABON, licence.getIdSupportAbon());
	...
	public void initCPThread(final TableViewer viewer, final ViewPart v,final ProgressBar bar, 
		...
		Thread t = new Thread() {
			public void run() {
				TaImmosManagerDAO dao =new TaImmosManagerDAO();
				Collection<TaImmosManager> l = null;
...
//					}
				...
				if(l != null) {
					final Collection<TaImmosManager> lfinal = l;
					...
					for(TaImmosManager ti : l){
						final TaImmosManager licence = ti;
...
//		TaImmosManagerDAO dao =new TaImmosManagerDAO(em);
		SWTSupportAbonLogiciel swtEpicea = null;
//		Date dateDeb = new Date();
//		Collection<TaImmosManager> l = new ArrayList<TaImmosManager>();
//		if(listeEntity==null) {
//			listeEntity = new LinkedList<TaImmosManager>();
//		}
//		TaImmosManager t = dao.findById(id);
//		if(t!=null) {
//			listeEntity.add(t);
//
//			TaImmosManager tiers = t;
...
//		logger.info("temp remplirListe "+new Date(dateFin.getTime()-dateDeb.getTime()));
	...
	public Collection<TaImmosManager> getListeEntity() {
		return listeEntity;
	}

	public void setListeEntity(Collection<TaImmosManager> listeEntity) {
	...
	public void customMapper(CustomFieldMapperIFLGR custom){
		//passage EJB
//		if(mapperModelToUI==null)mapperModelToUI=new LgrDozerMapper<TaImmosManager, SWTSupportAbonLogiciel>();
//		mapperModelToUI.getMapper().setCustomFieldMapper(custom);
	}

	public LgrDozerMapper<TaImmosManager, SWTSupportAbonLogiciel> getMapperModelToUI() {
		return mapperModelToUI;
	}

	public void setMapperModelToUI(LgrDozerMapper<TaImmosManager, SWTSupportAbonLogiciel> mapperModelToUI) {
