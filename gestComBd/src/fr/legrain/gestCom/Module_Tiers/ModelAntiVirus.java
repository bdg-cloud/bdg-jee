...
import fr.legrain.antiVirus.dao.TaAntiVirus;
import fr.legrain.antiVirus.dao.TaAntiVirusDAO;
...
public class ModelAntiVirus /*extends AbstractConcurrentModel*/ implements IModelGeneral, ILgrListModel<TaAntiVirus>  {
	static Logger logger = Logger.getLogger(ModelAntiVirus.class.getName());
	LinkedList<SWTSupportAbonLogiciel> listeObjet = new LinkedList<SWTSupportAbonLogiciel>();
	private Collection<TaAntiVirus> listeEntity = null;
	private String JPQLQuery = null;
	private Class<SWTSupportAbonLogiciel> typeObjet;
	private LgrDozerMapper<TaAntiVirus,SWTSupportAbonLogiciel> mapperModelToUI = null;
	private IlgrMapper<SWTSupportAbonLogiciel, TaAntiVirus> lgrMapper = null;
//	private AbstractLgrDAO<TaAntiVirus> dao;
...
//				viewer.replace(t, index);
	...
	public SWTSupportAbonLogiciel mapping(TaAntiVirus licence, SWTSupportAbonLogiciel swtSupport) {
		if(daoAbonnement==null)
			daoAbonnement= new TaAbonnementDAO();
		if(swtSupport==null)
			swtSupport = new SWTSupportAbonLogiciel();
		if(licence==null)licence=new TaAntiVirus();
	...
	public TaAntiVirus mapping( SWTSupportAbonLogiciel swtEpicea,TaAntiVirus licence) {
		if(licence==null)
			licence = new TaAntiVirus();
	...
	public void addToModel(final TableViewer viewer, TaAntiVirus licence) {
	...
	public void refreshModel(final TableViewer viewer, TaAntiVirus licence) {
	...
	public void removeFromModel(final TableViewer viewer, TaAntiVirus licence) {
		SWTSupportAbonLogiciel swtEpicea = recherche(Const.C_ID_SUPPORT_ABON, licence.getIdSupportAbon());
	...
	public void initCPThread(final TableViewer viewer, final ViewPart v,final ProgressBar bar, 
		...
		Thread t = new Thread() {
			public void run() {
				TaAntiVirusDAO dao =new TaAntiVirusDAO();
				Collection<TaAntiVirus> l = null;
...
//					listeEntity = l;
				...
				if(l != null) {
					final Collection<TaAntiVirus> lfinal = l;
					...
					for(TaAntiVirus ti : l){
						final TaAntiVirus licence = ti;
...
//		TaAntiVirusDAO dao =new TaAntiVirusDAO(em);
		SWTSupportAbonLogiciel swtEpicea = null;
//		Date dateDeb = new Date();
//		Collection<TaAntiVirus> l = new ArrayList<TaAntiVirus>();
//		if(listeEntity==null) {
//			listeEntity = new LinkedList<TaAntiVirus>();
//		}
//		TaAntiVirus t = dao.findById(id);
//		if(t!=null) {
//			listeEntity.add(t);
//
//			TaAntiVirus tiers = t;
...
//		logger.info("temp remplirListe "+new Date(dateFin.getTime()-dateDeb.getTime()));
	...
	public Collection<TaAntiVirus> getListeEntity() {
		return listeEntity;
	}

	public void setListeEntity(Collection<TaAntiVirus> listeEntity) {
	...
	public void customMapper(CustomFieldMapperIFLGR custom){
		//passage EJB
//		if(mapperModelToUI==null)mapperModelToUI=new LgrDozerMapper<TaAntiVirus, SWTSupportAbonLogiciel>();
//		mapperModelToUI.getMapper().setCustomFieldMapper(custom);
	}

	public LgrDozerMapper<TaAntiVirus, SWTSupportAbonLogiciel> getMapperModelToUI() {
		return mapperModelToUI;
	}

	public void setMapperModelToUI(LgrDozerMapper<TaAntiVirus, SWTSupportAbonLogiciel> mapperModelToUI) {
