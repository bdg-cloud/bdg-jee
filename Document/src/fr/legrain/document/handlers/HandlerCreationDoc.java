...
public class HandlerCreationDoc extends LgrAbstractHandler {
	
	static Logger logger = Logger.getLogger(HandlerCreationDoc.class.getName());
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			IEditorPart editor = EJBLgrEditorPart.verifEditeurOuvert(EditorCreationDoc.ID);
			if(editor==null){
			IEditorPart e = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().openEditor(new EditorInputCreationDoc(), EditorCreationDoc.ID);
			...
			((EJBLgrEditorPart)e).getController().configPanel(paramAfficheCreationDoc);
			} else {
