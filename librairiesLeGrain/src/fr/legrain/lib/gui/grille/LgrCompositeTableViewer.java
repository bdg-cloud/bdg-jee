...
public class LgrCompositeTableViewer<T> /*extends CheckboxTableViewer*/ {

	static Logger logger = Logger.getLogger(LgrCompositeTableViewer.class.getName());
...
//	 * {@inheritDoc}
//	 */
//	public LgrCompositeTableViewer(Composite parent) {
//		super(parent);
//	}
//
//	
...
//	 * {@inheritDoc}
//	 */
//	public LgrCompositeTableViewer(Composite parent, int style) {
//		super(parent, style);
//	}
//
//	public LgrCompositeTableViewer(Table table) {
//		super(table);
//    
//	}
	
	public LgrCompositeTableViewer(TableViewer viewer) {
		this.viewer = viewer;
	}
	
	public LgrCompositeTableViewer(CheckboxTableViewer viewer) {
		this.viewer = viewer;
	}
	...
	public boolean selectionGrille(int row) {
		...
		if (row>0 && row<viewer.getTable().getItemCount()){
			s = new StructuredSelection(viewer.getElementAt(row));
		} else {
			if(viewer.getTable().getItemCount()>0)
				if (viewer.getElementAt(0)!=null)
					s = new StructuredSelection(viewer.getElementAt(0));
		}
		if(s!=null) {
			viewer.setSelection(s,true);
			//viewer.setSelectionToWidget(s, true);
			result =true;
		}
		return result;
	}
	...
	public int selectionGrille(Object row) {
		int rang = 0;
		if (row!=null ){
			StructuredSelection s = new StructuredSelection(row);
			viewer.setSelection(s,true);
			rang = viewer.getTable().getSelectionIndex();
			//viewer.findItem(row);
			//rang = viewer.indexForElement(row)-1;
		}
		return rang;
	}
	...
	public int findPositionColonne(String titreColonne){
		...
		int i = 0;
		while(!trouve && i<viewer.getTable().getColumns().length){
			if(viewer.getTable().getColumns()[i].getText().equals(titreColonne)) {
				trouve = true;
			}
			i++;
		}
	...
	public String findTitreColonne(String titreColonne){
		...
		if(pos!=-1) {
			return viewer.getTable().getColumns()[pos].getText();
		} else {
			return null;
		}
	}
...
//			navigationStrategy();
		...
		if(column!=null) {
			...
			viewer.setComparator(new LgrComparator(column,SWT.UP));
		}
	}
...
//   			navigationStrategy();
	...
	public void videTableViewer(){
		...
		while(viewer.getTable().getColumnCount()>0){
			viewer.getTable().getColumn(viewer.getTable().getColumnCount()-1).setData(null);
			viewer.getTable().getColumn(viewer.getTable().getColumnCount()-1).dispose();
		}
	}
...
////});
	...
	public void brancheComparateur(final ViewerSorter comparateur){
		if(comparateur!=null){
			viewer.setSorter(comparateur);
		}
	}
	
//	public void brancheComparateur(){
...
//		});
//	}
	private Map<TableColumn,LgrComparator> mapComparatorUp = new HashMap<TableColumn,LgrComparator>();
	private Map<TableColumn,LgrComparator> mapComparatorDown = new HashMap<TableColumn,LgrComparator>();
			...
			this.typeObjetCompare = typeObjetCompare;
			 ...
			 Listener sortListener = new Listener() {
				 public void handleEvent(Event e) {
					 ...
					 Map<TableColumn,LgrComparator> mapComparator = null;
					 ...
					 //viewer.setSorter(new LgrSorter(currentColumn,dir));
					 if(dir == SWT.UP)
						 mapComparator = mapComparatorUp;
					 else
						mapComparator = mapComparatorDown;
					 
					 if(mapComparator.get(currentColumn)!=null) {
						 viewer.setComparator(mapComparator.get(currentColumn));
						 //logger.error("old");
					 } else {
						 mapComparator.put(currentColumn, new LgrComparator(currentColumn,dir));
						 viewer.setComparator(mapComparator.get(currentColumn));
						 //logger.error("new");
					 }
			 ...
			 for (int i = 0; i<viewer.getTable().getColumns().length; i++) {
				 viewer.getTable().getColumns()[i].addListener(SWT.Selection, sortListener);
			 }
	...
	private class LgrComparator extends ViewerComparator {
		private TableColumn column = null;
		private int dir = SWT.DOWN;
		private int i = -1;
		private boolean trouve = false;
		 ...
		 * @param dir - sens du tri (SWT.UP/SWT.DOWN)
		 */
		public LgrComparator(TableColumn column, int dir) {
			super();
			this.column = column;
			this.dir = dir;
	...
	CellNavigationStrategy naviStrat = new CellNavigationStrategy() {
		...
		public ViewerCell findSelectedCell(ColumnViewer viewer,
			...
			if (cell != null) {
				TableColumn t = LgrCompositeTableViewer.this.viewer.getTable().getColumn(
						cell.getColumnIndex());
				LgrCompositeTableViewer.this.viewer.getTable().showColumn(t);
			}
