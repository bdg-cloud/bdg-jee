...
import fr.legrain.reglement.reglementEnListe.FormPageControllerPrincipalReglement;
...
public class FormEditorReglement extends LgrFormEditor implements ILgrEditor, ILgrRetourEditeur, /*IEditorArticlesExtension,*/IPartListener{

	static Logger logger = Logger.getLogger(FormEditorReglement.class.getName());
	
	public static final String ID = "fr.legrain.reglement.reglementSurListe";
	...
	// FormPage principal
	private PaFormPageReglement defaultFormPage = null;
	private PaFormPageReglement remonterFormPage = null;
	private Map<PaFormPageReglement,FormPageControllerPrincipalReglement> listeController = new HashMap<PaFormPageReglement,FormPageControllerPrincipalReglement>();
			
	// Constructeur par défaut
	public FormEditorReglement() {
		super();
	}
	...
	protected void addPages() {
		try {
			...
			defaultFormPage = new PaFormPageReglement(this, PaFormPageReglement.id, PaFormPageReglement.title);
			remonterFormPage = new PaFormPageReglement(this, PaFormPageReglement.id, PaFormPageReglement.titleRemonter);
			// Création du Controller
			FormPageControllerPrincipalReglement defautController = new FormPageControllerPrincipalReglement(defaultFormPage);
			FormPageControllerPrincipalReglement remonterController = new FormPageControllerPrincipalReglement(remonterFormPage);
			...
			setPageImage(1, Activator.getImageDescriptor(iconPageTB).createImage());
	 ...
	 */
	private void liaisonControllerFormPage(FormPageControllerPrincipalReglement leController, PaFormPageReglement leFormPage) {
		leFormPage.setControllerPage((FormPageControllerPrincipalReglement)leController);
			...
			for (int i = 0; i < extensions.length; i++) {
				...
				for (int jj = 0; jj < confElements.length; jj++) {
					try {
						...
						addEditor((PaFormPageReglement)o,(FormPageControllerPrincipalReglement)c,editorName,icon);
...
//						}
	...
	private void addEditor(PaFormPageReglement e, FormPageControllerPrincipalReglement c, String label) {
		 addEditor(e,c,label,null);
	}
	
	private void addEditor(PaFormPageReglement e, FormPageControllerPrincipalReglement controllerPrincipal, String label, ImageDescriptor icon) {
...
//		}
	...
	public String getTitle() {
		return "Gestion des documents à règler";
	}
