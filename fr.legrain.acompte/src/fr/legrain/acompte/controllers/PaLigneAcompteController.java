...
import fr.legrain.acompte.ecrans.PaSaisieLigneSWT;
...
import fr.legrain.documents.dao.TaLAcompteDAO;
...
import fr.legrain.gestCom.Module_Document.IHMLAcompte;
import fr.legrain.gestCom.Module_Document.IHMLFacture;
...
public class PaLigneAcompteController extends JPABaseControllerSWTStandard
implements RetourEcranListener, SWTModificationDocumentListener, ISelectionProvider {

	static Logger logger = Logger.getLogger(PaLigneAcompteController.class.getName());
	
	public static final String C_COMMAND_LIGNE_FACTURE_AIDE_ID 			= "fr.legrain.ligneAcompte.aide";
	public static final String C_COMMAND_LIGNE_FACTURE_ANNULER_ID 		= "fr.legrain.ligneAcompte.annuler";
	public static final String C_COMMAND_LIGNE_FACTURE_ENREGISTRER_ID 	= "fr.legrain.ligneAcompte.enregsitrer";
	public static final String C_COMMAND_LIGNE_FACTURE_FERMER_ID 		= "fr.legrain.ligneAcompte.fermer";
	public static final String C_COMMAND_LIGNE_FACTURE_IMPRIMER_ID 		= "fr.legrain.ligneAcompte.imprimer";
	public static final String C_COMMAND_LIGNE_FACTURE_INSERER_ID 		= "fr.legrain.ligneAcompte.inserer";
	public static final String C_COMMAND_LIGNE_FACTURE_MODIFIER_ID 		= "fr.legrain.ligneAcompte.modifier";
	public static final String C_COMMAND_LIGNE_FACTURE_SUPPRIMER_ID 	= "fr.legrain.ligneAcompte.supprimer";
	public static final String C_COMMAND_LIGNE_FACTURE_REFRESH_ID 	    = "fr.legrain.ligneAcompte.refresh";
	public static final String C_COMMAND_LIGNE_FACTURE_ENTETE_ENREGISTRER_ID 	= "fr.legrain.ligneAcompteEntete.enregsitrer";
	protected ActionEnregistrerAcompte actionEnregistrerAcompte = new ActionEnregistrerAcompte("Enregistrer Acompte [Ctrl+F3]");
	protected HandlerEnregistrerAcompte handlerEnregistrerAcompte = new HandlerEnregistrerAcompte();
	
	
	public static final String C_COMMAND_LIGNE_FACTURE_AJOUT_ID 		= "fr.legrain.ligneAcompte.ajout";
	...
	public static final String C_COMMAND_LIGNE_FACTURE_AIDETVA_ID 		= "fr.legrain.ligneAcompte.aideTva";
...
//	private TaAcompte taAcompte = new TaAcompte();
//	private SWT_IB_TA_L_FACTURE ibTaTable = new SWT_IB_TA_L_FACTURE(taAcompte);
	private TaLAcompteDAO dao = null;//new TaLAcompteDAO();
	//private WorkbenchPart workbenchPart = null;

	private Object ecranAppelant = null;
	private IHMLAcompte ihmLAcompte;
	private IHMTotauxLignesDocument ihmTotauxLignesDocument;
	private IHMLAcompte ihmOldLAcompte;
	private Realm realm;
	private DataBindingContext dbc;
	private DataBindingContext dbcTotaux = null;
	public List<TaArticle> listeArticles = null;
//	private ModelLigneAcompte modelLAcompte /*=  new ModelLigneAcompte(ibTaTable)*/;
	private Class classModel = IHMLAcompte.class;
	private ModelGeneralObjet<IHMLAcompte,TaLAcompteDAO,TaLAcompte> modelLAcompte /*= new ModelGeneralObjet<IHMLAcompte,TaLAcompteDAO,TaLAcompte>(dao,classModel)*/;
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private IObservableValue selectedLigneAcompte;
	private Object selectedTotauxLigneAcompte;
	...
	private TaLAcompte taLAcompte = null;
	...
	private LgrDozerMapper<IHMLAcompte,TaLAcompte> mapperUIToModel = new LgrDozerMapper<IHMLAcompte,TaLAcompte>();
	private LgrDozerMapper<TaLAcompte,IHMLAcompte> mapperModelToUI = new LgrDozerMapper<TaLAcompte,IHMLAcompte>();
	
	private LgrDozerMapper<TaAcompte,IHMTotauxLignesDocument> mapperModelToUITotauxLignes = 
		new LgrDozerMapper<TaAcompte,IHMTotauxLignesDocument>();
	
	public PaLigneAcompteController(){
		
	}
	
	public PaLigneAcompteController(final PaSaisieLigneSWT vue) {
		this(vue,null);
	}
	
	public PaLigneAcompteController(final PaSaisieLigneSWT vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		try {
			dao = new TaLAcompteDAO(getEm());
			setVue(vue);
...
//							ibTaTable.inverseNumLigne(rang,rangTmp, ((SWTLigneAcompte)masterEntity.getLigne(rang)).getID_L_FACTURE());
...
//							ibTaTable.initLigneCourantSurRow();
	 ...
	 * @return
	 */
	public List<IHMLAcompte> IHMmodel() {
		LinkedList<TaLAcompte> ldao = new LinkedList<TaLAcompte>();
		LinkedList<IHMLAcompte> lswt = new LinkedList<IHMLAcompte>();
		
		if(masterEntity!=null && !masterEntity.getLignes().isEmpty()) {

			ldao.addAll(masterEntity.getLignes());

			LgrDozerMapper<TaLAcompte,IHMLAcompte> mapper = new LgrDozerMapper<TaLAcompte,IHMLAcompte>();
			for (TaLAcompte o : ldao) {
				IHMLAcompte t = null;
				t = (IHMLAcompte) mapper.map(o, IHMLAcompte.class);
				lswt.add(t);
			}
	...
	public void bind(PaSaisieLigneSWT paArticleSWT) {
		try {
			if (modelLAcompte==null)modelLAcompte = new ModelGeneralObjet<IHMLAcompte, TaLAcompteDAO, TaLAcompte>(dao,classModel);
...
//				tableViewer.setInput(writableList);
				...
				selectedLigneAcompte = ViewersObservables.observeSingleSelection(tableViewer);
			}
			...
			bindingFormMaitreDetail(dbc, realm, selectedLigneAcompte,classModel);
...
//						select.widgetSelected(null);
	...
	public void bindTotaux() {
		try {
			...
			selectedTotauxLigneAcompte = ihmTotauxLignesDocument;
			dbcTotaux = new DataBindingContext(realm);

//			dbc.getValidationStatusMap().addMapChangeListener(changeListener);
//			setDbcStandard(dbc);

			bindingFormSimple(masterDAO, mapComposantChampsTotauxLignes, dbcTotaux, realm, selectedTotauxLigneAcompte, IHMTotauxLignesDocument.class);
...
//			ibTaTable.ouvreDataset();
			if (((ParamAfficheLAcompte)param).getFocusSWT()!=null)
				((ParamAfficheLAcompte)param).getFocusSWT().forceFocus();
			else ((ParamAfficheLAcompte)param).setFocusSWT(vue.getTfCODE_ARTICLE());			
		...
		if (param.getModeEcran() != null
			...
			try {
				insertionLigne(false,((ParamAfficheLAcompte)param).getInitFocus());
				//actInserer(((ParamAfficheLAcompte)param).getInitFocus());
			} catch (Exception e) {
				//vue.getLaMessage().setText(e.getMessage());
				logger.error("", e);
			}
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfQTE_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_QTE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfQTE2_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_QTE2_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfMT_HT_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_MT_HT_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_MT_TTC_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfPRIX_U_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_PRIX_U_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfREM_TX_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_REM_TX_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfNUM_LIGNE_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_NUM_LIGNE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfCODE_ARTICLE(), new InfosVerifSaisie(new TaArticle(),Const.C_CODE_ARTICLE,null));
		mapInfosVerifSaisie.put(vue.getTfLIB_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_LIB_L_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfU1_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_U1_L_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfU2_L_FACTURE(), new InfosVerifSaisie(new TaLAcompte(),Const.C_U1_L_DOCUMENT,null));
...
//		vue.getTfQTE_L_FACTURE().addVerifyListener(dao.initialiseVerifyListener(new TaLAcompte(),Const.C_QTE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
//		vue.getTfQTE2_L_FACTURE().addVerifyListener(dao.initialiseVerifyListener(new TaLAcompte(),Const.C_QTE2_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
//		vue.getTfMT_HT_L_FACTURE().addVerifyListener(dao.initialiseVerifyListener(new TaLAcompte(),Const.C_MT_HT_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
//		vue.getTfMT_TTC_L_FACTURE().addVerifyListener(dao.initialiseVerifyListener(new TaLAcompte(),Const.C_MT_TTC_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
//		vue.getTfPRIX_U_L_FACTURE().addVerifyListener(dao.initialiseVerifyListener(new TaLAcompte(),Const.C_PRIX_U_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
//		vue.getTfREM_TX_L_FACTURE().addVerifyListener(dao.initialiseVerifyListener(new TaLAcompte(),Const.C_REM_TX_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
//		vue.getTfNUM_LIGNE_L_FACTURE().addVerifyListener(dao.initialiseVerifyListener(new TaLAcompte(),Const.C_NUM_LIGNE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
...
//			articleContentProposal.setContentProposalProvider(contentProposalProviderArticles());
			...
			articleContentProposal.addContentProposalListener(new IContentProposalListener2(){
				...
				public void proposalPopupOpened(ContentProposalAdapter adapter) {
					...
					if(!dao.dataSetEnModif()&& taLAcompte.ligneEstVide()  )
	...
	public void initWorkenchPartCommands(WorkbenchPart part){
		...
		handlerService.activateHandler(C_COMMAND_LIGNE_FACTURE_ENTETE_ENREGISTRER_ID, handlerEnregistrerAcompte,activePartExpression);
			
	}

	public PaLigneAcompteController getThis() {
		return this;
	}
			...
			if (getFocusAvantAideSWT() instanceof Text) {
				try {
					...
					if(getFocusAvantAideSWT().equals(vue.getTfCODE_ARTICLE())||getFocusAvantAideSWT().equals(vue.getTfLIB_L_FACTURE())){
						...
						t = null;
						taLAcompte.setLegrain(true);
							taLAcompte.setTaArticle(f);
					}
					if(getFocusAvantAideSWT().equals(vue.getTfU1_L_FACTURE())
						...
						taLAcompte.setLegrain(true);
						if(getFocusAvantAideSWT().equals(vue.getTfU1_L_FACTURE()))
							taLAcompte.setU1LDocument(f.getCodeUnite());
						else
							taLAcompte.setU2LDocument(f.getCodeUnite());
					}					
					if(getFocusAvantAideSWT().equals(vue.getTfPRIX_U_L_FACTURE())){
						...
						if(masterEntity.getTtc()==1)
							taLAcompte.setPrixULDocument(f.getPrixttcPrix());
						else
							taLAcompte.setPrixULDocument(f.getPrixPrix());
					}					
...
//					ibTaTable.lgrLocateID(((ResultAffiche) evt.getRetour()).getIdResult());
		...
		initEtatBouton(false);
		if(parentEcran!=null) {
			//((SWTPaEditorAcompteController)parentEcran).activeWorkenchPartCommands(true);
			((PaEditorAcompteController)parentEcran).initEtatBouton(false);
		}
	}
	}
		...
		if (getFocusCourantSWT()==vue.getTfPRIX_U_L_FACTURE()){
			try {
				...
				if(!LibChaine.empty(vue.getTfCODE_ARTICLE().getText())&& masterEntity.getLigne(masterEntity.getLigneCourante())!=null) {
					...
					TaArticle taArticle = taArticleDAO.findById(((TaLAcompte)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaArticle().getIdArticle());
...
//	);
	...
	protected void actChangeTva() throws Exception{
		if(!LibChaine.empty(((IHMLAcompte)selectedLigneAcompte.getValue()).getCodeArticle())
			...
			InputDialog input = new InputDialog(vue.getShell()
					...
					((IHMLFacture)selectedLigneAcompte.getValue()).getCodeTvaLDocument(),new IInputValidator() {
			...
			if(masterDAO.dataSetEnModif()){
				...
				if(input.getValue()!=null ){
					if(MessageDialog.openConfirm(vue.getShell()
						...
						((IHMLFacture)selectedLigneAcompte.getValue()).setCodeTvaLDocument(retour);
						if(input.getValue().equals("")){
							taLAcompte.setCodeTvaLDocument("");
							taLAcompte.setTauxTvaLDocument(BigDecimal.valueOf(0));
						}else{
							TaTvaDAO daoTva = new TaTvaDAO(getEm());
							taLAcompte.setCodeTvaLDocument(retour);
							taLAcompte.setTauxTvaLDocument(daoTva.findByCode(retour).getTauxTva());
						}
...
//			}
			if(continuer){
				...
				for (TaLAcompte p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument().equals(((IHMLAcompte) selectedLigneAcompte.getValue()).getNumLigneLDocument())) {
						taLAcompte = p;
						taLAcompte.setLegrain(true);
						ihmLAcompte= IHMLAcompte.copy((IHMLAcompte) selectedLigneAcompte.getValue());;
					}				
				}
				initEtatBouton(false);
				}		
	...
	protected void actSupprimer() throws Exception {
		...
		try {
			...
			for (TaLAcompte p : masterEntity.getLignes()) {
				...
				dao.supprimer(taLAcompte);
				taLAcompte.setTaDocument(null);
				masterEntity.removeLigne(taLAcompte);
				taLAcompte=null;
				actRefresh();
				refreshTotauxLignes();
				initEtatBouton();
	...
	protected void actAnnuler() throws Exception {
		...
		try {
			...
			case C_MO_INSERTION:
				message =!taLAcompte.ligneEstVide();
				//message =!((TaLAcompte)masterEntity.getLigne(masterEntity.getLigneCourante())).ligneEstVide();
				...
				if(repondu){
					if(selectedLigneAcompte!=null && selectedLigneAcompte.getValue() != null
							//&& ((IHMLAcompte) selectedLigneAcompte.getValue()).getID_ARTICLE()!=null
							){
//						modelLAcompte.getListeObjet().remove(((IHMLAcompte) selectedLigneAcompte.getValue()));
//						writableList = new WritableList(realm, modelLAcompte.getListeObjet(), classModel);
//						tableViewer.setInput(writableList);
//						tableViewer.refresh();
//						tableViewer.selectionGrille(0);
						masterEntity.removeLigne(taLAcompte);
...
//					int rang = modelLAcompte.getListeObjet().indexOf(selectedLigneAcompte.getValue());
//					// selectedLigneAcompte.setValue(ihmOldLAcompte);
//					modelLAcompte.getListeObjet().set(rang, ihmOldLAcompte);
//					writableList = new WritableList(realm, modelLAcompte.getListeObjet(), classModel);
//					tableViewer.setInput(writableList);
//					tableViewer.refresh();
//					tableViewer.setSelection(new StructuredSelection(ihmOldLAcompte), true);
//					dao.annuler(taLAcompte);
//					masterEntity.removeLigne(lignesupprimee);
////					initLigneCourantSurRow();
////					ibTaTable.initLigneCourantSurRow();
//					hideDecoratedFields();
					int rang =((WritableList)tableViewer.getInput()).indexOf(selectedLigneAcompte.getValue());
					List<IHMLAcompte> l = IHMmodel();
					if(rang!=-1)
					  l.set(rang, ihmOldLAcompte);
					
					writableList = new WritableList(realm, l, classModel);
					tableViewer.setInput(writableList);
					tableViewer.refresh();
					tableViewer.setSelection(new StructuredSelection(ihmOldLAcompte), true);
					mapperUIToModel.map(ihmOldLAcompte,taLAcompte);
					remetTousLesChampsApresAnnulationSWT(dbc);
					//mapperModelToUI.map(taLAcompte,(IHMLAcompte)selectedLigneAcompte.getValue());
//					ctrlTousLesChampsAvantEnregistrementSWT();//si on contrôle tous les champs
	...
	protected void actPrecedent() throws Exception {
		if(taLAcompte!=null && taLAcompte.ligneEstVide()){
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				switch ((PaLigneAcompteController.this.dao.getModeObjet().getMode())) {
				case C_MO_CONSULTATION:
				...
				case C_MO_INSERTION:
					...
					if (getFocusCourantSWT()==vue.getTfU1_L_FACTURE()||
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new TaUniteDAO(getEm()).modelObjetUniteArticle(taLAcompte.getTaArticle().getCodeArticle()));
					...
					if (getFocusCourantSWT()==vue.getTfPRIX_U_L_FACTURE()){
						...
						paramAffichePrix.setIdArticle(LibConversion.integerToString(getSelectedLigneAcompte().getIdArticle()));
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTPrix,TaPrixDAO,TaPrix>(taLAcompte.getTaArticle().getTaPrixes(),SWTPrix.class,getEm()));
				...
				if (paramAfficheAideRecherche.getJPQLQuery() != null) {
					...
					//if(parentEcran!=null) ((SWTPaEditorAcompteController)parentEcran).activeWorkenchPartCommands(false);
				}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "L_ACOMPTE";
		try {
			IStatus s = null;
			boolean change=true;
			if(nomChamp.equals(Const.C_CODE_ARTICLE)) {
				...
				if(s.getSeverity()!=IStatus.ERROR) {
					...
					if (ihmLAcompte!=null)
						change = !f.getCodeArticle().equals(ihmLAcompte.getCodeArticle());
					if(change){
						taLAcompte.setTaArticle(f);
						taLAcompte.initQte2(f);
						mapperModelToUI.map(taLAcompte, ihmLAcompte);
					}
				}
				dao = null;
			} else {
				TaLAcompte u = new TaLAcompte();
				...
				if (ihmLAcompte!=null)
					change=u.getQteLDocument()!=null&& !u.getQteLDocument().equals(ihmLAcompte.getQteLDocument());
				}
				
				if(s.getSeverity()!=IStatus.ERROR) {
					PropertyUtils.setSimpleProperty(taLAcompte, nomChamp, value);
				}
			}
			
			if(s.getSeverity()!=IStatus.ERROR) {
				if(nomChamp.equals(Const.C_QTE_L_DOCUMENT)&& change) {
					if(taLAcompte.getTaArticle()!=null){
						taLAcompte.initQte2(taLAcompte.getTaArticle());
					}
				}
				mapperModelToUI.map(taLAcompte, ((IHMLAcompte)selectedLigneAcompte.getValue()));
			}
...
//				ibTaTable.setChamp_Model_Obj(((TaLAcompte)masterEntity.getLigne(masterEntity.getLigneCourante())).setTaLAcompte(((IHMLAcompte)selectedLigneAcompte.getValue())));
...
//				ibTaTable.enregistrement(((TaLAcompte)masterEntity.getLigne(masterEntity.getLigneCourante())));
...
//				((IHMLAcompte) selectedLigneAcompte.getValue()).setIdLAcompte(LibConversion.stringToInteger(ibTaTable.valeurIdTable));
//				courant = new DataRow(ibTaTable.getFIBQuery());
//				ibTaTable.getFIBQuery().getDataRow(courant);
//				((IHMLAcompte) selectedLigneAcompte.getValue()).setIHMLAcompte(modelLAcompte.remplirObjet(courant));

//				select.widgetSelected(null);
//				try {
//				ibTaTable.setChamp_Model_Obj(((TaLAcompte)masterEntity.getLigne(masterEntity.getLigneCourante())));
//				} catch (Exception e1) {
//				logger.error("",e1);
//				}
...
//				dao.begin(transaction);
				if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
					LgrDozerMapper<IHMLAcompte,TaLAcompte> mapper = new LgrDozerMapper<IHMLAcompte,TaLAcompte>();
					mapper.map((IHMLAcompte) selectedLigneAcompte.getValue(),taLAcompte);

				} else if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)) {
					LgrDozerMapper<IHMLAcompte,TaLAcompte> mapper = new LgrDozerMapper<IHMLAcompte,TaLAcompte>();
					mapper.map((IHMLAcompte) selectedLigneAcompte.getValue(),taLAcompte);
//					taLAcompte.setTaAcompte(masterEntity);
//					masterEntity.addLigne(taLAcompte);
				}
...
//				actRefresh();
//				transaction = null;
				masterEntity.enregistreLigne(taLAcompte);
				dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_CONSULTATION);
				if(focusDansLigne()&& insertion)insertionLigne(true,true);//ajout d'une ligne à la suite des autres
			}
		} finally {
...
//			if(((TaLAcompte)swtAcompte.getLigne(swtAcompte.getLigneCourante())).ligneEstVide()) {
...
//			}
	...
	public boolean isUtilise(){
		return ((IHMLAcompte)selectedLigneAcompte.getValue()).getIdLDocument()!=null && 
		!dao.recordModifiable(dao.getNomTable(),
				((IHMLAcompte)selectedLigneAcompte.getValue()).getIdLDocument());		
	}

	public IHMLAcompte getIhmOldLAcompte() {
		return ihmOldLAcompte;
	}

	public void setIhmOldLAcompte(IHMLAcompte swtOldArticle) {
		this.ihmOldLAcompte = swtOldArticle;
	}

	public void setIhmOldLAcompte() {
		if (selectedLigneAcompte!=null && selectedLigneAcompte.getValue() != null)
			this.ihmOldLAcompte = IHMLAcompte.copy((IHMLAcompte) selectedLigneAcompte.getValue());
		else {
			if(tableViewer!=null && tableViewer.selectionGrille(0)){
				this.ihmOldLAcompte = IHMLAcompte.copy((IHMLAcompte) selectedLigneAcompte.getValue());
				tableViewer.setSelection(new StructuredSelection(
						(IHMLAcompte) selectedLigneAcompte.getValue()), true);
			}}
	}
	...
	protected void sortieChamps() {
		try {
			if (masterEntity.getLigne(masterEntity.getLigneCourante())!=null && 
					( (TaLAcompte)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaTLigne().getIdTLigne()!=0)
				initEtatComposant(( (TaLAcompte)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaTLigne().getCodeTLigne());
...
//		int row = vue.getGrille().getSelectionIndex();
		...
		tableViewer.selectionGrille(
				tableViewer.selectionGrille(selectedLigneAcompte.getValue()));
...
//	public int recupLignes(SWTAcompte swtAcompte) {
//		int nbLignes = 0;
//		try {
//			
//			setTaAcompte(swtAcompte);
//			ibTaTable.setAcompte(swtAcompte);
//			ibTaTable.videTableTemp();
//			swtAcompte.getLignes().clear();
//			swtAcompte.getLignesTVA().clear();
//		
//			ibTaTable.recupLignesAcompte(swtAcompte.getEntete().getCODE());
//			initLigneCourantSurRow();
//			ibTaTable.initLigneCourantSurRow();
//			writableList = new WritableList(realm, modelLAcompte.remplirListe(), classModel);
//			tableViewer.setInput(writableList);
//			nbLignes = modelLAcompte.getListeObjet().size();
//
//			for (IHMLAcompte ihmLigne : modelLAcompte.getListeObjet()) {
//				TaLAcompte swtLigne = new TaLAcompte((IHMLAcompte)selectedLigneAcompte.getValue());
//				swtLigne.setTaLAcompte(ihmLigne);
//				swtAcompte.addLigne(swtLigne);
//			}
//			tableViewer.selectionGrille(0);
//			initEtatBouton(false);
//			return nbLignes;
...
//	}
	...
	protected class ActionEnregistrerAcompte extends ActionEnregistrer {
		public ActionEnregistrerAcompte(String name) {
			super(name);
		}
...
//			}
	...
	private class SelectionGrille extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			setIhmOldLAcompte();
			if(selectedLigneAcompte.getValue()!=null && masterEntity.getLignes().size()>vue.getGrille().getSelectionIndex()){
//				((TaLAcompte)masterEntity.getLigne(vue.getGrille().getSelectionIndex())).setIhmLAcompte(((IHMLAcompte)selectedLigneAcompte.getValue()));
//				((TaLAcompte)masterEntity.getLigne(vue.getGrille().getSelectionIndex())).setTaLAcompte(((IHMLAcompte)selectedLigneAcompte.getValue()));
//				mapperUIToModel.map((IHMLAcompte)selectedLigneAcompte.getValue(), taLAcompte);
				for (TaLAcompte p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument()==((IHMLAcompte) selectedLigneAcompte.getValue()).getNumLigneLDocument()) {
						taLAcompte = p;
					}				
				}
			}
	...
	public IHMLAcompte getSelectedLigneAcompte() {
		IHMLAcompte resultat = null;
		if(selectedLigneAcompte!=null && selectedLigneAcompte.getValue()!=null)
			resultat = (IHMLAcompte)selectedLigneAcompte.getValue();
		return resultat;
	};	
	...
	protected void insertionLigne(Boolean ajout, boolean initFocus) throws Exception{
		...
		try {
			...
			if(masterDAO.dataSetEnModif()){
			setIhmOldLAcompte();
			ihmLAcompte = new IHMLAcompte();

			dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_INSERTION);
			taLAcompte = new TaLAcompte(true); 
			taLAcompte.setTaTLigne(new TaTLigneDAO(getEm()).findByCode(Const.C_CODE_T_LIGNE_C));
			
			List<IHMLAcompte> modelListeLigneAcompte = IHMmodel();
			
			if(ajout)
				masterEntity.addLigne(taLAcompte);
			else{
				if(vue.getGrille().getItemCount()!=0) 
				masterEntity.insertLigne(taLAcompte,vue.getGrille().getSelectionIndex());
				else
					masterEntity.insertLigne(taLAcompte,0);
					
			}
			ihmLAcompte.setNumLigneLDocument(taLAcompte.getNumLigneLDocument());
			taLAcompte.setTaDocument(masterEntity);
			taLAcompte.initialiseLigne(true);
			
			modelListeLigneAcompte.add(ihmLAcompte.getNumLigneLDocument(),ihmLAcompte);
			
//			taLAcompte.setTaLAcompte(ihmLAcompte);
//			ibTaTable.insertion(null,true);
//			modelLAcompte.getListeObjet().add(ihmLAcompte);
			writableList = new WritableList(realm, modelListeLigneAcompte, classModel);
			tableViewer.setInput(writableList);
			tableViewer.refresh();
			tableViewer.setSelection(new StructuredSelection(ihmLAcompte),true);
			initEtatComposant(taLAcompte.getTaTLigne().getCodeTLigne());
			//initLigneCourantSurRow();
//			ibTaTable.initLigneCourantSurRow();

//			masterEntity.insertLigne(taLAcompte,masterEntity.getLigneCourante());
//			masterEntity.getLigne(masterEntity.getLigneCourante()).setModeLigne(EnumModeObjet.C_MO_INSERTION);

//			try{
//				ibTaTable.insertion(ligneAcompte,true);
//			}catch (Exception ex1){
//				masterEntity.removeLigne(masterEntity.getLigneCourante());
//				throw new Exception();
//			}
			taLAcompte.setNumLigneLDocument(ihmLAcompte.getNumLigneLDocument());
//			ihmLAcompte.setNumLigneLAcompte(taLAcompte.getNumLigneLAcompte());
			//actualiserForm();
...
//			//enableActionAndHandler(actionAideTva,handlerAideTva,false);
			...
			break;
		case C_MO_CONSULTATION:
//			enableActionAndHandler(actionInserer,handlerInserer,etatAcompte);
//			enableActionAndHandler(actionModifier,handlerModifier,(trouve && etatAcompte));				
//			enableActionAndHandler(actionAjouter,handlerAjouter,etatAcompte);
//			//enableActionAndHandler(actionAideTva,handlerAideTva,trouve);
			enableActionAndHandler(C_COMMAND_GLOBAL_IMPRIMER_ID,false);
			enableActionAndHandler(C_COMMAND_GLOBAL_INSERER_ID,etatAcompte);
			enableActionAndHandler(C_COMMAND_GLOBAL_MODIFIER_ID,(trouve && etatAcompte));
			//enableActionAndHandler(C_COMMAND_LIGNE_FACTURE_AJOUT_ID,etatAcompte);
		...
		boolean editable =true;
//		if(swtAcompte.getLigne(swtAcompte.getLigneCourante())==null ||((TaLAcompte)swtAcompte.getLigne(swtAcompte.getLigneCourante())).ligneEstVide()) {
//		if(!ibTaTable.dataSetEnModif())editable=false;
//		}
		vue.getTfCODE_ARTICLE().setEditable(editable);
		vue.getTfLIB_L_FACTURE().setEditable(editable);
		...
		protected void actUp() throws Exception{
			int selectionGrille=getGrille().getSelectionIndex();
				try {
					if(dao.dataSetEnModif()) {
						//ctrlUnChampsSWT(getFocusCourantSWT());
//						dao.setChamp_Model_Obj(((TaLAcompte)masterEntity.getLigne(masterEntity.getLigneCourante())).setTaLAcompte(((IHMLAcompte)selectedLigneAcompte.getValue())));					
						if(taLAcompte.ligneEstVide()){
							actAnnuler();
						}else {
							ctrlUnChampsSWT(getFocusCourantSWT());
...
//								e.doit=false;
			...
			protected class HandlerEnregistrerAcompte extends LgrAbstractHandler {
			...
			public TaLAcompteDAO getDao() {
				return dao;
			}

			public void setDao(TaLAcompteDAO dao) {
				this.dao = dao;
			}
			...
			public boolean changementPageValide(){
				if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
					...
					if(taLAcompte!=null && selectedLigneAcompte!=null && ((IHMLAcompte) selectedLigneAcompte.getValue())!=null) {
						mapperUIToModel.map((IHMLAcompte) selectedLigneAcompte.getValue(),taLAcompte);
					}
				}
//				dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_CONSULTATION);
