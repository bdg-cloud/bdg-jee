...
import fr.legrain.acompte.controllers.PaEditorTotauxAcompteController;
import fr.legrain.acompte.controllers.PaLigneAcompteController;
 ...
 * </ul>
 */
public class AcompteMultiPageEditor extends AbstractMultiPageDocumentEditor implements ILgrRetourEditeur, IChangementDePageListener, IChangementMasterEntityListener {
	
	public static final String ID_EDITOR = "fr.legrain.editor.acompte.swt.multi"; //$NON-NLS-1$
	static Logger logger = Logger.getLogger(AcompteMultiPageEditor.class.getName());
	...
	private EditorAcompte editorFacture = null;
	private EditorLigne editorLigne = null;
	private EditorTotaux editorTotaux = null;
	
	public static final String PREFIXE_NOM_EDITEUR = "Acompte";
	

	/**
	 * Creates a multi-page editor example.
	 */
	public AcompteMultiPageEditor() {
		super();
	...
	void createPageQueEditeur() {
		try {
			...
			em = new TaAcompteDAO().getEntityManager();
			editorFacture = new EditorAcompte(this,em);
			...
			setPageImage(index, pluginAcompte.getImageDescriptor(iconPageEntete).createImage());
			
			index = addPage(editorLigne, getEditorInput());
			setPageText(index, labelPageCorps);
			setPageImage(index, pluginAcompte.getImageDescriptor(iconPageCorps).createImage());
			
			index = addPage(editorTotaux, getEditorInput());
			setPageText(index, labelPageTotaux);
			setPageImage(index, pluginAcompte.getImageDescriptor(iconPageTotaux).createImage());
			
			//liaison entre la selection du controller principal et le multipage editor
			((PaEditorAcompteController)editorFacture.getController()).addChangementMasterEntityListener(this);
			...
			masterDAO = ((PaEditorAcompteController)editorFacture.getController()).getDao();
			((PaLigneAcompteController)editorLigne.getController()).setMasterDAO(masterDAO);
			((PaEditorTotauxAcompteController)editorTotaux.getController()).setMasterDAO(masterDAO);
			...
			((PaEditorTotauxAcompteController)editorTotaux.getController()).initVerifySaisiePublic();
		
			//TODO #JPA
			//Spécifique a la facture
			((PaEditorAcompteController)editorFacture.getController()).setControllerLigne((PaLigneAcompteController)editorLigne.getController());
			((PaEditorAcompteController)editorFacture.getController()).getControllerLigne().setMasterEntity(((PaEditorAcompteController)editorFacture.getController()).getTaDocument());
//			
			((PaEditorAcompteController)editorFacture.getController()).setControllerTotaux((PaEditorTotauxAcompteController)editorTotaux.getController());
			((PaEditorAcompteController)editorFacture.getController()).getControllerTotaux().setMasterEntity(((PaEditorAcompteController)editorFacture.getController()).getTaDocument());
...
//				changeEditeurCourant(findPrecedent());
	...
	public void changeMasterEntity(TaAcompte newMasterEntity) {
		...
		masterEntity.addModificationDocumentListener(((PaEditorAcompteController)editorFacture.getController()));
		masterEntity.addModificationDocumentListener(((PaLigneAcompteController)editorLigne.getController()));
		masterEntity.addModificationDocumentListener(((PaEditorTotauxAcompteController)editorTotaux.getController()));
		
		updatePartName();
		
		//maj de l'editeur lignes
		ParamAfficheLAcompte paramAfficheLFacture = new ParamAfficheLAcompte();
		paramAfficheLFacture.setIdAcompte(LibConversion.integerToString(masterEntity.getIdDocument()));
		((PaLigneAcompteController)editorLigne.getController()).setMasterEntity(masterEntity);
		((PaLigneAcompteController)editorLigne.getController()).configPanel(paramAfficheLFacture);
...
//		paramAffiche.setIdArticle(LibConversion.integerToString(masterEntity.getIdFacture()));
		((PaEditorTotauxAcompteController)editorTotaux.getController()).setMasterEntity(masterEntity);
//		((SWTPaEditorTotauxController)editorTotaux.getController()).configPanel(paramAffiche);
		...
		((PaEditorAcompteController)editorFacture.getController()).modificationDocument(evt);
		((PaLigneAcompteController)editorLigne.getController()).modificationDocument(evt);
		((PaEditorTotauxAcompteController)editorTotaux.getController()).modificationDocument(evt);
	}
	
	public void changeMasterEntitySimple(TaAcompte newMasterEntity) {
		if(masterEntity!=null) {
			masterEntity.removeModificationDocumentListener(((SWTModificationDocumentListener)editorFacture.getController()));
			masterEntity.removeModificationDocumentListener(((SWTModificationDocumentListener)editorLigne.getController()));
			masterEntity.removeModificationDocumentListener(((SWTModificationDocumentListener)editorTotaux.getController()));
	...
	public void changementMasterEntity(ChangementMasterEntityEvent evt) {
		if(evt.isSimple())
			changeMasterEntitySimple((TaAcompte)evt.getNewMasterEntity());
		else
			changeMasterEntity((TaAcompte)evt.getNewMasterEntity());
	}
