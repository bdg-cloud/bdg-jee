...
public class PaCriterePrelevementRefusController extends JPABaseControllerSWTStandard 
implements RetourEcranListener{

	static Logger logger = Logger.getLogger(PaCriterePrelevementRefusController.class.getName());	
	private PaCriterePrelevementRefus  vue = null;
	private Date dateDeb=null;
	private Date dateFin=null;
	
//	private static String refus="refusé";
//	private static String paye="payé";
//	private static String attente="en attente";
...
//	private String compteCourant = null;
	...
	private ModelGeneralObjet<IHMEntetePrelevement,TaPrelevementDAO,TaPrelevement> modelRemise = null;
	private Object selectedPrelevement = new IHMEntetePrelevement();
	private PaSelectionLigneRefusControlleur paSelectionLigneRemiseControlleur=null;
	private MapChangeListener changeListener = new MapChangeListener();

	
	protected HandlerListeDocument handlerListeDocument = new HandlerListeDocument();
	
	public PaCriterePrelevementRefusController(PaCriterePrelevementRefus vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		dao =new TaPrelevementDAO(getEm());
		modelRemise = new ModelGeneralObjet<IHMEntetePrelevement,TaPrelevementDAO,TaPrelevement>
	...
	public PaCriterePrelevementRefusController(PaCriterePrelevementRefus vue) {
		this(vue,null);
	}
...
//			vue.getPaGrille().setMenu(popupMenuGrille);
			...
			int reponseCourante=0;
			int i=0;
			daoEtat =new TaEtatDAO();
			vue.getCbListeReponse().add("TOUS");
			List<TaEtat> listeEtat =daoEtat.selectAll();
			for (TaEtat taEtat : listeEtat) {		
				if(taEtat.getCodeEtat().toUpperCase().equals("REJETE"))reponseCourante=i;
				vue.getCbListeReponse().add(taEtat.getCodeEtat());
				i++;
			}
			
			vue.getCbListeReponse().select(reponseCourante);
			vue.getCbListeReponse().setVisibleItemCount(vue.getCbListeReponse().getItemCount());
			
//			vue.getCbListeReponse().add(refus);
//			vue.getCbListeReponse().add(paye);
//			vue.getCbListeReponse().add(attente);
...
//			List<TaCompteBanque> compte=daoCompte.findByTiersEntrepriseListe(daoPaiement.findByCode(typePaiementCourant));
...
//		List<TaCompteBanque> compte=daoCompte.findByTiersEntrepriseListe(daoPaiement.findByCode(typePaiementCourant));
//		for (TaCompteBanque taCompteBanque : compte) {
//			vue.getCbListeCompte().add(taCompteBanque.getIban());
//		}
...
//		mapInfosVerifSaisie.put(paFormulaireReglement.getTfTYPE_PAIEMENT(), new InfosVerifSaisie(new TaTPaiement(),Const.C_CODE_T_PAIEMENT,null));
	...
	private void initVue() {
		PaSelectionLigneRefus selectionLigneRelance = new PaSelectionLigneRefus(vue.getGroupFactureNonRegle(), SWT.PUSH,1,
				 SWT.FULL_SELECTION
					| SWT.H_SCROLL
					| SWT.V_SCROLL
					| SWT.BORDER
		...
		vue.layout();
		paSelectionLigneRemiseControlleur = new PaSelectionLigneRefusControlleur(selectionLigneRelance,getEm());
...
//		"Sélection des prélèvements refusés à regénérer", pluginPrelevement.getImageDescriptor(
...
//		}
	...
	protected void actImprimer() throws Exception { //Button VALIDER <--
		...
		String etat =vue.getCbListeReponse().getItem(vue.getCbListeReponse().getSelectionIndex()) ;
		//if((!etat.toUpperCase().equals("REFUSE")||!etat.equals("IMPAYE")))etat=null;
...
//		mapActions.put(vue.getBtnListeDoc(), C_COMMAND_DOCUMENT_LISTE_DOCUMENT_ID);
	...
	public void bind(){
		try {
			...
			modelRemise = new ModelGeneralObjet<IHMEntetePrelevement,TaPrelevementDAO,TaPrelevement>(dao,IHMEntetePrelevement.class);
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());

			if (modelRemise.getListeObjet().isEmpty()) {
				modelRemise.getListeObjet().add(new IHMEntetePrelevement());
			}
			selectedPrelevement = modelRemise.getListeObjet().getFirst();
...
//		listeComposantFocusable.add(vue.getTfDateRemise());
		...
		vue.getCbListeReponse().addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					if(vue.getCbListeReponse().getSelectionIndex()!=-1 && !typeReponseCourante.equals(vue.getCbListeReponse().getItem(vue.getCbListeReponse().getSelectionIndex()))){	
						if(dao.dataSetEnModif()){
...
//		vue.getBtnListeDoc().setImage(LibrairiesEcranPlugin.ir.get(LibrairiesEcranPlugin.IMAGE_EXPORT));
	...
	public void setVue(PaCriterePrelevementRefus vue) {
		super.setVue(vue);
		this.vue = vue;
	}


	public PaSelectionLigneRefusControlleur getPaSelectionLigneRemiseControlleur() {
		return paSelectionLigneRemiseControlleur;
	}

	public void setPaSelectionLigneRemiseControlleur(
			PaSelectionLigneRefusControlleur paSelectionLigneRelanceControlleur) {
