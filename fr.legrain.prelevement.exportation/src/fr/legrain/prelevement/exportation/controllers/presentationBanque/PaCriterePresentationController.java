...
public class PaCriterePresentationController extends JPABaseControllerSWTStandard 
implements RetourEcranListener{

	static Logger logger = Logger.getLogger(PaCriterePresentationController.class.getName());	
	private PaCriterePrelevementPresentation  vue = null;
...
//	private String typePaiementCourant=null;
	...
	private PaSelectionLignePresentationControlleur paSelectionLigneRemiseControlleur=null;
	private MapChangeListener changeListener = new MapChangeListener();

	
	protected HandlerListeDocument handlerListeDocument = new HandlerListeDocument();
	
	public PaCriterePresentationController(PaCriterePrelevementPresentation vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
	...
	public PaCriterePresentationController(PaCriterePrelevementPresentation vue) {
		this(vue,null);
	}
...
//			vue.getCbListePaiement().setVisible(false);
...
//		mapInfosVerifSaisie.put(paFormulaireReglement.getTfTYPE_PAIEMENT(), new InfosVerifSaisie(new TaTPaiement(),Const.C_CODE_T_PAIEMENT,null));
	...
	private void initVue() {
		PaSelectionLignePresentation selectionLigneRelance = new PaSelectionLignePresentation(vue.getGroupFactureNonRegle(), SWT.PUSH,1,
				 SWT.FULL_SELECTION
					| SWT.H_SCROLL
					| SWT.V_SCROLL
					| SWT.BORDER
		...
		vue.layout();
		paSelectionLigneRemiseControlleur = new PaSelectionLignePresentationControlleur(selectionLigneRelance,getEm());
...
//		"Sélection des prélèvements à présenter à la banque ", pluginPrelevement.getImageDescriptor(
//		"/icons/logo_lgr_16.png").createImage(), SWT.DEFAULT, 600);
...
//		}
	...
	protected void actImprimer() throws Exception { //Button VALIDER <--
		Integer filtre=null;
		if(vue.getPrelevExport().getSelection())filtre=1;
		if(vue.getPrelevNonExport().getSelection())filtre=0;
		...
		if(!vue.getTfCodePrelevement().getText().equals("") || vue.getTfTiers().getText().equals("") ){
			listeTaPrelevement = dao.recherchePrelevement(LibDateTime.getDate(vue.getTfDateDebutPeriode()),LibDateTime.getDate(vue.getTfDateFinPeriode()),
					vue.getTfCodePrelevement().getText(),filtre,null);
		}else{
			listeTaPrelevement = dao.recherchePrelevementTiers(LibDateTime.getDate(vue.getTfDateDebutPeriode()),LibDateTime.getDate(vue.getTfDateFinPeriode()),
					vue.getTfTiers().getText(),filtre,null);	
		}
	...
	protected void actRefresh() throws Exception {
		try{
			if(paSelectionLigneRemiseControlleur!=null){
				//<--
//				List <TaCompteBanque> selectCompteBanqueSEPA = new LinkedList<TaCompteBanque>() ; //<<--
//				TaCompteBanqueDAO daoCompteBanqueSepa = new TaCompteBanqueDAO();
//				selectCompteBanqueSEPA=daoCompteBanqueSepa.selectCompteEntreprise();
...
//		mapComposantChamps.put(vue.getTfDateFinPeriode(), Const.C_DATE_FIN);
		...
		mapComposantChamps.put(vue.getTfCodePrelevement(), Const.C_CODE_DOCUMENT);
//		mapComposantChamps.put(vue.getTfDateRemise(), Const.C_DATE_DOCUMENT);
		...
		listeComposantFocusable.add(vue.getPrelevTous());
...
//		listeComposantFocusable.add(vue.getTfDateRemise());
		...
		vue.getPrelevNonExport().setSelection(true);
		vue.getPrelevExport().addSelectionListener(new SelectionAdapter(){
...
//		vue.getBtnListeDoc().setImage(LibrairiesEcranPlugin.ir.get(LibrairiesEcranPlugin.IMAGE_EXPORT));
	...
	public void setVue(PaCriterePrelevementPresentation vue) {
		super.setVue(vue);
		this.vue = vue;
	}


	public PaSelectionLignePresentationControlleur getPaSelectionLigneRemiseControlleur() {
		return paSelectionLigneRemiseControlleur;
	}

	public void setPaSelectionLigneRemiseControlleur(
			PaSelectionLignePresentationControlleur paSelectionLigneRelanceControlleur) {
