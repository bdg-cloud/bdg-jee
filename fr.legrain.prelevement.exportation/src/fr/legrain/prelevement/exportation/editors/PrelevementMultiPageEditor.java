 ...
 * </ul>
 */
public class PrelevementMultiPageEditor extends AbstractLgrMultiPageEditor implements 
...
//public class MultiPageEditor extends SharedHeaderFormEditor {
	...
	public static final String ID_EDITOR = "fr.legrain.prelevement.exportation.multi"; //$NON-NLS-1$
	...
	protected EditorCriterePrelevementPresentation editorCriterePresentation = null;
	protected EditorCriterePrelevementRefus editorCritereRefus = null;
	protected EditorCriterePrelevementReponse editorCritereReponse = null;
	/**
	...
	protected void createPageQueEditeur() {
		try {
			...
			editorCriterePresentation = new EditorCriterePrelevementPresentation(this,em);
			editorCritereReponse= new EditorCriterePrelevementReponse(this,em);
			editorCritereRefus= new EditorCriterePrelevementRefus(this,em);
			
			listePageEditeur.add(editorCriterePresentation);
			listePageEditeur.add(editorCritereReponse);
			listePageEditeur.add(editorCritereRefus);
			
			
			int index = addPage(editorCriterePresentation, getEditorInput());
			setPageText(index, editorCriterePresentation.getTitle());
			setPageImage(index, pluginPrelevement.getImageDescriptor(iconPageRelance).createImage());
			
			int index2 = addPage(editorCritereReponse, getEditorInput());
			setPageText(index2, editorCritereReponse.getTitle());			
			setPageImage(index2, pluginPrelevement.getImageDescriptor(iconPageRelance).createImage());
			
			int index3 = addPage(editorCritereRefus, getEditorInput());
			setPageText(index3, editorCritereRefus.getTitle());
			setPageImage(index3, pluginPrelevement.getImageDescriptor(iconPageRelance).createImage());
			
			//liaison entre la selection du controller principal et le multipage editor
			editorCriterePresentation.getController().addChangementDeSelectionListener(this);
//			((SWTPaReglementMultipleController)editorCritereRelance.getController()).addChangementMasterEntityListener(this);

			((PaCriterePresentationController)editorCriterePresentation.getController()).
				getPaSelectionLigneRemiseControlleur().addDeclencheCommandeControllerListener(editorCriterePresentation.getController());
			((PaCriterePrelevementRefusController)editorCritereRefus.getController()).getPaSelectionLigneRemiseControlleur().
				addDeclencheCommandeControllerListener(editorCritereRefus.getController());
			((PaCriterePrelevementReponseController)editorCritereReponse.getController()).getPaSelectionLigneRemiseControlleur().
			addDeclencheCommandeControllerListener(editorCritereReponse.getController());
			
			mapEditorController.put(editorCriterePresentation, editorCriterePresentation.getController());
			mapEditorController.put(editorCritereRefus, editorCritereRefus.getController());
			mapEditorController.put(editorCritereReponse, editorCritereReponse.getController());

			
			editorCriterePresentation.getController().addChangementDePageListener(this);
			editorCritereRefus.getController().addChangementDePageListener(this);
			editorCritereReponse.getController().addChangementDePageListener(this);
			
			((PaCriterePresentationController)editorCriterePresentation.getController()).getPaSelectionLigneRemiseControlleur().addChangementDePageListener(this);
			((PaCriterePrelevementRefusController)editorCritereRefus.getController()).getPaSelectionLigneRemiseControlleur().addChangementDePageListener(this);
			((PaCriterePrelevementReponseController)editorCritereReponse.getController()).getPaSelectionLigneRemiseControlleur().addChangementDePageListener(this);
...
//			editorGestionReglement.getController().addDeclencheCommandeControllerListener(editorCritereRelance.getController());
			...
			editorCriterePresentation.getController().addAnnuleToutListener(this);
			editorCritereRefus.getController().addAnnuleToutListener(this);
			editorCritereReponse.getController().addAnnuleToutListener(this);

			
			editeurCourant = editorCriterePresentation;
			ParamAffichePresentation paramAfficheRelanceFacture = new ParamAffichePresentation();
			editorCriterePresentation.getController().configPanel(paramAfficheRelanceFacture);
			
//			ParamAffichePresentation paramAfficheRelanceFacture = new ParamAffichePresentation();
			editorCritereRefus.getController().configPanel(paramAfficheRelanceFacture);
			
////			ParamAffichePresentation paramAfficheRelanceFacture = new ParamAffichePresentation();
			editorCritereReponse.getController().configPanel(paramAfficheRelanceFacture);
...
//					|| masterEntity==null
		...
		setPartName(editeurCourant.getTitle());
		setCurrentPage(listePageEditeur.indexOf(editeurCourant));
