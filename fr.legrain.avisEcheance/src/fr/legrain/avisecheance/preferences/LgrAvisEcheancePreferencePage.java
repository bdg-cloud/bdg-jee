...
public class LgrAvisEcheancePreferencePage extends FieldEditorPreferencePageLGR{
	...
	static private PropertiesConfiguration listeGestCode = new PropertiesConfiguration();
	 static Logger logger = Logger.getLogger(LgrAvisEcheancePreferencePage.class.getName());
	  
	public LgrAvisEcheancePreferencePage() {
		super(GRID);
		String idPlugin = PlugInAvisEcheance.getDefault().getBundle().getSymbolicName();
		...
		this.setFolderEditionReportPlugin(constEdition.FICHE_FILE_REPORT_AVIS_ECHEANCE);
		this.setNamePlugin(idPlugin);
		
		reportPlugin =ConstEdition.pathRepertoireEditionsSpecifiques()+ConstEdition.SEPARATOR+idPlugin+
		ConstEdition.SEPARATOR+TaAvisEcheance.class.getSimpleName();
		
		reportPluginClients=ConstEdition.pathRepertoireEditionsSpecifiquesClient()+ConstEdition.SEPARATOR+idPlugin+
		ConstEdition.SEPARATOR+TaAvisEcheance.class.getSimpleName();
		
		setPreferenceStore(PlugInAvisEcheance.getDefault().getPreferenceStore());
		setDescription("Paramètres de codification des avis d'échéance");
	...
	public void createFieldEditors() {
		...
		String PathEditionDefaut = ConstEdition.pathFichierEditionsSpecifiques(ConstEdition.FICHE_FILE_REPORT_AVIS_ECHEANCE,
				   namePlugin);
		...
		constEdition.valuePropertiePathEditionDefaut = ConstEdition.FICHE_FILE_REPORT_AVIS_ECHEANCE;
		constEdition.valuePropertieCommentEditions = String.format(ConstEdition.COMMENT_ONE_EDITION_ENTITY, 
				namePlugin,namePlugin);
		
		File fileEditionsSuppProforma = constEdition.makeFolderEditions(reportEditionSupp);
		
		boolean flag = constEdition.getAllInfosEdition(filePathEditionDefaut,null,namePlugin,
				  fileEditionSpecifiquesClient,fileEditionSpecifiques,TaProforma.class.getSimpleName(),
				  null,false,fileEditionsSuppProforma);
		/********************/
	...
	protected void performApply() {
		...
		try {
			...
			taNouvelleValeur=PlugInAvisEcheance.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_FIXE_1) ;
			listeGestCode.setProperty(PreferenceConstants.TA_DOC_FIXE_1,taNouvelleValeur) ;

			taNouvelleValeur=PlugInAvisEcheance.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_EXO) ;
			listeGestCode.setProperty(PreferenceConstants.TA_DOC_EXO,taNouvelleValeur) ;

			taNouvelleValeur=PlugInAvisEcheance.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_COMPTEUR) ;
