...
import fr.legrain.avisecheance.editor.EditorAvisEcheance;
...
import fr.legrain.documents.dao.TaInfosAvisEcheanceDAO;
...
import fr.legrain.gestCom.Module_Document.IHMAideAvisEcheance;
...
public class PaEditorAvisEcheanceController extends JPABaseControllerSWTStandard
		implements   RetourEcranListener,
		SWTModificationDocumentListener, ChangeModeListener {

	static Logger logger = Logger.getLogger(PaEditorAvisEcheanceController.class.getName());
	private PaEditorAvisEcheance vue = null;
...
//	private Impression impressionAvisEcheance = null;
...
//	private List<TaAvisEcheance> listeDocument = null;
	private List<TaTiers>listeTiers = null;
	
	private TaAvisEcheanceDAO dao = null;//new TaAvisEcheanceDAO();

	private Object ecranAppelant = null;
	private IHMEnteteAvisEcheance ihmOldEnteteAvisEcheance;
	private Realm realm;
	private DataBindingContext dbc;
	private Object selectedAvisEcheance = new IHMEnteteAvisEcheance();
	private String nomClassController = this.getClass().getSimpleName();
	private Class classModel = IHMEnteteAvisEcheance.class;
	
	private ComboViewer viewerComboLocalisationTVA;
	
	private ModelGeneralObjet<IHMEnteteAvisEcheance,TaAvisEcheanceDAO,TaAvisEcheance> modelEnteteAvisEcheance /*= new ModelGeneralObjet<IHMEnteteAvisEcheance,TaAvisEcheanceDAO,TaAvisEcheance>(dao,classModel)*/;
	...
	private TaInfosAvisEcheanceDAO daoInfosAvisEcheance = null;//new TaInfosAvisEcheanceDAO();
	...
	private PaLigneAvisEcheanceController controllerLigne = null;
	private PaEditorTotauxAvisEcheanceController controllerTotaux = null;
	...
	private LgrDozerMapper<TaInfosAvisEcheance,IHMInfosCPaiement> mapperModelToUIInfosDocVersCPaiement = new LgrDozerMapper<TaInfosAvisEcheance,IHMInfosCPaiement>();
	private LgrDozerMapper<TaInfosAvisEcheance,IHMAdresseInfosFacturation> mapperModelToUIInfosDocVersAdresseFact = new LgrDozerMapper<TaInfosAvisEcheance,IHMAdresseInfosFacturation>();
	private LgrDozerMapper<TaInfosAvisEcheance,IHMAdresseInfosLivraison> mapperModelToUIInfosDocVersAdresseLiv = new LgrDozerMapper<TaInfosAvisEcheance,IHMAdresseInfosLivraison>();
	...
	private LgrDozerMapper<IHMEnteteAvisEcheance,TaAvisEcheance> mapperUIToModel = new LgrDozerMapper<IHMEnteteAvisEcheance,TaAvisEcheance>();
	private LgrDozerMapper<TaAvisEcheance,IHMEnteteAvisEcheance> mapperModelToUI = new LgrDozerMapper<TaAvisEcheance,IHMEnteteAvisEcheance>();
	
	private TaAvisEcheance taDocument = null;
	private TaInfosAvisEcheance taInfosDocument = null;
	...
	public PaEditorAvisEcheanceController() {
	}
	
	public PaEditorAvisEcheanceController(PaEditorAvisEcheance vue) {
		this(vue,null);
	}

	public PaEditorAvisEcheanceController(PaEditorAvisEcheance vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		dao = new TaAvisEcheanceDAO(getEm());
		daoInfosAvisEcheance = new TaInfosAvisEcheanceDAO(getEm());
...
//			impressionAvisEcheance = new Impression(vue.getShell(),getEm());
...
//			this.swtDevis.addChangeModeListener(this);
			...
			vue.getTfLIBELLE_DOCUMENT().addFocusListener(new FocusAdapter() {
				public void focusLost(FocusEvent e) {
					try {
//						ctrlUnChampsSWT(((PaEditorAvisEcheance) getVue()).getTfLIBELLE_DEVIS());
						validateUIField(Const.C_LIBELLE_DOCUMENT,((PaEditorAvisEcheance) getVue()).getTfLIBELLE_DOCUMENT());
					} catch (Exception e1) {
						logger.debug("", e1);
					}
				}
	...
	public void bind(PaEditorAvisEcheance PaEditorDevisSWT) {
		try {
			modelEnteteAvisEcheance = new ModelGeneralObjet<IHMEnteteAvisEcheance,TaAvisEcheanceDAO,TaAvisEcheance>(dao,classModel);
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());

//			modelEnteteFacture.remplirListe();
			if (modelEnteteAvisEcheance.getListeObjet().isEmpty()) {
				modelEnteteAvisEcheance.getListeObjet().add(new IHMEnteteAvisEcheance());
			}
			selectedAvisEcheance = modelEnteteAvisEcheance.getListeObjet().getFirst();
			...
			bindingFormSimple(dbc, realm, selectedAvisEcheance, classModel);
			bindAdresse();
			bindAdresseLiv();
			bindCPaiement();
...
//	}
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfCODE_DOCUMENT(), new InfosVerifSaisie(new TaAvisEcheance(),Const.C_CODE_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfCODE_TIERS(), new InfosVerifSaisie(new TaTiers(),Const.C_CODE_TIERS,null));
		mapInfosVerifSaisie.put(vue.getTfLIBELLE_DOCUMENT(), new InfosVerifSaisie(new TaAvisEcheance(),Const.C_LIBELLE_DOCUMENT,null));
		
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse1(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_ADRESSE1_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse2(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_ADRESSE2_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse3(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_ADRESSE3_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl())
				.getTfCodePostal(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_CODEPOSTAL_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfVille(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_VILLE_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfPays(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_PAYS_INFOS_DOCUMENT,null));

		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse1(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_ADRESSE1_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse2(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_ADRESSE2_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse3(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_ADRESSE3_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfCodePostal(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_CODEPOSTAL_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfVille(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_VILLE_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfPays(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_PAYS_LIV_INFOS_DOCUMENT,null));
				
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfCODE_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_CODE_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfLIB_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_LIB_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfREPORT_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_REPORT_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfFIN_MOIS_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosAvisEcheance(),Const.C_FIN_MOIS_C_PAIEMENT,null));
...
//				.getItem(3).getControl()).getBtnReinitialiser(),C_COMMAND_DOCUMENT_REINIT_INFOSTIERS_ID);
	...
	public PaEditorAvisEcheanceController getThis() {
		return this;
	}
	...
	public boolean onClose() throws ExceptLgr {
		...
		try{
			if (!vue.isDisposed() && !vue.getShell().isDisposed()) {
				...
				case C_MO_INSERTION:
				case C_MO_EDITION:
					boolean demande=!((IHMEnteteAvisEcheance) selectedAvisEcheance).DocumentEstVide(taDocument.dateDansExercice(new Date()));
						...
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_TIERS())){
							...
							taDocument.setTaTiers(u);
							 String nomTiers=taDocument.getTaTiers().getNomTiers();
								((IHMEnteteAvisEcheance)selectedAvisEcheance).setLibelleDocument("AvisEcheance N°"+taDocument.getCodeDocument()+" - "+nomTiers);
							
								if(vue.getCbTTC().isEnabled()){
									taDocument.setTtc(u.getTtcTiers());
									((IHMEnteteAvisEcheance)selectedAvisEcheance).setTtc(LibConversion.intToBoolean(taDocument.getTtc()));
								}
								
							changementTiers(true);
						}	
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_DOCUMENT())){
							TaAvisEcheance u = null;
							TaAvisEcheanceDAO t = new TaAvisEcheanceDAO(getEm());
							u = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
							t = null;
							taDocument=u;
	...
	public void calculDateEcheance(Boolean appliquer) {
		if(((IHMEnteteAvisEcheance)selectedAvisEcheance).getIdDocument()==0 || appliquer) {
			Integer report = null;
			...
			((IHMEnteteAvisEcheance)selectedAvisEcheance).setDateEchDocument(taDocument.calculDateEcheance(report, finDeMois));
		}
	}
	...
	private boolean changementTiers(boolean refresh) {
		...
		try {						
			...
			if(((IHMEnteteAvisEcheance)selectedAvisEcheance).getIdDocument()!=0 && 
				...
				if(!idTiersDocumentOriginal.equals(((IHMEnteteAvisEcheance)selectedAvisEcheance).getIdTiers())){
	...
	public void initialisationDesInfosComplementaires(Boolean recharger,String typeARecharger){
		 ...
		 * - le reste des adresses du tiers
		 */
		if(((IHMEnteteAvisEcheance)selectedAvisEcheance).getCodeDocument()!=null)
			taInfosDocument = daoInfosAvisEcheance.findByCodeAvisEcheance(((IHMEnteteAvisEcheance)selectedAvisEcheance).getCodeDocument());
		else
...
//	}
	...
	public void initialisationDesCPaiement(Boolean recharger) {
		// on vide les modeles
		modelCPaiement.getListeObjet().clear();
		TaInfosAvisEcheance taInfosDocument = null;
		if (taDocument != null) {
			if (taDocument.getCodeDocument()!=null&& taDocument.getCodeDocument() != "")
				taInfosDocument = daoInfosAvisEcheance
						.findByCodeAvisEcheance(taDocument.getCodeDocument());
			else
				taInfosDocument = new TaInfosAvisEcheance();

			TaTCPaiementDAO taTCPaiementDAO = new TaTCPaiementDAO(getEm());
			TaCPaiement taCPaiementDoc = null;
			if (taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_AVIS_ECHEANCE) != null
					&& taTCPaiementDAO.findByCode(
							TaTCPaiement.C_CODE_TYPE_AVIS_ECHEANCE).getTaCPaiement() != null) {
				taCPaiementDoc = taTCPaiementDAO.findByCode(
						TaTCPaiement.C_CODE_TYPE_AVIS_ECHEANCE).getTaCPaiement();
			}
			int report = 0;
			int finDeMois = 0;
...
//					dao.getEntityManager().clear();
					taDocument = new TaAvisEcheance(true);
					mapperModelToUI.map(taDocument, (IHMEnteteAvisEcheance) selectedAvisEcheance);
					((IHMEnteteAvisEcheance) selectedAvisEcheance).setCodeDocument(dao.genereCode());
					validateUIField(Const.C_CODE_DOCUMENT,((IHMEnteteAvisEcheance) selectedAvisEcheance).getCodeDocument());//permet de verrouiller le code genere
					((IHMEnteteAvisEcheance) selectedAvisEcheance).setCommentaire(PlugInAvisEcheance.getDefault().getPreferenceStore().getString(PreferenceConstants.COMMENTAIRE));

					validateUIField(Const.C_CODE_DOCUMENT,((IHMEnteteAvisEcheance) selectedAvisEcheance).getCodeDocument());
					taDocument.setCodeDocument(((IHMEnteteAvisEcheance) selectedAvisEcheance).getCodeDocument());
					changementTiers(true);
					Date date = new Date();
					...
					if (LibDate.compareTo(date, taInfoEntreprise.getDatedebInfoEntreprise()) == -1) {
						((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateDocument(taInfoEntreprise.getDatedebInfoEntreprise());
						((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateEchDocument(taInfoEntreprise.getDatedebInfoEntreprise());
						((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateLivDocument(taInfoEntreprise.getDatedebInfoEntreprise());
					} else
						// si date supérieur à dateFin dossier
						if (LibDate.compareTo(date, taInfoEntreprise.getDatefinInfoEntreprise()) == 1) {
							((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateDocument(taInfoEntreprise.getDatefinInfoEntreprise());
							((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateEchDocument(taInfoEntreprise.getDatefinInfoEntreprise());
							((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateLivDocument(taInfoEntreprise.getDatefinInfoEntreprise());
						} else {
							((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateDocument(new Date());
							((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateEchDocument(new Date());
							((IHMEnteteAvisEcheance) selectedAvisEcheance).setDateLivDocument(new Date());
						}
					((IHMEnteteAvisEcheance) selectedAvisEcheance).setCodeTiers("");
					((IHMEnteteAvisEcheance) selectedAvisEcheance).setIdAdresse(0);
					((IHMEnteteAvisEcheance) selectedAvisEcheance).setIdAdresseLiv(0);
					...
					if(typePaiementDefaut!=null){
						 ...
						 */
						if(taTPaiement!=null) {
							((IHMEnteteAvisEcheance) selectedAvisEcheance).setCodeTPaiement(typePaiementDefaut);
							taDocument.setTaTPaiement(taTPaiement);
						}
					}					
					mapperUIToModel.map(((IHMEnteteAvisEcheance) selectedAvisEcheance), taDocument);
...
//				setIhmOldEnteteDevis();
//				taDocument = dao.findById(((IHMEnteteAvisEcheance) selectedAvisEcheance).getIdDocument());
...
//				else 
	...
	public Boolean setIhmOldEnteteDocumentRefresh(){
		try {			
			if (selectedAvisEcheance != null){
				this.ihmOldEnteteAvisEcheance = IHMEnteteAvisEcheance.copy((IHMEnteteAvisEcheance) selectedAvisEcheance);
				if (taDocument!=null){
	...
	protected void actSupprimer() throws Exception {
		...
		TaComptePointDAO daoComptePoint = new TaComptePointDAO(dao.getEntityManager());
		...
		try {
			...
			if (MessageDialog.openConfirm(vue.getShell(),
				...
				TaAvisEcheance u = dao.findById(((IHMEnteteAvisEcheance) selectedAvisEcheance).getIdDocument());
				dao.supprimer(u);
				daoComptePoint.removeTousLesPointsBonus(u,true);
...
//					listeDocument.remove(taDocument);
	...
	protected void actAnnuler(boolean annulationForcee,boolean insertion,boolean messageAffiche) throws Exception {
		...
		if (!vue.isDisposed() && !vue.getShell().isDisposed()) {
			...
			boolean message = messageAffiche && !((IHMEnteteAvisEcheance) selectedAvisEcheance).DocumentEstVide(taDocument.dateDansExercice(new Date()));
					...
					case C_MO_EDITION:
						...
						case C_MO_EDITION:
							...
							if (repondu) {
								...
								dao.annulerCodeGenere(((IHMEnteteAvisEcheance) selectedAvisEcheance).getCodeDocument(),Const.C_CODE_DOCUMENT);
...
//		impressionAvisEcheance.imprimerSelection(((SWTEnteteDevis) swtDevis.getEntete()).getID_DEVIS(),
//				swtDevis.getEntete().getCODE(), true);
		
//		Collection<TaAvisEcheance> collectionTaAvisEcheance = dao.selectAll();
		Collection<TaAvisEcheance> collectionTaAvisEcheance = new LinkedList<TaAvisEcheance>();
//		taDocument.calculeTvaEtTotaux();
		collectionTaAvisEcheance.add(taDocument);
		
		ConstEdition constEdition = new ConstEdition(getEm()); 
		constEdition.setFlagEditionMultiple(true);
		
//		impressionAvisEcheance.setObject(taDocument);
//		impressionAvisEcheance.setConstEdition(constEdition);
//		impressionAvisEcheance.setCollection(collectionTaAvisEcheance);
		
		Bundle bundleCourant = PlugInAvisEcheance.getDefault().getBundle();
		String namePlugin = bundleCourant.getSymbolicName();
		
//		impressionAvisEcheance.imprimerSelection(taDocument.getIdDocument(),taDocument.getCodeDocument(),true,ConstEdition.FICHE_FILE_REPORT_PROFORMA,namePlugin,
//				TaAvisEcheance.class.getSimpleName());
		/** 01/03/2010 modifier les editions (zhaolin) **/
		baseImpressionEdition.setObject(taDocument);
		baseImpressionEdition.setConstEdition(constEdition);
		baseImpressionEdition.setCollection(collectionTaAvisEcheance);
	...
	protected boolean aideDisponible() {
		...
		switch ((PaEditorAvisEcheanceController.this.dao.getModeObjet().getMode())) {
		case C_MO_CONSULTATION:
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				case C_MO_CONSULTATION:
					if (getFocusCourantSWT().equals(vue.getTfCODE_DOCUMENT())) {
						...
						editorCreationId = EditorAvisEcheance.ID_EDITOR;
						editorInputCreation = new EditorInputAvisEcheance();
						...
						paramAfficheAideRecherche.setTypeEntite(TaAvisEcheance.class);
						...
						paramAfficheAideRecherche.setControllerAppelant(PaEditorAvisEcheanceController.this);
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<IHMAideAvisEcheance,TaAvisEcheanceDAO,TaAvisEcheance>(dao,IHMAideAvisEcheance.class));
						paramAfficheAideRecherche.setTypeObjet(IHMAideAvisEcheance.class);
...
//						l.add(TaAvisEcheance.getTaCPaiement());
						if(taDocument.getTaTiers()!=null)
							l.add(taDocument.getTaTiers().getTaCPaiement());
						TaTCPaiementDAO taTCPaiementDAO = new TaTCPaiementDAO(getEm());
						if(taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_AVIS_ECHEANCE)!=null
								&& taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_AVIS_ECHEANCE).getTaCPaiement()!=null)
								l.add(taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_AVIS_ECHEANCE).getTaCPaiement());
			...
			try {
				...
				if(sourceDeclencheCommandeController==null) {
					...
					if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
						...
						try{
							taDocument.setLegrain(false);
							for (TaLAvisEcheance ligne : taDocument.getLignes()) {
...
//			vue.getTfCODE_TIERS().setEnabled(taDocument.getTaRAcomptes().size()==0);
	...
	public IHMEnteteAvisEcheance getIhmOldEnteteAvisEcheance() {
		return ihmOldEnteteAvisEcheance;
	}

	public void setIhmOldEnteteDevis(IHMEnteteAvisEcheance ihmOldEnteteDevis) {
		this.ihmOldEnteteAvisEcheance = ihmOldEnteteDevis;
	}

	public void setIhmOldEnteteDevis() {
		if (selectedAvisEcheance != null)
			this.ihmOldEnteteAvisEcheance = IHMEnteteAvisEcheance
					.copy((IHMEnteteAvisEcheance) selectedAvisEcheance);
		else {
		}
	}

	public void setVue(PaEditorAvisEcheance vue) {
		super.setVue(vue);
		this.vue = vue;
	}
...
//			}
	...
	protected void actRefresh() throws Exception {
		try{
			vue.setCursor(Display.getCurrent().getSystemCursor(SWT.CURSOR_WAIT));
		if (dao.dataSetEnModif()) {
			if (taDocument!=null && selectedAvisEcheance!=null && (IHMEnteteAvisEcheance) selectedAvisEcheance!=null) {
				mapperModelToUI.map(taDocument, (IHMEnteteAvisEcheance) selectedAvisEcheance);
			}
		}
		listeDocument=null;
...
//				codeDevisContentProposal.setContentProposalProvider(contentProposalProviderDocument());
	...
	public PaLigneAvisEcheanceController getControllerLigne() {
		return controllerLigne;
	}

	public void setControllerLigne(PaLigneAvisEcheanceController controllerLigne) {
	...
	private Boolean remonterDocument(String codeDocument) throws Exception {
		try {
			...
			if(taDocument!=null) {
				factureVide = ((IHMEnteteAvisEcheance) selectedAvisEcheance).DocumentEstVide(taDocument.dateDansExercice(new Date()));
			}
			TaAvisEcheance documentRecherche = rechercheDevis(codeDocument, true, factureVide);
...
//				.isVerrouille();
//		String codeTmp = ((IHMEnteteAvisEcheance) selectedAvisEcheance)
//				.getCodeDevis();
//		try {
...
//							Const.C_CODE_DEVIS);
//				}
//				((IHMEnteteAvisEcheance) selectedAvisEcheance)
...
//
//				((IHMEnteteAvisEcheance) selectedAvisEcheance)
//						.setIHMEnteteAvisEcheance(((SWTEnteteDevis) swtDevis
//								.getEntete()));
//
//				// récupérer code validé dans facture, dataset et interface
...
//			TaAvisEcheanceDAO TaAvisEcheanceDAO = new TaAvisEcheanceDAO(getEm());
//			TaInfoEntrepriseDAO taInfoEntrepriseDAO = new TaInfoEntrepriseDAO(getEm());
//			TaInfoEntreprise taInfoEntreprise = taInfoEntrepriseDAO.findInstance();
//
//			//		List<TaBonliv> l = taBonlivDAO.selectAll();
//			//		if(listeDocument==null)
//			listeDocument =((IDocumentTiersDAO)TaAvisEcheanceDAO).rechercheDocument(taInfoEntreprise.getDatedebInfoEntreprise(),
//					taInfoEntreprise.getDatefinInfoEntreprise());
//			TaAvisEcheanceDAO = null;
//
//		}
////		List<TaAvisEcheance> l = TaAvisEcheanceDAO.selectAll();
...
//			+ " \r\n Echéance = " + LibDate.dateToStringAA(((TaAvisEcheance)taDocument).getDateEchDocument())
//			+ " \r\n Exportée = " 
//			+LibConversion.booleanToStringFrancais(LibConversion.intToBoolean(((TaAvisEcheance)taDocument).getExport()));
...
//	}
	public String[][] initAdapterDocument() {
		...
		if(affichageCtrlEspace){
		if(listeDocument==null){
			TaAvisEcheanceDAO taDocDAO = new TaAvisEcheanceDAO(getEm());
			TaInfoEntrepriseDAO taInfoEntrepriseDAO = new TaInfoEntrepriseDAO(getEm());
			TaInfoEntreprise taInfoEntreprise = taInfoEntrepriseDAO.findInstance();
...
//		//#AFAIRE
//		taTiersDAO.setPreferences(PlugInAvisEcheance.getDefault().getPreferenceStore().getString(PreferenceConstants.TYPE_TIERS_DOCUMENT));
//		if(listeTiers==null)
...
//			a.codeTiers,a.nomTiers,a.codeCompta,a.compte,ttiers.libelleTTiers
	...
	protected void initEtatBouton(boolean initFocus) {
		initEtatBoutonCommand(initFocus,modelEnteteAvisEcheance.getListeObjet());
...
//	}
	...
	public Date dateDansPeriode(Date newValue,String champ) throws Exception{
		if(champ.equals(Const.C_DATE_DOCUMENT)){
			...
			if(newValue!=null){
				if((((IHMEnteteAvisEcheance)selectedAvisEcheance).getDateEchDocument()!=null && 
						((IHMEnteteAvisEcheance)selectedAvisEcheance).getDateEchDocument().before(newValue))||
						((IHMEnteteAvisEcheance)selectedAvisEcheance).getDateEchDocument()==null ){
					taDocument.setDateEchDocument(newValue);
					((IHMEnteteAvisEcheance)selectedAvisEcheance).setDateEchDocument(newValue);
					}
//				if((((IHMEnteteAvisEcheance)selectedAvisEcheance).getDateLivDocument()!=null && 
//						((IHMEnteteAvisEcheance)selectedAvisEcheance).getDateLivDocument().before(newValue))||
//						((IHMEnteteAvisEcheance)selectedAvisEcheance).getDateLivDocument()==null ){
//					taDocument.setDateLivDocument(newValue);
//					((IHMEnteteAvisEcheance)selectedAvisEcheance).setDateLivDocument(newValue);
//					}
			}
		}
...
//	}
	...
	public PaEditorTotauxAvisEcheanceController getControllerTotaux() {
		return controllerTotaux;
	}

	public void setControllerTotaux(PaEditorTotauxAvisEcheanceController controllerTotaux) {
	...
	protected void actSelection() throws Exception {
		try{
			...
			paramAfficheSelectionVisualisation.setNomRequete(Const.C_NOM_VU_AVIS_ECHEANCE_REDUIT);
	...
	public TaAvisEcheance rechercheDevis(String codeDevis,boolean annule,boolean proformaVide) throws Exception {
		TaAvisEcheance fact = null;
	...
	public TaAvisEcheanceDAO getDao() {
		return dao;
	}

	public void setDao(TaAvisEcheanceDAO dao) {
		this.dao = dao;
	}
...
//				}
				...
				if(((IHMEnteteAvisEcheance) selectedAvisEcheance).getIdDocument()!=null) {
					u.setIdDocument(((IHMEnteteAvisEcheance) selectedAvisEcheance).getIdDocument());
				}
...
//					s = new Status(Status.OK,gestComBdPlugin.PLUGIN_ID,"OK");
			...
			if(s.getSeverity()!=Status.ERROR){
				if(nomChamp.equals(Const.C_DATE_DOCUMENT)){
					...
					((IHMEnteteAvisEcheance)selectedAvisEcheance).setDateDocument((Date)value);
					PropertyUtils.setSimpleProperty(taDocument, Const.C_DATE_DOCUMENT, value);
					if(changement){
						((IHMEnteteAvisEcheance)selectedAvisEcheance).setDateEchDocument(taDocument.getDateDocument());
						PropertyUtils.setSimpleProperty(taDocument, Const.C_DATE_ECH_DOCUMENT, taDocument.getDateDocument());
						((IHMEnteteAvisEcheance)selectedAvisEcheance).setDateLivDocument(taDocument.getDateDocument());
				...
				if(nomChamp.equals(Const.C_DATE_LIV_DOCUMENT)) {
					((IHMEnteteAvisEcheance)selectedAvisEcheance).setDateLivDocument((Date)value);
