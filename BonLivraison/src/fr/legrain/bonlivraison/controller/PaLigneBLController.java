package fr.legrain.bonlivraison.controller;
...
import fr.legrain.bonlivraison.ecran.PaSaisieLigneBL;
...
import fr.legrain.gestCom.Module_Document.IHMLBonliv;
...
public class PaLigneBLController extends JPABaseControllerSWTStandard
implements RetourEcranListener, SWTModificationDocumentListener, ISelectionProvider {

	static Logger logger = Logger.getLogger(PaLigneBLController.class.getName());
	...
	protected ActionEnregistrerBonliv actionEnregistrerBonliv = new ActionEnregistrerBonliv("Enregistrer Facture [Ctrl+F3]");
	protected HandlerEnregistrerBonliv handlerEnregistrerBonliv = new HandlerEnregistrerBonliv();
	...
	private PaSaisieLigneBL vue = null;


	private TaLBonlivDAO dao = null;//new TaLBonlivDAO();
	
	private Object ecranAppelant = null;
	private IHMLBonliv ihmLBonliv;
	private IHMTotauxLignesDocument ihmTotauxLignesDocument;
	private IHMLBonliv ihmOldLBonliv;
	private Realm realm;
	private DataBindingContext dbc;
	private DataBindingContext dbcTotaux = null;

//	private ModelLigneBonliv modelLBonliv /*=  new ModelLigneFacture(ibTaTable)*/;
	private Class classModel = IHMLBonliv.class;
	private ModelGeneralObjet<IHMLBonliv,TaLBonlivDAO,TaLBonliv> modelLDocument /*= new ModelGeneralObjet<IHMLBonliv,TaLBonlivDAO,TaLBonliv>(dao,classModel)*/;
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private IObservableValue selectedLigneBonliv;
	...
	private TaLBonliv taLBonliv = null;
	...
	private LgrDozerMapper<IHMLBonliv,TaLBonliv> mapperUIToModel = new LgrDozerMapper<IHMLBonliv,TaLBonliv>();
	private LgrDozerMapper<TaLBonliv,IHMLBonliv> mapperModelToUI = new LgrDozerMapper<TaLBonliv,IHMLBonliv>();
	
	private LgrDozerMapper<TaBonliv,IHMTotauxLignesDocument> mapperModelToUITotauxLignes = new LgrDozerMapper<TaBonliv,IHMTotauxLignesDocument>();

	public PaLigneBLController(){
	}
	
	public PaLigneBLController(final PaSaisieLigneBL vue/*, SWTFacture facture, WorkbenchPart part*/) {
		this(vue,null);
	}
	
	public PaLigneBLController(final PaSaisieLigneBL vue/*, SWTFacture facture, WorkbenchPart part*/,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		dao = new TaLBonlivDAO(getEm());
		try {
			setVue(vue);
...
//			// fenetre sur ESC
	 ...
	 * @return
	 */
	public List<IHMLBonliv> IHMmodel() {
		LinkedList<TaLBonliv> ldao = new LinkedList<TaLBonliv>();
		LinkedList<IHMLBonliv> lswt = new LinkedList<IHMLBonliv>();
		
		if(masterEntity!=null && !masterEntity.getLignes().isEmpty()) {

			ldao.addAll(masterEntity.getLignes());

			LgrDozerMapper<TaLBonliv,IHMLBonliv> mapper = new LgrDozerMapper<TaLBonliv,IHMLBonliv>();
			for (TaLBonliv o : ldao) {
				IHMLBonliv t = null;
				t = (IHMLBonliv) mapper.map(o, IHMLBonliv.class);
				lswt.add(t);
			}
	...
	public void bind(PaSaisieLigneBL paArticleSWT) {
		try {
			if (modelLDocument==null)modelLDocument = new ModelGeneralObjet<IHMLBonliv, TaLBonlivDAO, TaLBonliv>(dao,classModel);
...
//				tableViewer.setInput(writableList);
				...
				selectedLigneBonliv = ViewersObservables.observeSingleSelection(tableViewer);
			}
			...
			bindingFormMaitreDetail(dbc, realm, selectedLigneBonliv,classModel);
...
//			ibTaTable.ouvreDataset();
			if (((ParamAfficheLBL)param).getFocusSWT()!=null)
				((ParamAfficheLBL)param).getFocusSWT().forceFocus();
			else ((ParamAfficheLBL)param).setFocusSWT(vue.getTfCODE_ARTICLE());			
		...
		if (param.getModeEcran() != null
			...
			try {
				InsertionLigne(false,((ParamAfficheLBL)param).getInitFocus());
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfQTE_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_QTE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfQTE2_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_QTE2_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfMT_HT_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_MT_HT_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_MT_TTC_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfPRIX_U_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_PRIX_U_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfREM_TX_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_REM_TX_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfNUM_LIGNE_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_NUM_LIGNE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfCODE_ARTICLE(), new InfosVerifSaisie(new TaArticle(),Const.C_CODE_ARTICLE,null));
		mapInfosVerifSaisie.put(vue.getTfLIB_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_LIB_L_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfU1_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_U1_L_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfU2_L_BONLIV(), new InfosVerifSaisie(new TaLBonliv(),Const.C_U1_L_DOCUMENT,null));
	...
	protected void initMapComposantChamps() {
		...
		mapComposantChamps.put(vue.getTfLIB_L_BONLIV(), Const.C_LIB_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfQTE_L_BONLIV(), Const.C_QTE_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfU1_L_BONLIV(), Const.C_U1_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfQTE2_L_BONLIV(), Const.C_QTE2_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfU2_L_BONLIV(), Const.C_U2_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfPRIX_U_L_BONLIV(), Const.C_PRIX_U_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfREM_TX_L_BONLIV(), Const.C_REM_TX_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfMT_HT_L_BONLIV(), Const.C_MT_HT_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfMT_TTC_L_BONLIV(), Const.C_MT_TTC_L_DOCUMENT);
...
//				}
		...
		mapComposantChamps.put(vue.getTfNUM_LIGNE_L_BONLIV(), Const.C_NUM_LIGNE_L_DOCUMENT);
		...
		vue.getTfQTE_L_BONLIV().addVerifyListener(queDesChiffresPositifs);
		vue.getTfQTE2_L_BONLIV().addVerifyListener(queDesChiffresPositifs);
		vue.getTfMT_HT_L_BONLIV().addVerifyListener(queDesChiffresPositifs);
		vue.getTfMT_TTC_L_BONLIV().addVerifyListener(queDesChiffresPositifs);
		vue.getTfPRIX_U_L_BONLIV().addVerifyListener(queDesChiffres);
		vue.getTfREM_TX_L_BONLIV().addVerifyListener(queDesChiffresPositifs);
		vue.getTfNUM_LIGNE_L_BONLIV().addVerifyListener(queDesChiffresPositifs);
...
//			}
			...
			articleContentProposal.addContentProposalListener(new IContentProposalListener2(){
				...
				public void proposalPopupOpened(ContentProposalAdapter adapter) {
					...
					if(!dao.dataSetEnModif()&& taLBonliv.ligneEstVide()  )
	...
	public void initWorkenchPartCommands(WorkbenchPart part){
		...
		handlerService.activateHandler(C_COMMAND_LIGNE_FACTURE_ENTETE_ENREGISTRER_ID, handlerEnregistrerBonliv,activePartExpression);
			
	}

	public PaLigneBLController getThis() {
		return this;
	}
	...
	public void retourEcran(final RetourEcranEvent evt) {
		try {
			if (evt.getRetour() != null && (evt.getSource() instanceof SWTPaAideControllerSWT)) {
				if (getFocusAvantAideSWT() instanceof Text) {
					try {
						((Text) getFocusAvantAideSWT()).setText(((ResultAffiche) evt
								.getRetour()).getResult());	
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_ARTICLE())||getFocusAvantAideSWT().equals(vue.getTfLIB_L_BONLIV())){
							TaArticle f = null;
							TaArticleDAO t = new TaArticleDAO(getEm());
							f = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
							t = null;
							taLBonliv.setLegrain(true);
								taLBonliv.setTaArticle(f);
						}
						if(getFocusAvantAideSWT().equals(vue.getTfU1_L_BONLIV())
								||getFocusAvantAideSWT().equals(vue.getTfU2_L_BONLIV())){
							TaUnite f = null;
							TaUniteDAO t = new TaUniteDAO(getEm());
							f = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
							t = null;
							if(getFocusAvantAideSWT().equals(vue.getTfU1_L_BONLIV()))
								taLBonliv.setU1LDocument(f.getCodeUnite());
							else
								taLBonliv.setU2LDocument(f.getCodeUnite());
						}					
						if(getFocusAvantAideSWT().equals(vue.getTfPRIX_U_L_BONLIV())){
							TaPrix f = null;
							...
							if(masterEntity.getTtc()==1)
								taLBonliv.setPrixULDocument(f.getPrixttcPrix());
							else
								taLBonliv.setPrixULDocument(f.getPrixPrix());
						}					
				...
				if(this.getFocusAvantAideSWT() instanceof Text && this.getFocusAvantAideSWT()==vue.getTfPRIX_U_L_BONLIV()){
...
//						ibTaTable.lgrLocateID(((ResultAffiche) evt.getRetour()).getIdResult());
			...
			if(parentEcran!=null) {
				...
				((PaEditorBLController)parentEcran).initEtatBouton(false);
			}
		}
		}
...
//		public void run() {
		if (getFocusCourantSWT()==vue.getTfPRIX_U_L_BONLIV()){
			try {
				...
				if (masterEntity.getTtc()==0){
					...
					paramAideCalcTVA.setMtHT(vue.getTfPRIX_U_L_BONLIV().getText());
				}
				else{
					paramAideCalcTVA.setFocusSWT(paAideCalcTVA.getTfMtHT());
					paramAideCalcTVA.setMtTTC(vue.getTfPRIX_U_L_BONLIV().getText());
				}
...
//	);
	...
	protected void actChangeTva() throws Exception{
		if(!LibChaine.empty(((IHMLBonliv)selectedLigneBonliv.getValue()).getCodeArticle())
			...
			InputDialog input = new InputDialog(vue.getShell()
					...
					((IHMLBonliv)selectedLigneBonliv.getValue()).getCodeTvaLDocument(),new IInputValidator() {
			...
			if(masterDAO.dataSetEnModif()){
				...
				if(input.getValue()!=null ){
					if(MessageDialog.openConfirm(vue.getShell()
						...
						((IHMLBonliv)selectedLigneBonliv.getValue()).setCodeTvaLDocument(retour);
						if(input.getValue().equals("")){
							taLBonliv.setCodeTvaLDocument("");
							taLBonliv.setTauxTvaLDocument(BigDecimal.valueOf(0));
						}else{
							TaTvaDAO daoTva = new TaTvaDAO(getEm());
							taLBonliv.setCodeTvaLDocument(retour);
							taLBonliv.setTauxTvaLDocument(daoTva.findByCode(retour).getTauxTva());
						}
...
//			}
			if(continuer){
				...
				for (TaLBonliv p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument().equals(((IHMLBonliv) selectedLigneBonliv.getValue()).getNumLigneLDocument())) {
						taLBonliv = p;
						taLBonliv.setLegrain(true);
						ihmLBonliv= IHMLBonliv.copy((IHMLBonliv) selectedLigneBonliv.getValue());
					}				
				}
				initEtatBouton(false);
			}
	...
	protected void actSupprimer() throws Exception {
		...
		try {
			...
			for (TaLBonliv p : masterEntity.getLignes()) {
				...
				dao.supprimer(taLBonliv);
				taLBonliv.setTaDocument(null);
				masterEntity.removeLigne(taLBonliv);
				taLBonliv=null;
				actRefresh();
				refreshTotauxLignes();
				initEtatBouton();
	...
	protected void actAnnuler() throws Exception {
		...
		try {
			...
			case C_MO_INSERTION:
				message =!taLBonliv.ligneEstVide();
				...
				if(repondu){
					if(selectedLigneBonliv!=null && selectedLigneBonliv.getValue() != null
			...
			case C_MO_EDITION:
				...
				if(repondu){
					int rang =((WritableList)tableViewer.getInput()).indexOf(selectedLigneBonliv.getValue());
					List<IHMLBonliv> l = IHMmodel();
					if(rang!=-1)
					  l.set(rang, ihmOldLBonliv);
					
					writableList = new WritableList(realm, l, classModel);
					tableViewer.setInput(writableList);
					tableViewer.refresh();
					tableViewer.setSelection(new StructuredSelection(ihmOldLBonliv), true);
					
					mapperUIToModel.map(ihmOldLBonliv,taLBonliv);
//					ctrlTousLesChampsAvantEnregistrementSWT();//si on contrôle tous les champs
	...
	protected boolean aideDisponible() {
		...
		case C_MO_INSERTION:
			if (getFocusCourantSWT()==vue.getTfCODE_ARTICLE() 
					||getFocusCourantSWT()==vue.getTfLIB_L_BONLIV()
					|| getFocusCourantSWT()==vue.getTfU1_L_BONLIV() 
					|| getFocusCourantSWT()==vue.getTfU2_L_BONLIV() 
					|| getFocusCourantSWT()==vue.getTfPRIX_U_L_BONLIV())
				result = true;
			break;
		default:
			break;
	...
	protected void actPrecedent() throws Exception {
		if(taLBonliv!=null && taLBonliv.ligneEstVide()){
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				switch ((PaLigneBLController.this.dao.getModeObjet().getMode())) {
				case C_MO_CONSULTATION:
				...
				case C_MO_INSERTION:
					...
					if (getFocusCourantSWT()==vue.getTfLIB_L_BONLIV()){
						PaArticleSWT paArticleSWT = new PaArticleSWT(s2,SWT.NULL);
						SWTPaArticlesController swtPaArticlesController = new SWTPaArticlesController(paArticleSWT);
						paramAfficheAideRecherche.setForceAffichageAideRemplie(false);
						...
						paramAfficheAideRecherche.setDebutRecherche(vue.getTfLIB_L_BONLIV().getText());
						paramAfficheAideRecherche.setControllerAppelant(getThis());
					...
					if (getFocusCourantSWT()==vue.getTfU1_L_BONLIV()||
							getFocusCourantSWT()==vue.getTfU2_L_BONLIV()){
						...
						if(getFocusCourantSWT()==vue.getTfU1_L_BONLIV())
							paramAfficheAideRecherche.setDebutRecherche(vue.getTfU1_L_BONLIV().getText());
						else
							paramAfficheAideRecherche.setDebutRecherche(vue.getTfU2_L_BONLIV().getText());
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new TaUniteDAO(getEm()).modelObjetUniteArticle(taLBonliv.getTaArticle().getCodeArticle()));
					...
					if (getFocusCourantSWT()==vue.getTfPRIX_U_L_BONLIV()){
						...
						paramAffichePrix.setIdArticle(LibConversion.integerToString(getSelectedLigneBonliv().getIdArticle()));
						...
						paramAfficheAideRecherche.setDebutRecherche(vue.getTfPRIX_U_L_BONLIV().getText());
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTPrix,TaPrixDAO,TaPrix>(taLBonliv.getTaArticle().getTaPrixes(),SWTPrix.class,getEm()));
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "L_BONLIV";
		try {
			IStatus s = null;
			boolean change=true;
			if(nomChamp.equals(Const.C_CODE_ARTICLE)) {
				...
				if(ihmLBonliv!=null)
					change = !f.getCodeArticle().equals(ihmLBonliv.getCodeArticle());
				if(change){
					taLBonliv.setTaArticle(f);
					taLBonliv.initQte2(f);
					mapperModelToUI.map(taLBonliv, ihmLBonliv);
				}
				}
				dao = null;
			} else {
				TaLBonliv u = new TaLBonliv();
				...
				if(nomChamp.equals(Const.C_QTE_L_DOCUMENT)){
					if (ihmLBonliv!=null)
						change =u.getQteLDocument()!=null&& !u.getQteLDocument().equals(ihmLBonliv.getQteLDocument());
				}
				
				if(s.getSeverity()!=IStatus.ERROR) {
					PropertyUtils.setSimpleProperty(taLBonliv, nomChamp, value);
				}
			}
			
			if(s.getSeverity()!=IStatus.ERROR) {
				if(nomChamp.equals(Const.C_QTE_L_DOCUMENT)&& change) {
					if(taLBonliv.getTaArticle()!=null){
						taLBonliv.initQte2(taLBonliv.getTaArticle());
					}
				}				
				mapperModelToUI.map(taLBonliv, ((IHMLBonliv)selectedLigneBonliv.getValue()));
			}
	...
	protected void actEnregistrer(boolean insertion) throws Exception {
		try {
			if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
				...
				ctrlTousLesChampsAvantEnregistrementSWT();
				if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
					LgrDozerMapper<IHMLBonliv,TaLBonliv> mapper = new LgrDozerMapper<IHMLBonliv,TaLBonliv>();
					mapper.map((IHMLBonliv) selectedLigneBonliv.getValue(),taLBonliv);
				...
				masterEntity.enregistreLigne(taLBonliv);
				dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_CONSULTATION);
...
//	}
	...
	public void initEtatComposant(String type) throws SQLException{		
		initEtatComposant();
		vue.getTfQTE_L_BONLIV().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfQTE2_L_BONLIV().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfU1_L_BONLIV().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfU2_L_BONLIV().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfPRIX_U_L_BONLIV().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfREM_TX_L_BONLIV().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
	}

	public void initEtatComposant() {
		try {
			vue.getTfNUM_LIGNE_L_BONLIV().setEnabled(false);
			vue.getTfMT_HT_L_BONLIV().setEditable(false);
			vue.getTfMT_TTC_L_BONLIV().setEditable(false);
			...
			vue.getTfLIB_L_BONLIV().setEditable(editable);
			vue.getTfPRIX_U_L_BONLIV().setEditable(editable);
			vue.getTfQTE_L_BONLIV().setEditable(editable);
			vue.getTfQTE2_L_BONLIV().setEditable(editable);
			vue.getTfU1_L_BONLIV().setEditable(editable);
			vue.getTfU2_L_BONLIV().setEditable(editable);
			vue.getTfREM_TX_L_BONLIV().setEditable(editable);
			vue.getTfMT_HT_L_BONLIV().setEditable(false);
			vue.getTfMT_TTC_L_BONLIV().setEditable(false);
	...
	public boolean isUtilise(){
		return ((IHMLBonliv)selectedLigneBonliv.getValue()).getIdLDocument()!=null && 
		!dao.recordModifiable(dao.getNomTable(),
				((IHMLBonliv)selectedLigneBonliv.getValue()).getIdLDocument());		
	}
	public IHMLBonliv getIhmOldLBonliv() {
		return ihmOldLBonliv;
	}

	public void setIhmOldLBonliv(IHMLBonliv swtOldBonliv) {
		this.ihmOldLBonliv = swtOldBonliv;
	}

	public void setIhmOldLBonliv() {
		if (selectedLigneBonliv!=null && selectedLigneBonliv.getValue() != null)
			this.ihmOldLBonliv = IHMLBonliv.copy((IHMLBonliv) selectedLigneBonliv.getValue());
		else {
			if(tableViewer.selectionGrille(0)){
				this.ihmOldLBonliv = IHMLBonliv.copy((IHMLBonliv) selectedLigneBonliv.getValue());
				tableViewer.setSelection(new StructuredSelection(
						(IHMLBonliv) selectedLigneBonliv.getValue()), true);
			}}
	}

	public void setVue(final PaSaisieLigneBL vue) {
		super.setVue(vue);
		this.vue = vue;
	}
	...
	protected void initMapComposantDecoratedField() {
		...
		mapComposantDecoratedField.put(vue.getTfLIB_L_BONLIV(), vue.getFieldLIB_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfMT_HT_L_BONLIV(), vue.getFieldMT_HT_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfMT_TTC_L_BONLIV(), vue.getFieldMT_TTC_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfNUM_LIGNE_L_BONLIV(), vue.getFieldNUM_LIGNE_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfPRIX_U_L_BONLIV(), vue.getFieldPRIX_U_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfQTE_L_BONLIV(), vue.getFieldQTE_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfQTE2_L_BONLIV(), vue.getFieldQTE2_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfREM_TX_L_BONLIV(), vue.getFieldREM_TX_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfU1_L_BONLIV(), vue.getFieldU1_L_BONLIV());
		mapComposantDecoratedField.put(vue.getTfU2_L_BONLIV(), vue.getFieldU2_L_BONLIV());
	}
	...
	protected void sortieChamps() {
		try {
			if (masterEntity.getLigne(masterEntity.getLigneCourante())!=null && 
					( (TaLBonliv)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaTLigne().getIdTLigne()!=0)
				initEtatComposant(( (TaLBonliv)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaTLigne().getCodeTLigne());		
	...
	protected void actRefresh() throws Exception {
		...
		tableViewer.selectionGrille(
				tableViewer.selectionGrille(selectedLigneBonliv.getValue()));
	}
	...
	public void initTTC() {
		...
		if(masterEntity!=null && masterEntity.getTtc()==1) {
			...
			vue.getTfMT_HT_L_BONLIV().setEnabled(!ttc);
			vue.getTfMT_TTC_L_BONLIV().setEnabled(ttc);
			vue.getLaMT_HT_L_BONLIV().setEnabled(!ttc);
			vue.getLaMT_TTC_L_BONLIV().setEnabled(ttc);
	}
...
//	public int recupLignes(SWTBonliv swtBonliv) {
//		int nbLignes = 0;
//		try {
//			
//			setSwtBonliv(swtBonliv);
//			ibTaTable.setBonliv(swtBonliv);
//			ibTaTable.videTableTemp();
//			swtBonliv.getLignes().clear();
//			swtBonliv.getLignesTVA().clear();
//		
//			ibTaTable.recupLignesBonliv(swtBonliv.getEntete().getCODE());
//			initLigneCourantSurRow();
//			ibTaTable.initLigneCourantSurRow();
//			writableList = new WritableList(realm, modelLBonliv.remplirListe(), classModel);
//			tableViewer.setInput(writableList);
//			nbLignes = modelLBonliv.getListeObjet().size();
//
//			for (IHMLBonliv ihmLigne : modelLBonliv.getListeObjet()) {
//				SWTLigneBonliv swtLigne = new SWTLigneBonliv((IHMLBonliv)selectedLigneBonliv.getValue());
//				swtLigne.setSWTLigneBONLIV(ihmLigne);
//				swtBonliv.addLigne(swtLigne);
//			}
//			tableViewer.selectionGrille(0);
//			initEtatBouton(false);
//			return nbLignes;
...
//	}
	...
	protected class ActionEnregistrerBonliv extends ActionEnregistrer {
		public ActionEnregistrerBonliv(String name) {
			super(name);
		}
...
//			}
	...
	private class SelectionGrille extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			setIhmOldLBonliv();
			if(selectedLigneBonliv.getValue()!=null && masterEntity.getLignes().size()>vue.getGrille().getSelectionIndex()){
				for (TaLBonliv p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument()==((IHMLBonliv) selectedLigneBonliv.getValue()).getNumLigneLDocument()) {
						taLBonliv = p;
					}				
				}
			}
	...
	public IHMLBonliv getSelectedLigneBonliv() {
		IHMLBonliv resultat = null;
		if(selectedLigneBonliv!=null && selectedLigneBonliv.getValue()!=null)
			resultat = (IHMLBonliv)selectedLigneBonliv.getValue();
		return resultat;
	};	
	...
	protected void InsertionLigne(Boolean ajout, boolean initFocus) throws Exception{
		...
		try {
			...
			if(masterDAO.dataSetEnModif()){
			setIhmOldLBonliv();
			ihmLBonliv = new IHMLBonliv();
			dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_INSERTION);
			taLBonliv = new TaLBonliv(true); 
			taLBonliv.setTaTLigne(new TaTLigneDAO(getEm()).findByCode(Const.C_CODE_T_LIGNE_C));
			
			List<IHMLBonliv> modelListeLigneBonliv = IHMmodel();
			
			if(ajout)
				masterEntity.addLigne(taLBonliv);
			else{
				if(vue.getGrille().getItemCount()!=0) 
				masterEntity.insertLigne(taLBonliv,vue.getGrille().getSelectionIndex());
				else
					masterEntity.insertLigne(taLBonliv,0);
					
			}
			ihmLBonliv.setNumLigneLDocument(taLBonliv.getNumLigneLDocument());
			taLBonliv.setTaDocument(masterEntity);
			taLBonliv.initialiseLigne(true);
			
			modelListeLigneBonliv.add(ihmLBonliv.getNumLigneLDocument(),ihmLBonliv);
			
			writableList = new WritableList(realm, modelListeLigneBonliv, classModel);
			tableViewer.setInput(writableList);
			tableViewer.refresh();
			tableViewer.setSelection(new StructuredSelection(ihmLBonliv),true);
			initEtatComposant(taLBonliv.getTaTLigne().getCodeTLigne());

			taLBonliv.setNumLigneLDocument(ihmLBonliv.getNumLigneLDocument());
...
//			}		
	...
	protected void initEtatBouton(boolean initFocus) {
		...
		vue.getTfU2_L_BONLIV().setEditable(editable);
		vue.getTfREM_TX_L_BONLIV().setEditable(editable);
		vue.getTfMT_HT_L_BONLIV().setEditable(editable);
		vue.getTfMT_TTC_L_BONLIV().setEditable(editable);
...
//								e.doit=false;
			...
			protected class HandlerEnregistrerBonliv extends LgrAbstractHandler {
			...
			public boolean changementPageValide(){
				if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
					...
					if(taLBonliv!=null && selectedLigneBonliv!=null && ((IHMLBonliv) selectedLigneBonliv.getValue())!=null) {
						mapperUIToModel.map((IHMLBonliv) selectedLigneBonliv.getValue(),taLBonliv);
					}
				}
				return true;
			}

			public TaBonliv getMasterEntity() {
				return masterEntity;
			}

			public void setMasterEntity(TaBonliv masterEntity) {
				this.masterEntity = masterEntity;
			}

			public TaBonlivDAO getMasterDAO() {
				return masterDAO;
			}

			public void setMasterDAO(TaBonlivDAO masterDAO) {
