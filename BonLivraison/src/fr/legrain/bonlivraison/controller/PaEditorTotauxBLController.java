...
import fr.legrain.bonlivraison.divers.IHMLignesTVABL;
import fr.legrain.bonlivraison.divers.ModelLignesTVABL;
import fr.legrain.bonlivraison.divers.ModelTotauxBL;
import fr.legrain.bonlivraison.ecran.PaTotauxBL;
...
public class PaEditorTotauxBLController extends JPABaseControllerSWTStandard implements 
ISelectionProvider,   RetourEcranListener,
SWTModificationDocumentListener,ChangeModeListener {

	static Logger logger = Logger.getLogger(PaEditorTotauxBLController.class.getName());
	private PaTotauxBL vue = null;
		
	public List<Control> listeComposantEntete = null;
	protected Map<Control, String> mapComposantChampsCommentaire = null;

	private IHMTotauxBonliv ihmTotauxBonliv;
	private IHMTotauxBonliv ihmOldTotauxBonliv;
	private Realm realm;
	private DataBindingContext dbc;
	private String dataTVA[][] = {};
	private ModelTotauxBL modelTotaux = new ModelTotauxBL();
	private ModelLignesTVABL modelLignesTVA;
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private Object selectedTotauxBonliv = new IHMTotauxBonliv();
	private String[] listeChamp;
	private Class classModel = IHMTotauxBonliv.class;

	private MapChangeListener changeListener = new MapChangeListener();

	private TaBonliv masterEntity = null;
	private TaBonlivDAO masterDAO = null;
	private TaBonlivDAO daoLocal = null;

	
	private LgrDozerMapper<IHMTotauxBonliv,TaBonliv> mapperUIToModel = new LgrDozerMapper<IHMTotauxBonliv,TaBonliv>();
	private LgrDozerMapper<TaBonliv,IHMTotauxBonliv> mapperModelToUI = new LgrDozerMapper<TaBonliv,IHMTotauxBonliv>();
	
	public PaEditorTotauxBLController(){}
	
	public PaEditorTotauxBLController(PaTotauxBL vue/*, WorkbenchPart part*/) {
		this(vue,null);
	}
	
	public PaEditorTotauxBLController(PaTotauxBL vue/*, WorkbenchPart part*/,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		try {
			setVue(vue);
			daoLocal = new TaBonlivDAO(getEm());
	...
	private void initController() {
		try {
			...
			bind();
			try{
				((IHMTotauxBonliv)selectedTotauxBonliv).setImprimer(BonLivraisonPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.IMPRIMER_AUTO));
			}catch (Exception e) {	
			}
			//#AFAIRE
//			initEtatBouton();
	...
	public void bind() {
		try {
			...
			LgrDozerMapper<TaBonliv,IHMTotauxBonliv> mapper = new LgrDozerMapper<TaBonliv,IHMTotauxBonliv>();
			if(ihmTotauxBonliv==null) ihmTotauxBonliv = new IHMTotauxBonliv();
			if(masterEntity!=null)
				mapper.map(masterEntity,ihmTotauxBonliv);
			
			if(modelTotaux.getListeObjet().isEmpty()) {
				modelTotaux.getListeObjet().add(ihmTotauxBonliv);
			}

			selectedTotauxBonliv = ihmTotauxBonliv;
			dbc = new DataBindingContext(realm);

			dbc.getValidationStatusMap().addMapChangeListener(changeListener);
			setDbcStandard(dbc);

			bindingFormSimple(dbc, realm, selectedTotauxBonliv,classModel);
			bindingFormSimple(masterDAO, mapComposantChampsCommentaire, dbc, realm, selectedTotauxBonliv, classModel);
	...
	public void bindTVA() {
		try {
			modelLignesTVA = new ModelLignesTVABL();
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());

			tableViewer = new LgrTableViewer(vue.getTableTVA());
			tableViewer.createTableCol(vue.getTableTVA(), "BonLivLignesTVA", Const.C_FICHIER_LISTE_CHAMP_GRILLE);
			listeChamp = tableViewer.setListeChampGrille("BonLivLignesTVA", Const.C_FICHIER_LISTE_CHAMP_GRILLE);

//			ObservableListContentProvider viewerContent = new ObservableListContentProvider();
//			tableViewer.setContentProvider(viewerContent);
//
//			IObservableMap[] attributeMaps = BeansObservables.observeMaps(
//					viewerContent.getKnownElements(), IHMLignesTVABL.class, listeChamp);
//
//			tableViewer.setLabelProvider(new ObservableMapLabelProvider(attributeMaps));
//			writableList = new WritableList(realm, modelLignesTVA.remplirListe(masterEntity), IHMLignesTVABL.class);
//			tableViewer.setInput(writableList);
			
			LgrViewerSupport.bind(tableViewer, 
					new WritableList(modelLignesTVA.remplirListe(masterEntity), IHMLignesTVABL.class),
...
//			actInserer();
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfMT_HT_CALC(), new InfosVerifSaisie(new TaBonliv(),Const.C_MT_HT_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_CALC(), new InfosVerifSaisie(new TaBonliv(),Const.C_MT_TTC_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TVA_CALC(), new InfosVerifSaisie(new TaBonliv(),Const.C_MT_TVA_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_HT_BONLIV(), new InfosVerifSaisie(new TaBonliv(),Const.C_TX_REM_HT_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_TTC_BONLIV(), new InfosVerifSaisie(new TaBonliv(),Const.C_TX_REM_TTC_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
	...
	protected void initMapComposantChamps() {
		...
		//vue.getTfTX_REM_HT_BONLIV().setEnabled(false);
		vue.getTfTX_REM_TTC_BONLIV().setEnabled(false);

		mapComposantChamps.put(vue.getTfTX_REM_HT_BONLIV(), Const.C_TX_REM_HT_DOCUMENT);
		...
		mapComposantChamps.put(vue.getTfTX_REM_TTC_BONLIV(), Const.C_TX_REM_TTC_DOCUMENT);
		...
		listeComposantFocusable.add(vue.getTfTX_REM_HT_BONLIV());
		listeComposantFocusable.add(vue.getTfMT_HT_CALC());
		listeComposantFocusable.add(vue.getTfMT_TVA_CALC());
		listeComposantFocusable.add(vue.getTfTX_REM_TTC_BONLIV());
		...
		vue.getTfTX_REM_HT_BONLIV().addVerifyListener(queDesChiffresPositifs);
		vue.getTfTX_REM_TTC_BONLIV().addVerifyListener(queDesChiffresPositifs);
	}
...
//	}
	...
	public PaEditorTotauxBLController getThis() {
		return this;
	}
	...
	protected boolean aideDisponible() {
		...
		switch ((PaEditorTotauxBLController.this.masterDAO.getModeObjet().getMode())) {
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
	...
	public IStatus validateUI() throws Exception {
		if ((masterDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
			...
			mapperUIToModel.map(((IHMTotauxBonliv)selectedTotauxBonliv),masterEntity);
		}
		return null;
	}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		...
		try {
			...
			if(s.getSeverity()!=IStatus.ERROR) {
				mapperModelToUI.map(masterEntity, ((IHMTotauxBonliv)selectedTotauxBonliv));
			}
...
//	public SWT_IB_TA_BONLIV getIbTaTable() {
//		return ibTaTable;
//	}
	...
	public IHMTotauxBonliv getIhmOldEnteteBonliv() {
		return ihmOldTotauxBonliv;
	}

	public void setIhmOldTotauxBonliv(IHMTotauxBonliv ihmOldTotauxBonliv) {
		this.ihmOldTotauxBonliv = ihmOldTotauxBonliv;
	}

	public void setIhmOldTotauxBonliv() {
		if (selectedTotauxBonliv!= null)
			this.ihmOldTotauxBonliv = IHMTotauxBonliv.copy((IHMTotauxBonliv) selectedTotauxBonliv);
		else {
			if(tableViewer.selectionGrille(0)){
				setSelectedTotauxBonliv(selectedTotauxBonliv);
				this.ihmOldTotauxBonliv = IHMTotauxBonliv.copy((IHMTotauxBonliv) selectedTotauxBonliv);
				tableViewer.setSelection(new StructuredSelection(
						(IHMTotauxBonliv) selectedTotauxBonliv), true);
			}}
	}

	public void setVue(PaTotauxBL vue) {
		super.setVue(vue);
		this.vue = vue;
	...
	protected void initMapComposantDecoratedField() {
		...
		mapComposantDecoratedField.put(vue.getTfTX_REM_HT_BONLIV(), vue.getFieldTX_REM_HT_BONLIV());
		mapComposantDecoratedField.put(vue.getTfTX_REM_TTC_BONLIV(), vue.getFieldTX_REM_TTC_BONLIV());
	}
	...
	protected void actRefresh() throws Exception {
		if (masterDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION) != 0) {
			if (masterEntity!=null && selectedTotauxBonliv!=null && (IHMTotauxBonliv) selectedTotauxBonliv!=null) {
				mapperModelToUI.map(masterEntity, (IHMTotauxBonliv) selectedTotauxBonliv);
			}
		}
...
//	ISelectionProvider
	...
	public void modificationDocument(SWTModificationDocumentEvent evt) {
		...
		try {
			((IHMTotauxBonliv)selectedTotauxBonliv).setMtHtCalc(LibCalcul.arrondi(montantHT));
			((IHMTotauxBonliv)selectedTotauxBonliv).setMtTtcCalc(LibCalcul.arrondi(montantTTC));
			((IHMTotauxBonliv)selectedTotauxBonliv).setMtTvaCalc(LibCalcul.arrondi(montantTVA));
			((IHMTotauxBonliv)selectedTotauxBonliv).setNetHtCalc(LibCalcul.arrondi(NetHT));
			((IHMTotauxBonliv)selectedTotauxBonliv).setNetTtcCalc(LibCalcul.arrondi(NetTTC));
			((IHMTotauxBonliv)selectedTotauxBonliv).setNetTvaCalc(LibCalcul.arrondi(NetTVA));
			
			if(((IHMTotauxBonliv)selectedTotauxBonliv).getTxRemTtcDocument()==null)
				((IHMTotauxBonliv)selectedTotauxBonliv).setTxRemTtcDocument(new BigDecimal(0));


			if(((IHMTotauxBonliv)selectedTotauxBonliv).getTxRemHtDocument()==null)
				((IHMTotauxBonliv)selectedTotauxBonliv).setTxRemHtDocument(new BigDecimal(0));
...
//	}
	...
	protected void initFocusSWT(IBQuLgr ibTaTable, Map<ModeObjet.EnumModeObjet,Control> focus) {
		setFocusCourantSWT(vue.getTfTX_REM_HT_BONLIV());
...
public void setTaBonliv(TaBonliv masterEntity) {
	this.masterEntity = masterEntity;
	this.masterEntity.addModificationDocumentListener(this);
}

public Object getSelectedTotauxBonliv() {
	return selectedTotauxBonliv;
}

public void setSelectedTotauxBonliv(Object selectedTotauxBonliv) {
	this.selectedTotauxBonliv = selectedTotauxBonliv;
	((ModelObject)selectedTotauxBonliv).addPropertyChangeListener(new TotauxPropertyChangeListener());
}
...
private class TotauxPropertyChangeListener implements PropertyChangeListener {
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		else if(evt.getPropertyName().equals("TX_REM_TTC_BONLIV"))
			masterEntity.setTxRemTtcDocument((BigDecimal)evt.getNewValue());
		else if(evt.getPropertyName().equals("REM_TTC_BONLIV"))
			masterEntity.setRemTtcDocument((BigDecimal)evt.getNewValue());
		else if(evt.getPropertyName().equals("TX_REM_HT_BONLIV"))
			masterEntity.setTxRemHtDocument((BigDecimal)evt.getNewValue());
		else if(evt.getPropertyName().equals("REM_HT_BONLIV"))
...
private void initVue() {
	addExpandBarItem(vue.getExpandBar(), new PaCommentaire(vue
			...
			BonLivraisonPlugin.getImageDescriptor("/icons/creditcards.png")
					.createImage());
...
public void actCreateDoc() throws Exception {
	...
	param.setTypeSrc(TypeDoc.TYPE_BON_LIVRAISON);
