...
public class LgrBLPreferencePage extends FieldEditorPreferencePageLGR{
	...
	static private PropertiesConfiguration listeGestCode = new PropertiesConfiguration();
	 static Logger logger = Logger.getLogger(LgrBLPreferencePage.class.getName());
	  
	public LgrBLPreferencePage() {
		super(GRID);
		String idPlugin = BonLivraisonPlugin.getDefault().getBundle().getSymbolicName();
	...
	public void createFieldEditors() {
		...
		String PathEditionDefaut = ConstEdition.pathFichierEditionsSpecifiques(ConstEdition.FICHE_FILE_REPORT_BONLIV,
				   namePlugin);
		...
		constEdition.valuePropertiePathEditionDefaut = ConstEdition.FICHE_FILE_REPORT_BONLIV;
		constEdition.valuePropertieCommentEditions = String.format(ConstEdition.COMMENT_ONE_EDITION_ENTITY, 
				namePlugin,namePlugin);
		
		File fileEditionsSuppBonliv = constEdition.makeFolderEditions(reportEditionSupp);
		
		boolean flag = constEdition.getAllInfosEdition(filePathEditionDefaut,null,namePlugin,
				  fileEditionSpecifiquesClient,fileEditionSpecifiques,TaBonliv.class.getSimpleName(),
				  null,false,fileEditionsSuppBonliv);
	...
	protected void performApply() {
		...
		try {
			...
			IPreferenceStore store = BonLivraisonPlugin.getDefault().getPreferenceStore();

			// Change la valeur de la clé taCle dans l'objet Properties
			taNouvelleValeur=BonLivraisonPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_FIXE_1) ;
			listeGestCode.setProperty(PreferenceConstants.TA_DOC_FIXE_1,taNouvelleValeur) ;

			taNouvelleValeur=BonLivraisonPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_EXO) ;
			listeGestCode.setProperty(PreferenceConstants.TA_DOC_EXO,taNouvelleValeur) ;

			taNouvelleValeur=BonLivraisonPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_COMPTEUR) ;
