...
@Named
@ViewScoped 
public class MdpOublieBean implements Serializable {
	...
	private TaUtilisateurLogin uLogin = null;
	...
	//		String[][] toEmailandName = new String[][]{ {"nico290382@gmail.com","NP"}};
			...
			String lienCnx = "http://"+bdgProperties.getProperty(BdgProperties.KEY_PREFIXE_SOUS_DOMAINE,"")+tenantDossier+"."+bdgProperties.getProperty(BdgProperties.KEY_NOM_DOMAINE)+":"+bdgProperties.getProperty(BdgProperties.KEY_PORT_HTTP,"")+"/login/motdepasse_validation.xhtml?p="+URLEncoder.encode(b, "UTF-8")+"";
			String lienCnxTxt = "";
	...
	public void changeMotDePasse(ActionEvent event) throws Exception {
		...
		initTenant();
		u = userService.findByCode(identifiant);
		uLogin=utilisateurLoginService.findByCode(identifiant+"_"+tenantDossier);

		if(uLogin!=null && u!=null) {
			...
			if(nouveauMotDePasse!=null && !nouveauMotDePasse.equals("") 
				...
				if(verifComplexiteMotDePasse(nouveauMotDePasse)) {
					...
					uLogin.setPasswd(uLogin.passwordHashSHA256_Base64(nouveauMotDePasse));
					utilisateurLoginService.enregistrerMerge(uLogin);
