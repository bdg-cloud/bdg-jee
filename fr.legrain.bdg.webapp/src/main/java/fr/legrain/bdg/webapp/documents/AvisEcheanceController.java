...
@Named
@ViewScoped  
public class AvisEcheanceController extends AbstractDocumentController<TaAvisEcheance,TaAvisEcheanceDTO,TaLAvisEcheance,TaLAvisEcheanceDTO,TaLAvisEcheanceDTOJSF,TaInfosAvisEcheance> {  



	private @EJB ITaAvisEcheanceServiceRemote taAvisEcheanceService;
	private @EJB ITaInfosAvisEcheanceServiceRemote taInfosDocumentService;
	private @EJB ITaLAvisEcheanceServiceRemote taLAvisEcheanceService;
	
	private @EJB ITaLEcheanceServiceRemote taLEcheanceService;
	
	
	List<TaLAvisEcheanceDTOJSF> listeLigneLiaisonASupprimer = new ArrayList<TaLAvisEcheanceDTOJSF>();
	List<TaLEcheance> listeEcheanceLiaisonASupprimer = new ArrayList<TaLEcheance>();




	public AvisEcheanceController() {  
		//values = getValues();
	}  

	@PostConstruct
	public void postConstruct(){
		listeLigneLiaisonASupprimer.clear();
		listeEcheanceLiaisonASupprimer.clear();
		super.postConstruct();
		listePreferences= taPreferencesService.findByGroupe("AvisEcheance");
		nomDocument="Avis d'échéance";
		setTaDocumentService(taAvisEcheanceService);
		setTaLDocumentService(taLAvisEcheanceService);
		setTaInfosDocumentService(taInfosDocumentService);
		
		initListeEdition();
		stepSynthese = "idSyntheseAvisEcheance";
		stepEntete = "idEnteteAvisEcheance";
		stepLignes = "idLignesAvisEcheance";
		stepTotaux = "idTotauxFormAvisEcheance";
		stepValidation = "idValidationFormAvisEcheance";
		idMenuBouton = "form:tabView:idMenuBoutonAvisEcheance";
		wvEcran = LgrTab.WV_TAB_AVIS_ECHEANCE;
		classeCssDataTableLigne = "myclassAvisEcheance";
		refresh();
		initChoixEdition();
	}
	...
	public void initChoixEdition() {
		choixEdition = false;
		List<TaEdition> listeEditionDisponible = taEditionService.rechercheEditionDisponible("", ConstActionInterne.EDITION_AVIS_ECHEANCE_DEFAUT,null);
	...
	public void refresh(){
//		values = taAvisEcheanceService.selectAllDTO();
		values = taAvisEcheanceService.findAllLight();
		valuesLigne = IHMmodel();
		modeEcran.setMode(EnumModeObjet.C_MO_CONSULTATION);
	}

	public List<TaLAvisEcheanceDTOJSF> IHMmodel() {
		LinkedList<TaLAvisEcheance> ldao = new LinkedList<TaLAvisEcheance>();
		LinkedList<TaLAvisEcheanceDTOJSF> lswt = new LinkedList<TaLAvisEcheanceDTOJSF>();

		if(masterEntity!=null && !masterEntity.getLignes().isEmpty()) {

			ldao.addAll(masterEntity.getLignes());

			LgrDozerMapper<TaLAvisEcheance,TaLAvisEcheanceDTO> mapper = new LgrDozerMapper<TaLAvisEcheance,TaLAvisEcheanceDTO>();
			TaLAvisEcheanceDTO dto = null;
			for (TaLAvisEcheance o : ldao) {
				TaLAvisEcheanceDTOJSF t = new TaLAvisEcheanceDTOJSF();
				dto = (TaLAvisEcheanceDTO) mapper.map(o, TaLAvisEcheanceDTO.class);
				t.setDto(dto);
...
//			PrimeFaces.current().ajax().update("form:tabView:idMenuBoutonAvisEcheance");
//			return currentStepId;
//		} catch (Exception e) {
//			FacesContext context = FacesContext.getCurrentInstance();  
//			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "AvisEcheance", e.getMessage()));
...
//			}
			...
			LgrDozerMapper<TaLAvisEcheanceDTO,TaLAvisEcheance> mapper = new LgrDozerMapper<TaLAvisEcheanceDTO,TaLAvisEcheance>();
			mapper.map((TaLAvisEcheanceDTO) selectedLigneJSF.getDto(),masterEntityLigne);
...
//			modeEcranLigne.setMode(EnumModeObjet.C_MO_INSERTION);
		...
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("AvisEcheance", "actEnregisterLigne")); 
		}
	}

//	public TaLAvisEcheance rechercheLigne(int id){
//		TaLAvisEcheance obj=masterEntityLigne;
//		for (TaLAvisEcheance ligne : masterEntity.getLignes()) {
...
//	}
//	
//	public TaLAvisEcheance rechercheLigneNumLigne(int numLigne){
//		TaLAvisEcheance obj=masterEntityLigne;
//		for (TaLAvisEcheance ligne : masterEntity.getLignes()) {
...
//			
////			for (TaLAvisEcheance lbr : masterEntity.getLignes()) {
...
//			if(ConstWeb.DEBUG) {
//				FacesContext context = FacesContext.getCurrentInstance();  
//				context.addMessage(null, new FacesMessage("AvisEcheance", "actSupprimerLigne")); 
//			}
//		} catch(Exception e) {
//			e.printStackTrace();
//		}
...
//		//((TaLAvisEcheanceDTOJSF) event.getObject()).setAutoInsert(false);
...
////		}
//		selectionLigne((TaLAvisEcheanceDTOJSF) event.getObject());
...
//			selectedLigneJSF=(TaLAvisEcheanceDTOJSF) event.getObject();
...
//	}
//	
//	public void selectionLigne(TaLAvisEcheanceDTOJSF ligne){
...
//	}
	...
	public void actAnnuler(ActionEvent actionEvent) {
		try {
			...
			case C_MO_INSERTION:
				if(selectedDocumentDTO.getCodeDocument()!=null) {
					taAvisEcheanceService.annuleCode(selectedDocumentDTO.getCodeDocument());
				}
				masterEntity=new TaAvisEcheance();
				selectedDocumentDTO=new TaAvisEcheanceDTO();
			...
			case C_MO_EDITION:
				if(selectedDocumentDTO.getId()!=null && selectedDocumentDTO.getId()!=0){
					masterEntity = taAvisEcheanceService.findByIDFetch(selectedDocumentDTO.getId());
					selectedDocumentDTO=taAvisEcheanceService.findByIdDTO(selectedDocumentDTO.getId());
				}				
				break;
		...
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("AvisEcheance", "actAnnuler")); 
		}
...
//			}			
			...
			List<TaLAvisEcheance> listeLigneVide = new ArrayList<TaLAvisEcheance>(); 
			for (TaLAvisEcheance ligne : masterEntity.getLignes()) {
			...
			for (TaLAvisEcheance taLBonReception : listeLigneVide) {
...
//			for (TaLAvisEcheance ligne : masterEntity.getLignes()) { //@@ champs à ajouter aux devis
...
//			masterEntity.setTaGrMouvStock(grpMouvStock); //@@ champs à ajouter aux devis
			...
			masterEntity = taAvisEcheanceService.mergeAndFindById(masterEntity,ITaAvisEcheanceServiceRemote.validationContext);
			taAvisEcheanceService.annuleCode(selectedDocumentDTO.getCodeDocument());
			...
			selectedDocumentDTOs=new TaAvisEcheanceDTO[]{selectedDocumentDTO};
			...
			e.printStackTrace();
			FacesContext context = FacesContext.getCurrentInstance();  
			//context.addMessage(null, new FacesMessage("AvisEcheance", "actEnregistrer")); 
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "AvisEcheance", e.getMessage()));
		}
...
//			TaEtat etat = taEtatService.findByCode(TaEtat.ETAT_ENCOURS_NON_TRANSFORME);
			TaEtat etat = taAvisEcheanceService.rechercheEtatInitialDocument();
...
//			String newCode = taAvisEcheanceService.genereCode(paramsCode);
//			if(newCode!=null && !newCode.equals("")){
//				selectedDocumentDTO.setCodeDocument(newCode);
//			}
			selectedDocumentDTO.setCodeDocument(taAvisEcheanceService.genereCode(paramsCode));
...
//		if(ConstWeb.DEBUG) {
//			FacesContext context = FacesContext.getCurrentInstance();  
//			context.addMessage(null, new FacesMessage("AvisEcheance", "actModifier")); 	 
//		}
//	}
//
...
//		if(ConstWeb.DEBUG) {
//			FacesContext context = FacesContext.getCurrentInstance();  
//			context.addMessage(null, new FacesMessage("AvisEcheance", "actAide")); 	
//		}
//	}
...
//		tabsCenter.ajouterOnglet(b);
		...
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("AvisEcheance", 
					"Nouveau"
					)); 
		}
	} 
...
//					List<TaLigneALigneEcheance> liste  = taLigneALigneEcheanceService.findAllByIdLDocumentAndTypeDoc(selectedLigneJSF.getDto().getIdLDocument(), TaAvisEcheance.TYPE_DOC);
...
//					}
	...
	public void actSupprimer(ActionEvent actionEvent) {
		TaAvisEcheance obj = new TaAvisEcheance();
		try {
			if(autorisationLiaisonComplete()) {
			if(selectedDocumentDTO!=null && selectedDocumentDTO.getId()!=null){
				obj = taAvisEcheanceService.findByIDFetch(selectedDocumentDTO.getId());
			}

			taAvisEcheanceService.remove(obj);
			...
			if(!values.isEmpty()) {
				...
				selectedDocumentDTO = new TaAvisEcheanceDTO();
				selectedDocumentDTOs= new TaAvisEcheanceDTO[]{};	
			}
			...
			if(ConstWeb.DEBUG) {
				FacesContext context = FacesContext.getCurrentInstance();  
				context.addMessage(null, new FacesMessage("AvisEcheance", "actSupprimer"));
			}
			}
		} catch(Exception e) {
			e.printStackTrace();
...
//        requestContext.execute("PF('wvDataTableListeAvisEcheance').filter()");

		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("AvisEcheance", "actFermer")); 
		}
	}
	public void initListeEdition() {
		...
		editionDefautNulle.setLibelleEdition("Avis d'échéance modèle par défaut (V0220)");
...
//				}
			...
			editionParam.setIdActionInterne(ConstActionInterne.EDITION_AVIS_ECHEANCE_DEFAUT);
			...
			if(edition == null) {//si aucune edition n'est choisie
				...
				taEdition = taEditionService.rechercheEditionActionDefaut(null,action, TaAvisEcheance.TYPE_DOC);
				...
				if(taEdition == null) {
					taEdition = taEditionService.rechercheEditionDisponibleProgrammeDefaut("", ConstActionInterne.EDITION_AVIS_ECHEANCE_DEFAUT).get(0);
				}
...
//			}
			...
			if(edition == null) {//si aucune edition n'est choisie
				...
				pdf = taAvisEcheanceService.generePDF(selectedDocumentDTO.getId(),mapParametreEdition,null,null,actionImprimer);
				...
				if(taEdition == null) {
					...
					if(taEdition!=null) {
						...
						try { 
							...
							String localPath  = taAvisEcheanceService.writingFileEdition(taEdition);
							//on genere le pdf avec le fichier xml crée ci-dessus
							 pdf = taAvisEcheanceService.generePDF(selectedDocumentDTO.getId(),localPath,taEdition.getMapParametreEdition(),listeResourcesPath,taEdition.getTheme());
	...
	public void actImprimer(ActionEvent actionEvent) {
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("AvisEcheance", "actImprimer")); 
		}
		try {
			...
			TaAvisEcheance doc =taAvisEcheanceService.findById(selectedDocumentDTO.getId());
			...
			//sessionMap.put("doc", taAvisEcheanceService.findById(selectedDocumentDTO.getId()));
			Map<String,Object> mapParametreEdition = new HashMap<String,Object>();
			List<TaPreferences> liste= taPreferencesService.findByGroupe("avis_echeance");
			mapParametreEdition.put("preferences", liste);
//			mapParametreEdition.put("gestionLot", gestionLot);
			...
			System.out.println("AvisEcheanceController.actImprimer()");
		} catch (FinderException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void actImprimerListeDesAvisEcheance(ActionEvent actionEvent) {
		...
		try {
			...
			sessionMap.put("AvisEcheance", values);
...
//		sessionMap.put("numLot", numLot);
		...
		edition.setIdActionInterne(ConstActionInterne.EDITION_AVIS_ECHEANCE_DEFAUT);
		...
		if(event!=null && event.getObject()!=null) {
			...
			if(taEdition!=null) {
				...
				try { 
					...
					String pdf = taAvisEcheanceService.generePDF(selectedDocumentDTO.getId(),localPath,taEdition.getMapParametreEdition(),listeResourcesPath,taEdition.getTheme());
...
//		try {
//			for (TaLAvisEcheance l : masterEntity.getLignes()) {
...
//		if(ConstWeb.DEBUG) {
//		   	FacesContext context = FacesContext.getCurrentInstance();  
//		    context.addMessage(null, new FacesMessage("AvisEcheance", "actImprimerEtiquetteCB")); 
//		}
//	}
...
////				autoCompleteMapDTOtoUI();
////				
////				masterEntity = taAvisEcheanceService.findById(selectedDocumentDTO.getId());
////				masterEntity.calculeTvaEtTotaux();
////				
////				valuesLigne = IHMmodel();
////				
...
////					FacesContext context = FacesContext.getCurrentInstance();  
////					context.addMessage(null, new FacesMessage("AvisEcheance", 
////							"Chargement du AvisEcheance N°"+selectedDocumentDTO.getCodeTiers()
////							)); 
////				}
//			} else {
	...
	public void updateTab(){
		try {		
			...
			if(ConstWeb.DEBUG) {
				FacesContext context = FacesContext.getCurrentInstance();  
				context.addMessage(null, new FacesMessage("AvisEcheance", 
						"Chargement de l'avis d'Echeance N°"+selectedDocumentDTO.getCodeTiers()
						)); 
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
...
//
//		//		FacesContext context = FacesContext.getCurrentInstance();  
//		//		context.addMessage(null, new FacesMessage("AvisEcheance", "actAbout")); 	    
//	}
//
...
//	}
	...
	public void validateLignesDocument(FacesContext context, UIComponent component, Object value) throws ValidatorException {
		...
		try {
			...
			TaLAvisEcheanceDTO dtoTemp =new TaLAvisEcheanceDTO();
			PropertyUtils.setSimpleProperty(dtoTemp, nomChamp, value);
			taLAvisEcheanceService.validateDTOProperty(dtoTemp, nomChamp, ITaLAvisEcheanceServiceRemote.validationContext );
	...
	public void validateDocument(FacesContext context, UIComponent component, Object value) throws ValidatorException {
		...
		try {
			...
			Set<ConstraintViolation<TaAvisEcheanceDTO>> violations = factory.getValidator().validateValue(TaAvisEcheanceDTO.class,nomChamp,value);
			if (violations.size() > 0) {
				messageComplet = "Erreur de validation : ";
				for (ConstraintViolation<TaAvisEcheanceDTO> cv : violations) {
	...
	public void validateDocumentAvantEnregistrement( Object value) throws ValidatorException {
		...
		try {
			//taBonReceptionService.validateDTOProperty(selectedDocumentDTO,Const.C_CODE_TIERS,  ITaAvisEcheanceServiceRemote.validationContext );
		...
		try {
			if(selectedLigneJSF.getTaCoefficientUnite()!=null) {
				if(!calculCoherenceAffectationCoefficientQte2(selectedLigneJSF.getDto().getQte2LDocument())) {
					...
					PrimeFaces.current().executeScript("PF('wVdialogQte2AvisEcheance').show()");
				}
...
//		System.out.println("AvisEcheanceController.autcompleteSelection() : "+nomChamp);
//
//		if(value!=null) {
...
//						entity=(TaTiers) value;
				...
				if(changement){
					...
					((TaAvisEcheanceDTO)selectedDocumentDTO).setLibelleDocument("AvisEcheance N°"+selectedDocumentDTO.getCodeDocument()+" - "+nomTiers);										
					if(!disableTtc() && !disableTtcSiDocSansTVA()){
						masterEntity.setTtc(entity.getTtcTiers());
						((TaAvisEcheanceDTO)selectedDocumentDTO).setTtc(LibConversion.intToBoolean(masterEntity.getTtc()));
					}									
					changementTiers(true);
				}
...
//	}
	...
	public void initialisationDesInfosComplementaires(Boolean recharger,String typeARecharger){
		...
		try {
			if(((TaAvisEcheanceDTO)selectedDocumentDTO).getCodeDocument()!=null) {
				taInfosDocument = taInfosDocumentService.findByCodeAvisEcheance(((TaAvisEcheanceDTO)selectedDocumentDTO).getCodeDocument());
			} else {
...
//			
//			System.out.println("AvisEcheanceController.initialisationDesAdrFact() ** "+selectedAdresseFact.getAdresse1Adresse());
//		
//		}catch (FinderException e) {
//			e.printStackTrace();
//		}
...
//	}
		...
		if (masterEntity != null) {
			if (masterEntity.getCodeDocument()!=null&& masterEntity.getCodeDocument() != "")
				taInfosDocument = taInfosDocumentService
						.findByCodeAvisEcheance(masterEntity.getCodeDocument());
			else
				taInfosDocument = new TaInfosAvisEcheance();

			
			taCPaiementDoc = null;
			List<TaCPaiement> listeCPaiement=taCPaiementService.rechercheParType(TaAvisEcheance.TYPE_DOC);
			if(listeCPaiement!=null && !listeCPaiement.isEmpty())taCPaiementDoc=listeCPaiement.get(0);
			

			if (taTCPaiementService.findByCode(TaTCPaiement.C_CODE_TYPE_AVIS_ECHEANCE) != null
	...
	public void calculDateEcheance(Boolean appliquer) {
		...
		if(((TaAvisEcheanceDTO)selectedDocumentDTO).getId()==0|| appliquer) {
			...
			((TaAvisEcheanceDTO)selectedDocumentDTO).setDateEchDocument(taAvisEcheanceService.calculDateEcheance(masterEntity,report, finDeMois));
		}
		}
	}
...
@Override
public void IHMmodel(TaLAvisEcheanceDTOJSF dtoJSF, TaLAvisEcheance ligne) {
	LgrDozerMapper<TaLAvisEcheance,TaLAvisEcheanceDTO> mapper = new LgrDozerMapper<TaLAvisEcheance,TaLAvisEcheanceDTO>();
	TaLAvisEcheanceDTO dto = dtoJSF.getDto();

		dto = (TaLAvisEcheanceDTO) mapper.map(ligne, TaLAvisEcheanceDTO.class);
		dtoJSF.setDto(dto);
