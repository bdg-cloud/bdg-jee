...
@Named
@ViewScoped  
public class MensuraController extends AbstractController implements Serializable{
	...
	@EJB private ITaMensuraServiceWebExterneServiceRemote mensuraService;
	...
	private String typeDoc="Devis";
	
	@PostConstruct
	public void postConstruct(){
		try {
			serviceWeb = serviceWebExterneService.findByCode("MENSURA");
			typeEntite = TaSynchroServiceExterne.TYPE_ENTITE_DEVIS;
...
//		if(!listeSynchro.isEmpty()) {
//			derniereSynchro = listeSynchro.get(0);
//		}
		...
		selectedTypeCreation.setDisplayName(typeDoc);
		selectedTypeCreation.setName(typeDoc);
	...
	private void rempliNodes() {
		...
		for (String idCommande : mapLignesParCommandes.keySet()) {
			...
			if(!mapLignesParCommandes.get(idCommande).isEmpty()) {
				...
				dto.setLibelle("Devis n° "+idCommande);
//				dto.setMontantHt(montantHt);
				...
				if(mapLignesParCommandes.get(idCommande).get(0).getTaTiersDTO() != null) {
    ...
    public void actEnregistrer() {  	
		...
		FacesContext context = FacesContext.getCurrentInstance();
				...
				paramTmp.setLibelle("Lignes de commande Mensura séléctionnées");
		if(selectedNodesLignesDTOJSF != null && selectedNodesLignesDTOJSF.length>0) {
...
//		String libelle="Reprise de ";
    ...
    public void onRowSelectLigneMensuraAll(SelectEvent event){
    	System.out.println("onRowSelectLigneMensuraAll a etait cliquéééé");
    }
    public void onRowUnSelectLigneMensuraAll(UnselectEvent event){
    	System.out.println("onRowUnSelectLigneMensuraAll a etait cliquéééé");
    }
    public void onToggleSelectLigneMensuraAll(ToggleSelectEvent event){
    	System.out.println("onToggleSelectLigneMensuraAll a etait cliquéééé");
    }
	...
	//pour entete
		public List<TaLigneServiceWebExterne> editTiersEntete(TaLigneServiceWebExterneDTOJSF ligneCommandeJSF) {
			TaTiers tiers;
			...
			try {
				if(ligneCommandeJSF.getTaTiersDTO() != null) {
					...
					if(ligneCommandeJSF.getDto().getRefTiers() != null) {
						TaLiaisonServiceExterne liaison = liaisonService.findByIdTiersAndByIdServiceWebExterne(ligneCommandeJSF.getTaTiersDTO().getId(), serviceWeb.getIdServiceWebExterne());
						TaLiaisonServiceExterne liaisonRef = liaisonService.findByRefTiersAndByIdServiceWebExterne(ligneCommandeJSF.getDto().getRefTiers(), serviceWeb.getIdServiceWebExterne());
						...
						if(liaison != null) {
							liaison.setRefExterne(ligneCommandeJSF.getDto().getRefTiers());
						}else {
							//aucune liaison donc on l'a créé
							liaison = new TaLiaisonServiceExterne();
							liaison.setIdEntite(ligneCommandeJSF.getTaTiersDTO().getId());
							liaison.setRefExterne(ligneCommandeJSF.getDto().getRefTiers());
			...
			try {
				if(ligne != null) {
						...
						if(ligne.getDto().getEnteteDoc()) {
							...
							listeAMerge.addAll(editTiersEntete(ligne)); 
...
//		}
	...
	public void actTelecharger(ActionEvent actionEvent) {
		try {
			Integer nbFiles = mensuraService.recupereFichier();
			FacesContext context = FacesContext.getCurrentInstance();  
	...
	public void actParcourir(ActionEvent actionEvent) {
		Integer nbLignes;
		try {
			nbLignes = mensuraService.parcourir();
...
//				}
	...
	public TaLigneServiceWebExterne getSelectedligneMensura() {
		return selectedligneServiceWebExterne;
	}


	public void setSelectedligneMensura(TaLigneServiceWebExterne selectedligneServiceWebExterne) {
...
//	public TaSynchroServiceExterneDTO getDerniereSynchro() {
//		return derniereSynchro;
//	}
//
//
//	public void setDerniereSynchro(TaSynchroServiceExterneDTO derniereSynchro) {
//		this.derniereSynchro = derniereSynchro;
//	}
