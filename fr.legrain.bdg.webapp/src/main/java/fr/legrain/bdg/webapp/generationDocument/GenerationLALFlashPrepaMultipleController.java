...
@Named
@ViewScoped 
public class GenerationLALFlashPrepaMultipleController extends GenerationFlashLigneALigneMultipleController{
	...
	public void init() {
		super.init();
		selectedTypeSelection=TypeDoc.getInstance().getPathImageCouleurDoc().get(TaFlash.TYPE_DOC);
		selectedTypeCreation=TypeDoc.getInstance().getPathImageCouleurDoc().get(TaPreparation.TYPE_DOC);
		try {
			taTDocDest=taTDocService.findByCode(TaPreparation.TYPE_DOC);
	...
	public void actDialogGenerationDocument(ActionEvent actionEvent) {
		...
		paramGeneration.setTypeSrc(TypeDoc.TYPE_FLASH);
		paramGeneration.setTypeDest(TypeDoc.TYPE_PREPARATION);
