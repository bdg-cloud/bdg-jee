...
@Named
@ViewScoped  
public class DevisController extends AbstractDocumentController<TaDevis,TaDevisDTO,TaLDevis,TaLDevisDTO,TaLDevisDTOJSF,TaInfosDevis> {  
...
//	private String stepEntete = "idEnteteDevis";
//	private String stepLignes = "idLignesDevis";
//	private String stepTotaux = "idTotauxFormDevis";
//	private String stepValidation = "idValidationFormDevis";
...
//
//	private List<TaDevisDTO> values; 
//	private List<TaLDevisDTOJSF> valuesLigne;
//	
...
//
	...
	private @EJB ITaInfosDevisServiceRemote taInfosDocumentService;
	private @EJB ITaLDevisServiceRemote taLDevisService;
...
//	
//	private TaDevisDTO[] selectedDocumentDTOs; 
//	private TaDevis masterEntity = new TaDevis();
//	private TaDevisDTO newDocumentDTO = new TaDevisDTO();
//	private TaDevisDTO selectedDocumentDTO = new TaDevisDTO();
//
//	private TaInfosDevis taInfosDocument = new TaInfosDevis();
//
//	
//	private ILigneDocumentJSF[] selectedLigneJSFs; 
//	private TaLDevis masterEntityLigne = new TaLDevis();
//	private TaLDevisDTOJSF oldTaLDevisDTOJSF = new TaLDevisDTOJSF();// TODO gestion annulation ligne
//	private TaLDevisDTOJSF selectedLigneJSF = new TaLDevisDTOJSF();
//	
...
//	private boolean insertAuto = true;
	...
	public void postConstruct(){
		...
		nomDocument="Devis";
		setTaDocumentService(taDevisService);
		setTaLDocumentService(taLDevisService);
		setTaInfosDocumentService(taInfosDocumentService);
		
		initListeEdition();
		
		stepSynthese = "idSyntheseDevis";
		stepEntete = "idEnteteDevis";
		stepLignes = "idLignesDevis";
		stepTotaux = "idTotauxFormDevis";
		stepValidation = "idValidationFormDevis";
		idMenuBouton = "form:tabView:idMenuBoutonDevis";
		wvEcran = LgrTab.WV_TAB_DEVIS;
		classeCssDataTableLigne = "myclassDevis";
		idBtnWizardButtonPrecedent = "form:tabView:idBtnWizardButtonPrecedentDevis";
		idBtnWizardButtonSuivant = "form:tabView:idBtnWizardButtonSuivantDevis";
		
		JS_FONCTION_INITIALISE_POSITION_BOUTON_WIZARD = "initialisePositionBoutonWizard('.wizard-devis')";
	...
	public void refresh(){
		values = taDevisService.selectAllDTO();
		valuesLigne = IHMmodel();
		modeEcran.setMode(EnumModeObjet.C_MO_CONSULTATION);
	}

	public List<TaLDevisDTOJSF> IHMmodel() {
		LinkedList<TaLDevis> ldao = new LinkedList<TaLDevis>();
		LinkedList<TaLDevisDTOJSF> lswt = new LinkedList<TaLDevisDTOJSF>();
		...
		if(masterEntity!=null && !masterEntity.getLignes().isEmpty()) {
			...
			TaLDevisDTO dto = null;
			for (TaLDevis o : ldao) {
				TaLDevisDTOJSF t = new TaLDevisDTOJSF();
				dto = (TaLDevisDTO) mapper.map(o, TaLDevisDTO.class);
				t.setDto(dto);
...
//			PrimeFaces.current().ajax().update("form:tabView:idMenuBoutonDevis");
//			return currentStepId;
//		} catch (Exception e) {
//			FacesContext context = FacesContext.getCurrentInstance();  
//			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Devis", e.getMessage()));
...
//	}
	public void initListeEdition() {
		listeEdition = taEditionService.findByCodeTypeDTOAvecActionsEdition(TaDevis.TYPE_DOC.toUpperCase());
		TaEditionDTO editionDefautNulle = new TaEditionDTO();
		editionDefautNulle.setLibelleEdition("Devis modèle par défaut (V0220)");
...
//			}
			...
			editionParam.setIdActionInterne(ConstActionInterne.EDITION_DEVIS_DEFAUT);
			...
			if(edition == null) {//si aucune edition n'est choisie
				...
				taEdition = taEditionService.rechercheEditionActionDefaut(null,action, TaDevis.TYPE_DOC);
				...
				if(taEdition == null) {
					taEdition = taEditionService.rechercheEditionDisponibleProgrammeDefaut("", ConstActionInterne.EDITION_DEVIS_DEFAUT).get(0);
				}
		...
		//ICI COMMENCE LA PARTIE actDialogImprimer
		 Map<String,Object> options = new HashMap<String, Object>();
			...
			if(!editionClient) {
				...
				masterEntity = taDevisService.mergeAndFindById(masterEntity,ITaDevisServiceRemote.validationContext);
			}
...
//			}
			...
			if(edition == null) {//si aucune edition n'est choisie
				...
				pdf = taDevisService.generePDF(selectedDocumentDTO.getId(),mapParametreEdition,null,null,actionImprimer);
				...
				if(taEdition == null) {
					...
					if(taEdition!=null) {
						...
						try { 
							...
							String localPath  = taDevisService.writingFileEdition(taEdition);
							//on genere le pdf avec le fichier xml crée ci-dessus
							 pdf = taDevisService.generePDF(selectedDocumentDTO.getId(),localPath,taEdition.getMapParametreEdition(),listeResourcesPath,taEdition.getTheme());
			...
			try {
				...
				masterEntity = taDevisService.findById(masterEntity.getIdDocument());
			} catch (FinderException e) {
				e.printStackTrace();
...
//			}
			...
			mapper.map((TaLDevisDTO) selectedLigneJSF.getDto(),masterEntityLigne);
...
//			modeEcranLigne.setMode(EnumModeObjet.C_MO_INSERTION);
		...
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("Devis", "actEnregisterLigne")); 
		}
	}

//	public TaLDevis rechercheLigne(int id){
//		TaLDevis obj=masterEntityLigne;
//		for (TaLDevis ligne : masterEntity.getLignes()) {
...
//	}
//	
//	public TaLDevis rechercheLigneNumLigne(int numLigne){
//		TaLDevis obj=masterEntityLigne;
//		for (TaLDevis ligne : masterEntity.getLignes()) {
...
//			
////			for (TaLDevis lbr : masterEntity.getLignes()) {
...
//			if(ConstWeb.DEBUG) {
//				FacesContext context = FacesContext.getCurrentInstance();  
//				context.addMessage(null, new FacesMessage("Devis", "actSupprimerLigne")); 
//			}
//		} catch(Exception e) {
//			e.printStackTrace();
//		}
...
//		//((TaLDevisDTOJSF) event.getObject()).setAutoInsert(false);
...
////		}
//		selectionLigne((TaLDevisDTOJSF) event.getObject());
...
//			selectedLigneJSF=(TaLDevisDTOJSF) event.getObject();
...
//	}
//	
//	public void selectionLigne(TaLDevisDTOJSF ligne){
...
//	}
	...
	public void actAnnuler(ActionEvent actionEvent) {
		try {
			...
			case C_MO_INSERTION:
				if(selectedDocumentDTO.getCodeDocument()!=null) {
					taDevisService.annuleCode(selectedDocumentDTO.getCodeDocument());
				}
				masterEntity=new TaDevis();
				selectedDocumentDTO=new TaDevisDTO();
			...
			case C_MO_EDITION:
				if(selectedDocumentDTO.getId()!=null && selectedDocumentDTO.getId()!=0){
					masterEntity = taDevisService.findByIDFetch(selectedDocumentDTO.getId());
					selectedDocumentDTO=taDevisService.findByIdDTO(selectedDocumentDTO.getId());
				}				
				break;
		...
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("Devis", "actAnnuler")); 
		}
		} catch (FinderException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
...
//			}			
			...
			List<TaLDevis> listeLigneVide = new ArrayList<TaLDevis>(); 
			for (TaLDevis ligne : masterEntity.getLignes()) {
			...
			for (TaLDevis taLBonReception : listeLigneVide) {
...
//			for (TaLDevis ligne : masterEntity.getLignes()) { //@@ champs à ajouter aux devis
...
//			masterEntity.setTaGrMouvStock(grpMouvStock); //@@ champs à ajouter aux devis
			...
			selectedDocumentDTOs=new TaDevisDTO[]{selectedDocumentDTO};
			...
			e.printStackTrace();
			FacesContext context = FacesContext.getCurrentInstance();  
			//context.addMessage(null, new FacesMessage("Devis", "actEnregistrer")); 
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Devis", e.getMessage()));
		}
...
public void actInsererLigne(ActionEvent actionEvent) {
	...
	TaEtat etat = taDevisService.etatLigneInsertion(masterEntity);
...
//			TaEtat etat = taEtatService.findByCode(TaEtat.ETAT_ENCOURS_NON_TRANSFORME);
			TaEtat etat = taDevisService.rechercheEtatInitialDocument();
...
//			}
			selectedDocumentDTO.setCodeDocument(taDevisService.genereCode(paramsCode));
...
//			}
			...
			if(ConstWeb.DEBUG) {
				FacesContext context = FacesContext.getCurrentInstance();  
				context.addMessage(null, new FacesMessage("Devis", "actInserer"));
			}
...
//		if(ConstWeb.DEBUG) {
//			FacesContext context = FacesContext.getCurrentInstance();  
//			context.addMessage(null, new FacesMessage("Devis", "actModifier")); 	 
//		}
//	}
//
...
//		if(ConstWeb.DEBUG) {
//			FacesContext context = FacesContext.getCurrentInstance();  
//			context.addMessage(null, new FacesMessage("Devis", "actAide")); 	
//		}
//	}
...
//		tabsCenter.ajouterOnglet(b);
		...
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("Devis", 
					"Nouveau"
					)); 
		}
	} 
...
//					List<TaLigneALigneEcheance> liste  = taLigneALigneEcheanceService.findAllByIdLDocumentAndTypeDoc(selectedLigneJSF.getDto().getIdLDocument(), TaDevis.TYPE_DOC);
		...
		try {
			if(autorisationLiaisonComplete()) {
				...
				if(!values.isEmpty()) {
					...
					selectedDocumentDTO = new TaDevisDTO();
					selectedDocumentDTOs= new TaDevisDTO[]{};	
				}
				...
				if(ConstWeb.DEBUG) {
					FacesContext context = FacesContext.getCurrentInstance();  
					context.addMessage(null, new FacesMessage("Devis", "actSupprimer"));
				}
			}
...
//        requestContext.execute("PF('wvDataTableListeDevis').filter()");
		...
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("Devis", "actFermer")); 
		}
	}
	...
	public void actImprimer(ActionEvent actionEvent) {
		if(ConstWeb.DEBUG) {
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage("Devis", "actImprimer")); 
		}
		try {
			...
			TaDevis doc =taDevisService.findById(selectedDocumentDTO.getId());
			...
			System.out.println("DevisController.actImprimer()");
		} catch (FinderException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void actImprimerListeDesDevis(ActionEvent actionEvent) {
		...
		try {
			...
			sessionMap.put("listeDesDevis", values);
...
//		try {
//			for (TaLDevis l : masterEntity.getLignes()) {
...
//		if(ConstWeb.DEBUG) {
//		   	FacesContext context = FacesContext.getCurrentInstance();  
//		    context.addMessage(null, new FacesMessage("Devis", "actImprimerEtiquetteCB")); 
//		}
//	}
...
//
//		//		FacesContext context = FacesContext.getCurrentInstance();  
//		//		context.addMessage(null, new FacesMessage("Devis", "actAbout")); 	    
//	}
//
...
//	}
	...
	public void validateLignesDocument(FacesContext context, UIComponent component, Object value) throws ValidatorException {
		...
		try {
			...
			TaLDevisDTO dtoTemp =new TaLDevisDTO();
			PropertyUtils.setSimpleProperty(dtoTemp, nomChamp, value);
			taLDevisService.validateDTOProperty(dtoTemp, nomChamp, ITaLDevisServiceRemote.validationContext );
...
//
//			validateUIField(nomChamp,value);
//			TaDevisDTO dtoTemp = new TaDevisDTO();
//			PropertyUtils.setSimpleProperty(dtoTemp, nomChamp, value);
//			taBonReceptionService.validateDTOProperty(dtoTemp, nomChamp, ITaDevisServiceRemote.validationContext );
//
//			//selectedDocumentDTO.setAdresse1Adresse("abcd");
//
//			//		  if(selectedRadio.equals("aa")) {
...
//		}
		...
		try {
			...
			Set<ConstraintViolation<TaDevisDTO>> violations = factory.getValidator().validateValue(TaDevisDTO.class,nomChamp,value);
			if (violations.size() > 0) {
				messageComplet = "Erreur de validation : ";
				for (ConstraintViolation<TaDevisDTO> cv : violations) {
	...
	public void validateDocumentAvantEnregistrement( Object value) throws ValidatorException {
		...
		try {
			//taBonReceptionService.validateDTOProperty(selectedDocumentDTO,Const.C_CODE_TIERS,  ITaDevisServiceRemote.validationContext );
			...
			if(afficheUniteSaisie && selectedLigneJSF.getTaCoefficientUniteSaisie()!=null)
				...
				if(selectedLigneJSF.getTaCoefficientUnite()!=null) {
					if(!calculCoherenceAffectationCoefficientQte2(selectedLigneJSF.getDto().getQte2LDocument())) {
						...
						PrimeFaces.current().executeScript("PF('wVdialogQte2Devis').show()");
					}
...
//						entity=(TaTiers) value;
				...
				if(changement){
					...
					((TaDevisDTO)selectedDocumentDTO).setLibelleDocument("Devis N°"+selectedDocumentDTO.getCodeDocument()+" - "+nomTiers);										
					if(!disableTtc() && !disableTtcSiDocSansTVA()){
						masterEntity.setTtc(entity.getTtcTiers());
						((TaDevisDTO)selectedDocumentDTO).setTtc(LibConversion.intToBoolean(masterEntity.getTtc()));
					}									
					changementTiers(true);
				}
				...
				if(changementArticleLigne) {
					if(entity!=null)selectedLigneJSF.getDto().setLibLDocument(entity.getLibellecArticle());else selectedLigneJSF.getDto().setLibLDocument(null);
...
//	}
	...
	public void initialisationDesInfosComplementaires(Boolean recharger,String typeARecharger){
		...
		try {
			if(((TaDevisDTO)selectedDocumentDTO).getCodeDocument()!=null) {
				taInfosDocument = taInfosDocumentService.findByCodeDevis(((TaDevisDTO)selectedDocumentDTO).getCodeDocument());
			} else {
...
//			}    
		...
		edition.setIdActionInterne(ConstActionInterne.EDITION_DEVIS_DEFAUT);
...
//		context.addMessage(null, new FacesMessage("Tiers", "actAbout")); 	    
		...
		if(event!=null && event.getObject()!=null) {
			...
			if(taEdition!=null) {
				...
				try { 
					...
					String pdf = taDevisService.generePDF(selectedDocumentDTO.getId(),localPath,taEdition.getMapParametreEdition(),listeResourcesPath,taEdition.getTheme());
	...
	public void initialisationDesCPaiement(Boolean recharger) {
		...
		if (masterEntity != null) {
			...
			taCPaiementDoc = null;
			List<TaCPaiement> listeCPaiement=taCPaiementService.rechercheParType(TaDevis.TYPE_DOC);
	...
	public void calculDateEcheance(Boolean appliquer) {
		...
		if(((TaDevisDTO)selectedDocumentDTO).getId()==0|| appliquer) {
			...
			((TaDevisDTO)selectedDocumentDTO).setDateEchDocument(taDevisService.calculDateEcheance(masterEntity,report, finDeMois));
		}
		}
	}
...
@Override
public void IHMmodel(TaLDevisDTOJSF dtoJSF, TaLDevis ligne) {
	LgrDozerMapper<TaLDevis,TaLDevisDTO> mapper = new LgrDozerMapper<TaLDevis,TaLDevisDTO>();
	TaLDevisDTO dto = dtoJSF.getDto();

		dto = (TaLDevisDTO) mapper.map(ligne, TaLDevisDTO.class);
		dtoJSF.setDto(dto);
