...
@Named
@ViewScoped 
public class EtatMouvementCapsuleController implements Serializable {
	static Logger logger = Logger.getLogger(EtatMouvementCapsuleController.class.getName());		
	...
	public EtatMouvementCapsuleController() {
	...
	public void init() {
		...
		dateCalcul=new Date();
		dateCalculDeb=infos.getDatedebInfoEntreprise();
...
//				dateCalcul=LibDate.incrementDate(infos.getDatefinInfoEntreprise(), -1, 0, 0) ;
	...
	public void actImprimer(String synthese) throws Exception {
		...
		if(dto.getLibelleArticle()!=null && !dto.getLibelleArticle().equals("")){
			...
			criteres.setDateStockDeb(dateCalculDeb);

			
			criteres.setTypeEdition(synthese);

			criteres.setAvecReport(false);

			if(synthese.equals(C_SYNTHESE)){
				listeEtat = etatStocks.calculEtatStocks(criteres,criteres.getDateStockDeb(),false,true);
			}
			if(synthese.equals(C_MOUVEMENT_DATE)){
				listeEtat = etatStocks.calculEtatStocksMouvements(criteres,criteres.getDateStockDeb(),false);
			}
	...
	public void imprimer(TaEtatStockCapsules criteres) {
		if(ConstWeb.DEBUG) {
			...
			context.addMessage(null, new FacesMessage("Etat mouvements capsules", "actImprimer")); 
		}
		try {
			...
			List<String> listeEditionPossible = new LinkedList<String>();
			listeEditionPossible.add(C_MOUVEMENT_DATE);
			listeEditionPossible.add(C_MOUVEMENT_TYPE);
			listeEditionPossible.add(C_SYNTHESE);
