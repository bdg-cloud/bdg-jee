...
@Named
@ViewScoped 
public class EtatReportsCapsuleController implements Serializable {
	static Logger logger = Logger.getLogger(EtatReportsCapsuleController.class.getName());		
	...
	public EtatReportsCapsuleController() {
	...
	public void actImprimer(String synthese) throws Exception {
		...
		if(dto.getLibelleArticle()!=null && !dto.getLibelleArticle().equals("")){
			...
			criteres.setDateStock(etatStocks.recupDerniereDateReportStock());
			if(criteres.getDateStock()==null)criteres.setDateStock(new Date());
			criteres.setDateStockDeb(dateCalculDeb);

			

		listeEtat = etatStocks.calculEtatStocksPourReport(criteres,criteres.getDateStockDeb());
