package fr.legrain.gestiondms.controllers;
...
import fr.legrain.gestiondms.divers.EtatDms;
import fr.legrain.gestiondms.divers.Impression;
import fr.legrain.gestiondms.ecrans.PaEtatDms;
...
public class PaEtatDmsController extends JPABaseControllerSWTStandard implements
RetourEcranListener {
	
	static Logger logger = Logger.getLogger(PaEtatDmsController.class.getName());		
	private PaEtatDms vue = null; // vue
	...
	public PaEtatDmsController(PaEtatDms vue) {
		this(vue,null);
	}

	public PaEtatDmsController(PaEtatDms vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
...
//			Branchement action annuler : empeche la fermeture automatique de la
			...
			vue.getTfANNEE().setText(LibDate.getAnnee(new Date()));
			vue.getTfMOIS().setText(LibDate.getMois(new Date()));
...
//			vue.getLaMessage().setText(e.getMessage());
	...
	protected void initMapComposantChamps() {
		...
		mapComposantChamps.put(vue.getTfMOIS(), Const.C_MOIS);	
		mapComposantChamps.put(vue.getTfANNEE(), Const.C_ANNEE);	
...
//		listeComposantFocusable.add(vue.getCbChoix2());
		...
		mapInitFocus.put(ModeObjet.EnumModeObjet.C_MO_CONSULTATION,vue.getTfMOIS());
		mapInitFocus.put(ModeObjet.EnumModeObjet.C_MO_INSERTION,vue.getTfMOIS());
		mapInitFocus.put(ModeObjet.EnumModeObjet.C_MO_EDITION,vue.getTfMOIS());
...
//	}
	...
	public PaEtatDmsController getThis(){
		return this;
	}
...
//					vue.getLaMessage().setText(e.getMessage());
	...
	protected void actImprimer() throws Exception {
		...
		if(!vue.getTfMOIS().getText().equals("")){
			criteres.setMois(LibConversion.stringToInteger(vue.getTfMOIS().getText()));
		}
		if(!vue.getTfANNEE().getText().equals("")){
			criteres.setAnnee(LibConversion.stringToInteger(vue.getTfANNEE().getText()));
		}
		...
		if(vue.getCbChoix1().getSelection()){
			EtatDms etatDms = new EtatDms();
			List<TaEtatMouvementDms> listeEtat = etatDms.calculEtatDms(criteres,true);
			ConstEdition constEdition = new ConstEdition();
			Impression impression = new Impression(listeEtat,constEdition);
			impression.imprimer(ConstEdition.FICHE_FILE_REPORT_SYNTHESEDMS);

		}else{
			EtatDms etatDms = new EtatDms();
			List<TaEtatMouvementDms> listeEtat = etatDms.calculEtatDms(criteres,false);
			ConstEdition constEdition = new ConstEdition();
			Impression impression = new Impression(listeEtat,constEdition);
			impression.imprimer(ConstEdition.FICHE_FILE_REPORT_DMS);
		}
	}
	...
	protected void actAide(String message) throws Exception {
		...
		setActiveAide(true);
			try {
				...
				switch ((PaEtatDmsController.this.dao.getModeObjet().getMode())) {
				case C_MO_CONSULTATION:					
				case C_MO_EDITION:
				case C_MO_INSERTION:
				...
				if (paramAfficheAideRecherche.getJPQLQuery() != null) {
					...
					paAideController.addRetourEcranListener(PaEtatDmsController.this);
