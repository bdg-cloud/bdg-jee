package fr.legrain.gestiondms.editors;
...
import fr.legrain.gestiondms.Activator;
import fr.legrain.gestiondms.controllers.PaEtatDmsController;
 ...
 * </ul>
 */
public class DmsMultiPageEditor extends AbstractLgrMultiPageEditor implements ILgrRetourEditeur, IChangementDePageListener, IChangementDeSelectionListener{
//public class MultiPageEditor extends FormEditor {
//public class MultiPageEditor extends SharedHeaderFormEditor {
	
	public static final String ID_EDITOR = "fr.legrain.editor.dms.multi"; //$NON-NLS-1$
	static Logger logger = Logger.getLogger(DmsMultiPageEditor.class.getName());
	...
	private EditorEtatDms editorDms = null;
	
	String labelPageStocks = "Editions pour la Dms";

	/**
	 * Creates a multi-page editor example.
	 */
	public DmsMultiPageEditor() {
		super();
	...
	void createPageQueEditeur() {
		try {
			...
			editorDms = new EditorEtatDms(this,em);
			
			listePageEditeur.add(editorDms);
			
			int index = addPage(editorDms, getEditorInput());
			...
			editorDms.getController().addChangementDeSelectionListener(this);
			
			mapEditorController.put(editorDms, editorDms.getController());
			
			editorDms.getController().addChangementDePageListener(this);
					
			editeurCourant = editorDms;
