...
//	private @EJB IAdministrationServiceRemote adminService;
	...
	@Pattern(regexp = "^[a-z]+[a-z0-9\\-]*$", message="Seul les caractères alpha-numériques (a-z), les chiffres (0-9) et le tiret (-) sont autorisés dans le nom du dossier. Le nom du dossier ne peut pas commencer par un tiret ou un chiffre.")
	...
	public void refresh() {
		if(paramPartenaire!=null) {
			codePartenaire = paramPartenaire;
		}
	}
	...
	public void initListeModule() {
		...
		for (TaProduit p : listeModule) {
			if(p.isVendable()!=null && p.isVendable()
					&& p.getTaTNiveau().getCode().equals("PREMIUM") //pour la creation d'un dossier en version DEMO, on active tous les modules PREMIUM
					) {
	...
	public void ajoutDossier(ActionEvent event) {
		...
		try {
			...
			//supprime tout ce qui n'est pas un caractere alphanumerique ou un tiret "-" par sécurité (normalement déjà vérifié par l'annotation jsf
			//nomDossier.replaceAll("[^a-zA-Z0-9\\-]", ""); 
				...
				if(!wsMonCompte.dossierExiste(nomDossier)) {		
					...
					if(motDePasse!=null && confirmationMotDePasse!=null 
						...
						if(verifComplexiteMotDePasse(motDePasse)) {
							...
							String codeClientGenere = wsMonCompte.genereCodeClient(nomEntreprise);
							
							System.out.println("=========================================");
							System.out.println("Nom entreprise : "+nomEntreprise);
							System.out.println("Code dossier : "+nomDossier);
							System.out.println("Email : "+emailEntreprise);
							System.out.println("MDP : "+motDePasse);
							System.out.println("Code alliance : "+codePartenaire);
							System.out.println("Code client généré : "+codeClientGenere);
							System.out.println("=========================================");
...
//							motDePasse="mdp";
							...
							TaTNiveau niveauDossier = null;
							List<TaTNiveau> listeTaTNiveauTmp = wsMonCompte.selectAllTNiveau();
							for (TaTNiveau n : listeTaTNiveauTmp) {
								if(n.getCode().equals("DEMO")) {
									niveauDossier = n;
								}
							}
							dossier.setTaTNiveau(niveauDossier);
							
							if(codePartenaire!=null) {
								//TODO vérification du code partenaire ?
								codePartenaire = codePartenaire.toUpperCase();
								dossier.setCodePartenaire(codePartenaire);
							}
							...
							wsAutorisation.createUpdateDroits(dossier.getCode(), idProduit, listeXml);
				...
				context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,"Cet utilisateur existe déjà, veuillez vous connecté à votre compte pour céer un nouveau dossier.", "")); 
			}
	...
	public void emailConfirmation(TaDossier d) {
		...
		//ici pas de dossier, donc pas de schema pour le multi-tenant, donc l'enregistrement du mail plante, pour certains email programme, 
		//il ne faut pas les logger ou il faudrait peut être envisagé de les stockés dans une table de la base programme
//		lgrEmail.emailConfirmationCreationDossier(d, nomEntreprise, nomDossier, emailEntreprise);
