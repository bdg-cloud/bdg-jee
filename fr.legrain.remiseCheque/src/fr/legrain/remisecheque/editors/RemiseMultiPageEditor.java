package fr.legrain.remisecheque.editors;
...
import fr.legrain.remisecheque.controllers.PaCritereRemiseController;
import fr.legrain.remisecheque.controllers.PaCritereRemonterRemiseController;
 ...
 * </ul>
 */
public class RemiseMultiPageEditor extends AbstractLgrMultiPageEditor implements 
...
//public class MultiPageEditor extends SharedHeaderFormEditor {
	...
	public static final String ID_EDITOR = "fr.legrain.remise.multi"; //$NON-NLS-1$
	static Logger logger = Logger.getLogger(RemiseMultiPageEditor.class.getName());
	...
	protected EditorCritereRemise editorCritereRemise = null;
	protected EditorCritereRemonterRemise editorCritereRemonterRemise = null;
	/**
	 * Creates a multi-page editor example.
	 */
	public RemiseMultiPageEditor() {
		super();
	...
	protected void createPageQueEditeur() {
		try {
			...
			editorCritereRemise = new EditorCritereRemise(this,em);
			editorCritereRemonterRemise= new EditorCritereRemonterRemise(this,em);
			
			listePageEditeur.add(editorCritereRemise);
			listePageEditeur.add(editorCritereRemonterRemise);
			
			
			int index = addPage(editorCritereRemise, getEditorInput());
			setPageText(index, editorCritereRemise.getTitle());
			setPageImage(index, pluginRemise.getImageDescriptor(iconPageRelance).createImage());
			int index2 = addPage(editorCritereRemonterRemise, getEditorInput());
			setPageText(index2, editorCritereRemonterRemise.getTitle());
			setPageImage(index, pluginRemise.getImageDescriptor(iconPageRemonterRelance).createImage());			
			//liaison entre la selection du controller principal et le multipage editor
			editorCritereRemise.getController().addChangementDeSelectionListener(this);
//			((SWTPaReglementMultipleController)editorCritereRelance.getController()).addChangementMasterEntityListener(this);

			((PaCritereRemiseController)editorCritereRemise.getController()).
				getPaSelectionLigneRemiseControlleur().addDeclencheCommandeControllerListener(editorCritereRemise.getController());
			((PaCritereRemonterRemiseController)editorCritereRemonterRemise.getController()).getPaSelectionLigneRelanceControlleur().
				addDeclencheCommandeControllerListener(editorCritereRemonterRemise.getController());
			
			mapEditorController.put(editorCritereRemise, editorCritereRemise.getController());
			mapEditorController.put(editorCritereRemonterRemise, editorCritereRemonterRemise.getController());


			
			editorCritereRemise.getController().addChangementDePageListener(this);
			editorCritereRemonterRemise.getController().addChangementDePageListener(this);
			((PaCritereRemiseController)editorCritereRemise.getController()).getPaSelectionLigneRemiseControlleur().addChangementDePageListener(this);
			((PaCritereRemonterRemiseController)editorCritereRemonterRemise.getController()).getPaSelectionLigneRelanceControlleur().addChangementDePageListener(this);
...
//			editorGestionReglement.getController().addDeclencheCommandeControllerListener(editorCritereRelance.getController());
			...
			editorCritereRemise.getController().addAnnuleToutListener(this);
			editorCritereRemonterRemise.getController().addAnnuleToutListener(this);

			
			editeurCourant = editorCritereRemise;
			ParamAfficheRemise paramAfficheRelanceFacture = new ParamAfficheRemise();
			editorCritereRemise.getController().configPanel(paramAfficheRelanceFacture);
			editorCritereRemonterRemise.getController().configPanel(paramAfficheRelanceFacture);
