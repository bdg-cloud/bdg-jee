 ...
 * @author Le Grain S.A
 */
public class IbTaFinal  extends IbLiasse {
	static Logger logger = Logger.getLogger(IbTaFinal.class.getName());
	
	public IbTaFinal() {
		super();
		try {
			this.fIBQuery.setQuery(new QueryDescriptor(fIBBase,ConstLiasse.C_Debut_Requete+ ConstLiasse.C_NOM_VU_FINAL, true));
			this.fIBQuery.open();
			fIBBase.setAutoCommit(false);
			
			nomTable = ConstLiasse.C_NOM_TA_FINAL;
...
//					if(valeurChamps.getChamps().equals(ConstLiasse.C_FINAL_CLE )&& !valeurChamps.getValeur().equals(getChamp_Obj(ConstLiasse.C_FINAL_CLE)))
...
//						};
//				}
//				if(valeurChamps.getChamps().equals(ConstLiasse.C_FINAL_CLE)) {
...
//					return;            		
//				};
//				if(valeurChamps.getChamps().equals(ConstLiasse.C_FINAL_ID_DETAIL)) {
//					if (!LibChaine.emptyNumeric(valeurChamps.getValeur())){
...
//					return;            		
//				};
//				if(valeurChamps.getChamps().equals(ConstLiasse.C_FINAL_VALEUR)) {
//					if (!LibChaine.emptyNumeric(valeurChamps.getValeur())){
...
//				}
	...
	public void enregistrement(Final f) throws ExceptLgr {
		try{ 			
			if (!verifChamp(ConstLiasse.C_FINAL_CLE,f.getCle(),null,null))throw new ExceptLgr();
			if (!verifChamp(ConstLiasse.C_FINAL_ID_DETAIL,f.getIdDetail(),null,null))throw new ExceptLgr();
			if (!verifChamp(ConstLiasse.C_FINAL_VALEUR,f.getValeur(),null,null))throw new ExceptLgr();
	...
	public void insertion(Final f) throws ExceptLgr {
		try{
			this.fListeChampValue.clear();
			if (f!=null){
				if (!LibChaine.emptyNumeric(f.getCle())&&!verifChamp(ConstLiasse.C_FINAL_CLE,f.getCle(),null,null))throw new ExceptLgr();
				if (!LibChaine.emptyNumeric(f.getIdDetail())&&!verifChamp(ConstLiasse.C_FINAL_ID_DETAIL,f.getIdDetail(),null,null))throw new ExceptLgr();
				if (!LibChaine.emptyNumeric(f.getValeur())&&!verifChamp(ConstLiasse.C_FINAL_VALEUR,f.getValeur(),null,null))throw new ExceptLgr();
	...
	public void modification(Final f) {
		try{
...
//			//cas, on empêche la suppression.//TA_R_ADR (table de relation avec le tiers)
	...
	public static Final infosFinal(String idFinal, Final f) {
		if(f==null) f = new Final();
		StringBuffer requete = new StringBuffer("");
		try {
			requete.append("select ")
			.append(" * ").append(" from ").append(ConstLiasse.C_NOM_VU_FINAL)
			.append(" where ").append(ConstLiasse.C_FINAL_CLE).append(" = ").append(idFinal);
			PreparedStatement rqt = fIBBase.getJdbcConnection().prepareStatement(requete.toString());
			ResultSet res = rqt.executeQuery();
			if(res.next()){
				f.setCle(res.getInt(ConstLiasse.C_FINAL_CLE));
				f.setIdDetail(res.getInt(ConstLiasse.C_FINAL_ID_DETAIL));
				f.setValeur(res.getString(ConstLiasse.C_FINAL_VALEUR));
