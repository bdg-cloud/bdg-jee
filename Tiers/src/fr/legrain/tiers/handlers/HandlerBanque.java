...
public class HandlerBanque extends LgrAbstractHandler {
	
	static Logger logger = Logger.getLogger(HandlerBanque.class.getName());
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			IEditorPart editor = EJBLgrEditorPart.verifEditeurOuvert(EditorBanque.ID);
			if(editor==null){
			IEditorPart e = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage()
			.openEditor(new EditorInputBanque(), EditorBanque.ID);
			...
			((EJBLgrEditorPart)e).getController().configPanel(paramAfficheBanque);
			} else {
