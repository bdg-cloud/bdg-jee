...
import fr.legrain.tiers.ecran.ParamAfficheTypeBanque;
import fr.legrain.tiers.editor.EditorInputTypeBanque;
import fr.legrain.tiers.editor.EditorTypeBanque;


public class HandlerTypeBanque extends LgrAbstractHandler {
	
	static Logger logger = Logger.getLogger(HandlerTypeBanque.class.getName());
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			IEditorPart editor = EJBLgrEditorPart.verifEditeurOuvert(EditorTypeBanque.ID);
			if(editor==null){
			IEditorPart e = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().
			openEditor(new EditorInputTypeBanque(), EditorTypeBanque.ID);
			...
			((EJBLgrEditorPart)e).getController().configPanel(paramAfficheTypeBanque);
			} else {
