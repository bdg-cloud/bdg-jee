...
public class SWTPaTiersInfosEntrepriseController extends EJBBaseControllerSWTStandard 
...
//		daoDocumentTiers = new TaDocumentTiersDAO(getEm());
	...
	public void bind(PaTiersInfosEntrepriseSWT paTiersSWT, TaTiersDTO selection ){
		try {
...
////			);
			...
			// Set up data binding.
			LgrViewerSupport.bind(tableViewer, 
					new WritableList(modelTiers.remplirListe(), classModel),
					BeanProperties.values(listeChamp)
			);
//			LgrViewerSupport.bind(tableViewer, 
//					new WritableList(new LinkedList<TaTiersDTO>(), classModel),
...
//			);
	...
	private void changementDeSelection() {		
		if(selectedTiers!=null && selectedTiers.getValue()!=null) {
			if(((TaTiersDTO) selectedTiers.getValue()).getId()!=null) {
				try {
					taTiers = dao.findById(((TaTiersDTO) selectedTiers.getValue()).getId());
	...
	protected class HandlerAjoutAdresse extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheAdresse paramAfficheAdresse = new ParamAfficheAdresse();
				paramAfficheAdresse.setIdTiers(LibConversion.integerToString(((TaTiersDTO)selectedTiers.getValue()).getId()));
	...
	protected class HandlerAjoutCommercial extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheCommercial paramAfficheCommercial = new ParamAfficheCommercial();
				paramAfficheCommercial.setIdTiers(LibConversion.integerToString(((TaTiersDTO)selectedTiers.getValue()).getId()));
				((EJBLgrEditorPart)e).setPanel(((EJBLgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutCompl extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheCompl paramAfficheCompl = new ParamAfficheCompl();
				paramAfficheCompl.setIdTiers(((TaTiersDTO)selectedTiers.getValue()).getId());
				((EJBLgrEditorPart)e).setPanel(((EJBLgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutConditionPaiement extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheConditionPaiement paramAfficheConditionPaiement = new ParamAfficheConditionPaiement();
				paramAfficheConditionPaiement.setIdTiers(((TaTiersDTO)selectedTiers.getValue()).getId());
				((EJBLgrEditorPart)e).setPanel(((EJBLgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutEmail extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheEmail paramAfficheEmail = new ParamAfficheEmail();
				paramAfficheEmail.setIdTiers(LibConversion.integerToString(((TaTiersDTO)selectedTiers.getValue()).getId()));
				((EJBLgrEditorPart)e).setPanel(((EJBLgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutSiteWeb extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheWeb paramAfficheWeb = new ParamAfficheWeb();
				paramAfficheWeb.setIdTiers(((TaTiersDTO)selectedTiers.getValue()).getId());
				((EJBLgrEditorPart)e).setPanel(((EJBLgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutTelephone extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheTelephone paramAfficheTelephone = new ParamAfficheTelephone();
				paramAfficheTelephone.setIdTiers(((TaTiersDTO)selectedTiers.getValue()).getId());
				((EJBLgrEditorPart)e).setPanel(((EJBLgrEditorPart)e).getController().getVue());
	...
	protected void actInserer() throws Exception{
		try {
			if(getModeEcran().getMode().compareTo(EnumModeObjet.C_MO_CONSULTATION)==0) {
				...
				//swtTiers.setCodeTiers(dao.genereCode()); //ejb
...
//					dao.begin(transaction);
					TaTiers u = dao.findById(((TaTiersDTO) selectedTiers.getValue()).getId());
					dao.supprimer(u);
		...
		try {
			...
			case C_MO_INSERTION:
				if (silencieu || (!silencieu && MessageDialog.openQuestion(
					...
					if(((TaTiersDTO)selectedTiers.getValue()).getId()==null){
						modelTiers.getListeObjet().remove(((TaTiersDTO)selectedTiers.getValue()));
...
//		}
	...
	protected boolean aideDisponible() {
		...
		switch ((SWTPaTiersInfosEntrepriseController.this.getModeEcran().getMode())) {
		case C_MO_CONSULTATION:
...
//						paramAfficheAideRecherche.setChampsIdentifiant(Const.C_ID_T_PAIEMENT);
					...
					//					}
					if(SWTPaTiersInfosEntrepriseController.this.getModeEcran().getMode()==EnumModeObjet.C_MO_EDITION){
...
//			p.getTaTierses().add(taTiers);
	...
	private void initCodeCompta(){
		if(swtTiers==null){
			swtTiers=new TaTiersDTO();
			swtTiers = TaTiersDTO.copy(((TaTiersDTO)selectedTiers.getValue()));
		}
//		if(swtTiers!=null){
...
//		}
	...
	public IStatus validateUI() throws Exception {
		if ((getModeEcran().getMode().compareTo(EnumModeObjet.C_MO_INSERTION) == 0)
			...
			mapperUIToModel.map((TaTiersDTO) selectedTiers.getValue(),taTiers);
		}
		return null;
	}
...
//						else taTiers=dao.enregistrerMerge(taTiers);
						...
						//if(idTypeTiers!=null && idTypeTiers.equals(TaTiersDAO.C_ID_IDENTITE_ENTREPRISE_STR)) {
							...
							//TaInfoEntrepriseDAO infoDAO = new TaInfoEntrepriseDAO();
...
//			}
	...
	public boolean isUtilise(){
		return (selectedTiers!=null && selectedTiers.getValue()!=null &&
				((TaTiersDTO)selectedTiers.getValue()).getId()!=null && 
				!dao.recordModifiable(dao.getNomTable(),
						((TaTiersDTO)selectedTiers.getValue()).getId()))||
...
//				}
	...
	protected void actRefresh() throws Exception {
		...
		} else if(selectedTiers!=null && (TaTiersDTO) selectedTiers.getValue()!=null) {
			idActuel = ((TaTiersDTO) selectedTiers.getValue()).getId();
		}
		...
		if (getModeEcran().getMode().compareTo(EnumModeObjet.C_MO_CONSULTATION) == 0) {
			...
			if (taTiers!=null && selectedTiers!=null && (TaTiersDTO) selectedTiers.getValue()!=null) {
				mapperModelToUI.map(taTiers, (TaTiersDTO) selectedTiers.getValue());
			}
		}
		...
		if(!vue.isDisposed()) {
			if(selection instanceof IStructuredSelection) {
				...
				if(t!=null) {
					...
					paramAffiche.setIdFiche(String.valueOf(t.getId()));
