...
public class DocEcheanceController extends AbstractControllerMini {
	...
	private TaPrelevementDAO taPrelevementDAO = null;
	...
	protected List<TaPrelevement> listeDoc;
	private List<ModelObject> modele = null;
	protected  FormPageControllerPrelevement masterController = null;
	...
	/* Constructeur */
	public DocEcheanceController(FormPageControllerPrelevement masterContoller, PaFormPage vue, EntityManager em) {
		this.vue = vue;
	...
	public void initialiseModelIHM(int nbResult) {
		...
		taPrelevementDAO = new TaPrelevementDAO(masterController.getMasterDAOEM());
		...
		if(vue.getCompositeSectionParam().getTfCodeTiers().getText().equals("")) {
			listeDoc = taPrelevementDAO.rechercheDocumentEtat(
					LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb()),
					datefin,
					masterController.paramControllerMiniPrelevement.getMapEtatCodeLibelle().get(masterController.paramControllerMiniPrelevement.getCodeEtat()));
		} else {
			listeDoc = taPrelevementDAO.rechercheDocumentEtat(
					LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb()),
					datefin,
		...
		for (int i = 0; i < listeDoc.size() /*&& i < nbResult*/; i ++){
			...
			for (TaLPrelevement l : object.getLignes()) {
		...
		if(vue.getCompositeSectionParam().getCbEtat().getText().equals(fr.legrain.document.etat.devis.controllers.ParamControllerMini.etatEnCoursLibelle)) {
			libelleEcran = "Liste des prélèvements en cours arrivant à échéance avant le "
					+ LibDate.dateToString(datefin) + " ("+listDocumentSelection.size()+" documents)";
		} else {
			libelleEcran =
					"Liste des prélèvements arrivant à échéance entre le "
	...
	public void changeEtat(String v) {
		...
		String codeNouvelEtat = masterController.paramControllerMiniPrelevement.getMapEtatCodeLibelle().get(v);

		taPrelevementDAO = new TaPrelevementDAO(masterController.getMasterDAOEM());
		TaEtatDAO taEtatDAO = new TaEtatDAO(masterController.getMasterDAOEM());

		TaPrelevement taPrelevement =  taPrelevementDAO.findByCode(doc.code);
		TaEtat taEtat = null;

		if(codeNouvelEtat!=null)
			taEtat = taEtatDAO.findByCode(codeNouvelEtat);

		EntityTransaction transaction = taPrelevementDAO.getEntityManager().getTransaction();
		try {			
			taPrelevement.setTaEtat(taEtat);
			taPrelevementDAO.begin(transaction);
			
			taPrelevement.setLegrain(false); //Pour éviter setTaArticle sur les lignes suite au merge et donc la réinitialisation des informations de la ligne y compris pour les quantités

			//if ((taDevisDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
			taPrelevementDAO.enregistrerMerge(taPrelevement);
			//} 
			taPrelevementDAO.commit(transaction);
			transaction = null;
	...
	public final class ExampleEditingSupport extends ObservableValueEditingSupport {
		...
		private ExampleEditingSupport(ColumnViewer viewer) {
			...
			cellEditor.setInput(new WritableList(masterController.paramControllerMiniPrelevement.getMapEtatCodeLibelle().keySet(),String.class));
	...
	public void print() {
		...
		EditionTable ed = new EditionTable(infosDocuments, taPrelevementDAO.getEntityManager(), tableViewer, 
