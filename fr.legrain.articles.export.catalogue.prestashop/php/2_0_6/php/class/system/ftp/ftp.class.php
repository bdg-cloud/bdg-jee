...
class ftp {
    ...
    function ftp_sync ($dir_dist, $dir_local) {

    	if(!$this -> flux_connexion === false && $this -> flux_identification === true ) {

    		if ($dir_dist != ".") {
    			if (ftp_chdir ($this -> flux_connexion,$dir_dist) == false) {
    				echo ("Change Dir Failed: $dir_dist<BR>\r\n");
    				return;
    			}
    			if (!(is_dir($dir_local)))
    			mkdir($dir_local);
    		...
    		$contents = ftp_nlist($this -> flux_connexion, ".");
    		foreach ($contents as $file) {
    			 
    			if ($file == '.' || $file == '..')
    			continue;
    			 
    			if (@ftp_chdir($this -> flux_connexion, $file)) {
    				ftp_chdir ($this -> flux_connexion, "..");
    				$this ->ftp_sync ($file,$file);
    			}
    			else
    			ftp_get($this -> flux_connexion, $file, $file, FTP_BINARY);
    		}
