...
public class SWTPaEmailRappelerController extends JPABaseControllerSWTStandard
implements RetourEcranListener {

	static Logger logger = Logger.getLogger(SWTPaEmailRappelerController.class.getName()); 
	private PaEmailRappeler vue = null;
	private TaEmailRappelerDAO dao = new TaEmailRappelerDAO();

	private Object ecranAppelant = null;
	private SWTTaUserRappeler SWTTaEmailRappeler;
	private SWTTaUserRappeler swtOldEmailRappeler;
	private Realm realm;
...
//	private ModelGeneralObjet<SWTTaUserRappeler,TaUserRappelerDAO,TaUserRappeler> modelUserRappeler = new ModelGeneralObjet<SWTTaUserRappeler,TaUserRappelerDAO,TaUserRappeler>(dao,classModel);
	private ModelGeneralObjet<SWTTaUserRappeler,TaEmailRappelerDAO,TaEmailRappeler> modelListUserRappeler = new ModelGeneralObjet<SWTTaUserRappeler,TaEmailRappelerDAO,TaEmailRappeler>(dao,classModel);
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private IObservableValue selectedListEmailRappeler;
	...
	private LgrDozerMapper<SWTTaUserRappeler,TaEmailRappeler> mapperUIToModel = new LgrDozerMapper<SWTTaUserRappeler,TaEmailRappeler>();
	private LgrDozerMapper<TaEmailRappeler,SWTTaUserRappeler> mapperModelToUI = new LgrDozerMapper<TaEmailRappeler,SWTTaUserRappeler>();
	private TaEmailRappeler TaEmailRappeler = null;
	...
	public SWTPaEmailRappelerController(PaEmailRappeler vue) {
		setVue(vue);
...
//			});
	...
	public void bind(PaEmailRappeler PaEmailRappeler) {
		try {
...
//			//bindingFormSimple(dao, mapComposantChampsCommentaire, dbc, realm, selectedUserRappeler, classModel);
			...
			tableViewer = new LgrTableViewer(PaEmailRappeler.getGrille());
			tableViewer.createTableCol(classModel,PaEmailRappeler.getGrille(), nomClassController,
			...
			selectedListEmailRappeler = ViewersObservables.observeSingleSelection(tableViewer);
			...
			bindingFormMaitreDetail(dbc, realm, selectedListEmailRappeler,classModel);
...
//		mapComposantChamps.put(vue.getTfDIVERS_ARTICLE(), Const.C_DIVERS_ARTICLE);
		mapComposantChamps.put(vue.getTfSUJET_DEM() , Const.C_SUJET_DEM );
		mapComposantChamps.put(vue.getTfCONTENU_DEM(), Const.C_CONTENU_DEM);
		mapComposantChamps.put(vue.getTfSUJET_DEST() , Const.C_SUJET_DEST );
		mapComposantChamps.put(vue.getTfCONTENU_DEST(), Const.C_CONTENU_DEST);
	...
	public boolean onClose() throws ExceptLgr {
		...
		if (retour) {
			if (ecranAppelant instanceof SWTPaAideControllerSWT) {
				dao.initValeurIdTable(TaEmailRappeler);
				fireRetourEcran(new RetourEcranEvent(this, new ResultAide(
						dao.getChampIdEntite(), dao.getValeurIdTable(),
						selectedListEmailRappeler.getValue())));
	...
	protected void actInserer() throws Exception {
		try {
			if(dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION)==0) {
				...
				SWTTaEmailRappeler = new SWTTaUserRappeler();
...
//				SWTTaUserRappeler.setStockMinArticle(new BigDecimal(1));
				...
				TaEmailRappeler = new TaEmailRappeler();
				dao.inserer(TaEmailRappeler);
				modelListUserRappeler.getListeObjet().add(SWTTaEmailRappeler);
				writableList = new WritableList(realm, modelListUserRappeler.getListeObjet(), classModel);
				tableViewer.setInput(writableList);
				tableViewer.refresh();
				tableViewer.setSelection(new StructuredSelection(SWTTaEmailRappeler));
				initEtatBouton();
			}
	...
	protected void actModifier() throws Exception {
		try {
			if(dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION)==0) {
				setSwtOldUserRappeler();
				TaEmailRappeler = dao.findById(((SWTTaUserRappeler) selectedListEmailRappeler.getValue()).getIdEmailRappeler());
				dao.modifier(TaEmailRappeler);
		...
		try {
			...
			if(isUtilise())MessageDialog.openInformation(vue.getShell(), MessagesEcran
				...
				if (MessageDialog.openConfirm(vue.getShell(), MessagesEcran
					...
					TaEmailRappeler u = dao.findById(((SWTTaUserRappeler) selectedListEmailRappeler.getValue()).getIdEmailRappeler());
					dao.supprimer(u);
					TaEmailRappeler=null;
...
//					}
			...
			case C_MO_EDITION:
				if (MessageDialog.openQuestion(vue.getShell(), MessagesEcran
						.getString("Message.Attention"), MessagesEcran
						.getString("Articles.Message.Annuler"))) {
					int rang = modelListUserRappeler.getListeObjet().indexOf(selectedListEmailRappeler.getValue());
					modelListUserRappeler.getListeObjet().set(rang, swtOldEmailRappeler);
					writableList = new WritableList(realm, modelListUserRappeler.getListeObjet(), classModel);
					tableViewer.setInput(writableList);
					tableViewer.refresh();
					tableViewer.setSelection(new StructuredSelection(swtOldEmailRappeler), true);
					dao.annuler(TaEmailRappeler);
					hideDecoratedFields();
				}
				initEtatBouton();
...
//
	...
	protected boolean aideDisponible() {
		...
		switch ((SWTPaEmailRappelerController.this.dao.getModeObjet().getMode())) {
		case C_MO_CONSULTATION:
...
//				case C_MO_CONSULTATION:
//					if(getFocusCourantSWT().equals(vue.getGrille())){
//						PaEmailRappeler paArticlesSWT = new PaEmailRappeler(s2,SWT.NULL);
...
////		if(vue instanceof PaEmailRappeler) {
////			((PaEmailRappeler)vue).getBtnAjoutPrix().setImage(ArticlesPlugin.getImageDescriptor(C_IMAGE_AJOUT_PRIX).createImage());
////			vue.layout(true);
////		}
	}
	...
	public IStatus validateUI() throws Exception {
		if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
			...
			mapperUIToModel.map((SWTTaUserRappeler) selectedListEmailRappeler.getValue(),TaEmailRappeler);
		}
		return null;
	}
...
//			return s;
			...
			TaEmailRappeler u = new TaEmailRappeler();
			PropertyUtils.setSimpleProperty(u, nomChamp, value);
			if(((SWTTaUserRappeler) selectedListEmailRappeler.getValue()).getIdUserRappeler()!=null) {
				u.setIdEmailRappeler(((SWTTaUserRappeler) selectedListEmailRappeler.getValue()).getIdUserRappeler());
			}
	...
	protected void actEnregistrer() throws Exception {
		...
		try {
			...
			if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
				...
				if((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0))	
					TaEmailRappeler=dao.enregistrerMerge(TaEmailRappeler);
				else TaEmailRappeler=dao.enregistrerMerge(TaEmailRappeler);
...
//		}
	...
	public boolean isUtilise(){
		return ((SWTTaUserRappeler)selectedListEmailRappeler.getValue()).getIdUserRappeler()!=null && 
		!dao.recordModifiable(dao.getNomTable(),
				((SWTTaUserRappeler)selectedListEmailRappeler.getValue()).getIdUserRappeler());		
	}
	public SWTTaUserRappeler getSwtOldEmailRappeler() {
		return swtOldEmailRappeler;
	}

	public void setSwtOldUserRappeler(SWTTaUserRappeler swtOldEmailRappeler) {
		this.swtOldEmailRappeler = swtOldEmailRappeler;
	}

	public void setSwtOldUserRappeler() {
		if (selectedListEmailRappeler.getValue() != null){}
		//	this.swtOldUserRappeler = SWTTaUserRappeler.copy((SWTTaUserRappeler) selectedListUserRappeler.getValue());
		else {
			if(tableViewer.selectionGrille(0)){
				this.swtOldEmailRappeler = SWTTaEmailRappeler.copy((SWTTaUserRappeler) selectedListEmailRappeler.getValue());
				tableViewer.setSelection(new StructuredSelection(
						(SWTTaUserRappeler) selectedListEmailRappeler.getValue()), true);
			}}
	}

	public void setVue(PaEmailRappeler vue) {
		super.setVue(vue);
		this.vue = vue;
	}
	...
	protected void initMapComposantDecoratedField() {
		...
		mapComposantDecoratedField.put(vue.getTfSUJET_DEM(), vue.getFieldSUJET_DEM());
		mapComposantDecoratedField.put(vue.getTfCONTENU_DEM(), vue.getFieldCONTENU_DEM());
		mapComposantDecoratedField.put(vue.getTfSUJET_DEST(), vue.getFieldSUJET_DEST());
		mapComposantDecoratedField.put(vue.getTfCONTENU_DEST(), vue.getFieldCONTENU_DEST());
	...
	protected void actRefresh() throws Exception {
		if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION) == 0) {
			...
			if (TaEmailRappeler!=null && selectedListEmailRappeler.getValue()!=null && (SWTTaUserRappeler) selectedListEmailRappeler.getValue()!=null) {
				mapperModelToUI.map(TaEmailRappeler, (SWTTaUserRappeler) selectedListEmailRappeler.getValue());
			}
		}
	
		//repositionnement sur la valeur courante
		int idActuel = 0;
		if (TaEmailRappeler!=null) { //enregistrement en cours de modification/insertion
			idActuel = TaEmailRappeler.getIdEmailRappeler();
		} else if(selectedListEmailRappeler.getValue()!=null && (SWTTaUserRappeler) selectedListEmailRappeler.getValue()!=null) {
			idActuel = ((SWTTaUserRappeler) selectedListEmailRappeler.getValue()).getIdEmailRappeler();
		}
