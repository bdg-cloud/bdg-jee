package fr.legrain.generationdocument.facture.handlers;
...
import fr.legrain.generationdocument.facture.divers.ParamAfficheGenerationDocument;
import fr.legrain.generationdocument.facture.editor.EditorInputSelectionGenerationDocument;
import fr.legrain.generationdocument.facture.editor.EditorSelectionGenerationDocument;
import fr.legrain.gestCom.librairiesEcran.swt.LgrAbstractHandler;
import fr.legrain.gestCom.librairiesEcran.workbench.JPALgrEditorPart;
import fr.legrain.gestCom.librairiesEcran.workbench.LgrEditorPart;
import fr.legrain.gestCom.librairiesEcran.workbench.LgrPartListener;

public class HandlerGenerationDocument extends LgrAbstractHandler {
	
	static Logger logger = Logger.getLogger(HandlerGenerationDocument.class.getName());
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			IEditorPart editor = JPALgrEditorPart.verifEditeurOuvert(EditorSelectionGenerationDocument.ID);
			if(editor==null){
			IEditorPart e = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().
			openEditor(new EditorInputSelectionGenerationDocument(), EditorSelectionGenerationDocument.ID);
			LgrPartListener.getLgrPartListener().getLgrNavigation().add(e);
			
			ParamAfficheGenerationDocument paramAfficheGenerationDocument = new ParamAfficheGenerationDocument();
