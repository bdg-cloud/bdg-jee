 ...
 *
 */
public class GraphControllerAcompte extends GraphControllerMini{
	static Logger logger = Logger.getLogger(GraphControllerAcompte.class.getName());	
	...
	public GraphControllerAcompte(FormPageControllerPrincipal masterContoller,
	...
	public void initialiseModelIHM() {
		...
		class ResultatCA {
			 ...
			 * @param jour le jour d'édition de la acompte
			 * @param mois le mois " "
			 * @param annee l'année " "
			 * @param ht montant ht de la acompte
			 * @param tva tva de la acompte
			 * @param ttc montant ttc de la acompte
			 */
		...
		List<Object> listeObjectAcompte = taAcompteDAO.findChiffreAffaireTotal(
				datedeb,datefin,
				precision);
		...
		dataSetChart.getMapValuesYSeries().put("CA", serieAcompte);
		...
		MultiSerieBarChart multiSerieBarChart = new MultiSerieBarChart(vue.getCompositeSectionEvolution().getCompo(), SWT.NONE, 
				dataSetChart,"Evolution des factures acompte HT",ChartDimension.TWO_DIMENSIONAL_WITH_DEPTH_LITERAL,titreAbscisse,"Montant HT");
