package fr.legrain.prelevement.controller;
...
public class PaLignePrelevementController extends JPABaseControllerSWTStandard
implements RetourEcranListener, SWTModificationDocumentListener, ISelectionProvider {

	static Logger logger = Logger.getLogger(PaLignePrelevementController.class.getName());
	...
	private PaSaisieLignePrelevement vue = null;


	private TaLPrelevementDAO dao = null;//new TaLPrelevementDAO();
	
	private Object ecranAppelant = null;
	private IHMLPrelevement ihmLPrelevement;
	private IHMTotauxLignesDocument ihmTotauxLignesDocument;
	private IHMLPrelevement ihmOldLPrelevement;
	private Realm realm;
	private DataBindingContext dbc;
	private DataBindingContext dbcTotaux = null;

//	private ModelLigneDevis modelLDevis /*=  new ModelLigneFacture(ibTaTable)*/;
	private Class classModel = IHMLPrelevement.class;
	private ModelGeneralObjet<IHMLPrelevement,TaLPrelevementDAO,TaLPrelevement> modelLDocument /*= new ModelGeneralObjet<IHMLPrelevement,TaLPrelevementDAO,TaLPrelevement>(dao,classModel)*/;
	...
	private TaLPrelevement taLPrelevement = null;
	...
	private LgrDozerMapper<IHMLPrelevement,TaLPrelevement> mapperUIToModel = new LgrDozerMapper<IHMLPrelevement,TaLPrelevement>();
	private LgrDozerMapper<TaLPrelevement,IHMLPrelevement> mapperModelToUI = new LgrDozerMapper<TaLPrelevement,IHMLPrelevement>();
	
	private LgrDozerMapper<TaPrelevement,IHMTotauxLignesDocument> mapperModelToUITotauxLignes = new LgrDozerMapper<TaPrelevement,IHMTotauxLignesDocument>();

	public PaLignePrelevementController(){
	}
	
	public PaLignePrelevementController(final PaSaisieLignePrelevement vue/*, SWTFacture facture, WorkbenchPart part*/) {
		this(vue,null);
	}
	
	public PaLignePrelevementController(final PaSaisieLignePrelevement vue/*, SWTFacture facture, WorkbenchPart part*/,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		dao = new TaLPrelevementDAO(getEm());
		try {
			setVue(vue);
...
//			// fenetre sur ESC
	 ...
	 * @return
	 */
	public List<IHMLPrelevement> IHMmodel() {
		LinkedList<TaLPrelevement> ldao = new LinkedList<TaLPrelevement>();
		LinkedList<IHMLPrelevement> lswt = new LinkedList<IHMLPrelevement>();
		
		if(masterEntity!=null && !masterEntity.getLignes().isEmpty()) {

			ldao.addAll(masterEntity.getLignes());

			LgrDozerMapper<TaLPrelevement,IHMLPrelevement> mapper = new LgrDozerMapper<TaLPrelevement,IHMLPrelevement>();
			for (TaLPrelevement o : ldao) {
				IHMLPrelevement t = null;
				t = (IHMLPrelevement) mapper.map(o, IHMLPrelevement.class);
				lswt.add(t);
			}
	...
	public void bind(PaSaisieLignePrelevement paArticleSWT) {
		try {
			if (modelLDocument==null)modelLDocument = new ModelGeneralObjet<IHMLPrelevement, TaLPrelevementDAO, TaLPrelevement>(dao,classModel);
...
//				tableViewer.setInput(writableList);
				...
				selectedLignePrelevement = ViewersObservables.observeSingleSelection(tableViewer);
			}
...
//			ibTaTable.ouvreDataset();
			if (((ParamAfficheLPrelevement)param).getFocusSWT()!=null)
				((ParamAfficheLPrelevement)param).getFocusSWT().forceFocus();
			else ((ParamAfficheLPrelevement)param).setFocusSWT(vue.getTfCODE_ARTICLE());			
...
//			ibTaTable.getFIBQuery().setSort(null);
		...
		if (param.getModeEcran() != null
			...
			try {
				InsertionLigne(false,((ParamAfficheLPrelevement)param).getInitFocus());
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfQTE_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_QTE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfQTE2_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_QTE2_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfMT_HT_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_MT_HT_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_MT_TTC_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfPRIX_U_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_PRIX_U_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfREM_TX_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_REM_TX_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfNUM_LIGNE_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_NUM_LIGNE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfCODE_ARTICLE(), new InfosVerifSaisie(new TaArticle(),Const.C_CODE_ARTICLE,null));
		mapInfosVerifSaisie.put(vue.getTfLIB_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_LIB_L_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfU1_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_U1_L_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfU2_L_PRELEVEMENT(), new InfosVerifSaisie(new TaLPrelevement(),Const.C_U1_L_DOCUMENT,null));
	...
	protected void initMapComposantChamps() {
		...
		mapComposantChamps.put(vue.getTfLIB_L_PRELEVEMENT(), Const.C_LIB_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfQTE_L_PRELEVEMENT(), Const.C_QTE_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfU1_L_PRELEVEMENT(), Const.C_U1_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfQTE2_L_PRELEVEMENT(), Const.C_QTE2_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfU2_L_PRELEVEMENT(), Const.C_U2_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfPRIX_U_L_PRELEVEMENT(), Const.C_PRIX_U_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfREM_TX_L_PRELEVEMENT(), Const.C_REM_TX_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfMT_HT_L_PRELEVEMENT(), Const.C_MT_HT_L_DOCUMENT);
		mapComposantChamps.put(vue.getTfMT_TTC_L_PRELEVEMENT(), Const.C_MT_TTC_L_DOCUMENT);
...
//				}
		...
		mapComposantChamps.put(vue.getTfNUM_LIGNE_L_PRELEVEMENT(), Const.C_NUM_LIGNE_L_DOCUMENT);
		...
		vue.getTfQTE_L_PRELEVEMENT().addVerifyListener(queDesChiffresPositifs);
		vue.getTfQTE2_L_PRELEVEMENT().addVerifyListener(queDesChiffresPositifs);
		vue.getTfMT_HT_L_PRELEVEMENT().addVerifyListener(queDesChiffresPositifs);
		vue.getTfMT_TTC_L_PRELEVEMENT().addVerifyListener(queDesChiffresPositifs);
		vue.getTfPRIX_U_L_PRELEVEMENT().addVerifyListener(queDesChiffres);
		vue.getTfREM_TX_L_PRELEVEMENT().addVerifyListener(queDesChiffresPositifs);
		vue.getTfNUM_LIGNE_L_PRELEVEMENT().addVerifyListener(queDesChiffresPositifs);
...
//			}
			...
			articleContentProposal.addContentProposalListener(new IContentProposalListener2(){
				...
				public void proposalPopupOpened(ContentProposalAdapter adapter) {
					...
					if(!dao.dataSetEnModif()&& taLPrelevement.ligneEstVide()  )
	...
	public PaLignePrelevementController getThis() {
		return this;
	}
	...
	public void retourEcran(final RetourEcranEvent evt) {
		try {
			if (evt.getRetour() != null && (evt.getSource() instanceof SWTPaAideControllerSWT)) {
				if (getFocusAvantAideSWT() instanceof Text) {
					try {
						((Text) getFocusAvantAideSWT()).setText(((ResultAffiche) evt
								.getRetour()).getResult());	
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_ARTICLE())||getFocusAvantAideSWT().equals(vue.getTfLIB_L_PRELEVEMENT())){
							TaArticle f = null;
							TaArticleDAO t = new TaArticleDAO(getEm());
							f = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
							t = null;
							taLPrelevement.setLegrain(true);
								taLPrelevement.setTaArticle(f);
						}
						if(getFocusAvantAideSWT().equals(vue.getTfU1_L_PRELEVEMENT())
								||getFocusAvantAideSWT().equals(vue.getTfU2_L_PRELEVEMENT())){
							TaUnite f = null;
							TaUniteDAO t = new TaUniteDAO(getEm());
							f = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
							t = null;
							if(getFocusAvantAideSWT().equals(vue.getTfU1_L_PRELEVEMENT()))
								taLPrelevement.setU1LDocument(f.getCodeUnite());
							else
								taLPrelevement.setU2LDocument(f.getCodeUnite());
						}					
						if(getFocusAvantAideSWT().equals(vue.getTfPRIX_U_L_PRELEVEMENT())){
							TaPrix f = null;
							...
							if(masterEntity.getTtc()==1)
								taLPrelevement.setPrixULDocument(f.getPrixttcPrix());
							else
								taLPrelevement.setPrixULDocument(f.getPrixPrix());
						}					
				...
				if(this.getFocusAvantAideSWT() instanceof Text && this.getFocusAvantAideSWT()==vue.getTfPRIX_U_L_PRELEVEMENT()){
...
//						ibTaTable.lgrLocateID(((ResultAffiche) evt.getRetour()).getIdResult());
			...
			if(parentEcran!=null) {
				...
				((PaEditorPrelevementController)parentEcran).initEtatBouton(false);
			}
		}
		}
...
//		public void run() {
		if (getFocusCourantSWT()==vue.getTfPRIX_U_L_PRELEVEMENT()){
			try {
				...
				if(!LibChaine.empty(vue.getTfCODE_ARTICLE().getText())&& masterEntity.getLigne(masterEntity.getLigneCourante())!=null) {
					...
					TaArticle taArticle = taArticleDAO.findById(((TaLPrelevement)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaArticle().getIdArticle());
				...
				if (masterEntity.getTtc()==0){
					...
					paramAideCalcTVA.setMtHT(vue.getTfPRIX_U_L_PRELEVEMENT().getText());
				}
				else{
					paramAideCalcTVA.setFocusSWT(paAideCalcTVA.getTfMtHT());
					paramAideCalcTVA.setMtTTC(vue.getTfPRIX_U_L_PRELEVEMENT().getText());
				}
...
//	);
	...
	protected void actChangeTva() throws Exception{
		if(!LibChaine.empty(((IHMLPrelevement)selectedLignePrelevement.getValue()).getCodeArticle())
			...
			InputDialog input = new InputDialog(vue.getShell()
					...
					((IHMLPrelevement)selectedLignePrelevement.getValue()).getCodeTvaLDocument(),new IInputValidator() {
			...
			if(masterDAO.dataSetEnModif()){
				...
				if(input.getValue()!=null ){
					if(MessageDialog.openConfirm(vue.getShell()
						...
						((IHMLPrelevement)selectedLignePrelevement.getValue()).setCodeTvaLDocument(retour);
						if(input.getValue().equals("")){
							taLPrelevement.setCodeTvaLDocument("");
							taLPrelevement.setTauxTvaLDocument(BigDecimal.valueOf(0));
						}else{
							TaTvaDAO daoTva = new TaTvaDAO(getEm());
							taLPrelevement.setCodeTvaLDocument(retour);
							taLPrelevement.setTauxTvaLDocument(daoTva.findByCode(retour).getTauxTva());
						}
...
//			}
			if(continuer){
				...
				for (TaLPrelevement p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument().equals(((IHMLPrelevement) selectedLignePrelevement.getValue()).getNumLigneLDocument())) {
						taLPrelevement = p;
						taLPrelevement.setLegrain(true);
						ihmLPrelevement= IHMLPrelevement.copy((IHMLPrelevement) selectedLignePrelevement.getValue());;
					}				
				}
				initEtatBouton(false);
			}
	...
	protected void actSupprimer() throws Exception {
		...
		try {
			...
			for (TaLPrelevement p : masterEntity.getLignes()) {
				...
				dao.supprimer(taLPrelevement);
				taLPrelevement.setTaDocument(null);
				masterEntity.removeLigne(taLPrelevement);
				taLPrelevement=null;
				actRefresh();
				refreshTotauxLignes();
				initEtatBouton();
	...
	protected void actAnnuler() throws Exception {
		...
		try {
			...
			case C_MO_INSERTION:
				message =!taLPrelevement.ligneEstVide();
			...
			case C_MO_EDITION:
				...
				if(repondu){
					int rang =((WritableList)tableViewer.getInput()).indexOf(selectedLignePrelevement.getValue());
					List<IHMLPrelevement> l = IHMmodel();
					if(rang!=-1)
					  l.set(rang, ihmOldLPrelevement);
					
					writableList = new WritableList(realm, l, classModel);
					tableViewer.setInput(writableList);
					tableViewer.refresh();
					tableViewer.setSelection(new StructuredSelection(ihmOldLPrelevement), true);
					
					mapperUIToModel.map(ihmOldLPrelevement,taLPrelevement);
//					ctrlTousLesChampsAvantEnregistrementSWT();//si on contrôle tous les champs
	...
	protected boolean aideDisponible() {
		...
		case C_MO_INSERTION:
			if (getFocusCourantSWT()==vue.getTfCODE_ARTICLE() 
					||getFocusCourantSWT()==vue.getTfLIB_L_PRELEVEMENT()
					|| getFocusCourantSWT()==vue.getTfU1_L_PRELEVEMENT() 
					|| getFocusCourantSWT()==vue.getTfU2_L_PRELEVEMENT() 
					|| getFocusCourantSWT()==vue.getTfPRIX_U_L_PRELEVEMENT())
				result = true;
			break;
		default:
			break;
	...
	protected void actPrecedent() throws Exception {
		if(taLPrelevement!=null && taLPrelevement.ligneEstVide()){
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				switch ((PaLignePrelevementController.this.dao.getModeObjet().getMode())) {
				case C_MO_CONSULTATION:
				...
				case C_MO_INSERTION:
					...
					if (getFocusCourantSWT()==vue.getTfLIB_L_PRELEVEMENT()){
						PaArticleSWT paArticleSWT = new PaArticleSWT(s2,SWT.NULL);
						SWTPaArticlesController swtPaArticlesController = new SWTPaArticlesController(paArticleSWT);
						...
						paramAfficheAideRecherche.setDebutRecherche(vue.getTfLIB_L_PRELEVEMENT().getText());
						paramAfficheAideRecherche.setControllerAppelant(getThis());
					...
					if (getFocusCourantSWT()==vue.getTfU1_L_PRELEVEMENT()||
							getFocusCourantSWT()==vue.getTfU2_L_PRELEVEMENT()){
						...
						if(getFocusCourantSWT()==vue.getTfU1_L_PRELEVEMENT())
							paramAfficheAideRecherche.setDebutRecherche(vue.getTfU1_L_PRELEVEMENT().getText());
						else
							paramAfficheAideRecherche.setDebutRecherche(vue.getTfU2_L_PRELEVEMENT().getText());
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new TaUniteDAO(getEm()).modelObjetUniteArticle(taLPrelevement.getTaArticle().getCodeArticle()));
					...
					if (getFocusCourantSWT()==vue.getTfPRIX_U_L_PRELEVEMENT()){
						...
						paramAffichePrix.setIdArticle(LibConversion.integerToString(getSelectedLignePrelevement().getIdArticle()));
						...
						paramAfficheAideRecherche.setDebutRecherche(vue.getTfPRIX_U_L_PRELEVEMENT().getText());
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTPrix,TaPrixDAO,TaPrix>(taLPrelevement.getTaArticle().getTaPrixes(),SWTPrix.class,getEm()));
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "L_PRELEVEMENT";
		try {
			IStatus s = null;
			boolean change=true;
			if(nomChamp.equals(Const.C_CODE_ARTICLE)) {
				...
				if(ihmLPrelevement!=null)
					change = !f.getCodeArticle().equals(ihmLPrelevement.getCodeArticle());
				if(change){
					taLPrelevement.setTaArticle(f);
					taLPrelevement.initQte2(f);
					mapperModelToUI.map(taLPrelevement, ihmLPrelevement);
				}
				}
				dao = null;
			} else {
				TaLPrelevement u = new TaLPrelevement();
				...
				if(nomChamp.equals(Const.C_QTE_L_DOCUMENT)){
					if (ihmLPrelevement!=null)
						change = u.getQteLDocument()!=null&& !u.getQteLDocument().equals(ihmLPrelevement.getQteLDocument());
				}
				if(s.getSeverity()!=IStatus.ERROR) {
					PropertyUtils.setSimpleProperty(taLPrelevement, nomChamp, value);
				}
			}
			
			if(s.getSeverity()!=IStatus.ERROR) {
				if(nomChamp.equals(Const.C_QTE_L_DOCUMENT)&& change) {
					if(taLPrelevement.getTaArticle()!=null){
						taLPrelevement.initQte2(taLPrelevement.getTaArticle());
					}
				}				
				mapperModelToUI.map(taLPrelevement, ((IHMLPrelevement)selectedLignePrelevement.getValue()));
			}
	...
	protected void actEnregistrer(boolean insertion) throws Exception {
		try {
			if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
				...
				ctrlTousLesChampsAvantEnregistrementSWT();
				if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
					LgrDozerMapper<IHMLPrelevement,TaLPrelevement> mapper = new LgrDozerMapper<IHMLPrelevement,TaLPrelevement>();
					mapper.map((IHMLPrelevement) selectedLignePrelevement.getValue(),taLPrelevement);
				...
				masterEntity.enregistreLigne(taLPrelevement);
				dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_CONSULTATION);
...
//	}
	...
	public void initEtatComposant(String type) throws SQLException{		
		initEtatComposant();
		vue.getTfQTE_L_PRELEVEMENT().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfQTE2_L_PRELEVEMENT().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfU1_L_PRELEVEMENT().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfU2_L_PRELEVEMENT().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfPRIX_U_L_PRELEVEMENT().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
		vue.getTfREM_TX_L_PRELEVEMENT().setEditable(!type.equals(Const.C_CODE_T_LIGNE_C));
	}

	public void initEtatComposant() {
		try {
			vue.getTfNUM_LIGNE_L_PRELEVEMENT().setEnabled(false);
			vue.getTfMT_HT_L_PRELEVEMENT().setEditable(false);
			vue.getTfMT_TTC_L_PRELEVEMENT().setEditable(false);
			...
			vue.getTfLIB_L_PRELEVEMENT().setEditable(editable);
			vue.getTfPRIX_U_L_PRELEVEMENT().setEditable(editable);
			vue.getTfQTE_L_PRELEVEMENT().setEditable(editable);
			vue.getTfQTE2_L_PRELEVEMENT().setEditable(editable);
			vue.getTfU1_L_PRELEVEMENT().setEditable(editable);
			vue.getTfU2_L_PRELEVEMENT().setEditable(editable);
			vue.getTfREM_TX_L_PRELEVEMENT().setEditable(editable);
			vue.getTfMT_HT_L_PRELEVEMENT().setEditable(false);
			vue.getTfMT_TTC_L_PRELEVEMENT().setEditable(false);
	...
	public boolean isUtilise(){
		return ((IHMLPrelevement)selectedLignePrelevement.getValue()).getIdLDocument()!=null && 
		!dao.recordModifiable(dao.getNomTable(),
				((IHMLPrelevement)selectedLignePrelevement.getValue()).getIdLDocument());		
	}
	public IHMLPrelevement getIhmOldLPrelevement() {
		return ihmOldLPrelevement;
	}

	public void setIhmOldLPrelevement(IHMLPrelevement swtOldDevis) {
		this.ihmOldLPrelevement = swtOldDevis;
	}

	public void setIhmOldLDevis() {
		if (selectedLignePrelevement!=null && selectedLignePrelevement.getValue() != null)
			this.ihmOldLPrelevement = IHMLPrelevement.copy((IHMLPrelevement) selectedLignePrelevement.getValue());
		else {
			if(tableViewer.selectionGrille(0)){
				this.ihmOldLPrelevement = IHMLPrelevement.copy((IHMLPrelevement) selectedLignePrelevement.getValue());
				tableViewer.setSelection(new StructuredSelection(
						(IHMLPrelevement) selectedLignePrelevement.getValue()), true);
			}}
	}

	public void setVue(final PaSaisieLignePrelevement vue) {
		super.setVue(vue);
		this.vue = vue;
	}
	...
	protected void initMapComposantDecoratedField() {
		...
		mapComposantDecoratedField.put(vue.getTfLIB_L_PRELEVEMENT(), vue.getFieldLIB_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfMT_HT_L_PRELEVEMENT(), vue.getFieldMT_HT_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfMT_TTC_L_PRELEVEMENT(), vue.getFieldMT_TTC_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfNUM_LIGNE_L_PRELEVEMENT(), vue.getFieldNUM_LIGNE_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfPRIX_U_L_PRELEVEMENT(), vue.getFieldPRIX_U_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfQTE_L_PRELEVEMENT(), vue.getFieldQTE_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfQTE2_L_PRELEVEMENT(), vue.getFieldQTE2_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfREM_TX_L_PRELEVEMENT(), vue.getFieldREM_TX_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfU1_L_PRELEVEMENT(), vue.getFieldU1_L_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfU2_L_PRELEVEMENT(), vue.getFieldU2_L_PRELEVEMENT());
	}
	...
	protected void sortieChamps() {
		try {
			if (masterEntity.getLigne(masterEntity.getLigneCourante())!=null && 
					( (TaLPrelevement)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaTLigne().getIdTLigne()!=0)
				initEtatComposant(( (TaLPrelevement)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaTLigne().getCodeTLigne());		
	...
	protected void actRefresh() throws Exception {
		...
		tableViewer.selectionGrille(
				tableViewer.selectionGrille(selectedLignePrelevement.getValue()));	
	}
	...
	public void initTTC() {
		...
		if(masterEntity!=null && masterEntity.getTtc()==1) {
			...
			vue.getTfMT_HT_L_PRELEVEMENT().setEnabled(!ttc);
			vue.getTfMT_TTC_L_PRELEVEMENT().setEnabled(ttc);
			vue.getLaMT_HT_L_PRELEVEMENT().setEnabled(!ttc);
			vue.getLaMT_TTC_L_PRELEVEMENT().setEnabled(ttc);
	}
...
//
//			for (IHMLPrelevement ihmLigne : modelLDevis.getListeObjet()) {
//				SWTLigneDevis swtLigne = new SWTLigneDevis((IHMLPrelevement)selectedLigneProforma.getValue());
...
//			}
	...
	private class SelectionGrille extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			setIhmOldLDevis();
			if(selectedLignePrelevement.getValue()!=null && masterEntity.getLignes().size()>vue.getGrille().getSelectionIndex()){
				for (TaLPrelevement p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument()==((IHMLPrelevement) selectedLignePrelevement.getValue()).getNumLigneLDocument()) {
						taLPrelevement = p;
					}				
				}
			}
	...
	public IHMLPrelevement getSelectedLignePrelevement() {
		IHMLPrelevement resultat = null;
		if(selectedLignePrelevement!=null && selectedLignePrelevement.getValue()!=null)
			resultat = (IHMLPrelevement)selectedLignePrelevement.getValue();
		return resultat;
	};	
	...
	protected void InsertionLigne(Boolean ajout, boolean initFocus) throws Exception{
		...
		try {
			...
			if(masterDAO.dataSetEnModif()){
			setIhmOldLDevis();
			ihmLPrelevement = new IHMLPrelevement();
			dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_INSERTION);
			taLPrelevement = new TaLPrelevement(true); 
			taLPrelevement.setTaTLigne(new TaTLigneDAO(getEm()).findByCode(Const.C_CODE_T_LIGNE_C));
			
			List<IHMLPrelevement> modelListeLigneDevis = IHMmodel();
			
			if(ajout)
				masterEntity.addLigne(taLPrelevement);
			else{
				if(vue.getGrille().getItemCount()!=0) 
				masterEntity.insertLigne(taLPrelevement,vue.getGrille().getSelectionIndex());
				else
					masterEntity.insertLigne(taLPrelevement,0);
					
			}
			ihmLPrelevement.setNumLigneLDocument(taLPrelevement.getNumLigneLDocument());
			taLPrelevement.setTaDocument(masterEntity);
			taLPrelevement.initialiseLigne(true);
			
			modelListeLigneDevis.add(ihmLPrelevement.getNumLigneLDocument(),ihmLPrelevement);
			
			writableList = new WritableList(realm, modelListeLigneDevis, classModel);
			tableViewer.setInput(writableList);
			tableViewer.refresh();
			tableViewer.setSelection(new StructuredSelection(ihmLPrelevement),true);
			initEtatComposant(taLPrelevement.getTaTLigne().getCodeTLigne());

			taLPrelevement.setNumLigneLDocument(ihmLPrelevement.getNumLigneLDocument());
...
//			}		
	...
	protected void initEtatBouton(boolean initFocus) {
		...
		vue.getTfU2_L_PRELEVEMENT().setEditable(editable);
		vue.getTfREM_TX_L_PRELEVEMENT().setEditable(editable);
		vue.getTfMT_HT_L_PRELEVEMENT().setEditable(editable);
		vue.getTfMT_TTC_L_PRELEVEMENT().setEditable(editable);
...
//				ctrlUnChampsSWT(getFocusCourantSWT());
				if(taLPrelevement.ligneEstVide()){
						actAnnuler();
						annule=true;
					}else			{
						ctrlUnChampsSWT(getFocusCourantSWT());
...
//								e.doit=false;
			...
			public TaLPrelevementDAO getDao() {
				return dao;
			}
			...
			public boolean changementPageValide(){
				if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
					...
					if(taLPrelevement!=null && selectedLignePrelevement!=null && ((IHMLPrelevement) selectedLignePrelevement.getValue())!=null) {
						mapperUIToModel.map((IHMLPrelevement) selectedLignePrelevement.getValue(),taLPrelevement);
					}
				}
				return true;
			}

			public TaPrelevement getMasterEntity() {
				return masterEntity;
			}

			public void setMasterEntity(TaPrelevement masterEntity) {
