...
import fr.legrain.prelevement.divers.IHMLignesTVAPrelevement;
import fr.legrain.prelevement.divers.ModelLignesTVAPrelevement;
import fr.legrain.prelevement.divers.ModelTotaux;
...
public class PaEditorTotauxPrelevementController extends JPABaseControllerSWTStandard implements 
ISelectionProvider,   RetourEcranListener,
SWTModificationDocumentListener,ChangeModeListener {

	static Logger logger = Logger.getLogger(PaEditorTotauxPrelevementController.class.getName());
	private PaTotauxPrelevement vue = null;
		
	public List<Control> listeComposantEntete = null;
	protected Map<Control, String> mapComposantChampsCommentaire = null;

	private IHMTotauxPrelevement ihmTotauxPrelevement;
	private IHMTotauxPrelevement ihmOldTotauxPrelevement;
	private Realm realm;
	private DataBindingContext dbc;
	private String dataTVA[][] = {};
	private ModelTotaux modelTotaux = new ModelTotaux();
	private ModelLignesTVAPrelevement modelLignesTVA;
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private Object selectedTotauxPrelevement = new IHMTotauxPrelevement();
	private String[] listeChamp;
	private Class classModel = IHMTotauxPrelevement.class;

	private MapChangeListener changeListener = new MapChangeListener();

	private TaPrelevement masterEntity = null;
	private TaPrelevementDAO masterDAO = null;
	private TaPrelevementDAO daoLocal = null;

	
	private LgrDozerMapper<IHMTotauxPrelevement,TaPrelevement> mapperUIToModel = new LgrDozerMapper<IHMTotauxPrelevement,TaPrelevement>();
	private LgrDozerMapper<TaPrelevement,IHMTotauxPrelevement> mapperModelToUI = new LgrDozerMapper<TaPrelevement,IHMTotauxPrelevement>();
	
	public PaEditorTotauxPrelevementController(){}
	
	public PaEditorTotauxPrelevementController(PaTotauxPrelevement vue/*, WorkbenchPart part*/) {
		this(vue,null);
	}
	
	public PaEditorTotauxPrelevementController(PaTotauxPrelevement vue/*, WorkbenchPart part*/,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		try {
			setVue(vue);
			daoLocal = new TaPrelevementDAO(getEm());
	...
	private void initController() {
		try {
			...
			bind();
			try{
				((IHMTotauxPrelevement)selectedTotauxPrelevement).setImprimer(prelevementPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.IMPRIMER_AUTO));
			}catch (Exception e) {	
			}
			//#AFAIRE
//			initEtatBouton();
	...
	public void bind() {
		try {
			...
			LgrDozerMapper<TaPrelevement,IHMTotauxPrelevement> mapper = new LgrDozerMapper<TaPrelevement,IHMTotauxPrelevement>();
			if(ihmTotauxPrelevement==null) ihmTotauxPrelevement = new IHMTotauxPrelevement();
			if(masterEntity!=null)
				mapper.map(masterEntity,ihmTotauxPrelevement);
			
			if(modelTotaux.getListeObjet().isEmpty()) {
				modelTotaux.getListeObjet().add(ihmTotauxPrelevement);
			}

			selectedTotauxPrelevement = ihmTotauxPrelevement;
			dbc = new DataBindingContext(realm);

			dbc.getValidationStatusMap().addMapChangeListener(changeListener);
			setDbcStandard(dbc);

			bindingFormSimple(dbc, realm, selectedTotauxPrelevement,classModel);
			bindingFormSimple(masterDAO, mapComposantChampsCommentaire, dbc, realm, selectedTotauxPrelevement, classModel);
	...
	public void bindTVA() {
		try {
			modelLignesTVA = new ModelLignesTVAPrelevement();
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());
...
//					viewerContent.getKnownElements(), IHMLignesTVAProforma.class, listeChamp);
//
//			tableViewer.setLabelProvider(new ObservableMapLabelProvider(attributeMaps));
//			writableList = new WritableList(realm, modelLignesTVA.remplirListe(masterEntity), IHMLignesTVAProforma.class);
//			tableViewer.setInput(writableList);
			
			LgrViewerSupport.bind(tableViewer, 
					new WritableList(modelLignesTVA.remplirListe(masterEntity), IHMLignesTVAPrelevement.class),
...
//			actInserer();
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfMT_HT_CALC(), new InfosVerifSaisie(new TaPrelevement(),Const.C_MT_HT_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_CALC(), new InfosVerifSaisie(new TaPrelevement(),Const.C_MT_TTC_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TVA_CALC(), new InfosVerifSaisie(new TaPrelevement(),Const.C_MT_TVA_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfREGLE_DOCUMENT(), new InfosVerifSaisie(new TaPrelevement(),Const.C_REGLE_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_HT_PRELEVEMENT(), new InfosVerifSaisie(new TaPrelevement(),Const.C_TX_REM_HT_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_TTC_PRELEVEMENT(), new InfosVerifSaisie(new TaPrelevement(),Const.C_TX_REM_TTC_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
	...
	protected void initMapComposantChamps() {
		...
		mapComposantChamps.put(vue.getTfTX_REM_HT_PRELEVEMENT(), Const.C_TX_REM_HT_DOCUMENT);
		...
		mapComposantChamps.put(vue.getTfTX_REM_TTC_PRELEVEMENT(), Const.C_TX_REM_TTC_DOCUMENT);
		...
		listeComposantFocusable.add(vue.getTfTX_REM_HT_PRELEVEMENT());
		listeComposantFocusable.add(vue.getTfMT_HT_CALC());
		listeComposantFocusable.add(vue.getTfMT_TVA_CALC());
		listeComposantFocusable.add(vue.getTfTX_REM_TTC_PRELEVEMENT());
		...
		vue.getTfTX_REM_HT_PRELEVEMENT().addVerifyListener(queDesChiffresPositifs);
		vue.getTfTX_REM_TTC_PRELEVEMENT().addVerifyListener(queDesChiffresPositifs);
...
//	}
	...
	public PaEditorTotauxPrelevementController getThis() {
		return this;
	}
	...
	protected boolean aideDisponible() {
		...
		switch ((PaEditorTotauxPrelevementController.this.masterDAO.getModeObjet().getMode())) {
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
	...
	public IStatus validateUI() throws Exception {
		if ((masterDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
			...
			mapperUIToModel.map(((IHMTotauxPrelevement)selectedTotauxPrelevement),masterEntity);
		}
		return null;
	}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		...
		try {
			...
			if(s.getSeverity()!=IStatus.ERROR) {
				mapperModelToUI.map(masterEntity, ((IHMTotauxPrelevement)selectedTotauxPrelevement));
			}
...
//		sortieChamps();
	...
	public IHMTotauxPrelevement getIhmOldEnteteDevis() {
		return ihmOldTotauxPrelevement;
	}

	public void setIhmOldTotauxPrelevement(IHMTotauxPrelevement ihmOldTotauxDevis) {
		this.ihmOldTotauxPrelevement = ihmOldTotauxDevis;
	}

	public void setIhmOldTotauxDevis() {
		if (selectedTotauxPrelevement!= null)
			this.ihmOldTotauxPrelevement = IHMTotauxPrelevement.copy((IHMTotauxPrelevement) selectedTotauxPrelevement);
		else {
			if(tableViewer.selectionGrille(0)){
				setSelectedTotauxPrelevement(selectedTotauxPrelevement);
				this.ihmOldTotauxPrelevement = IHMTotauxPrelevement.copy((IHMTotauxPrelevement) selectedTotauxPrelevement);
				tableViewer.setSelection(new StructuredSelection(
						(IHMTotauxPrelevement) selectedTotauxPrelevement), true);
			}}
	}

	public void setVue(PaTotauxPrelevement vue) {
		super.setVue(vue);
		this.vue = vue;
	...
	protected void initMapComposantDecoratedField() {
		...
		mapComposantDecoratedField.put(vue.getTfTX_REM_HT_PRELEVEMENT(), vue.getFieldTX_REM_HT_PRELEVEMENT());
		mapComposantDecoratedField.put(vue.getTfTX_REM_TTC_PRELEVEMENT(), vue.getFieldTX_REM_TTC_PRELEVEMENT());
	...
	protected void actRefresh() throws Exception {
		if (masterDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION) != 0) {
			if (masterEntity!=null && selectedTotauxPrelevement!=null && (IHMTotauxPrelevement) selectedTotauxPrelevement!=null) {
				mapperModelToUI.map(masterEntity, (IHMTotauxPrelevement) selectedTotauxPrelevement);
			}
		}
...
//	ISelectionProvider
	...
	public void modificationDocument(SWTModificationDocumentEvent evt) {
		...
		writableList = new WritableList(realm, modelLignesTVA.remplirListe(masterEntity), IHMLignesTVAPrelevement.class);
		tableViewer.setInput(writableList);
		...
		try {
			((IHMTotauxPrelevement)selectedTotauxPrelevement).setMtHtCalc(LibCalcul.arrondi(montantHT));
			((IHMTotauxPrelevement)selectedTotauxPrelevement).setMtTtcCalc(LibCalcul.arrondi(montantTTC));
			((IHMTotauxPrelevement)selectedTotauxPrelevement).setMtTvaCalc(LibCalcul.arrondi(montantTVA));
			((IHMTotauxPrelevement)selectedTotauxPrelevement).setNetHtCalc(LibCalcul.arrondi(NetHT));
			((IHMTotauxPrelevement)selectedTotauxPrelevement).setNetTtcCalc(LibCalcul.arrondi(NetTTC));
			((IHMTotauxPrelevement)selectedTotauxPrelevement).setNetTvaCalc(LibCalcul.arrondi(NetTVA));

			if(((IHMTotauxPrelevement)selectedTotauxPrelevement).getTxRemTtcDocument()==null)
				((IHMTotauxPrelevement)selectedTotauxPrelevement).setTxRemTtcDocument(new BigDecimal(0));

			if(((IHMTotauxPrelevement)selectedTotauxPrelevement).getTxRemHtDocument()==null)
				((IHMTotauxPrelevement)selectedTotauxPrelevement).setTxRemHtDocument(new BigDecimal(0));
	...
	protected void initFocusSWT(IBQuLgr ibTaTable, Map<ModeObjet.EnumModeObjet,Control> focus) {
		setFocusCourantSWT(vue.getTfTX_REM_HT_PRELEVEMENT());
...
public void setTaPrelevement(TaPrelevement masterEntity) {
	this.masterEntity = masterEntity;
	this.masterEntity.addModificationDocumentListener(this);
}

public Object getSelectedTotauxPrelevement() {
	return selectedTotauxPrelevement;
}

public void setSelectedTotauxPrelevement(Object selectedTotauxDevis) {
	this.selectedTotauxPrelevement = selectedTotauxDevis;
...
private void initVue() {
	addExpandBarItem(vue.getExpandBar(), new PaCommentaire(vue
			...
			prelevementPlugin.getImageDescriptor("/icons/creditcards.png")
					.createImage());
...
public void actCreateDoc() throws Exception {
	...
	param.setTypeSrc(TypeDoc.TYPE_PROFORMA);
