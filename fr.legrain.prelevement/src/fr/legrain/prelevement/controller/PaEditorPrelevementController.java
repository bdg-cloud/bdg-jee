...
import fr.legrain.documents.dao.TaInfosPrelevementDAO;
...
import fr.legrain.prelevement.preferences.PreferenceConstants;
import fr.legrain.prelevement.preferences.PreferenceInitializer;
...
public class PaEditorPrelevementController extends JPABaseControllerSWTStandard
		implements   RetourEcranListener,
		SWTModificationDocumentListener, ChangeModeListener {

	static Logger logger = Logger.getLogger(PaEditorPrelevementController.class.getName());
	private PaEditorPrelevement vue = null;
...
//	private List<TaPrelevement> listeDocument = null;
	private List<TaTiers>listeTiers = null;
	
	private TaPrelevementDAO dao = null;//new TaPrelevementDAO();

	private Object ecranAppelant = null;
	private IHMEntetePrelevement ihmOldEntetePrelevement;
	private Realm realm;
	...
	private ModelGeneralObjet<IHMEntetePrelevement,TaPrelevementDAO,TaPrelevement> modelEnteteProforma /*= new ModelGeneralObjet<IHMEntetePrelevement,TaPrelevementDAO,TaPrelevement>(dao,classModel)*/;
	...
	private TaInfosPrelevementDAO daoInfosPrelevement = null;//new TaInfosPrelevementDAO();
	...
	private PaLignePrelevementController controllerLigne = null;
	private PaEditorTotauxPrelevementController controllerTotaux = null;
	...
	private LgrDozerMapper<TaInfosPrelevement,IHMInfosCPaiement> mapperModelToUIInfosDocVersCPaiement = new LgrDozerMapper<TaInfosPrelevement,IHMInfosCPaiement>();
	private LgrDozerMapper<TaInfosPrelevement,IHMAdresseInfosFacturation> mapperModelToUIInfosDocVersAdresseFact = new LgrDozerMapper<TaInfosPrelevement,IHMAdresseInfosFacturation>();
	private LgrDozerMapper<TaInfosPrelevement,IHMAdresseInfosLivraison> mapperModelToUIInfosDocVersAdresseLiv = new LgrDozerMapper<TaInfosPrelevement,IHMAdresseInfosLivraison>();
	...
	private LgrDozerMapper<IHMEntetePrelevement,TaPrelevement> mapperUIToModel = new LgrDozerMapper<IHMEntetePrelevement,TaPrelevement>();
	private LgrDozerMapper<TaPrelevement,IHMEntetePrelevement> mapperModelToUI = new LgrDozerMapper<TaPrelevement,IHMEntetePrelevement>();
	
	private TaPrelevement taDocument = null;
	private TaInfosPrelevement taInfosDocument = null;
	...
	public PaEditorPrelevementController() {
	}
	
	public PaEditorPrelevementController(PaEditorPrelevement vue) {
		this(vue,null);
	}

	public PaEditorPrelevementController(PaEditorPrelevement vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		dao = new TaPrelevementDAO(getEm());
		daoInfosPrelevement = new TaInfosPrelevementDAO(getEm());
...
//						ctrlUnChampsSWT(((PaEditorProforma) getVue()).getTfLIBELLE_DEVIS());
						validateUIField(Const.C_LIBELLE_DOCUMENT,((PaEditorPrelevement) getVue()).getTfLIBELLE_DOCUMENT());
					} catch (Exception e1) {
						logger.debug("", e1);
					}
				}
	...
	public void bind(PaEditorPrelevement PaEditorDevisSWT) {
		try {
			modelEnteteProforma = new ModelGeneralObjet<IHMEntetePrelevement,TaPrelevementDAO,TaPrelevement>(dao,classModel);
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());

//			modelEnteteFacture.remplirListe();
			if (modelEnteteProforma.getListeObjet().isEmpty()) {
				modelEnteteProforma.getListeObjet().add(new IHMEntetePrelevement());
			}
			selectedPrelevement = modelEnteteProforma.getListeObjet().getFirst();
			...
			bindingFormSimple(dbc, realm, selectedPrelevement, classModel);
			bindAdresse();
			bindAdresseLiv();
			bindCPaiement();
...
//	}
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfCODE_DOCUMENT(), new InfosVerifSaisie(new TaPrelevement(),Const.C_CODE_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfCODE_TIERS(), new InfosVerifSaisie(new TaTiers(),Const.C_CODE_TIERS,null));
		mapInfosVerifSaisie.put(vue.getTfLIBELLE_DOCUMENT(), new InfosVerifSaisie(new TaPrelevement(),Const.C_LIBELLE_DOCUMENT,null));
		
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse1(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_ADRESSE1_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse2(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_ADRESSE2_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse3(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_ADRESSE3_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl())
				.getTfCodePostal(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_CODEPOSTAL_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfVille(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_VILLE_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfPays(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_PAYS_INFOS_DOCUMENT,null));

		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse1(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_ADRESSE1_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse2(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_ADRESSE2_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse3(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_ADRESSE3_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfCodePostal(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_CODEPOSTAL_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfVille(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_VILLE_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfPays(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_PAYS_LIV_INFOS_DOCUMENT,null));
				
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfCODE_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_CODE_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfLIB_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_LIB_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfREPORT_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_REPORT_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfFIN_MOIS_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosPrelevement(),Const.C_FIN_MOIS_C_PAIEMENT,null));
...
//		mapActions.put(((PaIdentiteTiers) findExpandIem(vue.getExpandBar(), paIdentiteTiers).getControl()).getBtnAfficher(),C_COMMAND_DOCUMENT_AFFICHER_TIERS_ID);
	...
	public PaEditorPrelevementController getThis() {
		return this;
	}
	...
	public boolean onClose() throws ExceptLgr {
		...
		try{
			if (!vue.isDisposed() && !vue.getShell().isDisposed()) {
				...
				case C_MO_INSERTION:
				case C_MO_EDITION:
					boolean demande=!((IHMEntetePrelevement) selectedPrelevement).ProformaEstVide(taDocument.dateDansExercice(new Date()));
						...
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_TIERS())){
							...
							taDocument.setTaTiers(u);
							 String nomTiers=taDocument.getTaTiers().getNomTiers();
								((IHMEntetePrelevement)selectedPrelevement).setLibelleDocument("Proforma N°"+taDocument.getCodeDocument()+" - "+nomTiers);
								
								if(vue.getCbTTC().isEnabled()){
									taDocument.setTtc(u.getTtcTiers());
									((IHMEntetePrelevement)selectedPrelevement).setTtc(LibConversion.intToBoolean(taDocument.getTtc()));
								}
								
							changementTiers(true);
						}	
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_DOCUMENT())){
							TaPrelevement u = null;
							TaPrelevementDAO t = new TaPrelevementDAO(getEm());
							u = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
							t = null;
							taDocument=u;
	...
	public void calculDateEcheance(Boolean appliquer) {
		if(((IHMEntetePrelevement)selectedPrelevement).getIdDocument()==0 || appliquer) {
			...
			((IHMEntetePrelevement)selectedPrelevement).setDateEchDocument(taDocument.calculDateEcheance(report, finDeMois));
		}
	}
	...
	private boolean changementTiers(boolean refresh) {
		...
		try {						
			...
			if(((IHMEntetePrelevement)selectedPrelevement).getIdDocument()!=0 && 
				...
				if(!idTiersDocumentOriginal.equals(((IHMEntetePrelevement)selectedPrelevement).getIdTiers())){
	...
	public void initialisationDesInfosComplementaires(Boolean recharger,String typeARecharger){
		 ...
		 * - le reste des adresses du tiers
		 */
		if(((IHMEntetePrelevement)selectedPrelevement).getCodeDocument()!=null)
			taInfosDocument = daoInfosPrelevement.findByCodeProforma(((IHMEntetePrelevement)selectedPrelevement).getCodeDocument());
		else
	...
	public void initialisationDesCPaiement(Boolean recharger) {
		...
		if (taDocument != null) {
			if (taDocument.getCodeDocument()!=null&& taDocument.getCodeDocument() != "")
				taInfosDocument = daoInfosPrelevement
			...
			if (taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_PRELEVEMENT) != null
		...
		if (!vue.isDisposed()) {
			try {
				if(dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION)==0) {
					if(prelevementPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.FIXE)==null
							||prelevementPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.FIXE)==""){
...
//					dao.getEntityManager().clear();
					taDocument = new TaPrelevement(true);
					mapperModelToUI.map(taDocument, (IHMEntetePrelevement) selectedPrelevement);
					((IHMEntetePrelevement) selectedPrelevement).setCodeDocument(dao.genereCode());
					validateUIField(Const.C_CODE_DOCUMENT,((IHMEntetePrelevement) selectedPrelevement).getCodeDocument());//permet de verrouiller le code genere
					((IHMEntetePrelevement) selectedPrelevement).setCommentaire(prelevementPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.COMMENTAIRE));

					validateUIField(Const.C_CODE_DOCUMENT,((IHMEntetePrelevement) selectedPrelevement).getCodeDocument());
					taDocument.setCodeDocument(((IHMEntetePrelevement) selectedPrelevement).getCodeDocument());
					changementTiers(true);
					...
					if (LibDate.compareTo(date, taInfoEntreprise.getDatedebInfoEntreprise()) == -1) {
						((IHMEntetePrelevement) selectedPrelevement).setDateDocument(taInfoEntreprise.getDatedebInfoEntreprise());
						((IHMEntetePrelevement) selectedPrelevement).setDateEchDocument(taInfoEntreprise.getDatedebInfoEntreprise());
						((IHMEntetePrelevement) selectedPrelevement).setDateLivDocument(taInfoEntreprise.getDatedebInfoEntreprise());
					} else
						// si date supérieur à dateFin dossier
						if (LibDate.compareTo(date, taInfoEntreprise.getDatefinInfoEntreprise()) == 1) {
							((IHMEntetePrelevement) selectedPrelevement).setDateDocument(taInfoEntreprise.getDatefinInfoEntreprise());
							((IHMEntetePrelevement) selectedPrelevement).setDateEchDocument(taInfoEntreprise.getDatefinInfoEntreprise());
							((IHMEntetePrelevement) selectedPrelevement).setDateLivDocument(taInfoEntreprise.getDatefinInfoEntreprise());
						} else {
							((IHMEntetePrelevement) selectedPrelevement).setDateDocument(new Date());
							((IHMEntetePrelevement) selectedPrelevement).setDateEchDocument(new Date());
							((IHMEntetePrelevement) selectedPrelevement).setDateLivDocument(new Date());
						}
					((IHMEntetePrelevement) selectedPrelevement).setCodeTiers("");
					((IHMEntetePrelevement) selectedPrelevement).setIdAdresse(0);
					((IHMEntetePrelevement) selectedPrelevement).setIdAdresseLiv(0);
					...
					if(typePaiementDefaut!=null){
						 ...
						 */
						if(taTPaiement!=null) {
							((IHMEntetePrelevement) selectedPrelevement).setCodeTPaiement(typePaiementDefaut);
							taDocument.setTaTPaiement(taTPaiement);
						}
					}					
					mapperUIToModel.map(((IHMEntetePrelevement) selectedPrelevement), taDocument);
...
//				setIhmOldEnteteDevis();
//				taDocument = dao.findById(((IHMEntetePrelevement) selectedProforma).getIdDocument());
	...
	public Boolean setIhmOldEnteteDocumentRefresh(){
		try {			
			if (selectedPrelevement != null){
				this.ihmOldEntetePrelevement = IHMEntetePrelevement.copy((IHMEntetePrelevement) selectedPrelevement);
				if (taDocument!=null){
					taDocument =recherchePrelevement(taDocument.getCodeDocument(), false, true);
	...
	protected void actSupprimer() throws Exception {
		...
		try {
			...
			if (MessageDialog.openConfirm(vue.getShell(),
				...
				TaPrelevement u = dao.findById(((IHMEntetePrelevement) selectedPrelevement).getIdDocument());
				dao.supprimer(u);
...
//					listeDocument.remove(taDocument);
	...
	protected void actAnnuler(boolean annulationForcee,boolean insertion,boolean messageAffiche) throws Exception {
		...
		if (!vue.isDisposed() && !vue.getShell().isDisposed()) {
			...
			boolean message = messageAffiche && !((IHMEntetePrelevement) selectedPrelevement).ProformaEstVide(taDocument.dateDansExercice(new Date()));
					...
					case C_MO_EDITION:
						...
						case C_MO_EDITION:
							...
							if (repondu) {
								...
								dao.annulerCodeGenere(((IHMEntetePrelevement) selectedPrelevement).getCodeDocument(),Const.C_CODE_DOCUMENT);
...
//		Collection<TaPrelevement> collectionTaPrelevement = dao.selectAll();
		Collection<TaPrelevement> collectionTaPrelevement = new LinkedList<TaPrelevement>();
//		taDocument.calculeTvaEtTotaux();
		collectionTaPrelevement.add(taDocument);
		
		ConstEdition constEdition = new ConstEdition(getEm()); 
		constEdition.setFlagEditionMultiple(true);
		for (TaCompteBanque compte : taDocument.getTaTiers().getTaCompteBanques()) {
			if(compte.getTaTBanque()!=null && compte.getTaTBanque().getCodeTBanque().equals("")){
				compte.getNomBanque();
				compte.getCodeBanque();
				compte.getCodeGuichet();
				compte.getCompte();
				compte.getCleRib();
			}
		}
...
//		impressionProforma.setCollection(collectionTaPrelevement);
		
		Bundle bundleCourant = prelevementPlugin.getDefault().getBundle();
...
//				TaPrelevement.class.getSimpleName());
		/** 01/03/2010 modifier les editions (zhaolin) **/
		...
		IPreferenceStore preferenceStore = prelevementPlugin.getDefault().getPreferenceStore();
		baseImpressionEdition.impressionEdition(preferenceStore,simpleNameClass,/*true,*/null,namePlugin,
												ConstEdition.FICHE_FILE_REPORT_PRELEVEMENT,
...
//	}
	...
	protected boolean aideDisponible() {
		...
		switch ((PaEditorPrelevementController.this.dao.getModeObjet().getMode())) {
		case C_MO_CONSULTATION:
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				case C_MO_CONSULTATION:
					if (getFocusCourantSWT().equals(vue.getTfCODE_DOCUMENT())) {
						...
						paramAfficheAideRecherche.setControllerAppelant(PaEditorPrelevementController.this);
...
//						l.add(TaPrelevement.getTaCPaiement());
						if(taDocument.getTaTiers()!=null)
							l.add(taDocument.getTaTiers().getTaCPaiement());
						TaTCPaiementDAO taTCPaiementDAO = new TaTCPaiementDAO(getEm());
						if(taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_PRELEVEMENT)!=null
								&& taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_PRELEVEMENT).getTaCPaiement()!=null)
								l.add(taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_PRELEVEMENT).getTaCPaiement());
			...
			try {
				...
				if(sourceDeclencheCommandeController==null) {
					...
					if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
						...
						try{
							taDocument.setLegrain(false);
							for (TaLPrelevement ligne : taDocument.getLignes()) {
...
//							retourPage=ChangementDePageEvent.DEBUT;
	...
	public IHMEntetePrelevement getIhmOldEntetePrelevement() {
		return ihmOldEntetePrelevement;
	}

	public void setIhmOldEnteteDevis(IHMEntetePrelevement ihmOldEnteteDevis) {
		this.ihmOldEntetePrelevement = ihmOldEnteteDevis;
	}

	public void setIhmOldEnteteDevis() {
		if (selectedPrelevement != null)
			this.ihmOldEntetePrelevement = IHMEntetePrelevement
					.copy((IHMEntetePrelevement) selectedPrelevement);
		else {
		}
	}

	public void setVue(PaEditorPrelevement vue) {
		super.setVue(vue);
		this.vue = vue;
	}
...
//			}
	...
	protected void actRefresh() throws Exception {
		try{
			vue.setCursor(Display.getCurrent().getSystemCursor(SWT.CURSOR_WAIT));
		if (dao.dataSetEnModif()) {
			if (taDocument!=null && selectedPrelevement!=null && (IHMEntetePrelevement) selectedPrelevement!=null) {
				mapperModelToUI.map(taDocument, (IHMEntetePrelevement) selectedPrelevement);
			}
		}
		listeDocument=null;
...
//				codeDevisContentProposal.setContentProposalProvider(contentProposalProviderDocument());
	...
	public PaLignePrelevementController getControllerLigne() {
		return controllerLigne;
	}

	public void setControllerLigne(PaLignePrelevementController controllerLigne) {
	...
	private Boolean remonterDocument(String codeDocument) throws Exception {
		try {
			...
			if(taDocument!=null) {
				factureVide = ((IHMEntetePrelevement) selectedPrelevement).ProformaEstVide(taDocument.dateDansExercice(new Date()));
			}
			TaPrelevement documentRecherche = recherchePrelevement(codeDocument, true, factureVide);
...
//				.isVerrouille();
//		String codeTmp = ((IHMEntetePrelevement) selectedProforma)
//				.getCodeDevis();
//		try {
...
//							Const.C_CODE_DEVIS);
//				}
//				((IHMEntetePrelevement) selectedProforma)
...
//
//				((IHMEntetePrelevement) selectedProforma)
//						.setIHMEntetePrelevement(((SWTEnteteDevis) swtDevis
//								.getEntete()));
//
//				// récupérer code validé dans facture, dataset et interface
...
//			TaPrelevementDAO TaPrelevementDAO = new TaPrelevementDAO(getEm());
//			TaInfoEntrepriseDAO taInfoEntrepriseDAO = new TaInfoEntrepriseDAO(getEm());
//			TaInfoEntreprise taInfoEntreprise = taInfoEntrepriseDAO.findInstance();
//
//			//		List<TaBonliv> l = taBonlivDAO.selectAll();
//			//		if(listeDocument==null)
//			listeDocument =((IDocumentTiersDAO)TaPrelevementDAO).rechercheDocument(taInfoEntreprise.getDatedebInfoEntreprise(),
//					taInfoEntreprise.getDatefinInfoEntreprise());
//			TaPrelevementDAO = null;
//
//		}
////		List<TaPrelevement> l = TaPrelevementDAO.selectAll();
...
//			+ " \r\n Echéance = " + LibDate.dateToStringAA(((TaPrelevement)taDocument).getDateEchDocument())
//			+ " \r\n Exportée = " 
//			+LibConversion.booleanToStringFrancais(LibConversion.intToBoolean(((TaPrelevement)taDocument).getExport()));
...
//	}
	public String[][] initAdapterDocument() {
		...
		if(affichageCtrlEspace){
		if(listeDocument==null){
			TaPrelevementDAO taDocDAO = new TaPrelevementDAO(getEm());
			TaInfoEntrepriseDAO taInfoEntrepriseDAO = new TaInfoEntrepriseDAO(getEm());
			TaInfoEntreprise taInfoEntreprise = taInfoEntrepriseDAO.findInstance();
...
//		//#AFAIRE
//		taTiersDAO.setPreferences(prelevementPlugin.getDefault().getPreferenceStore().getString(fr.legrain.prelevement.preferences.PreferenceConstants.TYPE_TIERS_DOCUMENT));
//		if(listeTiers==null)
...
//	}
	...
	public Date dateDansPeriode(Date newValue,String champ) throws Exception{
		if(champ.equals(Const.C_DATE_DOCUMENT)){
			...
			if(newValue!=null){
				if((((IHMEntetePrelevement)selectedPrelevement).getDateEchDocument()!=null && 
						((IHMEntetePrelevement)selectedPrelevement).getDateEchDocument().before(newValue))||
						((IHMEntetePrelevement)selectedPrelevement).getDateEchDocument()==null ){
					taDocument.setDateEchDocument(newValue);
					((IHMEntetePrelevement)selectedPrelevement).setDateEchDocument(newValue);
					}
//				if((((IHMEntetePrelevement)selectedProforma).getDateLivDocument()!=null && 
//						((IHMEntetePrelevement)selectedProforma).getDateLivDocument().before(newValue))||
//						((IHMEntetePrelevement)selectedProforma).getDateLivDocument()==null ){
//					taDocument.setDateLivDocument(newValue);
//					((IHMEntetePrelevement)selectedProforma).setDateLivDocument(newValue);
//					}
			}
		}
...
//	}
	...
	public PaEditorTotauxPrelevementController getControllerTotaux() {
		return controllerTotaux;
	}

	public void setControllerTotaux(PaEditorTotauxPrelevementController controllerTotaux) {
	...
	protected void actSelection() throws Exception {
		try{
			...
			paramAfficheSelectionVisualisation.setModule("prelevement");
	...
	public TaPrelevement recherchePrelevement(String codeDevis,boolean annule,boolean proformaVide) throws Exception {
		TaPrelevement fact = null;
	...
	public void setTaPrelevement(TaPrelevement TaPrelevement) {
		this.taDocument = TaPrelevement;
	}

	public TaPrelevementDAO getDao() {
		return dao;
	}

	public void setDao(TaPrelevementDAO dao) {
		this.dao = dao;
	}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "PRELEVEMENT";
		try {
			IStatus s = null;
			...
			if(nomChamp.equals(Const.C_CODE_TIERS)) {
				...
				if(s.getSeverity()!=IStatus.ERROR && change){
					...
					taDocument.setTaTiers(f);
					 String nomTiers=taDocument.getTaTiers().getNomTiers();
						((IHMEntetePrelevement)selectedPrelevement).setLibelleDocument("Prélèvement N°"+taDocument.getCodeDocument()+" - "+nomTiers);
					if(vue.getCbTTC().isEnabled()){
...
//				}
				...
				TaPrelevement u = new TaPrelevement(true);
				PropertyUtils.setSimpleProperty(u, nomChamp, value);
				if(((IHMEntetePrelevement) selectedPrelevement).getIdDocument()!=null) {
					u.setIdDocument(((IHMEntetePrelevement) selectedPrelevement).getIdDocument());
				}
...
//					s = new Status(Status.OK,gestComBdPlugin.PLUGIN_ID,"OK");
			...
			if(s.getSeverity()!=Status.ERROR){
				if(nomChamp.equals(Const.C_DATE_DOCUMENT)){
					...
					((IHMEntetePrelevement)selectedPrelevement).setDateDocument((Date)value);
					PropertyUtils.setSimpleProperty(taDocument, Const.C_DATE_DOCUMENT, value);
					if(changement){
						((IHMEntetePrelevement)selectedPrelevement).setDateEchDocument(taDocument.getDateDocument());
						PropertyUtils.setSimpleProperty(taDocument, Const.C_DATE_ECH_DOCUMENT, taDocument.getDateDocument());
						((IHMEntetePrelevement)selectedPrelevement).setDateLivDocument(taDocument.getDateDocument());
				...
				if(nomChamp.equals(Const.C_DATE_LIV_DOCUMENT)) {
					((IHMEntetePrelevement)selectedPrelevement).setDateLivDocument((Date)value);
