...
//					vue.getLaMessage().setText(e.getMessage());
	...
	protected void actImprimer() throws Exception {
		...
		if(vue.getCbEditMouvDate().getSelection()){
			List<TaEtatStock> listeEtatStockFinal = etatStocks.calculEtatStocksMouvements(criteres,new TaReportStockDAO(getEm()).recupDerniereDateReportStock());
			ConstEdition constEdition = new ConstEdition();
			Impression impression = new Impression(listeEtatStockFinal,constEdition);
			impression.imprimer(ConstEdition.FICHE_FILE_REPORT_STOCKS_MOUVEMENTS_DATE,"&DateFin="+LibDate.dateToString(criteres.getDateStock()));
			}
		if(vue.getCbEditMouvType().getSelection()){
			List<TaEtatStock> listeEtatStockFinal = etatStocks.calculEtatStocksMouvements(criteres,new TaReportStockDAO(getEm()).recupDerniereDateReportStock());
			ConstEdition constEdition = new ConstEdition();
			Impression impression = new Impression(listeEtatStockFinal,constEdition);
			impression.imprimer(ConstEdition.FICHE_FILE_REPORT_STOCKS_MOUVEMENTS_TYPE,"&DateFin="+LibDate.dateToString(criteres.getDateStock()));
		}
	}
