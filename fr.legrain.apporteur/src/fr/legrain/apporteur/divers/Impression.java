...
public class Impression implements IImpressionDocumentTiers {
	...
	public void imprimerSelection(int idDoc,final String codeDoc,boolean preview,String fileEditionDefaut,
		...
		reportParam.put(ConstEditionApporteur.PARAM_CHOIX_DEST,choix);
...
//		constEdition.prepartionEditionBirt();
		...
		try {
			...
			if(!affiche){
				...
				ConstEdition.CONTENT_COMMENTS = ConstEditionApporteur.COMMENTAIRE_EDITION_DEFAUT;
				...
				if(ConstEdition.pathRepertoireEditionsSpecifiquesClient()!=null)
					FloderEditionSpecifiquesClient = ConstEdition.pathRepertoireEditionsSpecifiquesClient()+
													ConstEdition.SEPARATOR+namePlugin+ConstEdition.SEPARATOR+
													nomEntity;
...
//				constEdition.addValues(idSWTTiers,oneIDSWTTiers);
				...
				constEdition.setPARAM_ID_TABLE(ConstEditionApporteur.PARAM_ID_APPORTEUR);
				constEdition.paramId = ConstEditionApporteur.PARAM_ID_APPORTEUR;
				
				

				//#EDITION JPA A FAIRE
				//			constEdition.openDialogChoixEdition_Defaut(ecranDialogReport, FloderFileEditions, 
				//					reportFileLocation, namePlugin,nomOnglet,dialogShell,reportFile,null,affiche,reportParam,fileEditionSpecifiques,false);
				//		
//				constEdition.openDialogChoixEdition_Defaut(ecranDialogReport, fileEditionSpecifiquesClient, 
//						reportFileLocationDefaut, namePlugin, nomOnglet, dialogShell, null, affiche, reportParam, fileEditionSpecifiques, buttonEditionDefaut);
...
//		affichageChoixEdition.setAppContextEdition(new EditionAppContext().getExtensionMap());
		...
		if(!affiche_en_liste){
			...
			reportFileLocation = ConstEdition.pathFichierEditionsSpecifiques(reportFileLocation,apporteurPlugin.PLUGIN_ID);
			...
			if(ConstEdition.pathRepertoireEditionsSpecifiquesClient()!=null)	
				FloderEdition = ConstEdition.pathRepertoireEditionsSpecifiquesClient()+"//"+namePlugin+"/"+nomEntity;
...
//			constEdition.setCollection(collection);
			...
			String reportEditionsSpecifiques = ConstEdition.pathRepertoireEditionsSpecifiques()+"/"+namePlugin+"/"+nomEntity;
			
			constEdition.setCollection(collection);
			constEdition.setFlagEditionMultiple(false);
			
//			constEdition.openDialogChoixEdition_Defaut(dialogReport, FloderFileEditions, 
//					reportFileLocation, namePlugin,nomOnglet,dialogShell,reportFile,affiche,
//					reportParam,new File(reportEditionsSpecifiques), true);			
			/**
			 * with Expandbar
			 */
			constEdition.fillMapNameExpandbar(true);
