...
import fr.legrain.proforma.editor.ProformaMultiPageEditor;
...
public class DocEcheanceController extends AbstractControllerMini {
	...
	protected List<TaProforma> listeDoc;
	private List<ModelObject> modele = null;
	protected  FormPageControllerProforma masterController = null;
	...
	/* Constructeur */
	public DocEcheanceController(FormPageControllerProforma masterContoller, PaFormPage vue, EntityManager em) {
		this.vue = vue;
	...
	public void initialiseModelIHM(int nbResult) {
		...
		if(vue.getCompositeSectionParam().getTfCodeTiers().getText().equals("")) {
			listeDoc = taProformaDAO.rechercheDocumentEtat(
					LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb()),
					datefin,
					masterController.paramControllerMiniProforma.getMapEtatCodeLibelle().get(masterController.paramControllerMiniProforma.getCodeEtat()));
		} else {
			listeDoc = taProformaDAO.rechercheDocumentEtat(
					LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb()),
					datefin,
		...
		for (int i = 0; i < listeDoc.size() /*&& i < nbResult*/; i ++){
			...
			for (TaLProforma l : object.getLignes()) {
		...
		if(vue.getCompositeSectionParam().getCbEtat().getText().equals(fr.legrain.document.etat.devis.controllers.ParamControllerMini.etatEnCoursLibelle)) {
			libelleEcran = "Liste des proformas en cours arrivant à échéance avant le "
					+ LibDate.dateToString(datefin) + " ("+listDocumentSelection.size()+" documents)";
		} else {
			libelleEcran =
					"Liste des proformas arrivant à échéance entre le "
		...
		if(!evenementInitialise) {
			...
			vue.getCompositeSectionTableauGauche().getTable().addMouseListener(new MouseAdapter(){
				...
				public void mouseDoubleClick(MouseEvent e) {
					...
					String idEditor = ProformaMultiPageEditor.ID_EDITOR;
	...
	public void changeEtat(String v) {
		...
		String codeNouvelEtat = masterController.paramControllerMiniProforma.getMapEtatCodeLibelle().get(v);

		taProformaDAO = new TaProformaDAO(masterController.getMasterDAOEM());
		TaEtatDAO taEtatDAO = new TaEtatDAO(masterController.getMasterDAOEM());

		TaProforma taProforma =  taProformaDAO.findByCode(doc.code);
		TaEtat taEtat = null;

		if(codeNouvelEtat!=null)
			taEtat = taEtatDAO.findByCode(codeNouvelEtat);

		EntityTransaction transaction = taProformaDAO.getEntityManager().getTransaction();
		try {			
			taProforma.setTaEtat(taEtat);
			taProformaDAO.begin(transaction);
			
			taProforma.setLegrain(false); //Pour éviter setTaArticle sur les lignes suite au merge et donc la réinitialisation des informations de la ligne y compris pour les quantités.setLegrain(false); //Pour éviter setTaArticle sur les lignes suite au merge et donc la réinitialisation des informations de la ligne y compris pour les quantités

			//if ((taDevisDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
			taProformaDAO.enregistrerMerge(taProforma);
			//} 
			taProformaDAO.commit(transaction);
			transaction = null;
	...
	public final class ExampleEditingSupport extends ObservableValueEditingSupport {
		...
		private ExampleEditingSupport(ColumnViewer viewer) {
			...
			cellEditor.setInput(new WritableList(masterController.paramControllerMiniProforma.getMapEtatCodeLibelle().keySet(),String.class));
	...
	public void print() {
		...
		EditionTable ed = new EditionTable(infosDocuments, taProformaDAO.getEntityManager(), tableViewer, 
