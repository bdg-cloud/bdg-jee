...
public class WizardDialogModelLettre extends WizardDialog {
	...
	protected void finishPressed() {
		...
		String defautSavePubli = new File(parametrePublicPostage.getPathDefautFileSavePublipostage())
								.getPath();
		String savePubli = new File(parametrePublicPostage.getPathFileSavePublicPostage()).getPath();
		
		fonctionGeneral.connectOpenOffice(ValueSeparateur,defautSavePubli,savePubli);
...
//		}
		...
		if(isSavePublicpostage || isSavePublicpostagePagePlugin){
			...
			if(!newNameParamPublicPostage.equals(nameParamPublicPostage)){
				String pathFileParamEtiquette = Const.PATH_FOLDER_MODEL_LETTRE_DOSSIER+"/"+Const.FOLDER_PARAM_LETTRE_OO+
