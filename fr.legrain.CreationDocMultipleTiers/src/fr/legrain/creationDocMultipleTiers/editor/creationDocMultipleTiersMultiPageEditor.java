package fr.legrain.creationDocMultipleTiers.editor;
...
import fr.legrain.creationDocMultipleTiers.controllers.PaCriterecreationDocMultipleTiersController;
 ...
 * </ul>
 */
public class creationDocMultipleTiersMultiPageEditor extends AbstractLgrMultiPageEditor implements 
...
//public class MultiPageEditor extends SharedHeaderFormEditor {
	...
	public static final String ID_EDITOR = "fr.legrain.creationDocMultipleTiers.multi"; //$NON-NLS-1$
	static Logger logger = Logger.getLogger(creationDocMultipleTiersMultiPageEditor.class.getName());
	...
	protected EditorCriterecreationDocMultiple editorCritereCreationMultipleTiers = null;

	/**
	 * Creates a multi-page editor example.
	 */
	public creationDocMultipleTiersMultiPageEditor() {
		super();
	...
	protected void createPageQueEditeur() {
		try {
			...
			String labelPageRelance = "Association des documents";
			String labelPageRemonterRelance = "Liste des documents existants";
			...
			editorCritereCreationMultipleTiers = new EditorCriterecreationDocMultiple(this,em);
			
			listePageEditeur.add(editorCritereCreationMultipleTiers);
			
			
			int index = addPage(editorCritereCreationMultipleTiers, getEditorInput());
			setPageText(index, labelPageRelance);
			setPageImage(index, Activator.getImageDescriptor(iconPageRelance).createImage());
			
			//editorCritereCreationMultipleTiers.getController().addAnnuleToutListener(this);
			
			//liaison entre la selection du controller principal et le multipage editor
			editorCritereCreationMultipleTiers.getController().addChangementDeSelectionListener(this);
//			((SWTPaReglementMultipleController)editorCritereRelance.getController()).addChangementMasterEntityListener(this);

			
			mapEditorController.put(editorCritereCreationMultipleTiers, editorCritereCreationMultipleTiers.getController());


			
			editorCritereCreationMultipleTiers.getController().addChangementDePageListener(this);
			
			
			
			((PaCriterecreationDocMultipleTiersController)editorCritereCreationMultipleTiers.getController()).
			getPaSelectionMultiTiersControlleur().addDeclencheCommandeControllerListener(
					((PaCriterecreationDocMultipleTiersController)editorCritereCreationMultipleTiers.getController()).
					getPaSelectionLigneRelanceControlleur());
		((PaCriterecreationDocMultipleTiersController)editorCritereCreationMultipleTiers.getController()).
			getPaSelectionLigneRelanceControlleur().addDeclencheCommandeControllerListener(editorCritereCreationMultipleTiers.getController());
		
			

			
			editeurCourant = editorCritereCreationMultipleTiers;
			ParamAffichecreationDocMultiple paramAfficheRelanceFacture = new ParamAffichecreationDocMultiple();
			editorCritereCreationMultipleTiers.getController().configPanel(paramAfficheRelanceFacture);
