package fr.legrain.liaisondocument.editors;
...
import fr.legrain.liaisondocument.controllers.PaCriterecreationLiaisonDocController;
 ...
 * </ul>
 */
public class creationLiaisonDocMultiPageEditor extends AbstractLgrMultiPageEditor implements 
...
//public class MultiPageEditor extends SharedHeaderFormEditor {
	...
	public static final String ID_EDITOR = "fr.legrain.creationLiaisonDoc.multi"; //$NON-NLS-1$
	static Logger logger = Logger.getLogger(creationLiaisonDocMultiPageEditor.class.getName());
	...
	protected EditorCriterecreationLiaisonDoc editorCritereRelance = null;

	/**
	 * Creates a multi-page editor example.
	 */
	public creationLiaisonDocMultiPageEditor() {
		super();
	...
	protected void createPageQueEditeur() {
		try {
			...
			String iconPageRelance= "/icons/legrain.gif";
			String iconPageRemonterRelance= "/icons/legrain.gif";

			em = new TaTiersDAO().getEntityManager();
			editorCritereRelance = new EditorCriterecreationLiaisonDoc(this,em);
			
			listePageEditeur.add(editorCritereRelance);
			
			
			int index = addPage(editorCritereRelance, getEditorInput());
			...
			editorCritereRelance.getController().addChangementDeSelectionListener(this);
//			((SWTPaReglementMultipleController)editorCritereRelance.getController()).addChangementMasterEntityListener(this);

			
			mapEditorController.put(editorCritereRelance, editorCritereRelance.getController());


			
			editorCritereRelance.getController().addChangementDePageListener(this);
...
//				getAffectationReglementController().setMasterDAO(masterDAO);
			...
			((PaCriterecreationLiaisonDocController)editorCritereRelance.getController()).getPaSelectionLigneRelanceControlleur().
				addDeclencheCommandeControllerListener(editorCritereRelance.getController());
			((PaCriterecreationLiaisonDocController)editorCritereRelance.getController()).getPaSelectionLigneRelanceControlleur().
				getPaVisuDocumentControlleur().addDeclencheCommandeControllerListener(editorCritereRelance.getController());
			((PaCriterecreationLiaisonDocController)editorCritereRelance.getController()).getPaSelectionLigneDocControlleur2().
			addDeclencheCommandeControllerListener(editorCritereRelance.getController());
		((PaCriterecreationLiaisonDocController)editorCritereRelance.getController()).getPaSelectionLigneDocControlleur2().
			getPaVisuDocumentControlleur().addDeclencheCommandeControllerListener(editorCritereRelance.getController());			
			editorCritereRelance.getController().addAnnuleToutListener(this);

			
			editeurCourant = editorCritereRelance;
			ParamAffichecreationLiaisonDoc paramAfficheRelanceFacture = new ParamAffichecreationLiaisonDoc();
			editorCritereRelance.getController().configPanel(paramAfficheRelanceFacture);
