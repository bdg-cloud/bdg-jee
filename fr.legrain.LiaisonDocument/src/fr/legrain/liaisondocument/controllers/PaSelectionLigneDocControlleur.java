...
import fr.legrain.liaisondocument.divers.ViewerSupportLocal;
...
public class PaSelectionLigneDocControlleur extends
	...
	private AbstractLgrDAO dao = null;
	private TaBonliv taBonLiv=null;
	private IObservableValue selectedDocument;
	private ModelDocumentLiaisonDoc masterModelDocument = null;
	private PaVisuLiaisonDocumentControlleur paVisuDocumentControlleur=null;
//	private LgrDatabindingUtil lgrDatabindingUtil = new LgrDatabindingUtil();
	...
	private String selectedTypeAdvers = "";
...
//		mapInfosVerifSaisie.put(paFormulaireReglement.getTfTYPE_PAIEMENT(), new InfosVerifSaisie(new TaTPaiement(),Const.C_CODE_T_PAIEMENT,null));
	...
	private void initVue() {
		...
		DefaultFrameGrilleSansBouton visuLiaison = new DefaultFrameGrilleSansBouton(vue.getExpandBarVisualisation(), SWT.PUSH);
		paVisuDocumentControlleur = new PaVisuLiaisonDocumentControlleur(visuLiaison,getEm());
		
//		PaSelectionLigneDoc visuLiaison = new PaSelectionLigneDoc(vue.getExpandBarVisualisation(), SWT.PUSH);
//		paVisuDocumentControlleur = new PaVisuLiaisonDocumentControlleur(visuLiaison,getEm());
		addExpandBarItem(vue.getExpandBarVisualisation(), visuLiaison,
		"Documents liés au document en cours ", LibrairiesEcranPlugin.ir.get(LibrairiesEcranPlugin.IMAGE_GEN_DOCUMENT)
		, SWT.DEFAULT, 200);
		vue.getExpandBarVisualisation().getItem(0).setExpanded(true);	
	}
...
////			initEtatBouton(true);
	...
	protected void actRefresh() throws Exception {
		...
		try{	
			SelectionTTC=null;
			String codeActuel="";
			if((IHMEnteteDocument)selectedDocument.getValue()!=null)
				codeActuel =((IHMEnteteDocument)selectedDocument.getValue()).getCodeDocument();				
	...
	public void bind(PaSelectionLigneDoc paSelectionLigneRelance){
		try {
			...
			String[] titreColonnes =new String[] {"Code document","Date","Code tiers","Nom ","Net TTC","Ttc","Accepté"};
			getTableViewerStandard().createTableCol(vue.getGrille(),titreColonnes,
					new String[] {"100","80","100","150","70","30","30"},1);
			String[] listeChamp = new String[] {"codeDocument","dateDocument","codeTiers","nomTiers","netTtcCalc","ttc","accepte"};
...
//						IDocumentTiers document =rechercheEntity(((IHMEnteteDocument)event.getElement()).getCodeDocument());
//						if(masterModelDocument.rechercheRDocument(document,selectedTypeAdvers)){
//							message="Il existe déjà une liaison entre ces documents";
//						}
						if(message==""){
							...
							MessageDialog.openWarning(vue.getShell(),"Liaison existante",message);
						}
					} catch (Exception e) {
						logger.error("", e);
					}		
...
//		affectationReglementControllerMini.bind();
		vue.getGrille().addMouseListener(
				new MouseAdapter() {
					public void mouseDoubleClick(MouseEvent e) {
						...
						.get(((IHMEnteteDocument) selectedDocument
										.getValue())
										.getTypeDocument());
	...
	try {
		...
		paVisuDocumentControlleur.setMasterEntity(null);
		if(selectedDocument!=null && selectedDocument.getValue()!=null){
			//for (Object doc : masterModelDocument.getListeEntity()) {
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_ACOMPTE){
					dao=new TaAcompteDAO(getEm());
				}
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_APPORTEUR){
					dao=new TaApporteurDAO(getEm());

				}
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_AVOIR){
					dao=new TaAvoirDAO(getEm());

				}
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_FACTURE){
					dao=new TaFactureDAO(getEm());

				}
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_DEVIS){
					dao=new TaDevisDAO(getEm());

				}
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_BON_COMMANDE){
					dao=new TaBoncdeDAO(getEm());

				}
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_BON_LIVRAISON){
					dao=new TaBonlivDAO(getEm());

				}
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_PRELEVEMENT){
					dao=new TaPrelevementDAO(getEm());

				}
				if(((IHMEnteteDocument)selectedDocument.getValue()).getTypeDocument()==TypeDoc.TYPE_PROFORMA){
					dao=new TaProformaDAO(getEm());

				}
				paVisuDocumentControlleur.setMasterEntity(
						(IDocumentTiers)dao.findById(((IHMEnteteDocument)selectedDocument.getValue()).getIdDocument()));
			}
			paVisuDocumentControlleur.actRefresh();
//			vue.getExpandBarVisualisation().getItem(0).setExpanded(
//					paVisuDocumentControlleur.getModelDocument().getListeObjet().size()>0);
	...
	public ResultAffiche configPanel(ParamAffiche param) {
		if (param!=null){
			...
			if(param instanceof ParamAffichecreationLiaisonDoc){
				setEnregistreDirect(((ParamAffichecreationLiaisonDoc)param).getEnregistreDirect());
			}
		}	
...
//	}
	...
	public IDocumentTiers rechercheEntity( Object value) {
		boolean trouve = false;
		int i = 0;
		Collection<IDocumentTiers> model=masterModelDocument.getListeEntity();
		for (IDocumentTiers doc : model) {
			if(doc.getCodeDocument().equals(value))
				return doc;
		}
			return null;
	}
	...
	public ModelDocumentLiaisonDoc getMasterModelDocument() {
		return masterModelDocument;
	}

	public void setMasterModelDocument(
			ModelDocumentLiaisonDoc masterModelDocument) {
		this.masterModelDocument = masterModelDocument;
	}

	public PaVisuLiaisonDocumentControlleur getPaVisuDocumentControlleur() {
		return paVisuDocumentControlleur;
	}

	public void setPaVisuDocumentControlleur(
			PaVisuLiaisonDocumentControlleur paVisuDocumentControlleur) {
		this.paVisuDocumentControlleur = paVisuDocumentControlleur;
	}
	...
	public String getSelectedTypeAdvers() {
		return selectedTypeAdvers;
	}

	public void setSelectedTypeAdvers(String selectedTypeAdvers) {
		this.selectedTypeAdvers = selectedTypeAdvers;
	}
