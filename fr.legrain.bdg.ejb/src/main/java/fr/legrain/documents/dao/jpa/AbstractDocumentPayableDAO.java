...
public abstract class  AbstractDocumentPayableDAO	<Entity,InfosEntity,LigneEntity> extends AbstractDocumentDAO<Entity,InfosEntity,LigneEntity> {


	static Logger logger = Logger.getLogger(AbstractDocumentPayableDAO.class);
...
//						+" group by doc.id_Document, doc.code_Document, doc.date_Document, doc.libelle_Document, tiers.code_Tiers, infos.nom_Tiers,infos.prenom_Tiers, infos.nom_Entreprise,doc.date_Ech_Document,doc.date_Export,coalesce(doc.net_Ht_Calc,0),coalesce(doc.net_Tva_Calc,0),coalesce(doc.net_Ttc_Calc,0),s1.affectation,s2.affectationAvoir"  
		...
		SUM_RESTE_A_REGLER_JOUR_LIGTH_PERIODE_AVEC_ETAT = 
				...
				+ "sum(res.mtTtcCalc)as mtTtcCalc,sum(res.reglementComplet)as reglementComplet,sum(res.resteAReglercomplet)as resteAReglercomplet from ( "
						...
						+ documentChiffreAffaireDTO.construitSumCoallesceAsAlias("doc", "net_Tva_Calc")+","
						+ documentChiffreAffaireDTO.construitSumCoallesceAsAlias("doc", "net_Ttc_Calc")+","
						+"sum(s1.affectation+s2.affectationAvoir)as reglementComplet, "
						+" (coalesce(sum(doc.net_Ttc_Calc),0)-sum(s1.affectation+s2.affectationAvoir))as resteAReglercomplet "
						+" from "+nameEntitySQL+" doc  "
						+" join ta_tiers t on t.id_tiers=doc.id_tiers "  
						+ " join ta_R_Etat re on re.id_r_etat=doc.id_r_etat "
						+ " join ta_Etat et on et.id_etat=re.id_etat " 
						...
						+"  and et.identifiant like :etat " 
//						+" and coalesce((doc.net_Ttc_Calc),0)>(" +getRequeteSommeAffecte("doc") +")"  
						+" group by et.identifiant,relancer,extract(day from doc.date_Document),extract(month from doc.date_Document),extract(year from doc.date_Document)"  
						+" order by et.identifiant,relancer,extract(day from doc.date_Document),extract(month from doc.date_Document),extract(year from doc.date_Document))"
		...
		SUM_RESTE_A_REGLER_MOIS_LIGTH_PERIODE_AVEC_ETAT = 
				...
				+ "sum(res.mtTtcCalc)as mtTtcCalc,sum(res.reglementComplet)as reglementComplet,sum(res.resteAReglercomplet)as resteAReglercomplet from ( "
						...
						+"  and et.identifiant like :etat "  
//						+" and coalesce((doc.net_Ttc_Calc),0)>("+getRequeteSommeAffecte("doc") +")"
						+" group by et.identifiant,relancer,extract(month from doc.date_Document),extract(year from doc.date_Document)"  
						+" order by et.identifiant,relancer,extract(month from doc.date_Document),extract(year from doc.date_Document))"
		...
		SUM_RESTE_A_REGLER_ANNEE_LIGTH_PERIODE_AVEC_ETAT = 
				...
				+ "sum(res.mtTtcCalc)as mtTtcCalc,sum(res.reglementComplet)as reglementComplet,sum(res.resteAReglercomplet)as resteAReglercomplet from ( "
						...
						+"  and et.identifiant like :etat "   
//						+" and coalesce((doc.net_Ttc_Calc),0)>("+getRequeteSommeAffecte("doc") +")"
						+" group by et.identifiant,relancer,extract(year from doc.date_Document)" 
						+" order by et.identifiant,relancer,extract(year from doc.date_Document))"
						+ "as res group by res.identifiant,res.relancer,res.jour,res.mois,res.annee";
		...
		SUM_RESTE_A_REGLER_TOTAL_LIGTH_PERIODE_AVEC_ETAT = 
				...
				+ "sum(res.mtTtcCalc)as mtTtcCalc,sum(res.reglementComplet)as reglementComplet,sum(res.resteAReglercomplet)as resteAReglercomplet from ( "
						...
						+"  and et.identifiant like :etat "  
//						+" and coalesce((doc.net_Ttc_Calc),0)>("+getRequeteSommeAffecte("doc") +")"
						+" group by  et.identifiant,relancer)"
						+ "as res group by  res.identifiant,res.relancer,res.jour,res.mois,res.annee";	
	...
	public AbstractDocumentPayableDAO(){
		entity=newDocument();
...
//					List<DocumentDTO> l= query.unwrap(org.hibernate.Query.class).setResultTransformer(Transformers.aliasToBean(DocumentDTO.class)).list();
		...
		public List<DocumentChiffreAffaireDTO> listeChiffreAffaireTotalAvecEtatDTO(Date dateDebut, Date dateFin,String codeTiers, String etat) {
			try {
				...
				Date dateJour = LibDate.dateDuJour();
				String requete=SUM_RESTE_A_REGLER_TOTAL_LIGTH_PERIODE_AVEC_ETAT.replace(":dateRef", "cast('"+LibDate.dateToString(dateRef)+"' as date)").replace(":dateJour", "cast('"+LibDate.dateToString(dateJour)+"' as date)");
				query = entityManager.createNativeQuery(requete);
				query.setParameter("dateDebut", dateDebut, TemporalType.DATE);
				query.setParameter("dateFin", dateFin, TemporalType.DATE);
				query.setParameter("codeTiers",codeTiers);
				...
				addScalarDocumentChiffreAffaireAvecEtatDTO(query);
				List<DocumentChiffreAffaireDTO> l= query.getResultList();
//				List<DocumentChiffreAffaireDTO> l= query.unwrap(org.hibernate.Query.class).setResultTransformer(Transformers.aliasToBean(DocumentChiffreAffaireDTO.class)).list();
//				List<DocumentChiffreAffaireDTO> l = query.getResultList();;
				if (l == null || l.isEmpty() || l.get(0).getMtHtCalc() == null) {
		...
		public List<DocumentChiffreAffaireDTO> listeChiffreAffaireTotalJmaAvecEtatDTO(Date dateDebut, Date dateFin, int precision,String codeTiers,String etat) {
			...
			try {
				switch (precision) {
				case 0:
					requete=SUM_RESTE_A_REGLER_ANNEE_LIGTH_PERIODE_AVEC_ETAT.replace(":dateRef", "cast('"+LibDate.dateToString(dateRef)+"' as date)").replace(":dateJour", "cast('"+LibDate.dateToString(dateJour)+"' as date)");
					break;

				case 1:
					requete=SUM_RESTE_A_REGLER_MOIS_LIGTH_PERIODE_AVEC_ETAT.replace(":dateRef", "cast('"+LibDate.dateToString(dateRef)+"' as date)").replace(":dateJour", "cast('"+LibDate.dateToString(dateJour)+"' as date)");
					break;
				case 2:
					requete=SUM_RESTE_A_REGLER_JOUR_LIGTH_PERIODE_AVEC_ETAT.replace(":dateRef", "cast('"+LibDate.dateToString(dateRef)+"' as date)").replace(":dateJour", "cast('"+LibDate.dateToString(dateJour)+"' as date)");
					break;
...
//				List<DocumentChiffreAffaireDTO> l = query.unwrap(org.hibernate.Query.class).setResultTransformer(Transformers.aliasToBean(DocumentChiffreAffaireDTO.class)).list();
		...
		public List<DocumentDTO> findAllDTOPeriodeAvecEtat(Date dateDebut, Date dateFin,String codeTiers, String etat) {
			try {
				...
				Date dateRef = LibDate.incrementDate(LibDate.dateDuJour(), 15, 0, 0);
				Date dateJour = LibDate.dateDuJour();
				String requete = FIND_ALL_LIGHT_PERIODE_AVEC_ETAT.replace(":dateRef", "cast('"+LibDate.dateToString(dateRef)+"' as date)").replace(":dateJour", "cast('"+LibDate.dateToString(dateJour)+"' as date)");
				Query query = entityManager.createNativeQuery(requete);
				query.setParameter("dateDebut", dateDebut, TemporalType.DATE);
				query.setParameter("dateFin", dateFin, TemporalType.DATE);
				query.setParameter("codeTiers",codeTiers);
				query.setParameter("etat",etat);
				addScalarDocumentAvecEtatDTO(query);
//				List<DocumentDTO> l = query.getResultList();
				List<DocumentDTO> l = query.unwrap(org.hibernate.Query.class).setResultTransformer(Transformers.aliasToBean(DocumentDTO.class)).list();
//				List<DocumentDTO> l = query.getResultList();
				logger.debug("get successful");
		...
		public Query addScalarDocumentAvecEtatDTO(Query query ) {
			query.unwrap(SQLQuery.class)
			.addScalar(DocumentDTO.f_identifiantEtat, StringType.INSTANCE)
			.addScalar(DocumentDTO.f_relancer, BooleanType.INSTANCE)
	        .addScalar(DocumentDTO.f_idDocument, IntegerType.INSTANCE)
	        .addScalar(DocumentDTO.f_codeDocument, StringType.INSTANCE)
	        .addScalar(DocumentDTO.f_dateDocument, DateType.INSTANCE)
	        .addScalar(DocumentDTO.f_libelleDocument, StringType.INSTANCE)
