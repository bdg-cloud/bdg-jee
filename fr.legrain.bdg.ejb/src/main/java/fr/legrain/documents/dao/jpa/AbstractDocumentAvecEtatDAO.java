...
public abstract class AbstractDocumentAvecEtatDAO<Entity,InfosEntity,LigneEntity> extends AbstractDocumentDAO<Entity,InfosEntity,LigneEntity>   {


	static Logger logger = Logger.getLogger(AbstractDocumentAvecEtatDAO.class);
...
//						//unité2 et quantité2 à rajouter
	...
	public AbstractDocumentAvecEtatDAO(){
		entity=newDocument();
...
//				List<DocumentChiffreAffaireDTO> result =null;
				...
				try {
					...
					requete = "SELECT "
						+" count(distinct doc),et.identifiant  as identifiant,case  when "+getRequeteARelancerJPQL().replaceFirst("and", "")+" then true else false   END as relancer,"
						+" FROM "+nameEntity+" doc " 
						+" join doc.lignes ldoc "
						+" join ldoc.taArticle art "
...
//						+" and not exists ("+getRequeteDocumentTransforme("doc")+")";
					...
					query.setParameter("codeTiers", codeTiers);
					query.setParameter("codeArticle", codeArticle);
					query.setParameter("identifiant", etat);
