...
import fr.legrain.bdg.model.mapping.mapper.TaBoncdeMapper;
...
public class TaBoncdeService extends AbstractDocumentService<TaBoncde, TaBoncdeDTO> implements ITaBoncdeServiceRemote {

	static Logger logger = Logger.getLogger(TaBoncdeService.class);

	@Inject private IBoncdeDAO dao;
	 ...
	 * Default constructor. 
	 */
	public TaBoncdeService() {
		super(TaBoncde.class,TaBoncdeDTO.class);
...
//			    MT_TVA Numeric(15,2),
			...
			for (Object ligne : doc.getLignes()) {
				if(((TaLBoncde)ligne).getTaTLigne().getCodeTLigne().equals(SWTDocument.C_CODE_T_LIGNE_H)) {
					if(((TaLBoncde)ligne).getMtHtLApresRemiseGlobaleDocument()!=null)
						doc.setNetHtCalc(doc.getNetHtCalc().add(((TaLBoncde)ligne).getMtHtLApresRemiseGlobaleDocument()));
					if(((TaLBoncde)ligne).getMtTtcLApresRemiseGlobaleDocument()!=null)
						doc.setMtTtcCalc(doc.getMtTtcCalc().add(((TaLBoncde)ligne).getMtTtcLApresRemiseGlobaleDocument()));
					if(((TaLBoncde)ligne).getMtHtLDocument()!=null)
						doc.setMtHtCalc(doc.getMtHtCalc().add(((TaLBoncde)ligne).getMtHtLDocument()));
					if(((TaLBoncde)ligne).getMtTtcLDocument()!=null)
						doc.setMtTtcAvantRemiseGlobaleCalc(doc.getMtTtcAvantRemiseGlobaleCalc().add(((TaLBoncde)ligne).getMtTtcLDocument()));
				}
...
//			  netapayer = :mtnetttc - :regle;
	...
	public void dispatcherTva(TaBoncde doc) {
		...
		for (TaLBoncde ligne : doc.getLignes()) {
						...
						if(((TaLBoncde)ligne).getCodeTvaLDocument()!=null &&
							...
							if(doc.getTxRemHtDocument()!=null && doc.getTxRemHtDocument().signum()>0) {
								if  (lignepasse>= ligneTva.getNbLigneDocument()) {
									...
									((TaLBoncde)ligne).setMtHtLApresRemiseGlobaleDocument(((TaLBoncde)ligne).getMtHtLDocument().subtract(((TaLBoncde)ligne).getMtHtLDocument().multiply(doc.getTxRemHtDocument()).divide(new BigDecimal(100),MathContext.DECIMAL128).setScale(2,BigDecimal.ROUND_HALF_UP)));
									((TaLBoncde)ligne).setMtTtcLApresRemiseGlobaleDocument(((TaLBoncde)ligne).getMtTtcLDocument().subtract(((TaLBoncde)ligne).getMtTtcLDocument().multiply(doc.getTxRemHtDocument()).divide(new BigDecimal(100),MathContext.DECIMAL128).setScale(2,BigDecimal.ROUND_HALF_UP)));
								}
...
//				mtHtTotal = LibCalcul.arrondi(mtHtTotal.subtract(    mtHtTotal.multiply( (txRemHtDocument.divide(new BigDecimal(100))))     ) ) ;
	...
	public void persist(TaBoncde transientInstance) throws CreateException {
		persist(transientInstance, null);
	}

	//@TransactionAttribute(TransactionAttributeType.REQUIRED)
	@WebMethod(operationName = "persistValidationContext")
	public void persist(TaBoncde transientInstance, String validationContext) throws CreateException {
	...
	@Override
	public void remove(TaBoncde persistentInstance, boolean recharger) throws RemoveException {
	...
	public void remove(TaBoncde persistentInstance) throws RemoveException {
	...
	public TaBoncde merge(TaBoncde detachedInstance, String validationContext) {
		validateEntity(detachedInstance, validationContext);
		List<ILigneDocumentTiers> listeLien =null;
		TaBoncde objInitial = detachedInstance;
		try {
		...
		for (TaRReglementLiaison taRReglement : listeReglementTmp) {
			taRReglement.setTaBoncde(detachedInstance);
...
//				reglement.addRReglementLiaison(ligne);
		...
		for (TaRReglementLiaison ligne : detachedInstance.getTaRReglementLiaisons()) {
			if(ligne.getTaReglement()!=null) {
				...
				reglement=daoReglement.merge(ligne.getTaReglement());
				ligne.setTaBoncde(detachedInstance);
				ligne.setTaReglement(reglement);
			}
		}
	...
	public TaBoncde findById(int id) throws FinderException {
		return dao.findById(id);
	}

	public TaBoncde findByCode(String code) throws FinderException {
...
//	@RolesAllowed("admin")
	...
	public List<TaBoncdeDTO> findWithJPQLQueryDTO(String JPQLquery) throws FinderException {
		List<TaBoncde> entityList = dao.findWithJPQLQuery(JPQLquery);
	...
	public TaBoncdeDTO entityToDTO(TaBoncde entity) {
//		TaBoncdeDTO dto = new TaBoncdeDTO();
//		dto.setId(entity.getIdTCivilite());
//		dto.setCodeTCivilite(entity.getCodeTCivilite());
//		return dto;
		TaBoncdeMapper mapper = new TaBoncdeMapper();
		return mapper.mapEntityToDto(entity, null);
	}

	public List<TaBoncdeDTO> listEntityToListDTO(List<TaBoncde> entity) {
		List<TaBoncdeDTO> l = new ArrayList<TaBoncdeDTO>();

		for (TaBoncde taTCivilite : entity) {
...
//	@RolesAllowed("admin")
	public List<TaBoncdeDTO> selectAllDTO() {
		...
		List<TaBoncde> projects = selectAll();
		for(TaBoncde project : projects) {
	...
	public void persistDTO(TaBoncdeDTO dto, String validationContext) throws CreateException {
		try {
			TaBoncdeMapper mapper = new TaBoncdeMapper();
			TaBoncde entity = mapper.mapDtoToEntity(dto,null);
			//dao.persist(entity);
	...
	@Override
	protected TaBoncde refresh(TaBoncde persistentInstance) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public void validateEntity(TaBoncde value, String validationContext) /*throws ExceptLgr*/ {
		try {
	...
	public void validateEntityProperty(TaBoncde value, String propertyName, String validationContext) {
		try {
			if(validationContext==null) validationContext="";
...
//			validator.validateField(dto,propertyName);
	...
	public void validateEntity(TaBoncde value) {
		validateEntity(value,null);
	}

	@Override
	@WebMethod(operationName = "validateEntityProperty")
	public void validateEntityProperty(TaBoncde value, String propertyName) {
	...
	@Override
	public List<TaBoncde> findByCodeTiersAndDate(String codeTiers, Date debut,
			Date fin) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public TaBoncde findByIdDocument(int id) throws FinderException {
		return findById(id);
	}
	...
	public String genereCode(Map<String, String> params) {
		...
		try {
			return gencode.genereCodeJPA(TaBoncde.class.getSimpleName(),params);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void annuleCode(String code) {
		try {
			...
			gencode.annulerCodeGenere(gencode.findByCode(TaBoncde.class.getSimpleName()),code);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void verrouilleCode(String code) {
		try {
			gencode.rentreCodeGenere(gencode.findByCode(TaLBoncde.class.getSimpleName()),code, sessionInfo.getSessionID());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public TaBoncde mergeAndFindById(TaBoncde detachedInstance, String validationContext) {
		TaBoncde br = merge(detachedInstance,validationContext);
	try {
...
/////////Bloc de méthode pour notamment les onglets factures/devis/commandes/proforma dash par article et dash par tiers//////
   ...
   //Rajout YANN pour impression edition defaut choisi par l'utilisateur
	...
	public String generePDF(int idDoc, Map<String,Object> mapParametreEdition, List<String> listResourcesPath, String theme, TaActionEdition action) {
		if(action != null) {
			TaEdition edition = taEditionService.rechercheEditionActionDefaut(null, action, TaBoncde.TYPE_DOC);
	...
	public String generePDF(int idDoc, String modeleEdition, Map<String,Object> mapParametreEdition, List<String> listResourcesPath, String theme) {
		try {
			TaBoncde doc = findById(idDoc);
			SchemaResolver sr = new SchemaResolver();
			String localPath = bdgProperties.osTempDirDossier(sr.resolveCurrentTenantIdentifier())+"/"+bdgProperties.tmpFileName(doc.getCodeDocument(), doc.getTaTiers().getCodeTiers(),"bonCde.pdf");
...
//				doc = mergeAndFindById(doc,ITaBoncdeServiceRemote.validationContext);
//			}
	...
	@Override
	public List<TaBoncde> rechercheDocument(String codeTiers, Date dateExport, Date dateDeb, Date dateFin) {
		// TODO Auto-generated method stub
		return null;
	}
...
//	}
	...
	public TaBoncde findDocByLDoc(ILigneDocumentTiers lDoc) {
		TaBoncde o= (TaBoncde) dao.findDocByLDoc(lDoc);
		 recupSetREtat(o);
		 recupSetHistREtat(o);
		 recupSetLigneALigne(o);
		 recupSetRDocument(o);
	...
	@Override
	public TaBoncde mergeEtat(IDocumentTiers detachedInstance) {
//		à mettre si gestion des états !!!!
		modifEtatLigne(detachedInstance);		
		TaEtat etat=changeEtatDocument(detachedInstance);
		((TaBoncde) detachedInstance).addREtat(etat);
		
		detachedInstance=dao.merge((TaBoncde) detachedInstance);	
		return (TaBoncde) detachedInstance;
	}
	...
	@Override
	public TaBoncde findByCodeFetch(String code) throws FinderException {
		// TODO Auto-generated method stub
		TaBoncde o = (TaBoncde) dao.findByCodeFetch(code);
		if(o!=null) {
		recupSetREtat(o);
		recupSetHistREtat(o);
		recupSetLigneALigne(o);
	...
	@Override
	public TaBoncde findByIDFetch(int id) throws FinderException {
		// TODO Auto-generated method stub
		TaBoncde o = (TaBoncde) dao.findByIdFetch(id);
		if(o!=null) {
		recupSetREtat(o);
		recupSetHistREtat(o);
		recupSetLigneALigne(o);
	...
	@Override
	public List<DocumentChiffreAffaireDTO> listLigneArticlePeriodeBonlivDTOParFamille(Date dateDebut, Date dateFin,
			String codeArticle, Boolean synthese, String orderBy) {
		// TODO Auto-generated method stub
		return dao.listLigneArticlePeriodeBonlivDTOParFamille(dateDebut, dateFin, codeArticle, synthese, orderBy);
	}
