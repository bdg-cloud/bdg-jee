...
public  class GenereDocProformaVersAvoir extends AbstractGenereDocVersAvoir implements IGenereDocProformaVersAvoirServiceRemote{
	...
	public IDocumentTiers copieDocumentSpecifique(IDocumentTiers ds, IDocumentTiers dd
		...
		try {
			...
			if(multiple)((TaAvoir)dd).setTxRemHtDocument(BigDecimal.valueOf(0));else((TaAvoir)dd).setTxRemHtDocument(((TaProforma)ds).getTxRemHtDocument());
			...
			((TaAvoir)dd).setDateExport(((TaProforma)ds).getDateExport());
	...
	public ILigneDocumentTiers copieLigneDocumentSpecifique(ILigneDocumentTiers ls, ILigneDocumentTiers ld) {
		if(documentGereLesCrd) {			
			...
			if(ld.getTaArticle()!=null && ((TaLProforma)ls).getCodeTitreTransport()!=null ) {
				((TaLAvoir)ld).setCodeTitreTransport(((TaLProforma)ls).getCodeTitreTransport());
				((TaLAvoir)ld).setQteTitreTransport(((TaLProforma)ls).getQteTitreTransport());			
			}
			else
