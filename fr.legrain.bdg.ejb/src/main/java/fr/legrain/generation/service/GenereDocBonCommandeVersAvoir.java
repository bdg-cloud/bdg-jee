...
public  class GenereDocBonCommandeVersAvoir extends AbstractGenereDocVersAvoir implements IGenereDocBonCommandeVersAvoirServiceRemote{
...
//			((TaAvoir)dd).setRegleDocument(BigDecimal.valueOf(0));
			...
			if(multiple)((TaAvoir)dd).setTxRemHtDocument(BigDecimal.valueOf(0));else((TaAvoir)dd).setTxRemHtDocument(((TaBoncde)ds).getTxRemHtDocument());
	...
	public ILigneDocumentTiers copieLigneDocumentSpecifique(ILigneDocumentTiers ls, ILigneDocumentTiers ld) {
		if(documentGereLesCrd) {			
			...
			if(ld.getTaArticle()!=null && ((TaLBoncde)ls).getCodeTitreTransport()!=null ) {
				((TaLAvoir)ld).setCodeTitreTransport(((TaLBoncde)ls).getCodeTitreTransport());
				((TaLAvoir)ld).setQteTitreTransport(((TaLBoncde)ls).getQteTitreTransport());			
			}
			else
