...
public  class GenereDocBonLivraisonVersAvoir extends AbstractGenereDocVersAvoir implements IGenereDocBonLivraisonVersAvoirServiceRemote{
...
//			((TaAvoir)dd).setDateEchDocument(((TaBonliv)ds).getDateEchDocument());
			...
			if(multiple)((TaAvoir)dd).setTxRemHtDocument(BigDecimal.valueOf(0));else((TaAvoir)dd).setTxRemHtDocument(((TaBonliv)ds).getTxRemHtDocument());
...
//			((TaAvoir)dd).setExport(((TaBonliv)ds).getExport());
	...
	public ILigneDocumentTiers copieLigneDocumentSpecifique(ILigneDocumentTiers ls, ILigneDocumentTiers ld) {
		if(documentGereLesCrd) {
			...
			if(ld.getTaArticle()!=null && ((TaLBonliv)ls).getCodeTitreTransport()!=null  && documentGereLesCrd) {
				((TaLAvoir)ld).setCodeTitreTransport(((TaLBonliv)ls).getCodeTitreTransport());
				((TaLAvoir)ld).setQteTitreTransport(((TaLBonliv)ls).getQteTitreTransport());			
			}
			else
