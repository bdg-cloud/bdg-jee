...
public  class GenereDocTicketDeCaisseVersFacture extends AbstractGenereDocVersFacture implements IGenereDocTicketDeCaisseVersFactureServiceRemote{
	...
	public IDocumentTiers copieDocumentSpecifique(IDocumentTiers ds, IDocumentTiers dd
		...
		try {
//			((TaFacture)dd).setTaTPaiement(((TaTicketDeCaisse)ds).getTaTPaiement());
			((TaFacture)dd).setDateEchDocument(((TaTicketDeCaisse)ds).getDateEchDocument());
			//((TaFacture)dd).setRegleDocument(BigDecimal.valueOf(0));
			//isa le 4/03/2010 suite à demande philippe
			((TaFacture)dd).setRegleDocument(((TaTicketDeCaisse)ds).getRegleDocument());
			//
			((TaFacture)dd).setRemHtDocument(((TaTicketDeCaisse)ds).getRemHtDocument());
			if(multiple)((TaFacture)dd).setTxRemHtDocument(BigDecimal.valueOf(0));else((TaFacture)dd).setTxRemHtDocument(((TaTicketDeCaisse)ds).getTxRemHtDocument());
			((TaFacture)dd).setRemTtcDocument(((TaTicketDeCaisse)ds).getRemTtcDocument());
			((TaFacture)dd).setTxRemTtcDocument(((TaTicketDeCaisse)ds).getTxRemTtcDocument());
			((TaFacture)dd).setNbEDocument(((TaTicketDeCaisse)ds).getNbEDocument());
			((TaFacture)dd).setTtc(((TaTicketDeCaisse)ds).getTtc());
			((TaFacture)dd).setDateExport(((TaTicketDeCaisse)ds).getDateExport());
			((TaFacture)dd).setCommentaire(((TaTicketDeCaisse)ds).getCommentaire());
				...
				for (ILigneDocumentTiers ligne : ((TaTicketDeCaisse)ds).getLignes()) {
					...
					if(ligne.getGenereLigne()) {
						...
						for (TaLigneALigneSupplementDTO lSupp : ligne.getListeSupplement()) {
							...
							lal.setTaLTicketDeCaisse((TaLTicketDeCaisse) ligne);
							lal.setTaLFacture(temp);
							lal.setIdLigneSrc(ligne.getIdLDocument());
							temp.getTaLigneALignes().add(lal);
							((TaLTicketDeCaisse) ligne).getTaLigneALignes().add(lal);
						...
						if(ligne.getQteGenere()!=null && ligne.getQteGenere().compareTo(BigDecimal.ZERO)!=0) {
							...
							for (TaLigneALigneSupplementDTO o : ligne.getListeLigneAIntegrer()) {
								...
								TaLTicketDeCaisse ligneReel=(TaLTicketDeCaisse) o.getLigne();
								...
								lal2.setTaLTicketDeCaisse((TaLTicketDeCaisse) ligneReel);
								lal2.setTaLFacture(temp);
								lal2.setIdLigneSrc(ligneReel.getIdLDocument());
								temp.getTaLigneALignes().add(lal2);
								((TaLTicketDeCaisse) ligne).getTaLigneALignes().add(lal2);
								temp.addREtatLigneDoc(etatLigne);
							}
							if(temp.getTaArticle()!=null)temp.setTaArticle(taArticleService.findByIdAvecLazy(temp.getTaArticle().getIdArticle(), false));
							((TaFacture)dd).addLigne(temp);
			...
			TaInfosFacture infos = (TaInfosFacture)copieInfosDocument(((TaTicketDeCaisse)ds).getTaInfosDocument(),new TaInfosFacture());
	...
	public ILigneDocumentTiers copieLigneDocumentSpecifique(ILigneDocumentTiers ls, ILigneDocumentTiers ld) {
		...
		if(ld.getTaArticle()!=null && ((TaLTicketDeCaisse)ls).getCodeTitreTransport()!=null ) {
			((TaLFacture)ld).setCodeTitreTransport(((TaLTicketDeCaisse)ls).getCodeTitreTransport());
			((TaLFacture)ld).setQteTitreTransport(((TaLTicketDeCaisse)ls).getQteTitreTransport());			
		}
		else
	...
	public IDocumentTiers genereDocumentSpecifique(IDocumentTiers ds, IDocumentTiers dd,String codeTiers, boolean generationModele,boolean genereCode) {
		...
		try {
			...
			if(documentInit!=null){
				...
				taRDocument.setTaTicketDeCaisse(documentInit);
...
//			((TaFacture)dd).setNetAPayer(((TaFacture)dd).calculResteAReglerComplet());
	...
	public String creationRequeteDejaGenere(IDocumentTiers ds) {
		...
		String jpql = "select x from TaRDocument x where x.taTicketDeCaisse.idDocument="+idDoc+" and x.taFacture is not null";
		return jpql;
	}
