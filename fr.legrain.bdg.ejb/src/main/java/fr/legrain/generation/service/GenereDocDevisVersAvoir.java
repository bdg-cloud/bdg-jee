...
public  class GenereDocDevisVersAvoir extends AbstractGenereDocVersAvoir implements IGenereDocDevisVersAvoirServiceRemote{
	...
	public IDocumentTiers copieDocumentSpecifique(IDocumentTiers ds, IDocumentTiers dd
		...
		try {
			...
			if(multiple)((TaAvoir)dd).setTxRemHtDocument(BigDecimal.valueOf(0));else((TaAvoir)dd).setTxRemHtDocument(((TaDevis)ds).getTxRemHtDocument());
			...
			((TaAvoir)dd).setDateExport(((TaDevis)ds).getDateExport());
	...
	public ILigneDocumentTiers copieLigneDocumentSpecifique(ILigneDocumentTiers ls, ILigneDocumentTiers ld) {
		if(documentGereLesCrd) {			
			...
			if(ld.getTaArticle()!=null && ((TaLDevis)ls).getCodeTitreTransport()!=null ) {
				((TaLAvoir)ld).setCodeTitreTransport(((TaLDevis)ls).getCodeTitreTransport());
				((TaLAvoir)ld).setQteTitreTransport(((TaLDevis)ls).getQteTitreTransport());			
			}
			else
