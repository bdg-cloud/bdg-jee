 ...
 */
public class ReportStockCapsulesDAO implements IReportStockCapsulesDAO  {

	private static final Log logger = LogFactory.getLog(ReportStockCapsulesDAO.class);
	
	@PersistenceContext(unitName = "bdg")
	private EntityManager entityManager;
	
	@Inject private ITitreTransportCapsulesDAO daoTitreTransport;
...
//		    detachedInstance = entityManager.find(TaReportStockCapsules.class, detachedInstance.getIdReportStock());
	...
	public List<TaEtatStockCapsules> selectReportStocks(TaEtatStockCapsules criteres) {
		try {
			...
			JPQLQueryReportStock="select NEW fr.legrain.article.titretransport.model.TaEtatStockCapsules(a.codeTitreTransport,a.libelleTitreTransport,'R', "+
...
//			}
	...
	@Override
	public List<TaReportStockCapsules> findWithNamedQuery(String queryName) {
		try {
	...
	@Override
	public List<TaReportStockCapsules> findWithJPQLQuery(String JPQLquery) {
		try {
	...
	@Override
	public boolean validate(TaReportStockCapsules value) throws Exception {
		BeanValidator<TaReportStockCapsules> validator = new BeanValidator<TaReportStockCapsules>(TaReportStockCapsules.class);
		return validator.validate(value);
	}

	@Override
	public boolean validateField(TaReportStockCapsules value, String propertyName) throws Exception {
		BeanValidator<TaReportStockCapsules> validator = new BeanValidator<TaReportStockCapsules>(TaReportStockCapsules.class);
		return validator.validateField(value,propertyName);
	}

	@Override
	public void detach(TaReportStockCapsules transientInstance) {
		entityManager.detach(transientInstance);
	}

	@Override
	public TaReportStockCapsules findByCode(String code) {
		return null;
	}
	...
	public void recalculReport(List<TaEtatStockCapsules> listeReports, Date dateFin) {
		...
		for (TaEtatStockCapsules taEtatStock : listeReports) {
			TaReportStockCapsules taReportStock = new TaReportStockCapsules();
			taReportStock.setDateDebReportStock(recupDerniereDateReportStock());
			taReportStock.setDateFinReportStock(dateFin);
			TaTitreTransport taTitreTransport = daoTitreTransport.findByCode(taEtatStock.getCodeTitreTransport());
			taReportStock.setTaTitreTransport(taTitreTransport);
//			taReportStock.setUnite1ReportStock(taEtatStock.getUn1Stock());
//			taReportStock.setUnite2ReportStock(taEtatStock.getUn2Stock());
			taReportStock.setQte1ReportStock(taEtatStock.getQteCapsuleStock());
