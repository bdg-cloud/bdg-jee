...
import fr.legrain.bdg.general.service.remote.IGenericCRUDServiceRemoteDTO;
import fr.legrain.bdg.general.service.remote.IGenericCRUDServiceRemoteEntity;
...
//	private int lockType = LOCK_TYPE_JPA;
	...
	private static Map<String,String> mapChampIdTable = new HashMap<String,String>();
...
//	public abstract void validateDTOProperty(DTO dto, String propertyName);
	...
	public void enregistrerPersist(Entity transientInstance) throws Exception{
		enregistrer(transientInstance,TYPE_ENREGISTREMENT_PERSIST,null);
	}

	@Override
	public Entity enregistrerMerge(Entity persistentInstance) throws Exception{
		return enregistrer(persistentInstance,TYPE_ENREGISTREMENT_MERGE,null);
	}
	
	@Override
	public void enregistrerPersistDTO(DTO transientInstance) throws Exception{
		enregistrerDTO(transientInstance,TYPE_ENREGISTREMENT_PERSIST,null);
	}

	@Override
	public void enregistrerMergeDTO(DTO persistentInstance) throws Exception{
		enregistrerDTO(persistentInstance,TYPE_ENREGISTREMENT_MERGE,null);
	}
	
	@Override
	public void enregistrerPersist(Entity transientInstance, String validationContext) throws Exception{
		enregistrer(transientInstance,TYPE_ENREGISTREMENT_PERSIST,validationContext);
	}

	@Override
	public Entity enregistrerMerge(Entity persistentInstance, String validationContext) throws Exception{
		return enregistrer(persistentInstance,TYPE_ENREGISTREMENT_MERGE,validationContext);
	}
	
	@Override
	public void enregistrerPersistDTO(DTO transientInstance, String validationContext) throws Exception{
		enregistrerDTO(transientInstance,TYPE_ENREGISTREMENT_PERSIST,validationContext);
	}

	@Override
	public void enregistrerMergeDTO(DTO persistentInstance, String validationContext) throws Exception{
		enregistrerDTO(persistentInstance,TYPE_ENREGISTREMENT_MERGE,validationContext);
	}
	 ...
	 * @throws Exception
	 */
	private Entity enregistrer(Entity instance,int typeEnregistrement, String validationContext) throws Exception{
		Entity retour = null;
		init(instance);
		...
		if(lockType==LOCK_TYPE_LGR) {
			if(typeEnregistrement==TYPE_ENREGISTREMENT_MERGE)
				retour = merge(instance,validationContext);
			else if(typeEnregistrement==TYPE_ENREGISTREMENT_PERSIST)
				persist(instance,validationContext);
	...
	private void enregistrerDTO(DTO instance,int typeEnregistrement,String validationContext) throws Exception{
		//init(instance);
		
		if(lockType==LOCK_TYPE_LGR) {
			if(typeEnregistrement==TYPE_ENREGISTREMENT_MERGE)
				mergeDTO(instance,validationContext);
			else if(typeEnregistrement==TYPE_ENREGISTREMENT_PERSIST)
				persistDTO(instance,validationContext);
...
//	public abstract void ctrlSaisieSpecifique( ValeurChamps valeurChamps) throws ExceptLgr;
	...
	public int initValeurIdTable(Entity entity) {
		...
		while(!idTrouve && i<p.length) {
			if(p[i].getReadMethod()!=null && p[i].getReadMethod().getAnnotation(Id.class)!=null) {
				idTrouve = true;
			}
			if(!idTrouve) {
				i++;
		...
		if(idTrouve) {
			try {
				...
				if(valeurIdTable==null) {
					//Peut être null si la propriété "id" est de type "Integer" et non de type "int"
					valeurIdTable = 0;
				}
...
//			}
	...
	public boolean recordModifiable(String nomTable,Integer valeurChamp) {
		try{
			...
			boolean res=true;
			//TODO ré-écrire la procédure pour postgreSQL ou faire en java
...
//					res=false;
//			}
//			logger.debug("recordModifiable, nomTable : "+nomTable+" valeurChamp : "+valeurChamp);
			return res;
		}catch(Exception e){
			logger.error("",e);
			return false;
...
//		return selectAll().size();
	...
	protected String initChampId(Entity entity) {
			if(!mapChampIdTable.containsKey(entity.getClass().getCanonicalName())) {
				Class clazz = deproxyClass(entity);
				PropertyDescriptor[] p;
				if(clazz!=null) {
					p = PropertyUtils.getPropertyDescriptors(clazz);
				...
				if(idTrouve) {
					try {
						...
						mapChampIdTable.put(entity.getClass().getCanonicalName(), this.champIdTable);
					...
					mapChampIdTable.put(entity.getClass().getCanonicalName(), null);
					return null;
				}
			} else {
				return mapChampIdTable.get(entity.getClass().getCanonicalName());
			}
		}	
	
	protected String initChampId(Class entityClass) {	
		if(!mapChampIdTable.containsKey(entityClass.getClass().getCanonicalName())) {
			...
			if(idTrouve) {
				try {
					...
					mapChampIdTable.put(entityClass.getClass().getCanonicalName(), this.champIdTable);
					return this.champIdTable;
				} catch (IllegalArgumentException e) {
					logger.error("",e);
				} 
			...
			return mapChampIdTable.get(entityClass.getClass().getCanonicalName());
		}
	}
