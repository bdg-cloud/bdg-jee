...
import fr.legrain.hibernate.multitenant.TenantRequest;

public class TenantSoapHeaderHandler implements SOAPHandler<SOAPMessageContext>{
	
	public static final String TENANT_SOAP_TOKEN_KEY = "tenantSOAP";
	private static String TENANT_SOAP_TOKEN_VALUE = null;
	...
	public boolean handleMessage(SOAPMessageContext context) {
		...
		if(!isRequest){
			...
			try{
				...
				String paramTenant = (macNode == null) ? null : macNode.getValue();
...
//						tenantInfo.setTenantId(paramTenant);
//						// session.putValue("tenantInfo", t);
//						tenantInfoService.setTenantInfo(tenantInfo);
//						System.out.println("******************************** Tenant pour appel WS-SOAP --- : "+tenantInfo.getTenantId());
//					}
				
		
					 if(paramTenant!=null && !paramTenant.equals("")) {
...
//				         TENANT_TOKEN_VALUE = paramTenant;
//				         reg.putResource(TENANT_TOKEN_KEY, paramTenant);
						 
		            	 ThreadLocalContextHolder.cleanupThread();
						 ThreadLocalContextHolder.put(TENANT_SOAP_TOKEN_KEY, paramTenant);
				     }
