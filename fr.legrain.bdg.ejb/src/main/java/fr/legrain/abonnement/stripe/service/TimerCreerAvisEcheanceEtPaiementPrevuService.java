...
@Stateless
public class TimerCreerAvisEcheanceEtPaiementPrevuService {
	/*
...
//    }
	...
	public TimerHandle creerTimer(TaCron cron, Object param) {
		TaAbonnement n =  null;
    	if(param instanceof TaAbonnement) {
    		n = (TaAbonnement) param;
    	}
...
//			schedule.hour("2").minute("0").second("0");
			...
			if(cron!=null) {
				if(cron.getSecond()!=null) {
					schedule.second(cron.getSecond());
				}
				if(cron.getMinute()!=null) {
					schedule.minute(cron.getMinute());
				}
				if(cron.getHour()!=null) {
					schedule.hour(cron.getHour());
				}
				if(cron.getDayOfMonth()!=null) {
					schedule.dayOfMonth(cron.getDayOfMonth());
				}
				if(cron.getDayOfWeek()!=null) {
					schedule.dayOfWeek(cron.getDayOfWeek());
				}
				if(cron.getYear()!=null) {
					schedule.year(cron.getYear());
				}
			}
