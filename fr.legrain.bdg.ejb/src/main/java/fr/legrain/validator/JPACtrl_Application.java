...
public class JPACtrl_Application extends JPACtrlChamp implements ICtrlLgr {
	...
	//@Inject @MarqueX1 private	IListeControle listeControle;
	@Inject private	ListeControle listeControle;
//	@Inject private	IListeControle listeControle;
...
//		setFichierListeTitre(Const.C_FICHIER_LISTE_TITRE_BD);
	...
	public void init() {
		...
		if(listeControle.getListeCtrlMap()== null || listeControle.getListeCtrlMap().isEmpty()) {
			listeControle.setListeCtrlMap(new HashMap<String, TaControle>());
			
			List<TaControle> liste =  controleServiceRemote.selectAll();
			String key = "";
			for (TaControle taControle : liste) {
				if(taControle.getContexte()!=null && !taControle.getContexte().equals("")) {
					key = taControle.getContexte()+"."+taControle.getChamp();
					listeControle.getListeCtrlMap().put(taControle.getChamp() , taControle); ///-----------------------*****-*-*-**-*-*
				} else {
					key = taControle.getChamp();
				}
				listeControle.getListeCtrlMap().put(key , taControle);
			}
		}
		listeCtrlMap = listeControle.getListeCtrlMap();
//		if(listeCtrlMap == null || listeCtrlMap.isEmpty()) {
...
//				break;
			...
			logger.error("JPACtrl_Application (mode serveur = "+modeServeur+") : Champ ou Table inconnu dans la table ta_controle : "+key);
			//throw new ExceptLgr("JPACtrl_Application (mode serveur = "+modeServeur+") : Champ ou Table inconnu dans la table ta_controle : "+key);
