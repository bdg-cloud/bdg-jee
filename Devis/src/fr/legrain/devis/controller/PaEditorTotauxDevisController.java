...
import fr.legrain.devis.divers.LignesTVADevisDTO;
import fr.legrain.devis.divers.ModelLignesTVADevis;
import fr.legrain.devis.divers.ModelTotauxDevis;
import fr.legrain.devis.ecran.PaTotauxDevis;
...
public class PaEditorTotauxDevisController extends EJBBaseControllerSWTStandard implements 
ISelectionProvider,   RetourEcranListener,
SWTModificationDocumentListener,ChangeModeListener {

	static Logger logger = Logger.getLogger(PaEditorTotauxDevisController.class.getName());
	private PaTotauxDevis vue = null;
		
	public List<Control> listeComposantEntete = null;
	protected Map<Control, String> mapComposantChampsCommentaire = null;

	private IHMTotauxDocument ihmTotauxDevis;
	private IHMTotauxDocument ihmOldTotauxDevis;
	private Realm realm;
	private DataBindingContext dbc;
	private String dataTVA[][] = {};
	private ModelTotauxDevis modelTotaux = new ModelTotauxDevis();
	private ModelLignesTVADevis modelLignesTVA;
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private Object selectedTotauxDevis = new IHMTotauxDocument();
	private String[] listeChamp;
	private Class classModel = IHMTotauxDocument.class;

	private MapChangeListener changeListener = new MapChangeListener();

	private TaDevis masterEntity = null;
	private ITaDevisServiceRemote masterDAO = null;
	private ITaDevisServiceRemote daoLocal = null;

	
	private LgrDozerMapper<IHMTotauxDocument,TaDevis> mapperUIToModel = new LgrDozerMapper<IHMTotauxDocument,TaDevis>();
	private LgrDozerMapper<TaDevis,IHMTotauxDocument> mapperModelToUI = new LgrDozerMapper<TaDevis,IHMTotauxDocument>();
	
	public PaEditorTotauxDevisController(){}
	
	public PaEditorTotauxDevisController(PaTotauxDevis vue/*, WorkbenchPart part*/) {
		this(vue,null);
	}
	
	public PaEditorTotauxDevisController(PaTotauxDevis vue/*, WorkbenchPart part*/,EntityManager em) {
//		if(em!=null) {
//			setEm(em);
//		}
		try {
			setVue(vue);
			daoLocal = new EJBLookup<ITaDevisServiceRemote>().doLookup(JNDILookupClass.getInitialContext(),EJBLookup.EJB_NAME_TA_DEVIS_SERVICE, ITaDevisServiceRemote.class.getName());
	...
	private void initController() {
		try {
			...
			bind();
			try{
				((IHMTotauxDocument)selectedTotauxDevis).setImprimer(DevisPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.IMPRIMER_AUTO));
			}catch (Exception e) {	
			}
			//#AFAIRE
//			initEtatBouton();
	...
	public void bind() {
		try {
			...
			LgrDozerMapper<TaDevis,IHMTotauxDocument> mapper = new LgrDozerMapper<TaDevis,IHMTotauxDocument>();
			if(ihmTotauxDevis==null) ihmTotauxDevis = new IHMTotauxDocument();
			...
			bindingFormSimple(mapComposantChampsCommentaire, dbc, realm, selectedTotauxDevis, classModel);
	...
	public void bindTVA() {
		try {
			modelLignesTVA = new ModelLignesTVADevis();
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());
...
//					viewerContent.getKnownElements(), IHMLignesTVADevis.class, listeChamp);
//
//			tableViewer.setLabelProvider(new ObservableMapLabelProvider(attributeMaps));
//			writableList = new WritableList(realm, modelLignesTVA.remplirListe(masterEntity), IHMLignesTVADevis.class);
//			tableViewer.setInput(writableList);
			
			LgrViewerSupport.bind(tableViewer, 
					new WritableList(modelLignesTVA.remplirListe(masterEntity), LignesTVADevisDTO.class),
...
//			actInserer();
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfMT_HT_CALC(), new InfosVerifSaisie(new TaDevis(),Const.C_MT_HT_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_CALC(), new InfosVerifSaisie(new TaDevis(),Const.C_MT_TTC_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TVA_CALC(), new InfosVerifSaisie(new TaDevis(),Const.C_MT_TVA_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfREGLE_DOCUMENT(), new InfosVerifSaisie(new TaFacture(),Const.C_REGLE_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_HT_DEVIS(), new InfosVerifSaisie(new TaDevis(),Const.C_TX_REM_HT_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_TTC_DEVIS(), new InfosVerifSaisie(new TaDevis(),Const.C_TX_REM_TTC_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
	...
	protected void initMapComposantChamps() {
		...
		vue.getTfTX_REM_TTC_DEVIS().setEnabled(false);

		
		mapComposantChamps.put(vue.getTfTX_REM_HT_DEVIS(), Const.C_TX_REM_HT_DOCUMENT);
		...
		mapComposantChamps.put(vue.getTfTX_REM_TTC_DEVIS(), Const.C_TX_REM_TTC_DOCUMENT);
		...
		listeComposantFocusable.add(vue.getTfTX_REM_HT_DEVIS());
		listeComposantFocusable.add(vue.getTfMT_HT_CALC());
		listeComposantFocusable.add(vue.getTfMT_TVA_CALC());
		listeComposantFocusable.add(vue.getTfTX_REM_TTC_DEVIS());
		...
		vue.getTfTX_REM_HT_DEVIS().addVerifyListener(queDesChiffresPositifs);
		vue.getTfTX_REM_TTC_DEVIS().addVerifyListener(queDesChiffresPositifs);
...
//	}
	...
	public PaEditorTotauxDevisController getThis() {
		return this;
	}
	...
	public void retourEcran(final RetourEcranEvent evt) {
		try {
			//passage EJB attendre que le plugin fr.legrain.generationdocument soit OK
...
//			} else			
	...
	protected boolean aideDisponible() {
		...
		switch ((PaEditorTotauxDevisController.this.getMasterModeEcran().getMode())) {
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				case C_MO_CONSULTATION:
					if (getFocusCourantSWT().equals(vue.getTfCODE_T_PAIEMENT())) {
						...
						ModelGeneralObjetEJB<TaTPaiement,TaTPaiementDTO> modelTPaiement = new ModelGeneralObjetEJB<TaTPaiement,TaTPaiementDTO>(dao);
						paramAfficheAideRecherche.setModel(modelTPaiement);
...
//				}
	...
	public IStatus validateUI() throws Exception {
		if ((getMasterModeEcran().getMode().compareTo(EnumModeObjet.C_MO_INSERTION) == 0)
				|| (getMasterModeEcran().getMode().compareTo(EnumModeObjet.C_MO_EDITION) == 0)) {
			ctrlTousLesChampsAvantEnregistrementSWT();
			mapperUIToModel.map(((IHMTotauxDocument)selectedTotauxDevis),masterEntity);
		}
		return null;
	}
...
//				dao = null;	
				...
				try {
					masterDAO.validateEntityProperty(u,nomChamp,ITaDevisServiceRemote.validationContext);
				} catch(Exception e) {
					resultat = new Status(IStatus.ERROR,TiersPlugin.PLUGIN_ID, e.getMessage(), e);
				}
				if(resultat.getSeverity()!=IStatus.ERROR) {
...
//				TaDevis u = new TaDevis(true);
//				PropertyUtils.setSimpleProperty(u, nomChamp, value);
//				s = masterDAO.validate(u,nomChamp,ITaDevisServiceRemote.validationContext);
//				if(s.getSeverity()!=IStatus.ERROR) {
//					PropertyUtils.setSimpleProperty(masterEntity, nomChamp, value);
//				}
//				if(nomChamp.equals(Const.C_TX_REM_TTC_DOCUMENT)) {
...
//				}
			...
			if(s.getSeverity()!=IStatus.ERROR) {
				mapperModelToUI.map(masterEntity, ((IHMTotauxDocument)selectedTotauxDevis));
			}
...
//	public SWT_IB_TA_DEVIS getIbTaTable() {
//		return ibTaTable;
//	}
	...
	public IHMTotauxDocument getIhmOldEnteteDevis() {
		return ihmOldTotauxDevis;
	}

	public void setIhmOldTotauxDevis(IHMTotauxDocument ihmOldTotauxDevis) {
		this.ihmOldTotauxDevis = ihmOldTotauxDevis;
	}

	public void setIhmOldTotauxDevis() {
		if (selectedTotauxDevis!= null)
			this.ihmOldTotauxDevis = IHMTotauxDocument.copy((IHMTotauxDocument) selectedTotauxDevis);
		else {
			if(tableViewer.selectionGrille(0)){
				setSelectedTotauxDevis(selectedTotauxDevis);
				this.ihmOldTotauxDevis = IHMTotauxDocument.copy((IHMTotauxDocument) selectedTotauxDevis);
				tableViewer.setSelection(new StructuredSelection(
						(IHMTotauxDocument) selectedTotauxDevis), true);
			}}
	}

	public void setVue(PaTotauxDevis vue) {
		super.setVue(vue);
		this.vue = vue;
	...
	protected void initMapComposantDecoratedField() {
		...
		mapComposantDecoratedField.put(vue.getTfTX_REM_HT_DEVIS(), vue.getFieldTX_REM_HT_DEVIS());
		mapComposantDecoratedField.put(vue.getTfTX_REM_TTC_DEVIS(), vue.getFieldTX_REM_TTC_DEVIS());
	...
	protected void actRefresh() throws Exception {
		if (getMasterModeEcran().getMode().compareTo(EnumModeObjet.C_MO_CONSULTATION) != 0) {
			if (masterEntity!=null && selectedTotauxDevis!=null && (IHMTotauxDocument) selectedTotauxDevis!=null) {
				mapperModelToUI.map(masterEntity, (IHMTotauxDocument) selectedTotauxDevis);
			}
		}
...
//	ISelectionProvider
	...
	public void modificationDocument(SWTModificationDocumentEvent evt) {
		...
		writableList = new WritableList(realm, modelLignesTVA.remplirListe(masterEntity), LignesTVADevisDTO.class);
		tableViewer.setInput(writableList);
		...
		try {
			((IHMTotauxDocument)selectedTotauxDevis).setMtHtCalc(LibCalcul.arrondi(montantHT));
			((IHMTotauxDocument)selectedTotauxDevis).setMtTtcCalc(LibCalcul.arrondi(montantTTC));
			((IHMTotauxDocument)selectedTotauxDevis).setMtTvaCalc(LibCalcul.arrondi(montantTVA));
			((IHMTotauxDocument)selectedTotauxDevis).setNetHtCalc(LibCalcul.arrondi(NetHT));
			((IHMTotauxDocument)selectedTotauxDevis).setNetTtcCalc(LibCalcul.arrondi(NetTTC));
			((IHMTotauxDocument)selectedTotauxDevis).setNetTvaCalc(LibCalcul.arrondi(NetTVA));

			if(((IHMTotauxDocument)selectedTotauxDevis).getTxRemTtcDocument()==null)
				((IHMTotauxDocument)selectedTotauxDevis).setTxRemTtcDocument(new BigDecimal(0));


			if(((IHMTotauxDocument)selectedTotauxDevis).getRegleDocument()==null)
				((IHMTotauxDocument)selectedTotauxDevis).setRegleDocument(new BigDecimal(0));

			if(((IHMTotauxDocument)selectedTotauxDevis).getRegleDocument().compareTo(((IHMTotauxDocument)selectedTotauxDevis).getNetTtcCalc())>0
					&& ((IHMTotauxDocument)selectedTotauxDevis).getNetTtcCalc().compareTo(new BigDecimal(0))>0)
				((IHMTotauxDocument)selectedTotauxDevis).setRegleDocument(((IHMTotauxDocument)selectedTotauxDevis).getNetTtcCalc());
			else if(((IHMTotauxDocument)selectedTotauxDevis).getNetTtcCalc().signum()<0)
				((IHMTotauxDocument)selectedTotauxDevis).setRegleDocument(new BigDecimal(0));

			if(((IHMTotauxDocument)selectedTotauxDevis).getTxRemHtDocument()==null)
				((IHMTotauxDocument)selectedTotauxDevis).setTxRemHtDocument(new BigDecimal(0));
			
			//passage ejb
			masterEntity.calculeTvaEtTotaux();
			//masterDAO.calculeTvaEtTotaux(masterEntity);
	...
	public void activeModifytListener() {
		...
		//#AFAIRE
		if(masterDAO!=null){
			activeModifytListener(mapComposantChamps);
			activeModifytListener(mapComposantChampsCommentaire);
		}
	}
	 ...
	 */
	protected void initFocusSWT(/*IBQuLgr ibTaTable, Map<EnumModeObjet,Control> focus*/) {
		setFocusCourantSWT(vue.getTfTX_REM_HT_DEVIS());
...
//			}
	...
	public void setTaDevis(TaDevis masterEntity) {
	...
	private class TotauxPropertyChangeListener implements PropertyChangeListener {
		...
		public void propertyChange(PropertyChangeEvent evt) {
			...
			else if(evt.getPropertyName().equals("TX_REM_TTC_DEVIS"))
				masterEntity.setTxRemTtcDocument((BigDecimal)evt.getNewValue());
			else if(evt.getPropertyName().equals("REM_TTC_DEVIS"))
				masterEntity.setRemTtcDocument((BigDecimal)evt.getNewValue());
			else if(evt.getPropertyName().equals("TX_REM_HT_DEVIS"))
				masterEntity.setTxRemHtDocument((BigDecimal)evt.getNewValue());
			else if(evt.getPropertyName().equals("REM_HT_DEVIS"))
	...
	private void initVue() {
		addExpandBarItem(vue.getExpandBar(), new PaCommentaire(vue
				...
				DevisPlugin.getImageDescriptor("/icons/creditcards.png")
				.createImage());
...
//			param.setTypeSrc(TypeDoc.TYPE_DEVIS);
...
//			param.getFocusDefaut().requestFocus();
	...
	public boolean changementPageValide(){
		if (getMasterModeEcran().getMode().compareTo(EnumModeObjet.C_MO_EDITION) == 0
				|| getMasterModeEcran().getMode().compareTo(EnumModeObjet.C_MO_INSERTION) == 0) {
	...
	protected void initFocusSWT(Map<EnumModeObjet,Control> focus) {
		switch (getModeEcran().getMode()) {
		case C_MO_INSERTION:
			if (getModeEcran().getFocusCourantSWT()==null)
				getModeEcran().setFocusCourantSWT(focus.get(
						EnumModeObjet.C_MO_INSERTION));
			break;
		case C_MO_EDITION:
			if (getModeEcran().getFocusCourantSWT()==null){
				if (getFocusCourantSWT() instanceof Text){
					if (((Text)getFocusCourantSWT()).getEditable())
						getModeEcran().setFocusCourantSWT(getFocusCourantSWT());
				}
