...
pg_dump -i -h localhost -p 5432 -U bdg -F c -b -v -f "$BDG_FILESYSTEM_PATH/bdg/system/db_dumps/solstyce_"$DATE"_demo.backup" demo
pg_dump -i -h localhost -p 5432 -U bdg -F c -b -v -f "$BDG_FILESYSTEM_PATH/bdg/system/db_dumps/solstyce_"$DATE"_moncompte.backup" moncompte
pg_dump -i -h localhost -p 5432 -U bdg -F c -b -v -f "$BDG_FILESYSTEM_PATH/bdg/system/db_dumps/solstyce_"$DATE"_autorisations.backup" autorisations
pg_dump -i -h localhost -p 5432 -U bdg -F c -b -v -f "$BDG_FILESYSTEM_PATH/bdg/system/db_dumps/solstyce_"$DATE"_login_db.backup" login_db
...
echo "..Mise à jour de la base de données (tous les schémas)"
a=$(pwd)
cd $BDG_FILESYSTEM_PATH/bdg/update_db
$BDG_FILESYSTEM_PATH/bdg/update_db/update_db_dossier.sh $PGUSER $PGPASSWORD $BDG_FILESYSTEM_PATH
cd $a
