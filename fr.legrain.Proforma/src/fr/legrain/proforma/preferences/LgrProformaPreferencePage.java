...
public class LgrProformaPreferencePage extends FieldEditorPreferencePageLGR{
	...
	static private PropertiesConfiguration listeGestCode = new PropertiesConfiguration();
	 static Logger logger = Logger.getLogger(LgrProformaPreferencePage.class.getName());
	  
	public LgrProformaPreferencePage() {
		super(GRID);
		String idPlugin = proformaPlugin.getDefault().getBundle().getSymbolicName();
		...
		this.setFolderEditionReportPlugin(constEdition.FICHE_FILE_REPORT_PROFORMA);
		this.setNamePlugin(idPlugin);
		
		reportPlugin =ConstEdition.pathRepertoireEditionsSpecifiques()+ConstEdition.SEPARATOR+idPlugin+
		ConstEdition.SEPARATOR+TaProforma.class.getSimpleName();
		
		reportPluginClients=ConstEdition.pathRepertoireEditionsSpecifiquesClient()+ConstEdition.SEPARATOR+idPlugin+
		ConstEdition.SEPARATOR+TaProforma.class.getSimpleName();
		
		setPreferenceStore(proformaPlugin.getDefault().getPreferenceStore());
		setDescription("Paramètres de codification des proformas");
	...
	public void createFieldEditors() {
		...
		String PathEditionDefaut = ConstEdition.pathFichierEditionsSpecifiques(ConstEdition.FICHE_FILE_REPORT_PROFORMA,
				   namePlugin);
		...
		constEdition.valuePropertiePathEditionDefaut = ConstEdition.FICHE_FILE_REPORT_PROFORMA;
	...
	protected void performApply() {
		...
		try {
			...
			IPreferenceStore store = proformaPlugin.getDefault().getPreferenceStore();

			// Change la valeur de la clé taCle dans l'objet Properties
			taNouvelleValeur=proformaPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_FIXE_1) ;
			listeGestCode.setProperty(PreferenceConstants.TA_DOC_FIXE_1,taNouvelleValeur) ;

			taNouvelleValeur=proformaPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_EXO) ;
			listeGestCode.setProperty(PreferenceConstants.TA_DOC_EXO,taNouvelleValeur) ;

			taNouvelleValeur=proformaPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.TA_DOC_COMPTEUR) ;
