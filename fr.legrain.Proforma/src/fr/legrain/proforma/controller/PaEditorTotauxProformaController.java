...
import fr.legrain.proforma.divers.IHMLignesTVAProforma;
import fr.legrain.proforma.divers.ModelLignesTVAProforma;
import fr.legrain.proforma.divers.ModelTotaux;
...
public class PaEditorTotauxProformaController extends JPABaseControllerSWTStandard implements 
ISelectionProvider,   RetourEcranListener,
SWTModificationDocumentListener,ChangeModeListener {

	static Logger logger = Logger.getLogger(PaEditorTotauxProformaController.class.getName());
	private PaTotauxProforma vue = null;
		
	public List<Control> listeComposantEntete = null;
	protected Map<Control, String> mapComposantChampsCommentaire = null;

	private IHMTotauxProforma ihmTotauxDevis;
	private IHMTotauxProforma ihmOldTotauxDevis;
	private Realm realm;
	private DataBindingContext dbc;
	private String dataTVA[][] = {};
	private ModelTotaux modelTotaux = new ModelTotaux();
	private ModelLignesTVAProforma modelLignesTVA;
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private Object selectedTotauxDevis = new IHMTotauxProforma();
	private String[] listeChamp;
	private Class classModel = IHMTotauxProforma.class;

	private MapChangeListener changeListener = new MapChangeListener();

	private TaProforma masterEntity = null;
	private TaProformaDAO masterDAO = null;
	private TaProformaDAO daoLocal = null;

	
	private LgrDozerMapper<IHMTotauxProforma,TaProforma> mapperUIToModel = new LgrDozerMapper<IHMTotauxProforma,TaProforma>();
	private LgrDozerMapper<TaProforma,IHMTotauxProforma> mapperModelToUI = new LgrDozerMapper<TaProforma,IHMTotauxProforma>();
	
	public PaEditorTotauxProformaController(){}
	
	public PaEditorTotauxProformaController(PaTotauxProforma vue/*, WorkbenchPart part*/) {
		this(vue,null);
	}
	
	public PaEditorTotauxProformaController(PaTotauxProforma vue/*, WorkbenchPart part*/,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		try {
			setVue(vue);
			daoLocal = new TaProformaDAO(getEm());
	...
	private void initController() {
		try {
			...
			bind();
			try{
				((IHMTotauxProforma)selectedTotauxDevis).setImprimer(proformaPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.IMPRIMER_AUTO));
			}catch (Exception e) {	
			}
			//#AFAIRE
//			initEtatBouton();
	...
	public void bind() {
		try {
			...
			LgrDozerMapper<TaProforma,IHMTotauxProforma> mapper = new LgrDozerMapper<TaProforma,IHMTotauxProforma>();
			if(ihmTotauxDevis==null) ihmTotauxDevis = new IHMTotauxProforma();
	...
	public void bindTVA() {
		try {
			modelLignesTVA = new ModelLignesTVAProforma();
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());
...
//			tableViewer.setInput(writableList);
			...
			LgrViewerSupport.bind(tableViewer, 
					new WritableList(modelLignesTVA.remplirListe(masterEntity), IHMLignesTVAProforma.class),
...
//			actInserer();
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfMT_HT_CALC(), new InfosVerifSaisie(new TaProforma(),Const.C_MT_HT_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_CALC(), new InfosVerifSaisie(new TaProforma(),Const.C_MT_TTC_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TVA_CALC(), new InfosVerifSaisie(new TaProforma(),Const.C_MT_TVA_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfREGLE_DOCUMENT(), new InfosVerifSaisie(new TaProforma(),Const.C_REGLE_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_HT_PROFORMA(), new InfosVerifSaisie(new TaProforma(),Const.C_TX_REM_HT_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_TTC_PROFORMA(), new InfosVerifSaisie(new TaProforma(),Const.C_TX_REM_TTC_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
...
//	}
	...
	public PaEditorTotauxProformaController getThis() {
		return this;
	}
	...
	protected boolean aideDisponible() {
		...
		switch ((PaEditorTotauxProformaController.this.masterDAO.getModeObjet().getMode())) {
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
	...
	public IStatus validateUI() throws Exception {
		if ((masterDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
			...
			mapperUIToModel.map(((IHMTotauxProforma)selectedTotauxDevis),masterEntity);
		}
		return null;
	}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		...
		try {
			...
			if(s.getSeverity()!=IStatus.ERROR) {
				mapperModelToUI.map(masterEntity, ((IHMTotauxProforma)selectedTotauxDevis));
			}
...
//		sortieChamps();
	...
	public IHMTotauxProforma getIhmOldEnteteDevis() {
		return ihmOldTotauxDevis;
	}

	public void setIhmOldTotauxDevis(IHMTotauxProforma ihmOldTotauxDevis) {
		this.ihmOldTotauxDevis = ihmOldTotauxDevis;
	}

	public void setIhmOldTotauxDevis() {
		if (selectedTotauxDevis!= null)
			this.ihmOldTotauxDevis = IHMTotauxProforma.copy((IHMTotauxProforma) selectedTotauxDevis);
		else {
			if(tableViewer.selectionGrille(0)){
				setSelectedTotauxDevis(selectedTotauxDevis);
				this.ihmOldTotauxDevis = IHMTotauxProforma.copy((IHMTotauxProforma) selectedTotauxDevis);
				tableViewer.setSelection(new StructuredSelection(
						(IHMTotauxProforma) selectedTotauxDevis), true);
			}}
	}

	public void setVue(PaTotauxProforma vue) {
		super.setVue(vue);
		this.vue = vue;
	...
	protected void actRefresh() throws Exception {
		if (masterDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION) != 0) {
			if (masterEntity!=null && selectedTotauxDevis!=null && (IHMTotauxProforma) selectedTotauxDevis!=null) {
				mapperModelToUI.map(masterEntity, (IHMTotauxProforma) selectedTotauxDevis);
			}
		}
...
//	ISelectionProvider
	...
	public void modificationDocument(SWTModificationDocumentEvent evt) {
		...
		try {
			((IHMTotauxProforma)selectedTotauxDevis).setMtHtCalc(LibCalcul.arrondi(montantHT));
			((IHMTotauxProforma)selectedTotauxDevis).setMtTtcCalc(LibCalcul.arrondi(montantTTC));
			((IHMTotauxProforma)selectedTotauxDevis).setMtTvaCalc(LibCalcul.arrondi(montantTVA));
			((IHMTotauxProforma)selectedTotauxDevis).setNetHtCalc(LibCalcul.arrondi(NetHT));
			((IHMTotauxProforma)selectedTotauxDevis).setNetTtcCalc(LibCalcul.arrondi(NetTTC));
			((IHMTotauxProforma)selectedTotauxDevis).setNetTvaCalc(LibCalcul.arrondi(NetTVA));

			if(((IHMTotauxProforma)selectedTotauxDevis).getTxRemTtcDocument()==null)
				((IHMTotauxProforma)selectedTotauxDevis).setTxRemTtcDocument(new BigDecimal(0));

			if(((IHMTotauxProforma)selectedTotauxDevis).getTxRemHtDocument()==null)
				((IHMTotauxProforma)selectedTotauxDevis).setTxRemHtDocument(new BigDecimal(0));
...
public void setTaProforma(TaProforma masterEntity) {
...
private void initVue() {
	addExpandBarItem(vue.getExpandBar(), new PaCommentaire(vue
			...
			proformaPlugin.getImageDescriptor("/icons/creditcards.png")
					.createImage());
