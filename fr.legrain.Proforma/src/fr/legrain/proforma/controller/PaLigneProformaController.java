...
import fr.legrain.documents.dao.TaLProformaDAO;
...
import fr.legrain.gestCom.Module_Document.IHMLProforma;
...
import fr.legrain.proforma.ecran.PaSaisieLigneProforma;

public class PaLigneProformaController extends JPABaseControllerSWTStandard
implements RetourEcranListener, SWTModificationDocumentListener, ISelectionProvider {

	static Logger logger = Logger.getLogger(PaLigneProformaController.class.getName());
	...
	private PaSaisieLigneProforma vue = null;


	private TaLProformaDAO dao = null;//new TaLProformaDAO();
	
	private Object ecranAppelant = null;
	private IHMLProforma ihmLDevis;
	private IHMTotauxLignesDocument ihmTotauxLignesDocument;
	private IHMLProforma ihmOldLDevis;
	private Realm realm;
	private DataBindingContext dbc;
	private DataBindingContext dbcTotaux = null;

//	private ModelLigneDevis modelLDevis /*=  new ModelLigneFacture(ibTaTable)*/;
	private Class classModel = IHMLProforma.class;
	private ModelGeneralObjet<IHMLProforma,TaLProformaDAO,TaLProforma> modelLDocument /*= new ModelGeneralObjet<IHMLProforma,TaLProformaDAO,TaLProforma>(dao,classModel)*/;
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private IObservableValue selectedLigneProforma;
	...
	private TaLProforma taLProforma = null;
	...
	private LgrDozerMapper<IHMLProforma,TaLProforma> mapperUIToModel = new LgrDozerMapper<IHMLProforma,TaLProforma>();
	private LgrDozerMapper<TaLProforma,IHMLProforma> mapperModelToUI = new LgrDozerMapper<TaLProforma,IHMLProforma>();
	
	private LgrDozerMapper<TaProforma,IHMTotauxLignesDocument> mapperModelToUITotauxLignes = new LgrDozerMapper<TaProforma,IHMTotauxLignesDocument>();

	public PaLigneProformaController(){
	}
	
	public PaLigneProformaController(final PaSaisieLigneProforma vue/*, SWTFacture facture, WorkbenchPart part*/) {
		this(vue,null);
	}
	
	public PaLigneProformaController(final PaSaisieLigneProforma vue/*, SWTFacture facture, WorkbenchPart part*/,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		dao = new TaLProformaDAO(getEm());
		try {
			setVue(vue);
...
//			// fenetre sur ESC
	 ...
	 * @return
	 */
	public List<IHMLProforma> IHMmodel() {
		LinkedList<TaLProforma> ldao = new LinkedList<TaLProforma>();
		LinkedList<IHMLProforma> lswt = new LinkedList<IHMLProforma>();
		
		if(masterEntity!=null && !masterEntity.getLignes().isEmpty()) {

			ldao.addAll(masterEntity.getLignes());

			LgrDozerMapper<TaLProforma,IHMLProforma> mapper = new LgrDozerMapper<TaLProforma,IHMLProforma>();
			for (TaLProforma o : ldao) {
				IHMLProforma t = null;
				t = (IHMLProforma) mapper.map(o, IHMLProforma.class);
				lswt.add(t);
			}
	...
	public void bind(PaSaisieLigneProforma paArticleSWT) {
		try {
			if (modelLDocument==null)modelLDocument = new ModelGeneralObjet<IHMLProforma, TaLProformaDAO, TaLProforma>(dao,classModel);
...
//				tableViewer.setInput(writableList);
				...
				selectedLigneProforma = ViewersObservables.observeSingleSelection(tableViewer);
			}
			...
			bindingFormMaitreDetail(dbc, realm, selectedLigneProforma,classModel);
...
//			ibTaTable.ouvreDataset();
			if (((ParamAfficheLProforma)param).getFocusSWT()!=null)
				((ParamAfficheLProforma)param).getFocusSWT().forceFocus();
			else ((ParamAfficheLProforma)param).setFocusSWT(vue.getTfCODE_ARTICLE());			
...
//			ibTaTable.getFIBQuery().setSort(null);
		...
		if (param.getModeEcran() != null
			...
			try {
				InsertionLigne(false,((ParamAfficheLProforma)param).getInitFocus());
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfQTE_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_QTE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfQTE2_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_QTE2_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfMT_HT_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_MT_HT_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_MT_TTC_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfPRIX_U_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_PRIX_U_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfREM_TX_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_REM_TX_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfNUM_LIGNE_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_NUM_LIGNE_L_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS}));
		mapInfosVerifSaisie.put(vue.getTfCODE_ARTICLE(), new InfosVerifSaisie(new TaArticle(),Const.C_CODE_ARTICLE,null));
		mapInfosVerifSaisie.put(vue.getTfLIB_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_LIB_L_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfU1_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_U1_L_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfU2_L_PROFORMA(), new InfosVerifSaisie(new TaLProforma(),Const.C_U1_L_DOCUMENT,null));
...
//			}
			...
			articleContentProposal.addContentProposalListener(new IContentProposalListener2(){
				...
				public void proposalPopupOpened(ContentProposalAdapter adapter) {
					...
					if(!dao.dataSetEnModif()&& taLProforma.ligneEstVide()  )
	...
	public PaLigneProformaController getThis() {
		return this;
	}
				...
				if (getFocusAvantAideSWT() instanceof Text) {
					try {
						...
						if(getFocusAvantAideSWT().equals(vue.getTfU1_L_PROFORMA())
							...
							t = null;
							if(getFocusAvantAideSWT().equals(vue.getTfU1_L_PROFORMA()))
								taLProforma.setU1LDocument(f.getCodeUnite());
							else
								taLProforma.setU2LDocument(f.getCodeUnite());
						}					
						if(getFocusAvantAideSWT().equals(vue.getTfPRIX_U_L_PROFORMA())){
							...
							if(masterEntity.getTtc()==1)
								taLProforma.setPrixULDocument(f.getPrixttcPrix());
							else
								taLProforma.setPrixULDocument(f.getPrixPrix());
						}					
...
//						ibTaTable.lgrLocateID(((ResultAffiche) evt.getRetour()).getIdResult());
			...
			if(parentEcran!=null) {
				...
				((PaEditorProformaController)parentEcran).initEtatBouton(false);
			}
		}
		}
...
//	);
	...
	protected void actChangeTva() throws Exception{
		if(!LibChaine.empty(((IHMLProforma)selectedLigneProforma.getValue()).getCodeArticle())
			...
			InputDialog input = new InputDialog(vue.getShell()
					...
					((IHMLProforma)selectedLigneProforma.getValue()).getCodeTvaLDocument(),new IInputValidator() {
			...
			if(masterDAO.dataSetEnModif()){
				...
				if(input.getValue()!=null ){
					if(MessageDialog.openConfirm(vue.getShell()
						...
						((IHMLProforma)selectedLigneProforma.getValue()).setCodeTvaLDocument(retour);
						if(input.getValue().equals("")){
							taLProforma.setCodeTvaLDocument("");
							taLProforma.setTauxTvaLDocument(BigDecimal.valueOf(0));
						}else{
							TaTvaDAO daoTva = new TaTvaDAO(getEm());
							taLProforma.setCodeTvaLDocument(retour);
							taLProforma.setTauxTvaLDocument(daoTva.findByCode(retour).getTauxTva());
						}
...
//			}
			if(continuer){
				...
				for (TaLProforma p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument().equals(((IHMLProforma) selectedLigneProforma.getValue()).getNumLigneLDocument())) {
						taLProforma = p;
						taLProforma.setLegrain(true);
						ihmLDevis= IHMLProforma.copy((IHMLProforma) selectedLigneProforma.getValue());;
					}				
				}
				initEtatBouton(false);
			}
	...
	protected void actSupprimer() throws Exception {
		...
		try {
			...
			for (TaLProforma p : masterEntity.getLignes()) {
				...
				dao.supprimer(taLProforma);
				taLProforma.setTaDocument(null);
				masterEntity.removeLigne(taLProforma);
				taLProforma=null;
				actRefresh();
				refreshTotauxLignes();
				initEtatBouton();
	...
	protected void actAnnuler() throws Exception {
		...
		try {
			...
			case C_MO_INSERTION:
				message =!taLProforma.ligneEstVide();
				...
				if(repondu){
					if(selectedLigneProforma!=null && selectedLigneProforma.getValue() != null
			...
			case C_MO_EDITION:
				...
				if(repondu){
					int rang =((WritableList)tableViewer.getInput()).indexOf(selectedLigneProforma.getValue());
					List<IHMLProforma> l = IHMmodel();
					if(rang!=-1)
					...
					mapperUIToModel.map(ihmOldLDevis,taLProforma);
//					ctrlTousLesChampsAvantEnregistrementSWT();//si on contrôle tous les champs
	...
	protected void actPrecedent() throws Exception {
		if(taLProforma!=null && taLProforma.ligneEstVide()){
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				switch ((PaLigneProformaController.this.dao.getModeObjet().getMode())) {
				case C_MO_CONSULTATION:
				...
				case C_MO_INSERTION:
					...
					if (getFocusCourantSWT()==vue.getTfU1_L_PROFORMA()||
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new TaUniteDAO(getEm()).modelObjetUniteArticle(taLProforma.getTaArticle().getCodeArticle()));
					...
					if (getFocusCourantSWT()==vue.getTfPRIX_U_L_PROFORMA()){
						...
						paramAffichePrix.setIdArticle(LibConversion.integerToString(getSelectedLigneProforma().getIdArticle()));
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTPrix,TaPrixDAO,TaPrix>(taLProforma.getTaArticle().getTaPrixes(),SWTPrix.class,getEm()));
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "L_PROFORMA";
		try {
			IStatus s = null;
			boolean change=true;
			if(nomChamp.equals(Const.C_CODE_ARTICLE)) {
				...
				if(change){
					taLProforma.setTaArticle(f);
					taLProforma.initQte2(f);
					mapperModelToUI.map(taLProforma, ihmLDevis);
				}
				}
				dao = null;
			} else {
				TaLProforma u = new TaLProforma();
				...
				if(s.getSeverity()!=IStatus.ERROR) {
					PropertyUtils.setSimpleProperty(taLProforma, nomChamp, value);
				}
			}
			
			if(s.getSeverity()!=IStatus.ERROR) {
				if(nomChamp.equals(Const.C_QTE_L_DOCUMENT)&& change) {
					if(taLProforma.getTaArticle()!=null){
						taLProforma.initQte2(taLProforma.getTaArticle());
					}
				}				
				mapperModelToUI.map(taLProforma, ((IHMLProforma)selectedLigneProforma.getValue()));
			}
	...
	protected void actEnregistrer(boolean insertion) throws Exception {
		try {
			if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
				...
				ctrlTousLesChampsAvantEnregistrementSWT();
				if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
					LgrDozerMapper<IHMLProforma,TaLProforma> mapper = new LgrDozerMapper<IHMLProforma,TaLProforma>();
					mapper.map((IHMLProforma) selectedLigneProforma.getValue(),taLProforma);
				...
				masterEntity.enregistreLigne(taLProforma);
				dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_CONSULTATION);
...
//	}
	...
	public boolean isUtilise(){
		return ((IHMLProforma)selectedLigneProforma.getValue()).getIdLDocument()!=null && 
		!dao.recordModifiable(dao.getNomTable(),
				((IHMLProforma)selectedLigneProforma.getValue()).getIdLDocument());		
	}
	public IHMLProforma getIhmOldLDevis() {
		return ihmOldLDevis;
	}

	public void setIhmOldLDevis(IHMLProforma swtOldDevis) {
		this.ihmOldLDevis = swtOldDevis;
	}

	public void setIhmOldLDevis() {
		if (selectedLigneProforma!=null && selectedLigneProforma.getValue() != null)
			this.ihmOldLDevis = IHMLProforma.copy((IHMLProforma) selectedLigneProforma.getValue());
		else {
			if(tableViewer.selectionGrille(0)){
				this.ihmOldLDevis = IHMLProforma.copy((IHMLProforma) selectedLigneProforma.getValue());
				tableViewer.setSelection(new StructuredSelection(
						(IHMLProforma) selectedLigneProforma.getValue()), true);
			}}
	}

	public void setVue(final PaSaisieLigneProforma vue) {
		super.setVue(vue);
		this.vue = vue;
	}
	...
	protected void sortieChamps() {
		try {
			if (masterEntity.getLigne(masterEntity.getLigneCourante())!=null && 
					( (TaLProforma)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaTLigne().getIdTLigne()!=0)
				initEtatComposant(( (TaLProforma)masterEntity.getLigne(masterEntity.getLigneCourante())).getTaTLigne().getCodeTLigne());		
	...
	protected void actRefresh() throws Exception {
		...
		tableViewer.selectionGrille(
				tableViewer.selectionGrille(selectedLigneProforma.getValue()));	
	}
...
//
//			for (IHMLProforma ihmLigne : modelLDevis.getListeObjet()) {
//				SWTLigneDevis swtLigne = new SWTLigneDevis((IHMLProforma)selectedLigneProforma.getValue());
...
//			}
	...
	private class SelectionGrille extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			setIhmOldLDevis();
			if(selectedLigneProforma.getValue()!=null && masterEntity.getLignes().size()>vue.getGrille().getSelectionIndex()){
				for (TaLProforma p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument()==((IHMLProforma) selectedLigneProforma.getValue()).getNumLigneLDocument()) {
						taLProforma = p;
					}				
				}
			}
	...
	public IHMLProforma getSelectedLigneProforma() {
		IHMLProforma resultat = null;
		if(selectedLigneProforma!=null && selectedLigneProforma.getValue()!=null)
			resultat = (IHMLProforma)selectedLigneProforma.getValue();
		return resultat;
	};	
	...
	protected void InsertionLigne(Boolean ajout, boolean initFocus) throws Exception{
		...
		try {
			...
			if(masterDAO.dataSetEnModif()){
			setIhmOldLDevis();
			ihmLDevis = new IHMLProforma();
			dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_INSERTION);
			taLProforma = new TaLProforma(true); 
			taLProforma.setTaTLigne(new TaTLigneDAO(getEm()).findByCode(Const.C_CODE_T_LIGNE_C));
			
			List<IHMLProforma> modelListeLigneDevis = IHMmodel();
			
			if(ajout)
				masterEntity.addLigne(taLProforma);
			else{
				if(vue.getGrille().getItemCount()!=0) 
				masterEntity.insertLigne(taLProforma,vue.getGrille().getSelectionIndex());
				else
					masterEntity.insertLigne(taLProforma,0);
					
			}
			ihmLDevis.setNumLigneLDocument(taLProforma.getNumLigneLDocument());
			taLProforma.setTaDocument(masterEntity);
			taLProforma.initialiseLigne(true);
			...
			initEtatComposant(taLProforma.getTaTLigne().getCodeTLigne());

			taLProforma.setNumLigneLDocument(ihmLDevis.getNumLigneLDocument());
...
//								e.doit=false;
			...
			public boolean changementPageValide(){
				if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
					...
					if(taLProforma!=null && selectedLigneProforma!=null && ((IHMLProforma) selectedLigneProforma.getValue())!=null) {
						mapperUIToModel.map((IHMLProforma) selectedLigneProforma.getValue(),taLProforma);
					}
				}
				return true;
			}

			public TaProforma getMasterEntity() {
				return masterEntity;
			}

			public void setMasterEntity(TaProforma masterEntity) {
				this.masterEntity = masterEntity;
			}

			public TaProformaDAO getMasterDAO() {
				return masterDAO;
			}

			public void setMasterDAO(TaProformaDAO masterDAO) {
