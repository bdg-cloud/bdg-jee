...
import fr.legrain.boncde.divers.IHMLignesTVABoncde;
import fr.legrain.boncde.divers.ModelLignesTVABoncde;
import fr.legrain.boncde.divers.ModelTotaux;
import fr.legrain.boncde.ecran.PaTotauxBoncde;
...
public class PaEditorTotauxBoncdeController extends JPABaseControllerSWTStandard implements 
ISelectionProvider,   RetourEcranListener,
SWTModificationDocumentListener,ChangeModeListener {

	static Logger logger = Logger.getLogger(PaEditorTotauxBoncdeController.class.getName());
	private PaTotauxBoncde vue = null;
		
	public List<Control> listeComposantEntete = null;
	protected Map<Control, String> mapComposantChampsCommentaire = null;

	private IHMTotauxBoncde ihmTotauxDevis;
	private IHMTotauxBoncde ihmOldTotauxDevis;
	private Realm realm;
	private DataBindingContext dbc;
	private String dataTVA[][] = {};
	private ModelTotaux modelTotaux = new ModelTotaux();
	private ModelLignesTVABoncde modelLignesTVA;
	private LgrTableViewer tableViewer;
	private WritableList writableList;
	private Object selectedTotauxDevis = new IHMTotauxBoncde();
	private String[] listeChamp;
	private Class classModel = IHMTotauxBoncde.class;

	private MapChangeListener changeListener = new MapChangeListener();

	private TaBoncde masterEntity = null;
	private TaBoncdeDAO masterDAO = null;
	private TaBoncdeDAO daoLocal = null;
	
	private LgrDozerMapper<IHMTotauxBoncde,TaBoncde> mapperUIToModel = new LgrDozerMapper<IHMTotauxBoncde,TaBoncde>();
	private LgrDozerMapper<TaBoncde,IHMTotauxBoncde> mapperModelToUI = new LgrDozerMapper<TaBoncde,IHMTotauxBoncde>();
	
	public PaEditorTotauxBoncdeController(){}
	
	public PaEditorTotauxBoncdeController(PaTotauxBoncde vue/*, WorkbenchPart part*/) {
		this(vue,null);
	}
	
	public PaEditorTotauxBoncdeController(PaTotauxBoncde vue/*, WorkbenchPart part*/,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		try {
			setVue(vue);
			daoLocal = new TaBoncdeDAO(getEm());
	...
	private void initController() {
		try {
			...
			bind();
			try{
				((IHMTotauxBoncde)selectedTotauxDevis).setImprimer(boncdePlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.IMPRIMER_AUTO));
			}catch (Exception e) {	
			}
			//#AFAIRE
//			initEtatBouton();
	...
	public void bind() {
		try {
			...
			LgrDozerMapper<TaBoncde,IHMTotauxBoncde> mapper = new LgrDozerMapper<TaBoncde,IHMTotauxBoncde>();
			if(ihmTotauxDevis==null) ihmTotauxDevis = new IHMTotauxBoncde();
	...
	public void bindTVA() {
		try {
			modelLignesTVA = new ModelLignesTVABoncde();
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());
...
//					viewerContent.getKnownElements(), IHMLignesTVABoncde.class, listeChamp);
//
//			tableViewer.setLabelProvider(new ObservableMapLabelProvider(attributeMaps));
//			writableList = new WritableList(realm, modelLignesTVA.remplirListe(masterEntity), IHMLignesTVABoncde.class);
//			tableViewer.setInput(writableList);
			
			LgrViewerSupport.bind(tableViewer, 
					new WritableList(modelLignesTVA.remplirListe(masterEntity), IHMLignesTVABoncde.class),
...
//			actInserer();
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfMT_HT_CALC(), new InfosVerifSaisie(new TaBoncde(),Const.C_MT_HT_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TTC_CALC(), new InfosVerifSaisie(new TaBoncde(),Const.C_MT_TTC_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfMT_TVA_CALC(), new InfosVerifSaisie(new TaBoncde(),Const.C_MT_TVA_CALC,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfREGLE_DOCUMENT(), new InfosVerifSaisie(new TaBoncde(),Const.C_REGLE_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_HT_BONCDE(), new InfosVerifSaisie(new TaBoncde(),Const.C_TX_REM_HT_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
		mapInfosVerifSaisie.put(vue.getTfTX_REM_TTC_BONCDE(), new InfosVerifSaisie(new TaBoncde(),Const.C_TX_REM_TTC_DOCUMENT,new int[]{InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES}));
	...
	protected void initMapComposantChamps() {
		...
		//vue.getTfTX_REM_HT_BONCDE().setEnabled(false);
		vue.getTfTX_REM_TTC_BONCDE().setEnabled(false);

		
		mapComposantChamps.put(vue.getTfTX_REM_HT_BONCDE(), Const.C_TX_REM_HT_DOCUMENT);
		...
		mapComposantChamps.put(vue.getTfTX_REM_TTC_BONCDE(), Const.C_TX_REM_TTC_DOCUMENT);
		...
		listeComposantFocusable.add(vue.getTfTX_REM_HT_BONCDE());
		listeComposantFocusable.add(vue.getTfMT_HT_CALC());
		listeComposantFocusable.add(vue.getTfMT_TVA_CALC());
		listeComposantFocusable.add(vue.getTfTX_REM_TTC_BONCDE());
		...
		vue.getTfTX_REM_HT_BONCDE().addVerifyListener(queDesChiffresPositifs);
		vue.getTfTX_REM_TTC_BONCDE().addVerifyListener(queDesChiffresPositifs);
...
//	}
	...
	public PaEditorTotauxBoncdeController getThis() {
		return this;
	}
	...
	protected boolean aideDisponible() {
		...
		switch ((PaEditorTotauxBoncdeController.this.masterDAO.getModeObjet().getMode())) {
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
	...
	public IStatus validateUI() throws Exception {
		if ((masterDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
			...
			mapperUIToModel.map(((IHMTotauxBoncde)selectedTotauxDevis),masterEntity);
		}
		return null;
	}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		...
		try {
			...
			if(s.getSeverity()!=IStatus.ERROR) {
				mapperModelToUI.map(masterEntity, ((IHMTotauxBoncde)selectedTotauxDevis));
			}
...
//		sortieChamps();
	...
	public IHMTotauxBoncde getIhmOldEnteteDevis() {
		return ihmOldTotauxDevis;
	}

	public void setIhmOldTotauxDevis(IHMTotauxBoncde ihmOldTotauxDevis) {
		this.ihmOldTotauxDevis = ihmOldTotauxDevis;
	}

	public void setIhmOldTotauxDevis() {
		if (selectedTotauxDevis!= null)
			this.ihmOldTotauxDevis = IHMTotauxBoncde.copy((IHMTotauxBoncde) selectedTotauxDevis);
		else {
			if(tableViewer.selectionGrille(0)){
				setSelectedTotauxDevis(selectedTotauxDevis);
				this.ihmOldTotauxDevis = IHMTotauxBoncde.copy((IHMTotauxBoncde) selectedTotauxDevis);
				tableViewer.setSelection(new StructuredSelection(
						(IHMTotauxBoncde) selectedTotauxDevis), true);
			}}
	}

	public void setVue(PaTotauxBoncde vue) {
		super.setVue(vue);
		this.vue = vue;
	...
	protected void initMapComposantDecoratedField() {
		...
		mapComposantDecoratedField.put(vue.getTfTX_REM_HT_BONCDE(), vue.getFieldTX_REM_HT_BONCDE());
		mapComposantDecoratedField.put(vue.getTfTX_REM_TTC_BONCDE(), vue.getFieldTX_REM_TTC_BONCDE());
	...
	protected void actRefresh() throws Exception {
		if (masterDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION) != 0) {
			if (masterEntity!=null && selectedTotauxDevis!=null && (IHMTotauxBoncde) selectedTotauxDevis!=null) {
				mapperModelToUI.map(masterEntity, (IHMTotauxBoncde) selectedTotauxDevis);
			}
		}
...
//	ISelectionProvider
	...
	public void modificationDocument(SWTModificationDocumentEvent evt) {
		...
		try {
			((IHMTotauxBoncde)selectedTotauxDevis).setMtHtCalc(LibCalcul.arrondi(montantHT));
			((IHMTotauxBoncde)selectedTotauxDevis).setMtTtcCalc(LibCalcul.arrondi(montantTTC));
			((IHMTotauxBoncde)selectedTotauxDevis).setMtTvaCalc(LibCalcul.arrondi(montantTVA));
			((IHMTotauxBoncde)selectedTotauxDevis).setNetHtCalc(LibCalcul.arrondi(NetHT));
			((IHMTotauxBoncde)selectedTotauxDevis).setNetTtcCalc(LibCalcul.arrondi(NetTTC));
			((IHMTotauxBoncde)selectedTotauxDevis).setNetTvaCalc(LibCalcul.arrondi(NetTVA));

			if(((IHMTotauxBoncde)selectedTotauxDevis).getTxRemTtcDocument()==null)
				((IHMTotauxBoncde)selectedTotauxDevis).setTxRemTtcDocument(new BigDecimal(0));

			if(((IHMTotauxBoncde)selectedTotauxDevis).getTxRemHtDocument()==null)
				((IHMTotauxBoncde)selectedTotauxDevis).setTxRemHtDocument(new BigDecimal(0));
...
//			}
	...
	protected void initFocusSWT(IBQuLgr ibTaTable, Map<ModeObjet.EnumModeObjet,Control> focus) {
		setFocusCourantSWT(vue.getTfTX_REM_HT_BONCDE());
...
public void setTaBoncde(TaBoncde masterEntity) {
...
private class TotauxPropertyChangeListener implements PropertyChangeListener {
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		else if(evt.getPropertyName().equals("TX_REM_TTC_BONCDE"))
			masterEntity.setTxRemTtcDocument((BigDecimal)evt.getNewValue());
		else if(evt.getPropertyName().equals("REM_TTC_BONCDE"))
			masterEntity.setRemTtcDocument((BigDecimal)evt.getNewValue());
		else if(evt.getPropertyName().equals("TX_REM_HT_BONCDE"))
			masterEntity.setTxRemHtDocument((BigDecimal)evt.getNewValue());
		else if(evt.getPropertyName().equals("REM_HT_BONCDE"))
...
private void initVue() {
	addExpandBarItem(vue.getExpandBar(), new PaCommentaire(vue
			...
			boncdePlugin.getImageDescriptor("/icons/creditcards.png")
					.createImage());
...
public void actCreateDoc() throws Exception {
	...
	param.setTypeSrc(TypeDoc.TYPE_BON_COMMANDE);
