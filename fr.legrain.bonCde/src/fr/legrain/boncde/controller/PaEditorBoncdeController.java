...
import fr.legrain.boncde.ecran.PaEditorBoncde;
import fr.legrain.boncde.editor.EditorBoncde;
...
import fr.legrain.gestCom.Module_Document.IHMEnteteBoncde;
...
public class PaEditorBoncdeController extends JPABaseControllerSWTStandard
		implements   RetourEcranListener,
		SWTModificationDocumentListener, ChangeModeListener {

	static Logger logger = Logger.getLogger(PaEditorBoncdeController.class.getName());
	private PaEditorBoncde vue = null;
...
//	private Impression impressionBoncde = null;
...
//	private List<TaBoncde> listeDocument = null;
	private List<TaTiers>listeTiers = null;
	
	private TaBoncdeDAO dao = null;//new TaBoncdeDAO();

	private Object ecranAppelant = null;
	private IHMEnteteBoncde ihmOldEnteteBoncde;
	private Realm realm;
	private DataBindingContext dbc;
	private Object selectedBoncde = new IHMEnteteBoncde();
	private String nomClassController = this.getClass().getSimpleName();
	private Class classModel = IHMEnteteBoncde.class;
	
	private ComboViewer viewerComboLocalisationTVA;
	
	private ModelGeneralObjet<IHMEnteteBoncde,TaBoncdeDAO,TaBoncde> modelEnteteBoncde /*= new ModelGeneralObjet<IHMEnteteBoncde,TaBoncdeDAO,TaBoncde>(dao,classModel)*/;
	...
	private TaInfosBoncdeDAO daoInfosBoncde = null;//new TaInfosBoncdeDAO();
	...
	private PaLigneBoncdeController controllerLigne = null;
	private PaEditorTotauxBoncdeController controllerTotaux = null;
	...
	private LgrDozerMapper<TaInfosBoncde,IHMInfosCPaiement> mapperModelToUIInfosDocVersCPaiement = new LgrDozerMapper<TaInfosBoncde,IHMInfosCPaiement>();
	private LgrDozerMapper<TaInfosBoncde,IHMAdresseInfosFacturation> mapperModelToUIInfosDocVersAdresseFact = new LgrDozerMapper<TaInfosBoncde,IHMAdresseInfosFacturation>();
	private LgrDozerMapper<TaInfosBoncde,IHMAdresseInfosLivraison> mapperModelToUIInfosDocVersAdresseLiv = new LgrDozerMapper<TaInfosBoncde,IHMAdresseInfosLivraison>();
	...
	private LgrDozerMapper<IHMEnteteBoncde,TaBoncde> mapperUIToModel = new LgrDozerMapper<IHMEnteteBoncde,TaBoncde>();
	private LgrDozerMapper<TaBoncde,IHMEnteteBoncde> mapperModelToUI = new LgrDozerMapper<TaBoncde,IHMEnteteBoncde>();
	
	private TaBoncde taDocument = null;
	private TaInfosBoncde taInfosDocument = null;
	...
	public PaEditorBoncdeController() {
	}
	
	public PaEditorBoncdeController(PaEditorBoncde vue) {
		this(vue,null);
	}

	public PaEditorBoncdeController(PaEditorBoncde vue,EntityManager em) {
		if(em!=null) {
			setEm(em);
		}
		dao = new TaBoncdeDAO(getEm());
		daoInfosBoncde = new TaInfosBoncdeDAO(getEm());
...
//			impressionBoncde = new Impression(vue.getShell(),getEm());
...
//			this.swtDevis.addChangeModeListener(this);
			...
			vue.getTfLIBELLE_DOCUMENT().addFocusListener(new FocusAdapter() {
				public void focusLost(FocusEvent e) {
					try {
//						ctrlUnChampsSWT(((PaEditorBoncde) getVue()).getTfLIBELLE_DEVIS());
						validateUIField(Const.C_LIBELLE_DOCUMENT,((PaEditorBoncde) getVue()).getTfLIBELLE_DOCUMENT());
					} catch (Exception e1) {
						logger.debug("", e1);
					}
				}
	...
	public void bind(PaEditorBoncde PaEditorDevisSWT) {
		try {
			modelEnteteBoncde = new ModelGeneralObjet<IHMEnteteBoncde,TaBoncdeDAO,TaBoncde>(dao,classModel);
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());

//			modelEnteteFacture.remplirListe();
			if (modelEnteteBoncde.getListeObjet().isEmpty()) {
				modelEnteteBoncde.getListeObjet().add(new IHMEnteteBoncde());
			}
			selectedBoncde = modelEnteteBoncde.getListeObjet().getFirst();
			...
			bindingFormSimple(dbc, realm, selectedBoncde, classModel);
			bindAdresse();
			bindAdresseLiv();
			bindCPaiement();
...
//	}
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfCODE_DOCUMENT(), new InfosVerifSaisie(new TaBoncde(),Const.C_CODE_DOCUMENT,null));
		mapInfosVerifSaisie.put(vue.getTfCODE_TIERS(), new InfosVerifSaisie(new TaTiers(),Const.C_CODE_TIERS,null));
		mapInfosVerifSaisie.put(vue.getTfLIBELLE_DOCUMENT(), new InfosVerifSaisie(new TaBoncde(),Const.C_LIBELLE_DOCUMENT,null));
		
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse1(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_ADRESSE1_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse2(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_ADRESSE2_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfAdresse3(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_ADRESSE3_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl())
				.getTfCodePostal(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_CODEPOSTAL_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfVille(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_VILLE_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue.getExpandBar()
				.getItem(0).getControl()).getTfPays(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_PAYS_INFOS_DOCUMENT,null));

		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse1(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_ADRESSE1_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse2(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_ADRESSE2_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfAdresse3(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_ADRESSE3_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfCodePostal(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_CODEPOSTAL_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfVille(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_VILLE_LIV_INFOS_DOCUMENT,null));
		mapInfosVerifSaisie.put(((PaInfosAdresse) vue
				.getExpandBar().getItem(1).getControl())
				.getTfPays(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_PAYS_LIV_INFOS_DOCUMENT,null));
				
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfCODE_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_CODE_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfLIB_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_LIB_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfREPORT_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_REPORT_C_PAIEMENT,null));
		mapInfosVerifSaisie.put(((PaInfosCondPaiement) vue
				.getExpandBar().getItem(2).getControl())
				.getTfFIN_MOIS_C_PAIEMENT(), new InfosVerifSaisie(new TaInfosBoncde(),Const.C_FIN_MOIS_C_PAIEMENT,null));
...
//				.getItem(3).getControl()).getBtnReinitialiser(),C_COMMAND_DOCUMENT_REINIT_INFOSTIERS_ID);
	...
	public PaEditorBoncdeController getThis() {
		return this;
	}
	...
	public boolean onClose() throws ExceptLgr {
		...
		try{
			if (!vue.isDisposed() && !vue.getShell().isDisposed()) {
				...
				case C_MO_INSERTION:
				case C_MO_EDITION:
					boolean demande=!((IHMEnteteBoncde) selectedBoncde).BoncdeEstVide(taDocument.dateDansExercice(new Date()));
				...
				if (getFocusCourantSWT() instanceof Text) {
					try {
						...
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_TIERS())){
							...
							taDocument.setTaTiers(u);
							 String nomTiers=taDocument.getTaTiers().getNomTiers();
								((IHMEnteteBoncde)selectedBoncde).setLibelleDocument("Bon de commande N°"+taDocument.getCodeDocument()+" - "+nomTiers);
								
								if(vue.getCbTTC().isEnabled()){
									taDocument.setTtc(u.getTtcTiers());
									((IHMEnteteBoncde)selectedBoncde).setTtc(LibConversion.intToBoolean(taDocument.getTtc()));
								}
						...
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_DOCUMENT())){
							TaBoncde u = null;
							TaBoncdeDAO t = new TaBoncdeDAO(getEm());
							u = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
							t = null;
							taDocument=u;
	...
	public void calculDateEcheance(Boolean appliquer) {
		if(((IHMEnteteBoncde)selectedBoncde).getIdDocument()==0 || appliquer) {
			Integer report = null;
			...
			((IHMEnteteBoncde)selectedBoncde).setDateEchDocument(taDocument.calculDateEcheance(report, finDeMois));
		}
	}
	...
	private boolean changementTiers(boolean refresh) {
		...
		try {						
			...
			if(((IHMEnteteBoncde)selectedBoncde).getIdDocument()!=0 && 
				...
				if(!idTiersDocumentOriginal.equals(((IHMEnteteBoncde)selectedBoncde).getIdTiers())){
	...
	public void initialisationDesInfosComplementaires(Boolean recharger,String typeARecharger){
		 ...
		 * - le reste des adresses du tiers
		 */
		if(((IHMEnteteBoncde)selectedBoncde).getCodeDocument()!=null)
			taInfosDocument = daoInfosBoncde.findByCodeBoncde(((IHMEnteteBoncde)selectedBoncde).getCodeDocument());
		else
	...
	public void initialisationDesCPaiement(Boolean recharger) {
		...
		if (taDocument != null) {
			if (taDocument.getCodeDocument()!=null&& taDocument.getCodeDocument() != "")
				taInfosDocument = daoInfosBoncde
						.findByCodeBoncde(taDocument.getCodeDocument());
			else
				taInfosDocument = new TaInfosBoncde();

			TaTCPaiementDAO taTCPaiementDAO = new TaTCPaiementDAO(getEm());
			TaCPaiement taCPaiementDoc = null;
			if (taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_BON_COMMANDE) != null
...
//					dao.getEntityManager().clear();
					taDocument = new TaBoncde(true);
					mapperModelToUI.map(taDocument, (IHMEnteteBoncde) selectedBoncde);
					((IHMEnteteBoncde) selectedBoncde).setCodeDocument(dao.genereCode());
					...
					((IHMEnteteBoncde) selectedBoncde).setCommentaire(boncdePlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.COMMENTAIRE));

					validateUIField(Const.C_CODE_DOCUMENT,((IHMEnteteBoncde) selectedBoncde).getCodeDocument());
					taDocument.setCodeDocument(((IHMEnteteBoncde) selectedBoncde).getCodeDocument());
					changementTiers(true);
					Date date = new Date();
					...
					if (LibDate.compareTo(date, taInfoEntreprise.getDatedebInfoEntreprise()) == -1) {
						((IHMEnteteBoncde) selectedBoncde).setDateDocument(taInfoEntreprise.getDatedebInfoEntreprise());
						((IHMEnteteBoncde) selectedBoncde).setDateEchDocument(taInfoEntreprise.getDatedebInfoEntreprise());
						((IHMEnteteBoncde) selectedBoncde).setDateLivDocument(taInfoEntreprise.getDatedebInfoEntreprise());
					} else
						// si date supérieur à dateFin dossier
						if (LibDate.compareTo(date, taInfoEntreprise.getDatefinInfoEntreprise()) == 1) {
							((IHMEnteteBoncde) selectedBoncde).setDateDocument(taInfoEntreprise.getDatefinInfoEntreprise());
							((IHMEnteteBoncde) selectedBoncde).setDateEchDocument(taInfoEntreprise.getDatefinInfoEntreprise());
							((IHMEnteteBoncde) selectedBoncde).setDateLivDocument(taInfoEntreprise.getDatefinInfoEntreprise());
						} else {
							((IHMEnteteBoncde) selectedBoncde).setDateDocument(new Date());
							((IHMEnteteBoncde) selectedBoncde).setDateEchDocument(new Date());
							((IHMEnteteBoncde) selectedBoncde).setDateLivDocument(new Date());
						}
					((IHMEnteteBoncde) selectedBoncde).setCodeTiers("");
					((IHMEnteteBoncde) selectedBoncde).setIdAdresse(0);
					((IHMEnteteBoncde) selectedBoncde).setIdAdresseLiv(0);
					...
					if(typePaiementDefaut!=null){
						 ...
						 */
						if(taTPaiement!=null) {
							((IHMEnteteBoncde) selectedBoncde).setCodeTPaiement(typePaiementDefaut);
							taDocument.setTaTPaiement(taTPaiement);
						}
					}					
					mapperUIToModel.map(((IHMEnteteBoncde) selectedBoncde), taDocument);
...
//					setIhmOldEnteteDevis();
//					taDocument = dao.findById(((IHMEnteteBoncde) selectedBoncde).getIdDocument());
...
//				}else 
	...
	public Boolean setIhmOldEnteteDocumentRefresh(){
		try {			
			if (selectedBoncde != null){
				this.ihmOldEnteteBoncde = IHMEnteteBoncde.copy((IHMEnteteBoncde) selectedBoncde);
				if (taDocument!=null){
	...
	protected void actSupprimer() throws Exception {
		...
		try {
			...
			if (MessageDialog.openConfirm(vue.getShell(),
				...
				TaBoncde u = dao.findById(((IHMEnteteBoncde) selectedBoncde).getIdDocument());
				dao.supprimer(u);
...
//					listeDocument.remove(taDocument);
	...
	protected void actAnnuler(boolean annulationForcee,boolean insertion,boolean messageAffiche) throws Exception {
		...
		if (!vue.isDisposed() && !vue.getShell().isDisposed()) {
			...
			boolean message = messageAffiche && !((IHMEnteteBoncde) selectedBoncde).BoncdeEstVide(taDocument.dateDansExercice(new Date()));
					...
					case C_MO_EDITION:
						...
						case C_MO_EDITION:
							...
							if (repondu) {
								...
								dao.annulerCodeGenere(((IHMEnteteBoncde) selectedBoncde).getCodeDocument(),Const.C_CODE_DOCUMENT);
...
//				}
	...
	protected void actImprimer()throws Exception {
		...
		String simpleNameClass = TaBoncde.class.getSimpleName();
			
//		impressionBoncde.imprimerSelection(((SWTEnteteDevis) swtDevis.getEntete()).getID_DEVIS(),
//				swtDevis.getEntete().getCODE(), true);
		//impressionBoncde.imprimerSelection(TaBoncde.getIdDocument(),TaBoncde.getCodeDocument(), true);
//		Collection<TaBoncde> collectionTaBoncde = dao.selectAll();
		Collection<TaBoncde> collectionTaBoncde = new LinkedList<TaBoncde>();
//		taDocument.calculeTvaEtTotaux();
		collectionTaBoncde.add(taDocument);
		ConstEdition constEdition = new ConstEdition(getEm()); 
		constEdition.setFlagEditionMultiple(true);
//		Impression impression = new Impression(constEdition,taDocument);
//		impressionBoncde.setObject(taDocument);
//		impressionBoncde.setConstEdition(constEdition);
//		impressionBoncde.setCollection(collectionTaBoncde);
		
		Bundle bundleCourant = boncdePlugin.getDefault().getBundle();
		String namePlugin = bundleCourant.getSymbolicName();
		
//		impressionBoncde.imprimerSelection(taDocument.getIdDocument(),taDocument.getCodeDocument(),
//		true,ConstEdition.FICHE_FILE_REPORT_BONCDE,namePlugin,TaBoncde.class.getSimpleName());

		/** 01/03/2010 modifier les editions (zhaolin) **/
		baseImpressionEdition.setObject(taDocument);
		baseImpressionEdition.setConstEdition(constEdition);
		baseImpressionEdition.setCollection(collectionTaBoncde);
		...
		baseImpressionEdition.impressionEdition(preferenceStore,simpleNameClass,/*true,*/null,namePlugin,
												ConstEdition.FICHE_FILE_REPORT_BONCDE,
												true,false,true,false,false,false,"");
	}
	@Override
	protected boolean aideDisponible() {
		boolean result = false;
		switch ((PaEditorBoncdeController.this.dao.getModeObjet().getMode())) {
		case C_MO_CONSULTATION:
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				case C_MO_CONSULTATION:
					if (getFocusCourantSWT().equals(vue.getTfCODE_DOCUMENT())) {
						...
						editorCreationId = EditorBoncde.ID_EDITOR;
						editorInputCreation = new EditorInputBoncde();
						...
						paramAfficheAideRecherche.setControllerAppelant(PaEditorBoncdeController.this);
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<IHMAideBoncde,TaBoncdeDAO,TaBoncde>(dao,IHMAideBoncde.class));
...
//						l.add(TaBoncde.getTaCPaiement());
						if(taDocument.getTaTiers()!=null)
							l.add(taDocument.getTaTiers().getTaCPaiement());
						TaTCPaiementDAO taTCPaiementDAO = new TaTCPaiementDAO(getEm());
						if(taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_BON_COMMANDE)!=null
								&& taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_BON_COMMANDE).getTaCPaiement()!=null)
								l.add(taTCPaiementDAO.findByCode(TaTCPaiement.C_CODE_TYPE_BON_COMMANDE).getTaCPaiement());
			...
			try {
				...
				if(sourceDeclencheCommandeController==null) {
					...
					if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)
						...
						try{
							taDocument.setLegrain(false);
							for (TaLBoncde ligne : taDocument.getLignes()) {
...
//							retourPage=ChangementDePageEvent.DEBUT;
	...
	public IHMEnteteBoncde getIhmOldEnteteBoncde() {
		return ihmOldEnteteBoncde;
	}

	public void setIhmOldEnteteDevis(IHMEnteteBoncde ihmOldEnteteDevis) {
		this.ihmOldEnteteBoncde = ihmOldEnteteDevis;
	}

	public void setIhmOldEnteteDevis() {
		if (selectedBoncde != null)
			this.ihmOldEnteteBoncde = IHMEnteteBoncde
					.copy((IHMEnteteBoncde) selectedBoncde);
		else {
		}
	}

	public void setVue(PaEditorBoncde vue) {
		super.setVue(vue);
		this.vue = vue;
	}
...
//			}
	...
	protected void actRefresh() throws Exception {
		try{
			vue.setCursor(Display.getCurrent().getSystemCursor(SWT.CURSOR_WAIT));
		if (dao.dataSetEnModif()) {
			if (taDocument!=null && selectedBoncde!=null && (IHMEnteteBoncde) selectedBoncde!=null) {
				mapperModelToUI.map(taDocument, (IHMEnteteBoncde) selectedBoncde);
			}
		}
		listeDocument=null;
...
//				codeDevisContentProposal.setContentProposalProvider(contentProposalProviderDocument());
	...
	public PaLigneBoncdeController getControllerLigne() {
		return controllerLigne;
	}

	public void setControllerLigne(PaLigneBoncdeController controllerLigne) {
	...
	private Boolean remonterDocument(String codeDocument) throws Exception {
		try {
			...
			if(taDocument!=null) {
				factureVide = ((IHMEnteteBoncde) selectedBoncde).BoncdeEstVide(taDocument.dateDansExercice(new Date()));
			}
			TaBoncde documentRecherche = rechercheDevis(codeDocument, true, factureVide);
...
//				.isVerrouille();
//		String codeTmp = ((IHMEnteteBoncde) selectedBoncde)
//				.getCodeDevis();
//		try {
...
//							Const.C_CODE_DEVIS);
//				}
//				((IHMEnteteBoncde) selectedBoncde)
...
//
//				((IHMEnteteBoncde) selectedBoncde)
//						.setIHMEnteteBoncde(((SWTEnteteDevis) swtDevis
//								.getEntete()));
//
//				// récupérer code validé dans facture, dataset et interface
...
//			TaBoncdeDAO TaBoncdeDAO = new TaBoncdeDAO(getEm());
//			TaInfoEntrepriseDAO taInfoEntrepriseDAO = new TaInfoEntrepriseDAO(getEm());
//			TaInfoEntreprise taInfoEntreprise = taInfoEntrepriseDAO.findInstance();
//
//			//		List<TaBonliv> l = taBonlivDAO.selectAll();
//			//		if(listeDocument==null)
//			listeDocument =((IDocumentTiersDAO)TaBoncdeDAO).rechercheDocument(taInfoEntreprise.getDatedebInfoEntreprise(),
//					taInfoEntreprise.getDatefinInfoEntreprise());
//			TaBoncdeDAO = null;
//
//		}
////		List<TaBoncde> l = TaBoncdeDAO.selectAll();
...
//			+ " \r\n Echéance = " + LibDate.dateToStringAA(((TaBoncde)taDocument).getDateEchDocument())
//			+ " \r\n Exportée = " 
//			+LibConversion.booleanToStringFrancais(LibConversion.intToBoolean(((TaBoncde)taDocument).getExport()));
...
//	}
	public String[][] initAdapterDocument() {
		...
		if(affichageCtrlEspace){
		if(listeDocument==null){
			TaBoncdeDAO taDocDAO = new TaBoncdeDAO(getEm());
			TaInfoEntrepriseDAO taInfoEntrepriseDAO = new TaInfoEntrepriseDAO(getEm());
			TaInfoEntreprise taInfoEntreprise = taInfoEntrepriseDAO.findInstance();
...
//		//#AFAIRE
//		taTiersDAO.setPreferences(boncdePlugin.getDefault().getPreferenceStore().getString(fr.legrain.boncde.preferences.PreferenceConstants.TYPE_TIERS_DOCUMENT));
//		if(listeTiers==null)
...
//			a.codeTiers,a.nomTiers,a.codeCompta,a.compte,ttiers.libelleTTiers
	...
	protected void initEtatBouton(boolean initFocus) {
		initEtatBoutonCommand(initFocus,modelEnteteBoncde.getListeObjet());
...
//	}
	...
	public Date dateDansPeriode(Date newValue,String champ) throws Exception{
		if(champ.equals(Const.C_DATE_DOCUMENT)){
			...
			if(newValue!=null){
				if((((IHMEnteteBoncde)selectedBoncde).getDateEchDocument()!=null && 
						((IHMEnteteBoncde)selectedBoncde).getDateEchDocument().before(newValue))||
						((IHMEnteteBoncde)selectedBoncde).getDateEchDocument()==null ){
					taDocument.setDateEchDocument(newValue);
					((IHMEnteteBoncde)selectedBoncde).setDateEchDocument(newValue);
					}
//				if((((IHMEnteteBoncde)selectedBoncde).getDateLivDocument()!=null && 
//						((IHMEnteteBoncde)selectedBoncde).getDateLivDocument().before(newValue))||
//						((IHMEnteteBoncde)selectedBoncde).getDateLivDocument()==null ){
//					taDocument.setDateLivDocument(newValue);
//					((IHMEnteteBoncde)selectedBoncde).setDateLivDocument(newValue);
//					}
			}
		}
...
//	}
	...
	public PaEditorTotauxBoncdeController getControllerTotaux() {
		return controllerTotaux;
	}

	public void setControllerTotaux(PaEditorTotauxBoncdeController controllerTotaux) {
	...
	protected void actSelection() throws Exception {
		try{
			...
			paramAfficheSelectionVisualisation.setModule("bonCde");
			paramAfficheSelectionVisualisation.setNomClassController(nomClassController);
			paramAfficheSelectionVisualisation.setNomRequete(Const.C_NOM_VU_BONCDE_REDUIT);
	...
	public TaBoncde rechercheDevis(String codeDevis,boolean annule,boolean proformaVide) throws Exception {
		TaBoncde fact = null;
	...
	public TaBoncdeDAO getDao() {
		return dao;
	}

	public void setDao(TaBoncdeDAO dao) {
		this.dao = dao;
	}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "BONCDE";
		try {
			IStatus s = null;
...
//				}
				...
				TaBoncde u = new TaBoncde(true);
				PropertyUtils.setSimpleProperty(u, nomChamp, value);
				if(((IHMEnteteBoncde) selectedBoncde).getIdDocument()!=null) {
					u.setIdDocument(((IHMEnteteBoncde) selectedBoncde).getIdDocument());
				}
...
//					s = new Status(Status.OK,gestComBdPlugin.PLUGIN_ID,"OK");
			...
			if(s.getSeverity()!=Status.ERROR){
				if(nomChamp.equals(Const.C_DATE_DOCUMENT)){
					...
					((IHMEnteteBoncde)selectedBoncde).setDateDocument((Date)value);
					PropertyUtils.setSimpleProperty(taDocument, Const.C_DATE_DOCUMENT, value);
					if(changement){
						((IHMEnteteBoncde)selectedBoncde).setDateEchDocument(taDocument.getDateDocument());
						PropertyUtils.setSimpleProperty(taDocument, Const.C_DATE_ECH_DOCUMENT, taDocument.getDateDocument());
						((IHMEnteteBoncde)selectedBoncde).setDateLivDocument(taDocument.getDateDocument());
				...
				if(nomChamp.equals(Const.C_DATE_LIV_DOCUMENT)) {
					((IHMEnteteBoncde)selectedBoncde).setDateLivDocument((Date)value);
...
//		for (TaBoncde  document : listeDocument) {
