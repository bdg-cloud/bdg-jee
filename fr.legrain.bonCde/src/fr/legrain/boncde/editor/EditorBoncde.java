...
public class EditorBoncde extends JPALgrEditorPart implements ILgrEditor  {
	

	public static final String ID_EDITOR = "fr.legrain.editor.boncde.swt"; //$NON-NLS-1$
	static Logger logger = Logger.getLogger(EditorBoncde.class.getName());
	
	private boolean enabled = true;
	private PaEditorBoncde composite = null;
	private EntityManager em = null;
	
	
	public EditorBoncde(EditorPart parent) {
		super(parent);
	}
	
	public EditorBoncde(EditorPart parent,EntityManager em) {
		super(parent);
		this.em=em;
	}
	...
	public void createPartControl(Composite parent) {
    	composite = new PaEditorBoncde(parent,SWT.NONE);
		if(getController()==null) {
			setController(new PaEditorBoncdeController(composite,em));
		} else {
    ...
    class FenetrePart implements IPartListener{
		...
		public void partClosed(IWorkbenchPart part) {
			try {
				...
				if(part.getSite().getId().equals(EditorBoncde.ID_EDITOR)){
