...
public class SWTPaUniteController extends EJBBaseControllerSWTStandard
implements RetourEcranListener {

	static Logger logger = Logger.getLogger(SWTPaUniteController.class.getName());
	private PaUniteSWT vue = null;
	private ITaUniteServiceRemote dao = null;//new TaUniteDAO(getEm());

	private Object ecranAppelant = null;
	private TaUniteDTO TaUniteDTO;
	private TaUniteDTO swtOldUnite;
	private Realm realm;
	private DataBindingContext dbc;

	private Class classModel = TaUniteDTO.class;
	private ModelGeneralObjetEJB<TaUnite,TaUniteDTO> modelUnite = null;//new ModelGeneralObjet<TaUniteDTO,TaUniteDAO,TaUnite>(dao,classModel);
	...
	public SWTPaUniteController(PaUniteSWT vue) {
		this(vue,null);
	}

	public SWTPaUniteController(PaUniteSWT vue,EntityManager em) {
//		if(em!=null) {
//			setEm(em);
//		}
		try {
	...
	public void bind(PaUniteSWT paUniteSWT) {
		try {
...
//			tableViewer.setInput(writableList);
			...
			LgrViewerSupport.bind(tableViewer, 
					new WritableList(modelUnite.remplirListe(), classModel),
	...
	private void changementDeSelection() {
		if(selectedUnite!=null && selectedUnite.getValue()!=null) {
			...
			fireChangementDeSelection(new ChangementDeSelectionEvent(SWTPaUniteController.this));
		}
	}
	...
	public ResultAffiche configPanel(ParamAffiche param) {
		if (param != null) {
			if (((ParamAfficheUnite) param).getFocusDefautSWT() != null && !((ParamAfficheUnite) param).getFocusDefautSWT().isDisposed())
	...
	protected void initVerifySaisie() {
		...
		mapInfosVerifSaisie.put(vue.getTfLIBL_UNITE(), new InfosVerifSaisie(new TaUnite(),Const.C_LIBL_UNITE,null));
	...
	public SWTPaUniteController getThis() {
		return this;
	}
	...
	public boolean onClose() throws ExceptLgr {
		...
		if (retour) {
			...
			if (ecranAppelant instanceof SWTPaAideControllerSWT) {
				setListeEntity(getModelUnite().remplirListe());
			...
			if (getFocusAvantAideSWT() instanceof Text) {
				try {
					...
					if(getFocusAvantAideSWT().equals(vue.getTfCODE_FAMILLE())){
						...
						ITaFamilleUniteServiceRemote t = new EJBLookup<ITaFamilleUniteServiceRemote>().doLookup(JNDILookupClass.getInitialContext(),EJBLookup.EJB_NAME_TA_FAMILLE_UNITE_SERVICE, ITaFamilleUniteServiceRemote.class.getName());
						f = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
						t = null;
						taUnite.setTaFamilleUnite(f);
					}					
	...
	protected void actInserer() throws Exception {
		try {
			if(getModeEcran().getMode().compareTo(EnumModeObjet.C_MO_CONSULTATION)==0) {
				VerrouInterface.setVerrouille(true);
				setSwtOldUnite();
				TaUniteDTO = new TaUniteDTO();

				taUnite = new TaUnite();
//				dao.inserer(taUnite);
				modelUnite.getListeObjet().add(TaUniteDTO);
				writableList = new WritableList(realm, modelUnite.getListeObjet(), classModel);
				tableViewer.setInput(writableList);
				tableViewer.refresh();
				tableViewer.setSelection(new StructuredSelection(TaUniteDTO));
	...
	public boolean containsEntity(TaUnite entite){
		if(modelUnite.getListeEntity()!=null){
			for (Object e : modelUnite.getListeEntity()) {
				if(((TaUnite)e).getIdUnite()==
					entite.getIdUnite())return true;
			}
		}
		return false;
	}
	...
	public boolean setSwtOldUniteRefresh() {
		try {	
			if (selectedUnite.getValue()!=null){
				TaUnite taArticleOld =dao.findById(taUnite.getIdUnite());
//				taArticleOld=dao.refresh(taArticleOld);
				if(containsEntity(taUnite)) 
					modelUnite.getListeEntity().remove(taUnite);
				if(!taUnite.getVersionObj().equals(taArticleOld.getVersionObj())){
					taUnite=taArticleOld;
					if(!containsEntity(taUnite)) 
...
//		if (selectedUnite.getValue()!=null){
//			if(LgrMess.isDOSSIER_EN_RESEAU())dao.refresh(dao.findById(((TaUniteDTO) selectedUnite.getValue()).getIdUnite()));
//			taUnite=dao.findById(((TaUniteDTO) selectedUnite.getValue()).getIdUnite());
//			try {
...
//					dao.begin(transaction);
					TaUnite u = dao.findById(((TaUniteDTO) selectedUnite.getValue()).getId());
					dao.supprimer(u);
//					dao.commit(transaction);
					Object suivant=tableViewer.getElementAt(tableViewer.getTable().getSelectionIndex()+1);
					modelUnite.removeEntity(u);
					taUnite=null;
		...
		try {
			...
			case C_MO_INSERTION:
				if (silencieu || (!silencieu && MessageDialog.openQuestion(vue.getShell(), MessagesEcran
					...
					if(((TaUniteDTO) selectedUnite.getValue()).getId()==null){
						modelUnite.getListeObjet().remove(
								((TaUniteDTO) selectedUnite.getValue()));
						writableList = new WritableList(realm, modelUnite
			...
			case C_MO_EDITION:
				if (silencieu || (!silencieu && MessageDialog.openQuestion(vue.getShell(), MessagesEcran
						.getString("Message.Attention"), MessagesEcran
						.getString("Unite.Message.Annuler")))) {
					int rang = modelUnite.getListeObjet().indexOf(selectedUnite.getValue());
					remetTousLesChampsApresAnnulationSWT(dbc);
					modelUnite.getListeObjet().set(rang, swtOldUnite);
					writableList = new WritableList(realm, modelUnite.getListeObjet(), classModel);
					tableViewer.setInput(writableList);
					tableViewer.refresh();
					tableViewer.setSelection(new StructuredSelection(swtOldUnite), true);
...
//
//		fonctionGetInfosXmlAndProperties.setValueMapAttributeTable(TaUnite.class.getSimpleName()+".head");
//		fonctionGetInfosXmlAndProperties.setValueMapAttributeTable(TaUnite.class.getSimpleName()+".detail");
//		
//
//		LinkedHashMap<String,AttributElementResport> mapAttributeTablHead = fonctionGetInfosXmlAndProperties.getMapAttributeTablHead(); 
//		LinkedHashMap<String,AttributElementResport> mapAttributeTablDetail = fonctionGetInfosXmlAndProperties.getMapAttributeTablDetail();
//		
//		Collection<TaUnite> collectionTaUnite = modelUnite.getListeEntity();
//
//		fonctionGetInfosXmlAndProperties.findInfosFileXml(TaUnite.class.getName(),TaUniteDTO.class.getName(),
//				listeChamp,"mapping");
//		fonctionGetInfosXmlAndProperties.getInfosObjetJPA(taUnite);
//		
...
//					Const.C_NOM_PROJET_TMP+ConstEdition.SEPARATOR+TaUnite.class.getSimpleName();
...
//			DynamiqueReport.setNomObjet(TaUnite.class.getSimpleName());
//			/**************************************************************/
//
...
//
//			DynamiqueReport.setSimpleNameEntity(TaUnite.class.getSimpleName());
//			
...
////			impression.imprimer(true,pathFileReportDynamic,null,"Unite",TaUnite.class.getSimpleName(),false);
//			
//			BaseImpressionEdition impressionEdition = new BaseImpressionEdition(constEdition,taUnite,
//					getEm(),collectionTaUnite,taUnite.getIdUnite());
//			
	...
	protected void actAide(String message) throws Exception {
		if(aideDisponible()){
			try {
				...
				switch (SWTPaUniteController.this.getModeEcran().getMode()) {
				case C_MO_CONSULTATION:
					if(getFocusCourantSWT().equals(vue.getGrille())){
...
//						paramAfficheAideRecherche.setChampsIdentifiant(Const.C_ID_UNITE);
				...
				case C_MO_INSERTION:
					if(getFocusCourantSWT().equals(vue.getTfCODE_FAMILLE())){
						...
						ITaFamilleUniteServiceRemote dao = new EJBLookup<ITaFamilleUniteServiceRemote>().doLookup(JNDILookupClass.getInitialContext(),EJBLookup.EJB_NAME_TA_FAMILLE_UNITE_SERVICE, ITaFamilleUniteServiceRemote.class.getName());
						paramAfficheAideRecherche.setJPQLQuery(dao.getJPQLQuery());
						...
						paramAfficheAideRecherche.setControllerAppelant(SWTPaUniteController.this);
						ModelGeneralObjetEJB<TaFamilleUnite, TaFamilleDTO> modelFamilleUnite = new ModelGeneralObjetEJB<TaFamilleUnite, TaFamilleDTO>(dao);
						paramAfficheAideRecherche.setModel(modelFamilleUnite);
...
//				if (paramAfficheAideRecherche.getJPQLQuery() != null) {
					...
					paAideController.addRetourEcranListener(SWTPaUniteController.this);
...
//				}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		try {
			...
			AbstractApplicationDAOClient<TaUniteDTO> a = new AbstractApplicationDAOClient<TaUniteDTO>();
...
//				dao = null;
				...
				try {
					if(TYPE_VALIDATION==VALIDATION_CLIENT || TYPE_VALIDATION==VALIDATION_CLIENT_ET_SERVEUR) {
						AbstractApplicationDAOClient<TaFamilleUniteDTO> validationClient = new AbstractApplicationDAOClient<TaFamilleUniteDTO>();
						PropertyUtils.setSimpleProperty(dto, nomChamp, value);
						validationClient.validate(dto,nomChamp,ITaUniteServiceRemote.validationContext);
					}
					if(resultat==null && (TYPE_VALIDATION==VALIDATION_SERVEUR || TYPE_VALIDATION==VALIDATION_CLIENT_ET_SERVEUR)) {
						PropertyUtils.setSimpleProperty(dto, nomChamp, value);
						dao.validateDTOProperty(dto, nomChamp,ITaUniteServiceRemote.validationContext);
					}
				} catch(Exception e) {
				...
				if(resultat!=null && resultat.getSeverity()!=IStatus.ERROR ){
					...
					taUnite.setTaFamilleUnite(entity);
				}
				dao = null;
			} else {
...
//				if(((TaUniteDTO) selectedUnite.getValue()).getIdUnite()!=null) {
//					u.setIdUnite(((TaUniteDTO) selectedUnite.getValue()).getIdUnite());
//				}
...
//				}
				try {
					TaUniteDTO u = new TaUniteDTO();
					//u.setTaTiers(masterEntity);
					PropertyUtils.setSimpleProperty(u, nomChamp, value);
					dao.validateDTOProperty(u,nomChamp,ITaUniteServiceRemote.validationContext);
				} catch(Exception e) {
					resultat = new Status(IStatus.ERROR,ArticlesPlugin.PLUGIN_ID, e.getMessage(), e);
				}
			}
...
//				
//				if (modeEcran.getMode().compareTo(EnumModeObjet.C_MO_EDITION) == 0)
//					taUnite=dao.enregistrerMerge(taUnite);
//				else if (modeEcran.getMode().compareTo(EnumModeObjet.C_MO_INSERTION) == 0)
//					dao.enregistrerPersist(taUnite);
					
//mapper sur serveur, envoi d'un DTO					
					if (modeEcran.getMode().compareTo(EnumModeObjet.C_MO_EDITION) == 0)
						dao.enregistrerMergeDTO((TaUniteDTO) selectedUnite.getValue(),ITaUniteServiceRemote.validationContext);
					else if (modeEcran.getMode().compareTo(EnumModeObjet.C_MO_INSERTION) == 0)
						dao.enregistrerPersistDTO((TaUniteDTO) selectedUnite.getValue(),ITaUniteServiceRemote.validationContext);
				
				
			} 
			
//			dao.commit(transaction);
			modelUnite.addEntity(taUnite);
...
//			transaction = null;
			...
			afficheDecoratedField(vue.getTfCODE_UNITE(),e.getMessage());
		} finally {
//			if(transaction!=null && transaction.isActive()) {
//				transaction.rollback();
//			}
	...
	public boolean isUtilise(){
		...
		!dao.recordModifiable(dao.getNomTable(),
				...
				!dao.autoriseModification(taUnite);		
	}
	public TaUniteDTO getSwtOldUnite() {
		return swtOldUnite;
	}

	public void setSwtOldUnite(TaUniteDTO swtOldUnite) {
		this.swtOldUnite = swtOldUnite;
	}
	...
	public void setVue(PaUniteSWT vue) {
		super.setVue(vue);
		this.vue = vue;
	}
	...
	protected void initMapComposantDecoratedField() {
		...
		mapComposantDecoratedField.put(vue.getTfLIBL_UNITE(), vue.getFieldLIBL_UNITE());
	...
	protected void actRefresh() throws Exception {
		...
		} else if(selectedUnite!=null && (TaUniteDTO) selectedUnite.getValue()!=null) {
		...
		if (getModeEcran().getMode().compareTo(EnumModeObjet.C_MO_CONSULTATION) == 0) {
			...
			if (taUnite!=null && selectedUnite!=null && (TaUniteDTO) selectedUnite.getValue()!=null) {
				new LgrDozerMapper<TaUnite, TaUniteDTO>().
					map(taUnite, (TaUniteDTO) selectedUnite.getValue());
			}
		}

		if(idActuel!=0) {
			tableViewer.setSelection(new StructuredSelection(modelUnite.recherche(Const.C_ID_UNITE, idActuel)));
		}else
			tableViewer.selectionGrille(0);

	}

	public ModelGeneralObjetEJB<TaUnite,TaUniteDTO> getModelUnite() {
		return modelUnite;
	}
	...
	public void actAfficherTous() throws Exception{
		...
		modelUnite.setJPQLQuery(null);
		modelUnite.setListeEntity(null);
		try {
			actRefresh();
		} catch (Exception e1) {
			logger.error("", e1);
