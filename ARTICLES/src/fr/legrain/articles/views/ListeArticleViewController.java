...
public class ListeArticleViewController implements ILgrListView<TaArticle>, RetourEcranListener {
	
	static Logger logger = Logger.getLogger(ListeArticleViewController.class.getName());
	...
	private String nomClass = SWTPaArticlesController.class.getSimpleName();
	...
	public ListeArticleViewController(PaListeArticleView vue, ViewPart view) {
		/*
...
//		contributeToActionBars();
	...
	private void initController()	{
		try {	
			vue.getTfCodeArticle().addKeyListener(new KeyListener() {
				...
				public void keyPressed(KeyEvent e) {
					if(e.keyCode == SWT.CR || e.keyCode == SWT.KEYPAD_CR) {
						...
						TaArticleDTO tiersRetour = modelArticle.recherche(Const.C_CODE_ARTICLE, vue.getTfCodeArticle().getText().toUpperCase(), true);
	...
	private void bind() {
		try {
			...
			ModelArticles.MyContentProvider cp = modelArticle.initCP(vue.getLgrViewer().getViewer(),view,vue.getBar(),vue.getBarContainer(),vue.getLgrViewer());
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				ListeArticleViewController.this.fillContextMenu(manager);
			}
		});
...
//			}
			...
			public void run() {
				if(EJBAbstractLgrMultiPageEditor.isEditeurOuvert(ArticleMultiPageEditor.ID_EDITOR)) {
...
//		actionSupprimer.setImageDescriptor(LibrairiesEcranPlugin.getImageDescriptor("/icons/delete.png"));
	...
	public void update(TaArticle TaArticle) {

		modelArticle.addToModel(vue.getLgrViewer().getViewer(), TaArticle);
	}
	
	public void refresh(TaArticle TaArticle) {

		modelArticle.refreshModel(vue.getLgrViewer().getViewer(), TaArticle);
	}
	
	public void remove(TaArticle TaArticle) {

		modelArticle.removeFromModel(vue.getLgrViewer().getViewer(), TaArticle);
	}

	@Override
	public void select(TaArticle t) {
		if(t!=null) {
			TaArticleDTO tiers = modelArticle.recherche(Const.C_ID_DTO_GENERAL, t.getIdArticle());
...
//				paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTTiers, TaArticleDAO, TaArticle>(SWTTiers.class,new TaArticleDAO().getEntityManager()));
	...
	public void retourEcran(RetourEcranEvent evt) {
		if (evt.getRetour() != null
			...
			if(((ResultAffiche) evt.getRetour()).getSelection()!=null){
				TaArticleDTO tiersRetour =modelArticle.recherche(Const.C_ID_DTO_GENERAL, LibConversion.stringToInteger(((ResultAffiche) evt.getRetour()).getIdResult())); 
				if(tiersRetour!=null){
					vue.getLgrViewer().getViewer().setSelection(new StructuredSelection(tiersRetour),true);
				}
			}
