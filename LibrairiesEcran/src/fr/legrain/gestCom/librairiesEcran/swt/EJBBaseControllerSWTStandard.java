...
public abstract class EJBBaseControllerSWTStandard extends EJBBaseControllerSWT implements
...
//	private EntityManager em = null;
	...
	static Logger logger = Logger.getLogger(EJBBaseControllerSWTStandard.class.getName());
	//protected AbstractLgrDAO daoStandard=null;
	protected IGenericCRUDServiceRemote daoStandard = null;
	protected ModeObjetEcran modeEcran = new ModeObjetEcran();
	protected ModeObjetEcran masterModeEcran = new ModeObjetEcran();
...
//	protected EventListenerList listenerList = new EventListenerList();
	...
	protected Map<Control,ControlDecoration> mapComposantDecoratedField = null;
	...
	protected Map<EnumModeObjet,Control> mapInitFocus = null;
...
//	abstract protected void initEtatBouton();
	...
	public EJBBaseControllerSWTStandard() {
		super();
	}
...
//		}
	...
	public static String getLookupName(String appName,String moduleName,
		...
		return EJBLookupName.getLookupName(appName, moduleName, distinctName, beanName, interfaceName);
	}
	
	public static String getLookupName(String moduleName,
			String beanName,final String interfaceName ) {
		
		return EJBLookupName.getLookupName(moduleName, beanName, interfaceName);
	}
	
	public static String getLookupName(String beanName,final String interfaceName ) {
		
		return EJBLookupName.getLookupName(beanName, interfaceName);
	}
...
//				uiToModel.setAfterConvertValidator(new EJBCtrlInterface(this,mapComposantChamps.get(c)));
				uiToModel.setAfterConvertValidator(new BeanValidatorJFaceDatabinding(selectedObjectClass, mapComposantChamps.get(c), this));
			}
	...
	public void bindingFormSimple(DataBindingContext dbc, Realm realm, Object selectedObject, Class selectedObjectClass) {
		bindingFormSimple(mapComposantChamps, dbc, realm, selectedObject, selectedObjectClass);
	}
...
//				}
				...
				//branchement des validators apres le binding pour eviter le 1er controle qui risque d'etre faux
				uiToModel.setAfterConvertValidator(new EJBCtrlInterface(this,nomChamp));
			}
		} catch (SecurityException e) {
			logger.error("",e);
		} catch (NoSuchFieldException e) {
	...
	public void retourEcran(RetourEcranEvent evt) {
		...
		vue.getDisplay().asyncExec(new Runnable() {
			public void run() {
				...
				else initFocusSWT(modeEcran, mapInitFocus);	
			}			
		}); 
		} finally {				
...
//		}
	 ...
	 */
	protected void initVerifyListener(Map<Control,InfosVerifSaisie> mapComposantInfosVerif, IGenericCRUDServiceRemote dao) {
	 ...
	 */
	protected void initVerifyListener(Map<Control,InfosVerifSaisie> mapComposantInfosVerif,IGenericCRUDServiceRemote dao, boolean verifieLongueurTexte) {
		try {
						...
						if(mapComposantInfosVerif.get(control).getListeControles()!=null) {
							...
							for (int ctrl : mapComposantInfosVerif.get(control).getListeControles()) {
								...
								if(ctrl==InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_POSITIFS) {
									((Text)control).addVerifyListener(new QueDesChiffresPositif(annotation.champBd(),annotation.table(), dao));
									numerique = true;
								}
								if(ctrl==InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES) {
									((Text)control).addVerifyListener(new QueDesChiffres(annotation.champBd(),annotation.table(), dao));
									numerique = true;
								}
								if(ctrl==InfosVerifSaisie.CTRL_QUE_DES_CHIFFRES_INTERVALE) {
									((Text)control).addVerifyListener(new QueDesChiffresIntervaleDecimale(annotation.champBd(),annotation.table(), dao));
									numerique = true;
								}
								if(((ctrl==InfosVerifSaisie.CTRL_LONGUEUR_TEXTE) || verifieLongueurTexte) && !numerique) {
									int longueurMax =0;
									longueurMax= dao.getDbMetaDataLongeur(annotation.table(), annotation.champBd());
...
//				containsKey(ibTaTableStandard.getNomTable() + "." +nomChamp));
	...
	public void activeModifytListener() {
		activeModifytListener(mapComposantChamps/*,modeEcran*/);
	}
...
//		annulerListeContext();
	...
	public IGenericCRUDServiceRemote getDaoStandard() {
		return daoStandard;
	}
	public void setDaoStandard(IGenericCRUDServiceRemote daoStandard) {
...
//					false);
			...
			if(FieldDecorationRegistry.getDefault().getFieldDecoration(monIdImageTest)==null) {
...
//			);
			if (!getActiveAide()){
				ControlDecoration deco = mapComposantDecoratedField.get(control);
				if(deco!=null) {
					Image image = FieldDecorationRegistry.getDefault().getFieldDecoration(monIdImageTest).getImage();
					...
					logger.error("Pas de champ décoré pour "+control.getToolTipText()+" : "+message);
				}
			}
	...
	public void hideDecoratedField(Control control,Map<Control,String>mapComposantChamps){
		if (control!=null){	
			//if(registry.getFieldDecoration("error.field."+mapComposantChamps.get(control))!=null){
				...
				mapComposantDecoratedField.get(control).hide();
...
//					if(registry.getFieldDecoration("error.field."+mapComposantChamps.get(control))!=null){
						//mapComposantDecoratedField.get(control).hideDecoration(registry.getFieldDecoration("error.field."+mapComposantChamps.get(control)));
						if(mapComposantDecoratedField.get(control)!=null) {
							mapComposantDecoratedField.get(control).hide();
							mapComposantDecoratedField.get(control).getControl().setToolTipText("");
						} else {
							System.out.println("Pas de champ décoré pour "+control);
						}
//					}
				}
			}
		...
		for (final Object binding : dbcStandard.getValidationStatusMap().keySet()) {
			...
			if (((IStatus) dbcStandard.getValidationStatusMap().get(binding)).getSeverity() == IStatus.ERROR) {
				// mettre le focus sur le champ en question
					...
					IStatus[] listChild = null;
					if(((IStatus) dbcStandard.getValidationStatusMap().get(binding)).isMultiStatus()) {
						listChild = ((IStatus) dbcStandard.getValidationStatusMap().get(binding)).getChildren();
					} else {
						listChild = new IStatus[] { ((IStatus) dbcStandard.getValidationStatusMap().get(binding)) };
					}
					String messageErreur="";
					if(listChild[0].isMultiStatus()) {
						messageErreur = LibConversion.StringNotNull(listChild[0].getChildren()[0].getMessage());
					} else {
						messageErreur = LibConversion.StringNotNull(listChild[0].getMessage());
					}
				...
				afficheDecoratedField(((Control) ((ISWTObservable) ((Binding) binding)
						.getTarget()).getWidget()),messageErreur ,mapComposantChamps);
				throw new Exception();
			}else{
				try {
...
//								.getTarget()).getWidget()), status.getChildren()[0].getMessage());
						try {
							if (multiStatus != null
									&& multiStatus.getMessage().equals(EJBCtrlInterface.C_CONTROLE_INTERFACE)) {
...
//	}
	...
	public void ouvreDocument(String valeurIdentifiant, String idEditor,Boolean message) {
		EJBAbstractLgrMultiPageEditor.ouvreFiche(null,valeurIdentifiant, idEditor,null,message);
...
//		//return new Date();
	...
	public IEditorPart ouvreFiche(String valeurIdentifiant, String idEditor,ParamAffiche paramAfficheIn,Boolean message) {
		return EJBAbstractLgrMultiPageEditor.ouvreFiche(valeurIdentifiant, null, idEditor,paramAfficheIn,message);
	}
	...
	public ModeObjetEcran getModeEcran() {
		return modeEcran;
	}
	public void setModeEcran(ModeObjetEcran modeEcran) {
		this.modeEcran = modeEcran;
	}
	public ModeObjetEcran getMasterModeEcran() {
		return masterModeEcran;
	}
	public void setMasterModeEcran(ModeObjetEcran masterModeEcran) {
		this.masterModeEcran = masterModeEcran;
	}
}
