...
import fr.legrain.document.etat.devis.ecrans.ToolTipDetail;
...
public class DocEcheanceController extends AbstractControllerMini {
	...
	public void initialiseModelIHM(int nbResult) {
		...
		if(vue.getCompositeSectionParam().getTfCodeTiers().getText().equals("")) {
			listeDoc = taDevisDAO.rechercheDocumentEtat(
					LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb()),
					datefin,
					masterController.paramControllerMini.getMapEtatCodeLibelle().get(masterController.paramControllerMini.getCodeEtat()));
		} else {
			listeDoc = taDevisDAO.rechercheDocumentEtat(
					LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb()),
					datefin,
		...
		for (int i = 0; i < listeDoc.size() /*&& i < nbResult*/; i ++){
			...
			documentSelection = new DocumentSelection(
					...
					object.getMtHtCalc(),
					object.getMtTtcCalc(),
					object.getTaEtat()!=null?object.getTaEtat().getLibEtat():masterController.paramControllerMini.etatEnCoursLibelle);

			for (TaLDevis l : object.getLignes()) {
		...
		if(vue.getCompositeSectionParam().getCbEtat().getText().equals(masterController.paramControllerMini.etatEnCoursLibelle)) {
			libelleEcran = "Liste des devis en cours arrivant à échéance avant le "
					+ LibDate.dateToString(datefin) + " ("+listDocumentSelection.size()+" documents)";
		} else {
			libelleEcran =
					"Liste des devis arrivant à échéance entre le "
	...
	public void changeEtat(String v) {
		...
		TaDevis taDevis =  taDevisDAO.findByCode(doc.code);
		TaEtat taEtat = null;

		if(codeNouvelEtat!=null)
			taEtat = taEtatDAO.findByCode(codeNouvelEtat);

		EntityTransaction transaction = taDevisDAO.getEntityManager().getTransaction();
		try {			
			taDevis.setLegrain(false); //Pour éviter setTaArticle sur les lignes suite au merge et donc la réinitialisation des informations de la ligne y compris pour les quantités
			taDevis.setTaEtat(taEtat);
			taDevisDAO.begin(transaction);
			

			//if ((taDevisDAO.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
			taDevisDAO.enregistrerMerge(taDevis);
			//} 
			taDevisDAO.commit(transaction);
			transaction = null;
	...
	public final class ExampleEditingSupport extends ObservableValueEditingSupport {
		...
		private ExampleEditingSupport(ColumnViewer viewer) {
			...
			cellEditor.setInput(new WritableList(masterController.paramControllerMini.getMapEtatCodeLibelle().keySet(),String.class));
