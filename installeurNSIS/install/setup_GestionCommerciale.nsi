...
; ----------------------------------------
; ---------------------------------------------------------------------------------------------------------------
; - Fichier a ne surtout pas mettre en unicode (UTF-8),
; - il doit rester en encodage Windows cp1252, ou il faut utiliser une version de nsis qui supporte l'unicode.
; - Idem pour le fichier licence.txt
; ---------------------------------------------------------------------------------------------------------------
...
;"Constantes"
!ifndef OUT_FILE
!define OUT_FILE "Setups\setup_cogere\a_tester\setup_BdG_Cogere_a_tester.exe"
!endif
!ifndef REPERTOIRE_EXPORT_RCP
!define REPERTOIRE_EXPORT_RCP "/var/www/projetPHP/plugins_features/tmp_zip/BG/"
!endif
!ifndef REPERTOIRE_EXPORT_RCP_UTIL
!define REPERTOIRE_EXPORT_RCP_UTIL "/var/www/projetPHP/plugins_features/tmp_zip/BG/utilitaires/"
!endif
!ifndef NOM_REPERTOIRE_UTIL_RESTAURATION
!define NOM_REPERTOIRE_UTIL_RESTAURATION "restauration"
!endif
!define INSTALLEUR_JRE "jre-6u1-windows-i586-p.exe"
!define INSTALLEUR_SHOWMYPC "ShowMyPC3010.exe"
!define INSTALLEUR_FIREBIRD "Firebird-2.1.1.17910-0_Win32.exe"
...
!define FIREBIRD_BIN_DIRECTORY "C:\Program Files\Firebird\Firebird_2_1\bin"
!define NOM_REPERTOIRE_SORTIE_UTIL "util"

; HM NIS Edit Wizard helper defines
!define PRODUCT_NAME "Bureau de gestion"
!define PRODUCT_VERSION "2.0"
!define PRODUCT_VERSION_DETAIL "2.0.13.0"
...
;LangString DESC_SEC__DESINSTALL_BUREAU_DE_GESTION ${LANG_FRENCH} "Désinstallation Bureau de gestion."
...
;------------------------------- Sections -------------------------------------------
Section "-D?sinstallation Bureau de gestion" SEC_DESINSTALL_BUREAU_DE_GESTION
  SectionIn RO ;toujours coche
  DetailPrint "Désinstallation du bureau de gestion : "
  DetailPrint "Veuillez patienter ..."
  SetDetailsPrint none
  call desinstallBDG 
  SetDetailsPrint both
  DetailPrint "Désinstallation du bureau de gestion terminée."
...
Section "Bureau de gestion" SEC_BUREAU_DE_GESTION
  ...
  ;File "$PLUGINSDIR\install_showmypc.exe"
  File "${INSTALLEUR_SHOWMYPC}"
  ;File "..\Sources\TransDocW.exe" 
  ${registerExtension} "$INSTDIR\GestionCommerciale.exe" ".bdg" "DossierBDG"
...
Section /o "Java" SEC_JAVA
  ...
  ;MessageBox MB_OK "retour java $1"           ; meme version deja installe = 1603
SectionEnd
...
Section "-addDatabaseUser" ;section cachee et toujours executee
...
Section "-updateDB" ;section cachee et toujours executee
...
Section -AdditionalIcons
  ...
  CreateDirectory "$SMPROGRAMS\Legrain\Bureau de gestion"
  CreateDirectory "$SMPROGRAMS\Legrain\Utilitaires"
  CreateShortCut "$SMPROGRAMS\Legrain\Bureau de gestion\Le Grain.lnk" "$INSTDIR\${PRODUCT_PUBLISHER}.url"
  CreateShortCut "$SMPROGRAMS\Legrain\Bureau de gestion\logiciel-gestion.fr.lnk" "$INSTDIR\${PRODUCT_NAME}.url"
  CreateShortCut "$SMPROGRAMS\Legrain\Bureau de gestion\Désinstallation.lnk" "$INSTDIR\uninst.exe"
  SetOutPath "$INSTDIR"
  CreateShortCut "$SMPROGRAMS\Legrain\Bureau de gestion\Bureau de gestion.lnk" "$INSTDIR\GestionCommerciale.exe"
  CreateShortCut "$DESKTOP\Bureau de gestion.lnk" "$INSTDIR\GestionCommerciale.exe"     

  CreateShortCut "$SMPROGRAMS\Legrain\Utilitaires\ShowMyPC.lnk" "$INSTDIR\${INSTALLEUR_SHOWMYPC}"
  CreateShortCut "$DESKTOP\ShowMyPC.lnk" "$INSTDIR\${INSTALLEUR_SHOWMYPC}"
...
Function .onInit
  ...
  File /oname=$PLUGINSDIR\install_showmypc.exe "${INSTALLEUR_SHOWMYPC}"
...
Function un.onUninstSuccess
  HideWindow
  MessageBox MB_ICONINFORMATION|MB_OK "$(^Name) a été désinstallé avec succés de votre ordinateur."
FunctionEnd

Function un.onInit
  MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "Etes-vous certains de vouloir désinstaller totalement $(^Name) et tous ses composants ?" IDYES +2
  Abort
FunctionEnd
...
Function supprFirebird
  ...
  ${If} $0 <> 0
    ...
    nsSCM::QueryStatus "${FIREBIRD_GUARDIAN_SERVICE_NAME}"
    Pop $0
    Pop $1 
    ;MessageBox MB_OK 'status du service $1'
    ${If} $1 <> 1
      DetailPrint "Service Firebird lancé, arrêt du service en cours ..."
      ;nsSCM::Stop "${FIREBIRD_GUARDIAN_SERVICE_NAME}"
      ExecWait 'net stop "${FIREBIRD_GUARDIAN_SERVICE_NAME}"' ;
      DetailPrint "Service Firebird arrêté"
    ${EndIf}
  
    ;nsSCM::QueryStatus "${FIREBIRD_GUARDIAN_SERVICE_NAME}"
    ;Pop $0
    ;Pop $1 
    ;MessageBox MB_OK 'status du service avant suppression $1'
  
    ;Nouvelle recherche du désinstalleur (car variable $0 changer depuis la 1ere recherche)
    ${locate::Open} "$PROGRAMFILES\Firebird" "/F=1 /D=0 /M=unins000.exe" $0
...
Function addAttenteLancementFirebird
  ...
  ; Utilise Version pour detecter la version de Windows (http://nsis.sourceforge.net/Detect_Windows_Version)
...
Function desinstallBDG
  Delete "$INSTDIR\${PRODUCT_PUBLISHER}.url"
  Delete "$INSTDIR\${PRODUCT_NAME}.url"
  Delete "$INSTDIR\uninst.exe"  
  ;------------------------------------------------------------------------
  ; uniquement present dans la desistallation avant une install
  ;------------------------------------------------------------------------
  Delete "$DESKTOP\Restauration.lnk"
  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\Restauration.lnk"
  RMDir /r "$INSTDIR\util"
  ;------------------------------------------------------------------------


  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\Désinstallation.lnk"
  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\Le Grain.lnk"
  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\logiciel-gestion.fr.lnk"
  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\Bureau de gestion.lnk"
  Delete "$DESKTOP\Bureau de gestion.lnk"
  
  Delete "$SMPROGRAMS\Legrain\Utilitaires\ShowMyPC.lnk" 
  Delete "$DESKTOP\ShowMyPC.lnk" 

  RMDir "$SMPROGRAMS\Legrain\Bureau de gestion"
  ...
  Delete "$INSTDIR\GestionCommerciale.exe"
  Delete "$INSTDIR\GestionCommerciale.ini"
  Delete "$INSTDIR\artifacts.xml"
  Delete "$INSTDIR\log_gestCom.log"
  Delete "$INSTDIR\${INSTALLEUR_SHOWMYPC}"
...
Section Uninstall
  ...
  Delete "$INSTDIR\${PRODUCT_NAME}.url"
  Delete "$INSTDIR\uninst.exe"
  ${unregisterExtension} ".bdg" "DossierBDG"

  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\D?sinstallation.lnk"
  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\Le Grain.lnk"
  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\logiciel-gestion.fr.lnk"
  Delete "$SMPROGRAMS\Legrain\Bureau de gestion\Bureau de gestion.lnk"
  Delete "$DESKTOP\Bureau de gestion.lnk"
