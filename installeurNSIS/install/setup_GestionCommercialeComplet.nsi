...
;"Constantes"
!define REPERTOIRE_EXPORT_RCP "C:\Workspace_trunk\installeurNSIS\install\Build_GestionCommercialeComplet\"
!define REPERTOIRE_EXPORT_RCP_UTIL "C:\Workspace_trunk\installeurNSIS\install\utilitaires\"
...
!define PRODUCT_VERSION_DETAIL "1.8.7.1"
...
XPStyle on
OutFile "Setups\setup_general\a_tester\setup_BdG_Complet_a_tester.exe"
...
;------------------------------- Sections -------------------------------------------
Section "-Désinstallation Bureau de gestion" SEC_DESINSTALL_BUREAU_DE_GESTION
...
Section "-updateDB" ;section cachée et toujours exécutée
    ...
    ;Exec '"C:\Program Files\Firebird\Firebird_2_1\bin\isql.exe" -i $PLUGINSDIR\script_updateDB.sql'
    SetDetailsPrint both
    Goto End
    NotMaj:
...
Function supprFirebird
  ...
  ${locate::Open} "$PROGRAMFILES\Firebird" "/F=1 /D=0 /M=unins000.exe" $0
  ${If} $0 <> 0
    ;MessageBox MB_OK '$PROGRAMFILES\Firebird\Firebird_2_0'
    ;MessageBox MB_OK '$0'
    ${locate::Find} "$0" $1 $2 $3 $4 $5 $6
    ;MessageBox MB_OK '$1'
    ExecWait '$1 /VERYSILENT';
    ;ExecWait '$PROGRAMFILES\Firebird\Firebird_2_0\unins000.exe';
    RMDir /r "$PROGRAMFILES\Firebird\"
    ;MessageBox MB_OK 'stop2'
  ${EndIf}
FunctionEnd
...
Section Uninstall
  ...
  RMDir /r "$INSTDIR\jre"
  RMDir /r "$INSTDIR\.eclipseproduct"
  RMDir /r "$INSTDIR\GestionCommerciale.exe"
  RMDir /r "$INSTDIR\GestionCommerciale.ini"
  RMDir /r "$INSTDIR\log_gestCom.log"
