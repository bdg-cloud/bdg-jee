...
public class GraphQuantite2ControllerMini extends AbstractControllerMini {
	
	static Logger logger = Logger.getLogger(GraphQuantite2ControllerMini.class.getName());	
	...
	public GraphQuantite2ControllerMini(DefaultFormPageController masterContoller, DefaultFormPage vue, EntityManager em) {
		this.vue = vue;
	...
	public void initialiseModelIHM(TaArticle masterEntity,TaArticleDAO masterDAO) {
		...
		for (int i = 0; i < vue.getCompositeSectionGraphQuantite2().getCompo().getChildren().length; i++) {
			vue.getCompositeSectionGraphQuantite2().getCompo().getChildren()[i].dispose();
		}
...
//					&& ((Object[])object)[4]!=null /*&& !((String)((Object[])object)[4]).equals("")*/
		...
		MultiSerieBarChart multiSerieBarChart = new MultiSerieBarChart(vue.getCompositeSectionGraphQuantite2().getCompo(), SWT.NONE, 
				dataSetChart,"Quantités 2 facturées",ChartDimension.TWO_DIMENSIONAL_WITH_DEPTH_LITERAL,titreAbscisse,"Quantité");

		multiSerieBarChart.build();
		vue.getCompositeSectionGraphQuantite2().getCompo().layout();
	}
