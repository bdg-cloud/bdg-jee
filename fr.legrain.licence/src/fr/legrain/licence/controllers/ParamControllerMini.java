...
import fr.legrain.gestCom.librairiesEcran.swt.JPABaseControllerSWTStandard.DateTraverse;
...
import fr.legrain.licence.ecrans.PaCompositeSectionParam;
...
//	public static final String etatEnCoursCode = "ENCOURS";
	...
	private Map<String,String> mapTArticle = new HashMap<String, String>();
	...
	public void actCreer(IStructuredSelection selection,final ParamAffiche param ){
		try{
			...
			if(documentSelection!=null && documentSelection.getCodeArticle()!=null){
				...
				((SWTSupportAbonLogiciel)param.getSelected()).setCodeFamille(documentSelection.getGroupe());
				((SWTSupportAbonLogiciel)param.getSelected()).setCodeEntite(documentSelection.getEntite());
				((SWTSupportAbonLogiciel)param.getSelected()).setEntreprise(documentSelection.getEntreprise());
				((SWTSupportAbonLogiciel)param.getSelected()).setCommercial(documentSelection.getCommercial());
	...
	private Action refreshAction = new Action("Rechercher",pluginLicence.getImageDescriptor(PaCompositeSectionParam.iconPath)) { 
		@Override 
		public void run() { 
			datedeb = LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb());
...
//						String contributorName = confElements[jj].getContributor().getName();
						if(classNameSupport != null){
							TSupport=daoTSupport.findById(LibConversion.stringToInteger(classNameSupport));
						}
						...
						if(classEditor!=null && classTabItem!=null && ClassEditorName!=null){
							listeGestionnaireEditorSupportAbon.put(TSupport.getCodeTSupport(),classEditor);
						}
	...
	protected void initActions() {	
		if(!toolBarInitialise) {
			...
			vue.getCompositeSectionParam().getBtnRefesh().setImage(pluginLicence.getImageDescriptor(PaCompositeSectionParam.iconRefreshPath).createImage());
			...
			vue.getCompositeSectionTableauGauche().getBtnCreer().setImage(pluginLicence.getImageDescriptor(PaCompositeSectionParam.iconRefreshPath).createImage());
			vue.getCompositeSectionTableauGauche().getBtnCreer().addSelectionListener(new SelectionListener() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					for (Object key : listeGestionnaireEditorSupportAbon.keySet()) {
						if(key.equals(documentSelection.getCodeTSupport())){
							final AbstractLgrMultiPageEditorSupport editor =(AbstractLgrMultiPageEditorSupport) listeGestionnaireEditorSupportAbon.get(key);
			...
			vue.getCompositeSectionParam().getBtnAideTiers().setImage(pluginLicence.getImageDescriptor(PaCompositeSectionParam.iconFindPath).createImage());
			...
			TaTSupportDAO taTArticleDAO = new TaTSupportDAO();
			List<TaTSupport> listeTArticle = taTArticleDAO.selectAll();
			
			for (TaTSupport taTArticle : listeTArticle) {
				
			}
			//mapTArticle.put(etatEnCoursLibelle, null);
//			vue.getCompositeSectionParam().getCbEtat().add(etatEnCoursLibelle);
			
			//String etatDefaut = pluginLicence.getDefault().getPreferenceStore().getString(PreferenceConstants.P_ETAT_DEFAUT);

			int i = 1;
			int selection = -1;
			for (TaTSupport taTSupport : listeTArticle) {
				if(taTSupport.getTypes()!=null  && 
						listeGestionnaireEditorSupportAbon.containsKey(taTSupport.getCodeTSupport())){
							mapTArticle.put(taTSupport.getCodeTSupport(), taTSupport.getCodeTSupport());
							vue.getCompositeSectionParam().getCbEtat().add(taTSupport.getCodeTSupport());
				}
				i++;
			}
			if(selection==-1)
...
//			refreshAction.run();
	...
	protected void initMapComposantChamps() {
		...
		//vue.getCompositeSectionParam().getCdateDeb().addTraverseListener(new DateTraverse());
...
//					ctrlUnChampsSWT(getFocusAvantAideSWT());
	...
	public Map<String, String> getMapTArticle() {
		return mapTArticle;
	}
