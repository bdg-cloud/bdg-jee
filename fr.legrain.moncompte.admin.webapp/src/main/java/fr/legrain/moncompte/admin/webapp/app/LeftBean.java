...
//        LayoutUnit layoutUnit = (LayoutUnit)event.getComponent();
		Panel layoutUnit = (Panel)event.getComponent();
...
//        } else {
        	...
        	if(collapsed)
        		PrimeFaces.current().ajax().addCallbackParam("collapsed", "true");
    		else
    			PrimeFaces.current().ajax().addCallbackParam("collapsed", "false");
